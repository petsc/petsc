  ...
  for f in filenames:
    if fnmatch.fnmatch(f, "build_" + sys.argv[1] + "_*.log"):
      ...
      for line in open(logfile_examples_full):
        ...
        if re.search(r'not ok', line) or re.search(r'[Pp]ossible [Pp]roblem', line) or re.search(r'error: ld returned', line) or re.search(r'warning:', line) or re.search(r': warning ', line):
