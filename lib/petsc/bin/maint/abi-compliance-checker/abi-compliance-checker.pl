...
sub getReportPath($)
{
    ...
    if($In::Desc{1}{"Version"} == "" || $In::Desc{2}{"Version"} == "")
    { # String version might contain \, / or spaces, which are hassles in making a path, so we just use a simpiler V1_to_V2.
      # Note we do not change Desc{}{"Version"}. So in the generated html file, the version description is clearly shown.
      $Dir = "compat_reports/".$In::Opt{"TargetLib"}."/V1_to_V2";
    }
    else
    {
      $Dir = "compat_reports/".$In::Opt{"TargetLib"}."/".$In::Desc{1}{"Version"}."_to_".$In::Desc{2}{"Version"};
    }
