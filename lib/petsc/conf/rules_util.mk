...
checkclangformatversion:
	@version=`${PETSCCLANGFORMAT} --version | cut -d" " -f3 | cut -d"." -f 1` ;\
         ...
         if [ $$version != 22 ]; then echo "Require clang-format version 22! Currently used ${PETSCCLANGFORMAT} version is $$version"; false; fi
...
checkbadSource:
	...
	@a=`cat checkbadSource.out | wc -l`; l=`expr $$a - 45` ;\
         if [ $$l -gt 0 ] ; then \
...
fixbadSource:
	@command -v claude >/dev/null 2>&1 || { echo "Claude Code not installed"; exit 1; }
	@git diff --quiet || { echo "Warning: committing all current changes to prevent data loss"; git commit -a -m "WIP: auto-save before fixbadSource"; }
	-@make checkbadSource
	@if [ -s checkbadSource.out ]; then \
          claude "The file checkbadSource.out contains the results of running make checkbadSource. Update all the needed files to remove all of the errors detected in checkbadSource.out. Ensure you fix all errors. Report what files you have updated." && make clangformat fprettify checkbadSource && echo "Warning: code generated by an LLM, use git diff to verify all changes are appropriate";\
        else echo "checkbadSource successful, nothing to do!"; fi
