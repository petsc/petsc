...
class Configure(config.package.Package):
  ...
  def Install(self):
    ...
    if self.installNeeded('make.inc'):
      ...
      config.package.Package.executeShellCommand('cd '+self.packageDir+' && make clean && make lib',timeout=500, log = self.log)
      config.package.Package.executeShellCommand('cd '+self.packageDir+' && make install ',timeout=250, log = self.log)
