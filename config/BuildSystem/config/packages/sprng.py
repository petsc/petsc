  ...
  def Install(self):
    ...
    if self.installNeeded(os.path.join('SRC','make.PETSC')):
      try:
        ...
        config.package.Package.executeShellCommand('cd '+self.packageDir+' && make realclean && cd SRC && make && cd .. && cp -f lib/*.a '+os.path.join(self.libDir,'')+' && cp -f include/*.h '+os.path.join(self.installDir,self.includedir,''), timeout=2500, log = self.log)
      except RuntimeError as e:
        raise RuntimeError('Error running make on SPRNG: '+str(e))
      self.postInstall(os.path.join('SRC','make.PETSC'))
