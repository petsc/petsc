...
class generateExamples(Petsc):
  ...
  def _isRun(self,testDict, debug=False):
    """
    Determine whether the test requirements are satisfied by the configuration.
    """
    indent="  "
    ...
    if 'requires' in testDict:
      for requirement in testDict['requires'].split():
        ...
        if requirement=="datafilespath" and not isNull:
          ...
          continue
        # Package versions generated by configure in petscpkg_version.h
        version = re.fullmatch(r'(?:PETSC_PKG_)?([A-Z0-9_]+_VERSION_)(EQ|LT|LE|GT|GE)\((\d+),(\d+),(\d+)\)', requirement.upper())
        if version:
          prefix, comparison, major, minor, subminor = version.groups()
          prefix = 'PETSC_PKG_' + prefix
          keys = [prefix + component for component in ('MAJOR', 'MINOR', 'SUBMINOR')]
          matches = False
          if all(key in self.conf for key in keys):
            installed = tuple(int(self.conf[key]) for key in keys)
            requested = (int(major), int(minor), int(subminor))
            matches = getattr(operator, comparison.lower())(installed, requested)
          if matches == isNull:
            testDict['SKIP'].append(('Not ' if isNull else 'Required: ') + requirement)
          continue
