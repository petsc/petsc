  ...
  def configureETags(self):
    ...
    if not os.path.exists(os.path.join(self.petscdir.dir, 'TAGS')):
      ...
      if hasattr(self.framework, 'etags'):
        ...
        try:
          (output, error, status) = config.base.Configure.executeShellCommand(os.path.join(self.petscdir.dir,'bin','maint','generateetags.py'), timeout = 15*60.0, log = self.framework.log)
