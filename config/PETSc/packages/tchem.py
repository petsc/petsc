  ...
  def Install(self):
    ...
    if self.installNeeded(conffile):
      try:
        ...
        output1,err1,ret1  = PETSc.package.NewPackage.executeShellCommand('cd '+self.packageDir+' && ./configure '+args, timeout=300, log = self.framework.log)
      ...
      try:
        ...
        output2,err2,ret2  = PETSc.package.NewPackage.executeShellCommand('cd '+self.packageDir+' && make && cp include/TC_*.h %(includeDir)s && cp lib/libtchem* %(libDir)s' % dict(includeDir=includeDir,libDir=libDir), timeout=500, log = self.framework.log)
      except RuntimeError, e:
