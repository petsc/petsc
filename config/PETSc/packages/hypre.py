  ...
  def Install(self):
    ...
    if self.installNeeded('hypre'):
      try:
        ...
        output,err,ret  = PETSc.package.NewPackage.executeShellCommand('cd '+os.path.join(self.packageDir,'src')+';make distclean;./configure '+args, timeout=900, log = self.framework.log)
      ...
      try:
        ...
        output,err,ret  = PETSc.package.NewPackage.executeShellCommand('cd '+os.path.join(self.packageDir,'src')+';HYPRE_INSTALL_DIR='+self.installDir+';export HYPRE_INSTALL_DIR; make install', timeout=2500, log = self.framework.log)
      except RuntimeError, e:
      ...
      self.postInstall(output+err,'hypre')
