...
class PETScMaker(script.Script):
 ...
 def buildDir(self, libname, dirname, fnames):
   ...
   for f in os.listdir(dirname):
     ...
     if ext == '.c':
       cnames.append(f)
       cufile = f.replace('.c', '.cu')
       cunames.append(cufile)
       if not os.path.islink(os.path.join(dirname,cufile)):
         os.symlink(os.path.join(dirname,f),os.path.join(dirname,cufile))
