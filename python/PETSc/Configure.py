  ...
  def configureWin32NonCygwin(self):
    ...
    if self.framework.argDB['CC'].find('win32fe') >= 0:
      ...
      if not self.checkCompile('#include <sys/types.h>\n','uid_t u;\n'):
        self.addPrototype('typedef int uid_t;')
        self.addPrototype('typedef int gid_t;')
      if not self.checkCompile('#include <sys/stat.h>\n','int a=R_OK;\n'):
        self.addPrototype('#define R_OK 04')
        self.addPrototype('#define W_OK 02')
        self.addPrototype('#define X_OK 01')
      if not self.checkLink('#include <sys/stat.h>\n','int a=0;\nif (S_ISDIR(a)){}\n'):
        self.addPrototype('#define S_ISREG(a) (((a)&_S_IFMT) == _S_IFREG)')
        self.addPrototype('#define S_ISDIR(a) (((a)&_S_IFMT) == _S_IFDIR)')
