 ...
 * also be passed in. */
#define MBERR(msg,rval) do{if (rval != moab::MB_SUCCESS) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"MOAB ERROR (%i): %s",(PetscErrorCode)rval,msg);} while (0)
#define MBERRNM(rval) do{if (rval != moab::MB_SUCCESS) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"MOAB ERROR (%i)",rval);} while (0)
