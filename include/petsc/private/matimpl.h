...
#  define MatCheckPreallocated(A,arg) do {                              \
    PetscAssert((A)->preallocated,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Must call MatXXXSetPreallocation(), MatSetUp() or the matrix has not yet been factored on argument %d \"%s\" before %s()",(arg),#A,PETSC_FUNCTION_NAME); \
  } while (0)
#else
...
#  define MatCheckProduct(A,arg) do {                              \
    PetscAssert((A)->product,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Argument %d \"%s\" is not a matrix obtained from MatProductCreate()",(arg),#A); \
  } while (0)
#else
...
#define MatCheckSameLocalSize(A,ar1,B,ar2) do {                         \
    ...
    PetscAssert(((A)->rmap->n == (B)->rmap->n) && ((A)->cmap->n == (B)->cmap->n),PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Incompatible matrix local sizes: parameter # %d (%" PetscInt_FMT " x %" PetscInt_FMT ") != parameter # %d (%" PetscInt_FMT " x %" PetscInt_FMT ")",ar1,(A)->rmap->n,(A)->cmap->n,ar2,(B)->rmap->n,(B)->cmap->n); \
  } while (0)
#define MatCheckSameSize(A,ar1,B,ar2) do {                              \
    PetscAssert(((A)->rmap->N == (B)->rmap->N) && ((A)->cmap->N == (B)->cmap->N),PetscObjectComm((PetscObject)(A)),PETSC_ERR_ARG_INCOMP,"Incompatible matrix global sizes: parameter # %d (%" PetscInt_FMT " x %" PetscInt_FMT ") != parameter # %d (%" PetscInt_FMT " x %" PetscInt_FMT ")",ar1,(A)->rmap->N,(A)->cmap->N,ar2,(B)->rmap->N,(B)->cmap->N); \
...
#define VecCheckMatCompatible(M,x,ar1,b,ar2) do { \
    PetscAssert((M)->cmap->N == (x)->map->N,PetscObjectComm((PetscObject)(M)),PETSC_ERR_ARG_SIZ,"Vector global length incompatible with matrix: parameter # %d global size %" PetscInt_FMT " != matrix column global size %" PetscInt_FMT,ar1,(x)->map->N,(M)->cmap->N); \
    PetscAssert((M)->rmap->N == (b)->map->N,PetscObjectComm((PetscObject)(M)),PETSC_ERR_ARG_SIZ,"Vector global length incompatible with matrix: parameter # %d global size %" PetscInt_FMT " != matrix row global size %" PetscInt_FMT,ar2,(b)->map->N,(M)->rmap->N); \
  } while (0)
