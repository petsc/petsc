...
#define PetscCheckScaLapackInfo(routine,info) \
  do { \
    if (info) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"Error in ScaLAPACK subroutine %s: info=%d",routine,(int)info); \
  } while (0)
...
#define MatScaLAPACKCheckDistribution(a,arga,b,argb) \
  do { \
    ...
    if ((_aa)->mb!=(_bb)->mb || (_aa)->nb!=(_bb)->nb || (_aa)->rsrc!=(_bb)->rsrc || (_aa)->csrc!=(_bb)->csrc || (_aa)->grid->nprow!=(_bb)->grid->nprow || (_aa)->grid->npcol!=(_bb)->grid->npcol || (_aa)->grid->myrow!=(_bb)->grid->myrow || (_aa)->grid->mycol!=(_bb)->grid->mycol) SETERRQ(PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_INCOMP,"Arguments #%d and #%d have different ScaLAPACK distribution",arga,argb); \
  } while (0)
