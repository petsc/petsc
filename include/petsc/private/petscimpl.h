...
#define PetscValidHeaderSpecificType(h,ck,arg,t) \
  do {   \
    ...
    PetscAssertFalse(!_7_same,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Wrong subtype object:Parameter # %d must have implementation %s it is %s",arg,t,((PetscObject)(h))->type_name); \
  } while (0)

#define PetscValidHeaderSpecific(h,ck,arg)                              \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Object: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_OBJECT),PETSC_COMM_SELF,PETSC_ERR_ARG_CORRUPT,"Invalid Pointer to Object: Parameter # %d",arg); \
    if (((PetscObject)(h))->classid != ck) {                            \
      PetscAssertFalse(((PetscObject)(h))->classid == PETSCFREEDHEADER,PETSC_COMM_SELF,PETSC_ERR_ARG_CORRUPT,"Object already free: Parameter # %d",arg); \
...
#define PetscValidHeader(h,arg)                                         \
  do {                                                                  \
    ...
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_OBJECT),PETSC_COMM_SELF,PETSC_ERR_ARG_CORRUPT,"Invalid Pointer to Object: Parameter # %d",arg); \
    PetscAssertFalse(((PetscObject)(h))->classid == PETSCFREEDHEADER,PETSC_COMM_SELF,PETSC_ERR_ARG_CORRUPT,"Object already free: Parameter # %d",arg); \
    else PetscAssertFalse(((PetscObject)(h))->classid < PETSC_SMALLEST_CLASSID || ((PetscObject)(h))->classid > PETSC_LARGEST_CLASSID,PETSC_COMM_SELF,PETSC_ERR_ARG_CORRUPT,"Invalid type of object: Parameter # %d",arg); \
  } while (0)

#define PetscValidPointer(h,arg)                                        \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Pointer: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_CHAR),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer: Parameter # %d",arg); \
  } while (0)

#define PetscValidCharPointer(h,arg)                                    \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Pointer: Parameter # %d",arg);\
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_CHAR),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer to char: Parameter # %d",arg); \
  } while (0)

#define PetscValidIntPointer(h,arg)                                     \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Null Pointer: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_INT),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer to PetscInt: Parameter # %d",arg); \
  } while (0)

#define PetscValidBoolPointer(h,arg)                                    \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Null Pointer: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_BOOL),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer to PetscBool: Parameter # %d",arg); \
  } while (0)

#define PetscValidScalarPointer(h,arg)                                  \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Pointer: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_SCALAR),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer to PetscScalar: Parameter # %d",arg); \
  } while (0)

#define PetscValidRealPointer(h,arg)                                    \
  do {                                                                  \
    PetscAssertFalse(!(h),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Pointer: Parameter # %d",arg); \
    PetscAssertFalse(!PetscCheckPointer(h,PETSC_REAL),PETSC_COMM_SELF,PETSC_ERR_ARG_BADPTR,"Invalid Pointer to PetscReal: Parameter # %d",arg); \
  } while (0)

#define PetscValidFunction(f,arg)                                       \
  do {                                                                  \
    PetscAssertFalse(!(f),PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Null Function Pointer: Parameter # %d",arg); \
  } while (0)
#endif
...
#define PetscCheckSameType(a,arga,b,argb)                               \
  do {                                                                  \
    PetscAssertFalse(((PetscObject)(a))->type != ((PetscObject)(b))->type,PETSC_COMM_SELF,PETSC_ERR_ARG_NOTSAMETYPE,"Objects not of same type: Argument # %d and %d",arga,argb); \
  } while (0)
...
#define PetscCheckTypeName(a,type)                                      \
  do {                                                                  \
    ...
    PetscAssertFalse(!_7_match,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Object (%s) is not %s",(char*)(((PetscObject)(a))->type_name),type); \
  } while (0)
...
#define PetscCheckTypeNames(a,type1,type2)                              \
  do {                                                                  \
    ...
    PetscAssertFalse(!_7_match,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Object (%s) is not %s or %s",(char*)(((PetscObject)(a))->type_name),type1,type2); \
  } while (0)
/*
...
#define PetscValidType(a,arg)                                           \
  do {                                                                  \
    PetscAssertFalse(!((PetscObject)(a))->type_name,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"%s object's type is not set: Argument # %d",((PetscObject)(a))->class_name,arg); \
  } while (0)
/*
...
#define PetscCheckSameComm(a,arga,b,argb)                               \
  do {                                                                  \
    ...
    PetscAssertFalse(_7_flag != MPI_CONGRUENT && _7_flag != MPI_IDENT,PETSC_COMM_SELF,PETSC_ERR_ARG_NOTSAMECOMM,"Different communicators in the two objects: Argument # %d and %d flag %d",arga,argb,_7_flag); \
  } while (0)
...
#define PetscValidLogicalCollectiveScalar(a,b,arg)                      \
  do {                                                                  \
    ...
    PetscAssertFalse(!(b2[4] > 0) && !(PetscEqualReal(-b2[0],b2[1]) && PetscEqualReal(-b2[2],b2[3])),PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"Scalar value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscValidLogicalCollectiveReal(a,b,arg)                        \
  do {                                                                  \
    ...
    PetscAssertFalse(!(b2[2] > 0) && !PetscEqualReal(-b2[0],b2[1]),PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"Real value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscValidLogicalCollectiveInt(a,b,arg)                         \
  do {                                                                  \
    ...
    PetscAssertFalse(-b2[0] != b2[1],PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"Int value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscValidLogicalCollectiveMPIInt(a,b,arg)                      \
  do {                                                                  \
    ...
    PetscAssertFalse(-b2[0] != b2[1],PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"PetscMPIInt value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscValidLogicalCollectiveBool(a,b,arg)                        \
  do {                                                                  \
    ...
    b1[0] = -b0; b1[1] = b0;                                            \
    _7_ierr = MPIU_Allreduce(b1,b2,2,MPI_INT,MPI_MAX,PetscObjectComm((PetscObject)(a)));CHKERRMPI(_7_ierr); \
    PetscAssertFalse(-b2[0] != b2[1],PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"Bool value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscValidLogicalCollectiveEnum(a,b,arg)                        \
  do {                                                                  \
    ...
    b1[0] = -b0; b1[1] = b0;                                            \
    _7_ierr = MPIU_Allreduce(b1,b2,2,MPI_INT,MPI_MAX,PetscObjectComm((PetscObject)(a)));CHKERRMPI(_7_ierr); \
    PetscAssertFalse(-b2[0] != b2[1],PetscObjectComm((PetscObject)(a)),PETSC_ERR_ARG_WRONG,"Enum value must be same on all processes, argument # %d",arg); \
  } while (0)
...
#define PetscCheckSorted(n,idx)                                                                   \
  do {                                                                                            \
    ...
    PetscAssertFalse(!_1_flg,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Input array needs to be sorted"); \
  } while (0)
