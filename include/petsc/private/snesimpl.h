...
#define SNESCheckFunctionNorm(snes,beta) do { \
  if (PetscIsInfOrNanReal(beta)) {\
    PetscCheck(!snes->errorifnotconverged,PetscObjectComm((PetscObject)snes),PETSC_ERR_NOT_CONVERGED,"SNESSolve has not converged due to Nan or Inf norm");\
      ...
      CHKERRMPI(MPIU_Allreduce(&snes->domainerror,&domainerror,1,MPIU_BOOL,MPI_LOR,PetscObjectComm((PetscObject)snes)));\
      if (domainerror)  {\
...
#define SNESCheckJacobianDomainerror(snes) do { \
  if (snes->checkjacdomainerror) {\
    PetscBool domainerror;\
    CHKERRMPI(MPIU_Allreduce(&snes->jacobiandomainerror,&domainerror,1,MPIU_BOOL,MPI_LOR,PetscObjectComm((PetscObject)snes)));\
    if (domainerror) {\
    ...
    if (kspreason < 0) {\
      if (kspreason == KSP_DIVERGED_NANORINF) {\
        ...
        if (++snes->numLinearSolveFailures >= snes->maxLinearSolveFailures) {\
          CHKERRQ(PetscInfo(snes,"iter=%D, number linear solve failures %D greater than current SNES allowed %D, stopping solve\n",snes->iter,snes->numLinearSolveFailures,snes->maxLinearSolveFailures));\
