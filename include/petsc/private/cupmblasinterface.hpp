...
#define PetscCallCUPMBLAS(...) \
  do { \
    ...
    if (PetscUnlikely(cberr_p_ != CUPMBLAS_STATUS_SUCCESS)) { \
      if (((cberr_p_ == CUPMBLAS_STATUS_NOT_INITIALIZED) || (cberr_p_ == CUPMBLAS_STATUS_ALLOC_FAILED)) && PetscDeviceInitialized(cupmDeviceTypeToPetscDeviceType())) { \
...
// }
#define PETSC_CUPMBLAS_ALIAS_BLAS_FUNCTION_EXACT(MACRO_SUFFIX, our_suffix, their_suffix) PETSC_CUPM_ALIAS_FUNCTION_EXACT(cupmBlasX, our_suffix, PETSC_CUPMBLAS_PREFIX, PetscConcat(PETSC_CUPMBLAS_BUILD_BLAS_FUNCTION_ALIAS_, MACRO_SUFFIX)(their_suffix))
