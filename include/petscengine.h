...
#define PreLoadBegin(flag,name) {PetscTruth PreLoading = flag, PreLoad; int PreLoadIt, __ierr;\
                                 __ierr = OptionsGetLogical(PETSC_NULL,"-preload",&PreLoading,PETSC_NULL);CHKERRQ(__ierr);\
                                 if (PreLoading) PreLoad = PETSC_TRUE;\
                                 for (PreLoadIt=0; PreLoadIt<=(int)PreLoading; PreLoadIt++) {\
...
#define PreLoadEnd()               __ierr = PLogStagePop();CHKERRQ(__ierr);PreLoad = PETSC_FALSE;}}
