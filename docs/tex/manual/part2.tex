% $Id: part2.tex,v 1.358 2001/04/06 03:00:10 bsmith Exp bsmith $ 
%
% NOTES:  
...
\findex{VIEWER_STDOUT_WORLD} \trl{VIEWER_STDOUT_WORLD}, which ensures
that parallel vectors are printed correctly to \trl{stdout}. To display the vector in an X-window,
one can use the default X-windows viewer \trl{VIEWER_DRAW_WORLD},
\findex{VIEWER_DRAW_WORLD}
or one can create a viewer with the routine \trl{ViewerDrawOpenX()}.
...
the user is responsible for providing enough space in the array; \trl{m*sizeof(Scalar)}.
...
\end{verbatim}
where \trl{viewer} was obtained with \trl{ViewerDrawOpenX()}.
...
the options database command \trl{ksp_preres} or by calling the routine
\begin{verbatim}
...
one of the following:  \trl{-pc_lu_ordering_type natural, -pc_lu_ordering_type nd, 
-pc_lu_ordering_type 1wd, -pc_lu_ordering_type rcm, -pc_lu_ordering_type qmd}.
In addition, see 
...
Let \trl{B1} and \trl{B2} represent the application of two 
...
residual. For example, with two composed preconditioners: \trl{B1} and 
\trl{B2}; $ y = B x $ is obtained from
\[
  y   = B1 x \\
  w1  = x - A y \\
  y   = y + B2 w1 
...
See also \trl{$PETSC_DIR/src/snes/examples/tutorials/ex19.c} for a higher level
interface to the multigrid solvers using the \trl{DMMG} object.
...
\begin{verbatim}
  MatSNESMFDefaultSetUmin(Mat mat,double);
\end{verbatim}
...
\begin{verbatim}
  PetscViewerDrawOpenX(MPI_Comm comm,char *display,char *title,int x,int y,int w,
...
Predefined viewers, \trl{VIEWER_DRAW_WORLD} 
and \trl{VIEWER_DRAW_SELF}, may be used at any time. Their initial
...
\begin{verbatim}
  PetscDrawOpenX(MPI_Comm comm,char *display,char *title,int x,int y,int w,
                    int h,PetscDraw *win);
\end{verbatim}
...
\trl{${PETSC_DIR}/bmake/${PETSC_ARCH}/base} and remove the flag
\trl{-PETSC_DHAVE_X11} from the CONF variable definition.  Then (re)compile
the PETSc libraries.
...
MatView() functions with a viewer type of ASCII (see \trl{ViewerASCIIOpen()}, 
\trl{VIEWER_STDOUT_WORLD}, \trl{VIEWER_STDOUT_SELF}, or \trl{VIEWER_STDOUT_(MPI_Comm)}). Before calling
the viewer set the output type with \trl{ViewerSetFormat(viewer,PETSC_VIEWER_ASCII_MATLAB,char *name)} or
\trl{ViewerPushFormat(viewer,PETSC_VIEWER_ASCII_MATLAB}. \findex{PETSC_VIEWER_ASCII_MATLAB}
\begin{verbatim}
...
One creates a viewer to Matlab via \trl{ViewerSocketOpen(MPI_Comm,char *machine,int port,PetscViewer *v)}
(port is usally set to \trl{PETSC_DEFAULT}) and then sends matrices or vectors via
\begin{verbatim}
  VecView(Vec A,v);
  MatView(Mat B,v);
\end{verbatim}
...
To receive the objects in Matlab you must first make sure that \trl{${PETSC_DIR}/src/sys/src/viewer/impls/socket/matlab}
is in your Matlab path. Use \trl{p = openport;} (or \trl{p = openport(portnum)} if you provided a port number in
your call to \trl{ViewerSocketOpen()}), then \trl{a = receive(p);} returns the object you have passed from PETSc.
\trl{receive()} may be called any number of times. You many call \trl{closeport()} to close the connection from Matlab.
It is also possible to start your PETSc program from Matlab via \trl{launch()}.

\section{Using the Matlab Compute Engine}

One creates access to the Matlab engine via \trl{PetscMatlabEngineCreate(MPI_Comm comm,char *machine,PetscMatlabEngine *e);}
(\trl{machine} may be \trl{PETSC_NULL}).
One can send objects to Matlab via \trl{PetscMatlabEnginePut(PetscMatlabEngine e,PetscObject obj);} One can get objects
via \trl{PetscMatlabEngineGet(PetscMatlabEngine e,PetscObject obj);}. Similarly one can send arrays via
\trl{PetscMatlabEnginePutArray(PetscMatlabEngine e,int m,int n,Scalar *array,char *name);} and get them back via
\trl{PetscMatlabEngineGetArray(PetscMatlabEngine e,int m,int n,Scalar *array,char *name);} One cannot use Matlab
interactively in this mode but you can send Matlab commands via
\trl{PetscMatlabEngineEvaluate(PetscMatlabEngine,''format'',...);} where \trl{format} has the usual \trl{printf()} format.
Responses can be returned from Matlab via \trl{PetscMatlabEngineGetOutput(PetscMatlabEngine,char **);} or 
\trl{PetscMatlabEnginedPrintOutput(PetscMatlabEngine,FILE*)}. There is a short-cut to starting the Matlab engine
with \trl{MATLAB_ENGINE_(MPI_Comm)}.
...
If working with \trl{.f} files is absolutely essential (perhaps as
...
\end{verbatim}
Some Fortran compilers will accept a \trl{-I<directory>}, but depending on the
Fortran compiler, they may use the \trl{-I} list only for the \trl{\#include}
style of include.
...
In addition, the user must declare all PETSc objects as \trl{integer} 
rather than by their name. For example, declarations within Fortran
\trl{.F} files have the form
\begin{verbatim}
   SLES    solver
   Mat     A, B
   Vec     x, y
   integer i
\end{verbatim}
while the analogous statements within \trl{.f} files are
\begin{verbatim}
   integer solver
   integer A, B
   integer x, y
...
Fortran programmers using the \trl{.F} file suffix, as discussed in
...
\end{verbatim}
Note that users of the Fortran \trl{.f} suffix {\em cannot} employ the
macros \trl{SETERRQ()} and \trl{CHKERRQ()}.
...
PETSc objects (such as \trl{Mat} and \trl{SLES}) between the two
...
%first convert the C pointer to a Fortran integer with \trl{
%PetscCObjectToFortranObject(PetscObject,PetscFortranAddr *)}, for example,
%\begin{verbatim}
%   PetscFortranAddr fmat;
%   Mat              mat;
%    ....
%  PetscCObjectToFortranObject(mat,&fmat);
...
%\trl{PetscFortranObjectToCObject(PetscFortranAddr,PetscObject *)}, for example,
%\findex{PetscFortranObjectToCObject()}
%\begin{verbatim}
%   int cfunction(PetscFortranAddr *fmat,....)
%   Mat mat;
%  PetscFortranObjectToCObject(*fmat,&mat);
%\end{verbatim}
%See \trl{${PETSC_DIR}/src/vec/examples/tests/ex24.c} for a complete
...
\trl{PetscInitializeFortran()}, which should be used if one is using a
C main program that calls Fortran routines that in turn call PETSc
routines.  In this case, \trl{PetscInitializeFortran()} should be
called from C after the call to \trl{PetscInitialize()} to initialize
some of the default viewers, communicators, etc.  for use in the
Fortran.  \trl{PetscInitializeFortran()} is {\em not} needed if a
user's main program is written in Fortran; in this case, just calling
\trl{PetscInitialize()} in the main program is sufficient.
...
routine \trl{OptionsGetInt()}, one must use the following command in
Fortran:
\begin{verbatim}
...
of the Fortran program \trl{ex3.F} with the actions \trl{make BOPT=g ex3} and
\trl{make runex3}, respectively. The compilation command is restated below:
\begin{verbatim}
   ex3: ex3.o 
...
\begin{itemize}
\item \trl{PetscInitialize(char *filename,int ierr)}
\item \trl{PetscError(int err,char *message,int ierr)}
\item \trl{VecGetArray()}, \trl{MatGetArray()}
\item \trl{ISGetIndices()}, \trl{DAGetGlobalIndices()}
\item \trl{VecDuplicateVecs()}, \trl{VecDestroyVecs()}
\item \trl{OptionsGetString()}
\end{itemize}
...
\item \trl{ViewerASCIIGetPointer()}, \trl{ViewerBinaryGetDescriptor()}
\item \trl{ViewerStringOpen()}, \trl{ViewerStringSPrintf()}
\item \trl{OptionsGetStringArray()}
\end{itemize}
...
\trl{petscmat.h90}, and \trl{petscis.h90}; the conventional Fortran style
...
${PETSC_DIR}/bmake/${PETSC_ARCH}/base_variables}, as discussed in
...
\trl{PetscEventRegister()} (as described in
...
\begin{verbatim}
  PLogEventRegister(int *e,char *string,char *color);
\end{verbatim}
...
be passed to the \trl{PLogEventBegin()} and \trl{PLogEventEnd()}
routines.
...
\begin{verbatim}
    PLogFlops(number of flops for this code segment);
\end{verbatim}
between the calls to \trl{PLogEventBegin()} and \trl{PLogEventEnd()}.
...
summaries are generated with \trl{-log_summary} or \trl{PLogPrintSummary()}.
...
The \trl{event} may be either a predefined PETSc event (as listed in
the file \trl{${PETSC_DIR}/include/petsclog.h}) or one obtained with
\trl{PetscEventRegister()} (as described in Section~\ref{sec:profileuser}).
...
In particular, the macros \trl{PreLoadBegin(PetscTruth,char* stagename)}, \trl{
PreLoadStage(char *stagename)}, and \trl{PreLoadEnd()} can be used to easily 
...
\trl{${PETSC_DIR}/bmake/${PETSC_ARCH}/base.$\{BOPT\}}.
...
\begin{verbatim}
  PetscInitialize(int *argc,char ***args,char *file_name,char *help_message);
   call PetscInitialize(character file_name,integer ierr)
\end{verbatim}
The arguments \trl{argc} and \trl{args} (in the C/C++ version only) are
the usual command line arguments, while the \trl{file_name} is a name of
...
\trl{XXX}, \findex{Viewer} where \trl{XXX} 
...
\item Passing in a zero for the viewer causes the object to be printed 
      ...
      a debugger.
\item \trl{VIEWER_STDOUT_SELF} \findex{VIEWER_STDOUT_SELF}
...
\item \trl{VIEWER_STDOUT_WORLD} \findex{VIEWER_STDOUT_WORLD}
      is similar to \trl{VIEWER_STDOUT_SELF}, except that 
      ...
      this viewer uses the communicator \trl{PETSC_COMM_WORLD}.
\item \trl{VIEWER_DRAW_WORLD} \findex{VIEWER_DRAW_WORLD} causes the 
      object to be drawn in a default X window.
\item \trl{VIEWER_DRAW_SELF} \findex{VIEWER_DRAW_SELF} causes the 
      object to be drawn in a default X window.
\item Passing in a viewer obtained by
      \trl{ViewerDrawOpenX()} causes the object to be displayed graphically.
\item To save an object to a file in ASCII format, the user creates
      the viewer object with the command
      \trl{ViewerASCIIOpen(MPI_Comm comm, char* file, PetscViewer *viewer)}.  
      This object is \findex{ViewerASCIIOpen()}
      analogous to \trl{VIEWER_STDOUT_SELF} (for a communicator of
      \trl{MPI_COMM_SELF}) and 
      \trl{VIEWER_STDOUT_WORLD} (for a parallel communicator).
\item To save an object to a file in binary format, the user creates
      the viewer object with the command
      \trl{ViewerBinaryOpen(MPI_Comm comm,char* file,PetscViewerBinaryType type,
...
\item Vector and matrix objects can be passed to a running Matlab process
      with a viewer created by \trl{ViewerMatlabOpen(MPI_Comm comm,
      ...
      the viewer object with \findex{ViewerDestroy()} \trl{
...
created by \trl{ViewerASCIIOpen()} by calling
\begin{verbatim}
...
first opening a binary viewer with \trl{ViewerBinaryOpen()} and
...
command \trl{PLogTraceBegin(FILE*)}. \findex{-log_trace} \findex{PLogTraceBegin()}
...
or \trl{CHKERRQ()}.  A simple example of usage is given below.
\begin{verbatim}
    #undef __FUNCT__  
...
not needed by users.
\begin{itemize}
\item \trl{ci} - Uses the RCS \trl{ci} mechanism to check in all files in the
      directory. 
\item \trl{co} - Uses the RCS \trl{co} mechanism to check out 
      all files in the directory. 
\item \trl{fortranstubs} - Generates the Fortran wrapper routines.  
\item \trl{latexpages} - Updates the LaTeX version of the manual pages.
\item \trl{manualpages} - Updates the HTML version of the manual pages.
\end{itemize}
...
two base makefiles:
\begin{itemize}
\item \trl{base.site} - locations of all needed include
      and library files for a particular site. This file (discussed
      below) is usually the only one that the user needs to alter.
\item \trl{base_variables} - definitions of the compilers, linkers, etc.
\end{itemize}
Each architecture base file, denoted by
\trl{${PETSC_DIR}/bmake/${PETSC_ARCH}/base},
includes the file \break \trl{${PETSC_DIR}/bmake/common},
...
\> \trl{EXAMPLES_7} \> - C examples that require BlockSolve\\
...
\begin{verbatim}
  ...
  MatLUFactor(Mat matrix,IS rowpermutation,IS columnpermutation,
                      MatLUInfo *info); 
\end{verbatim}
The argument \trl{info->fill $ \ge $ 1} is the predicted fill
