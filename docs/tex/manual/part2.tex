...
\item (IS, VecScatter) Section \ref{sec_unstruct} - management of vectors related to unstructured grids
\end{itemize}
...
More generally, one can use the routines \findex{VecCreate()} \findex{VecSetSizes()} \findex{VecSetFromOptions()}
\begin{tabbing}
...
ISGlobalToLocalMappingApply() with \trl{idxout} equal to \trl{
...
\begin{tabbing}
   ...
   PetscScalar *values;\\
   int         idx\_from[] = {100,200}, idx\_to[] = {0,1};\\
...
column (\trl{in}) indices set with MatSetValues() are sorted,
...
\begin{tabbing}
   ...
   \trl{-pc_ilu_use_drop_tolerance <dt>,<dtcount>}\\
...
as well as the code to calculate residuals. The PC component 
...
discretization of the Laplacian operator with Neummann boundary conditions
...
BUT if one chooses a direct solver (or an incomplete factorization)
...
To create a SNES solver, one must first call SNESCreate() and indicate
...
for a list of SNESConvergedReason. \findex{SNESConvergedReason}
...
interpreting profile data for the SLES component, 
...
is composed of a PC (preconditioner) and KSP (Krylov
subspace) component, which are in turn built on top of the Mat 
(matrix) and Vec (vector) modules.  Thus, operations in the
SLES module are composed of lower-level operations in these
components.  Note also that the nonlinear solvers component, SNES, 
...
about higher level PETSc components (e.g., TS and SNES) without 
