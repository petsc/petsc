% $Id: part2.tex,v 1.340 2000/06/23 18:16:18 bsmith Exp bsmith $ 
%
% NOTES:  
...
dimension, but not both, can be set to \url{PETSC\_DECIDE} to 
...
The option \url{-vec\_type mpi} can be used in conjunction with 
...
PETSc does not allow the simultaneous use of \url{INSERT\_VALUES} and
\url{ADD\_VALUES} this nondeterministic behavior will not occur in PETSc.)
...
\findex{VIEWER_STDOUT_WORLD} \url{VIEWER\_STDOUT\_WORLD}, which ensures
that parallel vectors are printed correctly to \url{stdout}.  The
viewer \url{VIEWER\_STDOUT\_SELF} \findex{VIEWER_STDOUT_SELF} 
can be employed if
...
one can use the default X-windows viewer \url{VIEWER\_DRAW\_WORLD},
...
\end{verbatim}
Note that here one must provide the value \url{m}, it cannot be \url{PETSC\_DECIDE} and
...
\findex{NormType} \url{NORM\_1}, \url{NORM\_2}, or \url{NORM\_INFINITY}.
...
and viewed with \url{AOView(AO ao,Viewer viewer)}.
...
\url{idxin} are left unmapped.  But, in addition, if \url{type} is set to \url{IS\_GTOLM\_MASK},
...
are replaced by -1. When \url{type} is set to \url{IS\_GTOLM\_DROP},
...
\url{ISGlobalToLocalMappingApply()} with \url{idxout} equal to {\tt
PETSC\_NULL} to determine the required length (returned in \url{nout})
...
\url{PETSC\_DECIDE} for \url{m} and \url{n} 
...
\url{DA\_NONPERIODIC} \findex{DA_NONPERIODIC} (no periodicity), 
\url{DA\_XYPERIODIC} \findex{DA_XYPERIODIC} (periodic in
both x- and y-directions), \url{DA\_XPERIODIC} \findex{DA_XPERIODIC}, 
or \url{DA\_YPERIODIC}.\findex{DA_YPERIODIC}  The argument \url{dof} 
...
\url{m} and the dimension of \url{ly} is \url{n}; or \url{PETSC\_NULL} 
may be passed in.
...
directions are indicated by \url{DA\_STENCIL\_STAR},
\findex{DA_STENCIL_STAR} while box-type stencils are specified by
\url{DA\_STENCIL\_BOX}. \findex{DA_STENCIL_BOX} For example, for the
two-dimensional case,
\url{DA\_STENCIL\_STAR} with width 1 corresponds to the standard 5-point
stencil, while \url{DA\_STENCIL\_BOX} with width 1 denotes the
...
the number of messages sent.  Note that the \url{DA\_STENCIL\_STAR}
...
\end{verbatim}
\url{DA\_ZPERIODIC}, \findex{DA_ZPERIODIC}
\url{DA\_XZPERIODIC}, \findex{DA_XZPERIODIC}
\url{DA\_YZPERIODIC}, and \findex{DA_YZPERIODIC}
\url{DA\_XYZPERIODIC} \findex{DA_XYZPERIODIC}
...
The \url{InsertMode} can be either \url{ADD\_VALUES} or \url{INSERT\_VALUES}.
...
When either type of stencil is used, \url{DA\_STENCIL\_STAR} or 
\url{DA\_STENCIL\_BOX}, the local vectors (with the ghost points) 
represent rectangular arrays, including the extra corner elements in 
the \url{DA\_STENCIL\_STAR} case. This configuration provides simple 
access to the elements by employing two- (or three-) dimensional indexing. 
The only difference between the 
two cases is that when \url{DA\_STENCIL\_STAR} is used, the extra 
...
\end{verbatim} 
Here \url{ix} denotes the index set of the first vector, while {\tt
...
index sets.  The argument \url{INSERT\_VALUES} specifies that the
...
\url{ADD\_VALUES} \findex{ADD_VALUES} instead of \url{INSERT\_VALUES}.
...
{\em Caution}: When \url{INSERT\_VALUES} is used, if two different
...
two arguments to these routines must be swapped whenever the \url{SCATTER\_REVERSE}
option is used.
...
global entries (or \url{PETSC\_NULL}) and \url{nghost} is the number of 
...
The input arguments \url{INSERT\_VALUES} and \url{SCATTER\_FORWARD}
cause the ghost values to be correctly updated from the appropriate
processor. The arguments \url{ADD\_VALUES} and \url{SCATTER\_REVERSE }
...
Calls to \url{MatSetValues()} with the \url{INSERT\_VALUES} and {\tt
...
second routine argument  typically should be \url{MAT\_FLUSH\_ASSEMBLY},
\findex{MAT_FLUSH_ASSEMBLY} which omits some of the work of the full 
assembly process.  \url{MAT\_FINAL\_ASSEMBLY} \findex{MAT_FINAL_ASSEMBLY} is
...
\end{verbatim}
informs the user that all rows from \url{first\_row} to 
\url{last\_row-1} will be stored on the local processor.
...
See \url{\$\{PETSC\_DIR\}/src/sles/examples/tutorials/ex5.c} for a simple example of
...
as discussed below. The user can set \url{nz=0} and {\tt
...
keys are \url{-mat\_aij\_no\_inode} (do not use inodes) and {\tt
...
SPARSKIT, \sindex{SPARSKIT} the option \url{-mat\_aij\_oneindex}
...
The \url{-log\_info} \findex{-log_info} option causes the routines
...
global parameters can be replaced with \url{PETSC\_DECIDE}, so that 
...
If \url{PETSC\_DECIDE} is not used for the arguments
...
The user must set \url{d\_nz=0}, \url{o\_nz=0}, \url{d\_nnz=PETSC\_NULL}, and 
\url{o\_nnz=PETSC\_NULL} for PETSc to control dynamic allocation of matrix
memory space.  Analogous to \url{nz} and \url{nnz} for the routine 
\url{MatCreateSeqAIJ()}, these arguments optionally specify 
nonzero information for the diagonal (\url{d\_nz} and \url{d\_nnz}) and 
off-diagonal (\url{o\_nz} and \url{o\_nnz}) parts of the matrix. 
...
For the first processor one could set \url{d\_nz} to 2 (since each
row has 2 nonzeros) or, alternatively, set \url{d\_nnz} to \{2,2,2\}.
The \url{o\_nz} could be set to 2 since each row of the \url{o} matrix
has 2 nonzeros, or \url{o\_nnz} could be set to \{2,2,2\}.
...
Thus, one could set \url{d\_nz} to 3, since the maximum number of
nonzeros in each row is 3, or alternatively one could set \url{d\_nnz} to
...
so that one could set \url{o\_nz} to 2 or \url{o\_nnz} to \{2,1,1\}.
...
As described above, the option \url{-log\_info} \findex{-log_info}
...
set \url{data} to \url{PETSC\_NULL} for PETSc to control matrix memory allocation.
...
be stored locally (by passing in \url{PETSC\_DECIDE} as the second
...
\findex{NormType} \url{NORM\_1} or \url{NORM\_INFINITY}.
...
\end{verbatim}
Here \url{MATOP\_MULT} is the name of the operation for matrix-vector
...
\url{\$\{PETSC\_DIR\}/include/petscmat.h} provides a complete list of matrix
operations, which have the form \url{MATOP\_<OPERATION>}, where {\tt
<OPERATION>} is the name (in all capital letters) of the user
interface routine (for example, \url{MatMult()} $ \rightarrow $ \url{MATOP\_MULT}).  All
...
of the matrix. If the pointer \url{diag\_value} is \url{PETSC\_NULL}, it 
...
\url{\$\{PETSC\_DIR\}/include/petscmat.h} for other available matrix types;
standard ones are \url{MATSEQDENSE, MATSEQAIJ,   MATMPIAIJ,  
...
can use \url{PETSC\_NULL} for the \url{vals} argument. Similarly,
one can use \url{PETSC\_NULL} for the \url{cols} argument.
...
\begin{itemize}
\item \url{SAME\_NONZERO\_PATTERN} - the preconditioning matrix has the
    same \findex{SAME_NONZERO_PATTERN} nonzero structure during successive
    linear solves,
\item \url{DIFFERENT\_NONZERO\_PATTERN} - the preconditioning matrix does
     not have the same nonzero structure during successive linear solves,
   \findex{DIFFERENT_NONZERO_PATTERN}
\item \url{SAME\_PRECONDITIONER} - the preconditioner matrix is identical
...
the flag \url{DIFFERENT\_NONZERO\_PATTERN}.
...
\url{-pc\_type lu -ksp\_type preonly} (see below).
...
\end{verbatim}
The type can be one of \url{KSPRICHARDSON, KSPCHEBYCHEV, KSPCG, KSPGMRES, 
KSPTCQMR, KSPBCGS, KSPCGS, KSPTFQMR, KSPCR, KSPLSQR, KSPBICG}, or \url{KSPPREONLY.}
...
\url{-ksp\_type},
followed by one of the options \url{richardson, chebychev, cg, gmres, tcqmr, 
bcgs, cgs, tfqmr, cr, lsqr, bicg}, or \url{preonly.} \findex{-ksp_type}
...
\end{verbatim}
The default parameter values are \url{damping\_factor=1.0, 
emax=0.01, emin=100.0}, and \url{max\_steps=30}. The GMRES 
\sindex{restart} restart and Richardson damping factor
can also be set with the options \url{-ksp\_gmres\_restart <n>}
and \url{-ksp\_richardson\_scale <factor>}. \findex{-ksp_gmres_restart} 
...
\url{-ksp\_gmres\_irorthog}. \findex{-ksp_gmres_irorthog} Or
via 
\begin{verbatim}
...
command \url{-ksp\_gmres\_unmodifiedgramschmidt}.
...
\url{-ksp\_gmres\_modifiedgramschmidt}. \findex{-ksp_gmres_modifiedgramschmidt}
...
using the options database command \url{-ksp\_right\_pc} or
...
the options database command \url{ksp\_preres} or by calling the routine
\begin{verbatim}
...
specifying \url{PETSC\_DEFAULT} \findex{PETSC_DEFAULT} as the 
...
commands \url{-ksp\_rtol <rtol>, -ksp\_atol <atol>}, \url{-ksp\_divtol <dtol>},
\findex{-ksp_rtol} \findex{-ksp_atol} \findex{-ksp_divtol}
and \url{-ksp\_max\_it <its>}. \findex{-ksp_max_it}
...
\begin{verbatim}
   ierr = KSPSetConvergenceTest(KSP ksp,
                                int (*test)(KSP ksp,int it,double rnorm,void *ctx),
                                void *ctx);
\end{verbatim}
...
The routine for detecting convergence, \url{test}, should return the 
...
\url{-ksp\_monitor} within the options database.  
To display the residual norms in a graphical window (running under X Windows),
one should use \url{-ksp\_xmonitor [x,y,w,h]}, where either all or none of 
...
\url{PetscObjectGetComm((PetscObject)ksp,MPI\_Comm *comm)}, which returns
...
\url{-ksp\_monitor, -ksp\_singmonitor}, and \url{-ksp\_truemonitor}.
...
with the command \url{-ksp\_xmonitor [x,y,w,h]}. \findex{-ksp_xmonitor} 
...
\url{-ksp\_cancelmonitors}. \findex{-ksp_cancelmonitors}

As the Krylov method converges so that the residual norm is small,
say $ 10^{-10} $ many of the final digits printed with the \url{-ksp\_monitor}
...
difficult. The option \url{-ksp\_smonitor} \findex{-ksp_smonitor}
...
data base commands \url{-ksp\_plot\_eigenvalues} and \url{-ksp\_plot\_eigenvalues\_explicitly}.
\findex{-ksp_plot_eigenvalues} \findex{-ksp_plot_eigenvalues_explicitly}
Or they can be dumped to the screen in ASCII text via
\url{-ksp\_compute\_eigenvalues} and \url{-ksp\_compute\_eigenvalues\_explicitly}.
...
\url{w} is \url{PETSC\_NULL}, space allocated by PETSc in the KSP context is 
...
\url{-pc\_type <methodname>} or set the method with the 
...
\url{PC<TYPE>Option} and \url{-pc\_<type>\_option [value]}.  A
...
\url{-mat\_order order}) computed in the first factorization to be reused
for later factorizations. \sindex{orderings} 
The \url{ PCILUSetReuseFill()} causes the 
...
\url{MatCreate()} with the option \url{-mat\_mpirowbs}, or by directly
...
with \url{PETSC\_DECIDE}, so that PETSc will determine it.  The matrix
is stored with a fixed number of rows on each processor, given by {\tt
m}, or determined by PETSc if \url{m} is \url{PETSC\_DECIDE}.  The
...
\url{nnz=PETSC\_NULL} for PETSc to control all matrix memory
...
\url{PETSC\_NULL}, so that PETSc will create and initialize this context.
...
option \url{-ksp\_truemonitor} \findex{-ksp_truemonitor} to print both the
...
where the argument \url{type} can be one of \url{SOR\_FORWARD\_SWEEP,
SOR\_BACKWARD\_SWEEP} or \url{SOR\_SYMMETRIC\_SWEEP}, the default
being \url{SOR\_FORWARD\_SWEEP}. Setting the type to be {\tt
...
variant of SOR with the types \url{SOR\_LOCAL\_FORWARD\_SWEEP, 
SOR\_LOCAL\_BACKWARD\_SWEEP}, and \url{SOR\_LOCAL\_SYMMETRIC\_SWEEP}.
...
also be set with the options \url{-pc\_sor\_omega <omega>}, 
\url{-pc\_sor\_its <its>}, \url{-pc\_sor\_backward}, \url{-pc\_sor\_symmetric}, 
\break \url{-pc\_sor\_local\_forward}, \url{-pc\_sor\_local\_backward}, and 
\url{-pc\_sor\_local\_symmetric}.
...
\break \url{-pc\_eisenstat\_no\_diagonal\_scaling}) 
...
the option \url{-pc\_eisenstat\_omega <omega>} (or the routine
...
this command is \url{-pc\_lu\_in\_place}. \findex{-pc_lu_in_place}
...
\begin{itemize}
\item \url{MATORDERING\_NATURAL} - Natural
\item \url{MATORDERING\_ND} - Nested Dissection
\item \url{MATORDERING\_1WD} - One-way Dissection
\item \url{MATORDERING\_RCM} - Reverse Cuthill-McKee
\item \url{MATORDERING\_QMD} - Quotient Minimum Degree
\end{itemize}
...
one of the following:  \url{-mat\_ordering\_type natural, -mat\_ordering\_type nd, 
-mat\_ordering\_type 1wd, -mat\_ordering\_type rcm, -mat\_ordering\_type qmd}.
In addition, see 
...
\url{-pc\_lu\_nonzeros\_along\_diagonal <tol>} \findex{-pc_lu_nonzeros_along_diagonal}
...
\url{-sub\_ksp\_type} and \url{-sub\_pc\_type}. In fact, all of the KSP
and PC options can be applied to the subproblems by inserting the prefix
\url{-sub\_} at the beginning of the option name. \sindex{local linear solves}
...
block.  The argument \url{n\_local} is the number of blocks on the 
calling processor, and \url{first\_local} indicates the global number 
...
example \url{\$\{PETSC\_DIR\}/src/sles/examples/tutorials/ex7.c}.
...
options database commands to set this value are \url{-pc\_bjacobi\_blocks n}
and \url{-pc\_bgs\_blocks n}, and, within a program, the corresponding routines
...
\url{PCASMType} is one of \url{PC\_ASM\_BASIC},
\url{PC\_ASM\_INTERPOLATE}, \url{PC\_ASM\_RESTRICT}, \url{PC\_ASM\_NONE}
and may also be set with the options database \url{-pc\_asm\_type [basic,interpolate,restrict,none]}.
...
The type \url{PC\_ASM\_BASIC} (or \url{-pc\_asm\_type basic}) corresponds to the
standard additive Schwarz method that uses the full restriction and
interpolation operators.
The type \url{PC\_ASM\_RESTRICT} (or \url{-pc\_asm\_type restrict}) uses a full
restriction operator, but during the interpolation process ignores the off-processor
values.
Similarly, \url{PC\_ASM\_INTERPOLATE} (or \url{-pc\_asm\_type interpolate}) uses a limited
restriction process in conjunction with a full interpolation, while
\url{PC\_ASM\_NONE} (or \url{-pc\_asm\_type none}) ignores off-processor valies
...
\url{PC\_ASM\_RESTRICT} is the PETSc default, as it saves substantial communication
...
\url{-pc\_type composite} \findex{composite} \url{-pc\_composite\_pcs jacobi,ilu}
\findex{-pc_composite_pcs} causes the composite preconditioner to be used with 
two preconditioners: Jacobi and ILU. The option \url{-pc\_composite\_type multiplicative}
\findex{-pc_composite_type} initiates the multiplicative version of the algorithm,
while \url{-pc\_composite\_type additive} the additive version. Using the true
preconditioner is obtained with the option \url{-pc\_composite\_true}. 
\findex{-pc_composite_true} One sets options for the subpreconditioners with the 
extra prefix \url{-sub\_N\_} where \url{N} is the number of the subpreconditioner.
For example, \url{-sub\_0\_pc\_ilu\_fill 0}.
...
\url{-pc\_type sles -sles\_pc\_type ilu -sles\_ksp\_max\_it 5 -sles\_ksp\_type bcgs}. 
...
\end{verbatim}
at the command line with \url{-pc\_sles\_true}. \findex{-pc_sles_true}

Naturally one can use a SLES preconditioner inside a composite preconditioner. For example,
\url{-pc\_type composite -pc\_composite\_pcs ilu,sles -sub\_1\_pc\_type jacobi -sub\_1\_ksp\_max\_it 10}
...
\end{verbatim}
with a value of \url{MG\_W\_CYCLE} for \url{cycles}. 
The commands above can also be set from the options database. The option 
names are \url{-pc\_mg\_type [multiplicative, additive, full, kaskade]},
and \url{-pc\_mg\_cycles cycles}. \findex{-pc_mg_type} \findex{-pc_mg_cycles}
...
\url{-pc\_mg\_smoothup~m} and \url{-pc\_mg\_smoothdown~n} or
...
(as well as the coarse grid) using the prefix \url{mg\_levels\_} (
\url{mg\_coarse\_} for the coarse grid).
...
\url{\$\{PETSC\_DIR\}/src/snes/examples/tutorials/ex1.c}.  
...
\end{verbatim}
or use the the option \url{-snes\_type <method>}, \findex{-snes_type} 
...
function evaluation routine; \url{PETSC\_NULL} should be used if such information
...
pointers to PETSc objects. \url{\$\{PETSC\_DIR\}/src/snes/examples/tutorials/ex5.c} and
\url{\$\{PETSC\_DIR\}/src/snes/examples/ex5f.F} give examples of user-defined
...
\url{flag} are identical to those for the flag of {\tt
...
in the \url{FormJacobian()} routine to be \url{SAME\_NONZERO\_PATTERN} 
...
by setting \url{flag} to \url{SAME\_PRECONDITIONER}.  See the
discussion of \url{SLESSetOperators()} in Section~\ref{sec:usingsles} for
details.

The directory \url{\$\{PETSC\_DIR\}/src/snes/examples/tutorials} provides
...
The directory \url{\$\{PETSC\_DIR\}/src/snes/examples/tests/umin} provides
...
\url{SNES\_EQ} and \url{SNES\_UM}, respectively.  
...
The method \url{SNES\_EQ\_NLS} (\url{-snes\_type ls}) provides a line
...
\url{-snes\_eq\_ls [cubic, quadratic, basic, basicnonorms]}. \findex{-snes_eq_ls}
...
\url{-snes\_eq\_ls\_alpha <alpha>}, \findex{-snes_eq_ls_alpha}
\url{-snes\_eq\_ls\_maxstep <max>}, and \findex{-snes_eq_ls_maxstep}
\url{-snes\_eq\_ls\_steptol <tol>}. \findex{-snes_eq_ls_steptol}

The method \url{SNES\_UM\_NLS} (\url{-snes\_type umls}) provides a line
...
equations, \url{SNES\_EQ\_NTR} ({\tt-snes\_type tr}), is taken from
...
\url{-snes\_eq\_tr\_delta0 <delta0>}.

The default trust region method for unconstrained minimization, 
\url{SNES\_UM\_NTR} (\url{-snes\_type umtr}), is based on the work of
...
with the options \url{-pc\_type jacobi}, \url{-pc\_type icc}, and {\tt
...
are \url{-snes\_atol <atol>}, \url{-snes\_rtol <rtol>}, \url{-snes\_stol <stol>},
\findex{-snes_atol} \findex{-snes_rtol}  \findex{-snes_stol}
\findex{-snes_max_it} \findex{-snes_max_funcs}
\url{-snes\_max\_it <its>}, and \url{-snes\_max\_funcs <fcts>}.
...
The user can set this parameter with the option \url{-snes\_trtol <trtol>}
...
be set with the option \url{-snes\_fmin <ftol>} \findex{-snes_fmin} or
with the routine
\begin{verbatim}
...
unconstrained minimization problems, the arguments \url{xnorm}, {\tt
...
\url{-snes\_monitor} \findex{-snes_monitor} activates the default
SNES monitor routine, \url{SNESDefaultMonitor()}, \findex{SNESDefaultMonitor()}
while \url{-snes\_xmonitor} \findex{-snes_xmonitor} draws
...
\url{-snes\_cancelmonitors}. \findex{-snes_cancelmonitors}

As the Newton method converges so that the residual norm is small,
say $ 10^{-10} $, many of the final digits printed with the \url{-snes\_monitor}
option are meaningless. Worse, they are different on different 
machines; due to different round-off rules used by, say, the IBM RS6000
and the Sun Sparc. This makes testing between different machines
difficult. The option \url{-snes\_smonitor} \findex{-snes_smonitor}
causes PETSc to print fewer of the digits of the residual norm 
as it gets smaller; thus on most of the machines it will always
print the same numbers making cross processor testing easier.
...
form of comparison, users can employ the option \url{-snes\_type test}
...
difference or matrix-free variant, \url{-snes\_fd} or \url{-snes\_mf}.
...
\url{-ksp\_type <ksp\_method>} and \url{-pc\_type <pc\_method>},
...
option \url{-snes\_ksp\_ew\_conv} \findex{-snes_ksp_ew_conv}. In addition,
...
in combination with no preconditioning (\url{PCNONE} or \url{-pc\_type none}),
...
approximations can also be invoked with the command \url{-snes\_mf}. \findex{-snes_mf}
...
option \url{-snes\_mf\_operator}. \findex{-snes_mf_operator}
...
\end{verbatim}
and the runtime option  \url{-snes\_mf\_ksp\_monitor} \findex{snes_mf_ksp_monitor}
...
\url{-snes\_mf} one can easily convert any SNES code to use a matrix-free
...
\url{\$\{PETSC\_DIR\}/src/snes/\-examples/ex5.c},
...
\end{verbatim}
Alternatively, one can set a coloring type of \url{COLORING\_SL}, {\tt
COLORING\_ID}, \url{COLORING\_LF}, or \url{COLORING\_NATURAL} 
...
is one of \url{TS\_LINEAR} or \url{TS\_NONLINEAR},
...
\end{verbatim}
\findex{TSSetType()} Currently supported types are \url{TS\_EULER},
\url{TS\_BEULER}, and \url{TS\_PSEUDO}
\findex{TS_EULER} \findex{TS_BEULER} \findex{TS_PSEUDO}
or the command line option
\url{-ts\_type euler, beuler, pseudo}. \findex{-ts_type} \sindex{Euler}
...
pass in \url{PETSC\_NULL} for \url{f}.  The variable \url{fP} allows 
...
\end{verbatim}
or use the command line option \url{-ts\_type pvode}. \findex{TS_PVODE} 
...
\end{verbatim}
or the command line option \url{-ts\_pvode\_type <adams,bdf>}. BDF is
...
via \url{-pc\_xxx}. \findex{TSPVodeGetPC()}
...
\end{verbatim}
where \url{type} is either \url{PVODE\_MODIFIED\_GS} or 
\url{PVODE\_UNMODIFIED\_GS}. \findex{TSPVodeSetGramSchmidtType()}
...
with \url{-ts\_pvode\_gramschmidt\_type <modifed,unmodified>}.
...
database with \url{-ts\_pvode\_gmres\_restart restart}. \findex{-ts_pvode_gmres_restart}
...
the option \url{-ts\_pseudo\_increment\_dt\_from\_initial\_dt}. 
...
\url{ -ts\_pseudo\_increment <inc>}. \findex{-ts_pseudo_increment}
...
Predefined viewers, \url{VIEWER\_DRAW\_WORLD} 
and \url{VIEWER\_DRAW\_SELF}, may be used at any time. Their initial
...
colormap can be used via the option \url{-draw\_x\_shared\_colormap}.
...
$ 0 \le \url{xl} \le \url{xr} \le 1 $ and $ 0 \le \url{yl} \le \url{yr} \le 1.$ 
...
and includes \url{DRAW\_BLACK}, \url{DRAW\_RED}, \url{DRAW\_BLUE} etc.
...
\url{-draw\_pause n}, which \findex{-draw_pause} causes the PETSc application 
...
\url{-ksp\_xmonitor} and \url{-snes\_xmonitor}. \findex{-ksp_xmonitor}
...
\url{\$\{PETSC\_DIR\}/bmake/\$\{PETSC\_ARCH\}/base} and remove the flag
\url{-DHAVE\_X11} from the CONF variable definition.  Then (re)compile
the PETSc libraries.
...
command line options -vec\_view\_matlab -mat\_view\_matlab. This causes the PETSc program
...
\url{VIEWER\_STDOUT\_WORLD}, \url{VIEWER\_STDOUT\_SELF}, or \url{VIEWER\_STDOUT\_(MPI\_Comm)}). Before calling
the viewer set the output type with \url{ViewerSetFormat(viewer,VIEWER\_FORMAT\_ASCII\_MATLAB,char *name)} or
\url{ViewerPushFormat(viewer,VIEWER\_FORMAT\_ASCII\_MATLAB,char *name)}. \findex{VIEWER_FORMAT_ASCII_MATLAB}
\begin{verbatim}
  ViewerSetFormat(VIEWER_STDOUT_WORLD,VIEWER_FORMAT_ASCII_MATLAB,''VecA'');
  VecView(A,VIEWER_STDOUT_WORLD);
  ViewerSetFormat(VIEWER_STDOUT_WORLD,VIEWER_FORMAT_ASCII_MATLAB,''MatB'');
  MatView(B,VIEWER_STDOUT_WORLD);
\end{verbatim}

\section{Sending Data to Running Matlab}

One creates a viewer to Matlab via \url{ViewerSocketOpen(MPI\_Comm,char *machine,int port,Viewer *v)}
(port is usally set to \url{PETSC\_DEFAULT}) and then sends matrices or vectors via
\begin{verbatim}
  VecView(Vec A,v);
  MatView(Mat B,v);
\end{verbatim}
...
\begin{verbatim}
  ViewerSocketPutScalar(v,int m,int n,Scalar *array);
  ViewerSocketPutReal(v,int m,int n,double *array);
  ViewerSocketPutInt(v,int m,int *array);
\end{verbatim}
One may start the Matlab program manually or use the PETSc command
\url{PetscStartMatlab(MPI\_Comm,char *machine,char *script,FILE **fp);} where machine and script may be \url{PETSC\_NULL}.

To receive the objects in Matlab you must first make sure that \url{\${PETSC\_DIR}/src/sys/src/viewer/impls/socket/matlab}
is in your Matlab path. Use \url{p = openport;} (or \url{p = openport(portnumber)} if you provided a port number in
...
One creates access to the Matlab engine via \url{PetscMatlabEngineCreate(MPI\_Comm comm,char *machine,PetscMatlabEngine *e);}
(\url{machine} may be \url{PETSC\_NULL}).
...
with \url{MATLAB\_ENGINE\_(MPI\_Comm)}.
...
\url{\$\{PETSC\_DIR\}/include/finclude} and should be used via statements 
...
\url{CHKERRA(ierr)}, which terminates all process when an error is
encountered.  Likewise, one can set error codes within Fortran programs by
using \url{SETERRA(ierr,p,' ')}, which again terminates all processes
...
macros \url{SETERRA()} and \url{CHKERRA()}.
...
indexing begins with 1, not 0 (unless the array is declared as \url{xx\_v(0:1)}).
...
%\end{verbatim}
%See \url{\$\{PETSC\_DIR\}/src/vec/examples/tests/ex24.c} for a complete
...
would be called from C with \url{dabsc\_()}.  Other machines
...
\url{PETSC\_HAVE\_FORTRAN\_UNDERSCORE} and \url{PETSC\_HAVE\_FORTRAN\_CAPS}
\findex{PETSC_HAVE_FORTRAN_UNDERSCORE} \findex{PETSC_HAVE_FORTRAN_CAPS},
which are defined in the file \url{\$\{PETSC\_DIR\}/bmake/\$\{PETSC\_ARCH\}/petscconf.h}.
...
From Fortran, users {\em must} pass \url{PETSC\_NULL\_XXX} to indicate a
...
the code.   Note
that the C convention of passing \url{PETSC\_NULL} (or 0) {\em cannot}
...
user can employ \url{PETSC\_NULL} for all null arguments. 
...
\url{v\_new} of size \url{n}.  Then, after \url{VecDuplicateVecs()} has
been called, \url{v\_new} will contain (pointers to) the new PETSc
...
duplicates \url{v\_old} to form two new vectors, \url{v\_new(1)} and \url{v\_new(2)}.
\begin{verbatim}
   Vec     v_old, v_new(2)
   integer ierr
   Scalar  alpha
...
\url{\$\{PETSC\_FORTRAN\_LIB\}}, {\em must}  \findex{PETSC_FORTRAN_LIB} precede
the base PETSc libraries, given by \url{\$\{PETSC\_LIB\}}, \findex{PETSC_LIB}
on the link line.
...
\item \url{PetscError(int errno,char *message,int ierr)}
...
corresponding to
\url{\$\{PETSC\_DIR\}/src/vec/examples/tests/ex19.F}, 
\url{\$\{PETSC\_DIR\}/src/vec/examples/tutorials/ex4f.F}, 
\break \url{\$\{PETSC\_DIR\}/src/draw/examples/tests/ex5.F}, and 
\url{\$\{PETSC\_DIR\}/src/snes/examples/ex1f.F}, respectively.  We also
...
the \url{-DPETSC\_LOG} flag (which is the default for all versions),
...
activated at runtime.  Note that the flag \url{-DPETSC\_LOG} can be
...
\findex{-log_trace}
\begin{itemize}
\item \url{-log\_summary} - Prints an ASCII version of performance data
     at program's conclusion. These statistics are comprehensive and concise
     and require little overhead; thus, \url{-log\_summary} is intended as
     the primary means of monitoring the performance of PETSc codes.
\item \url{-log\_info} - Prints verbose information about code to the screen. 
...
\item \url{-log\_trace [logfile]} - Traces the beginning and ending of all
     PETSc events.  This option, which can be used in conjunction with 
     \url{-log\_info}, is useful to see where a program is hanging
...
The summaries printed via \url{-log\_summary} reflect this 
...
and ILU, as described above), so that interpreting the \url{-log\_summary}
...
\url{\$\{PETSC\_DIR\}/src/sles/examples/ex21.c}.  The code loads a
...
operations, as given by the routines \url{VecScatterBegin()} and {\tt
VecScatterEnd()}. 
%
%  Temp removed
%
...
can turn on MPE logging by specifying \url{-DHAVE\_MPE} in the {\tt
...
the file \url{\$\{PETSC\_DIR\}/include/petsclog.h}) or one obtained with
...
summaries are generated with \url{-log\_summary} or \url{PLogPrintSummary()}.
...
generated by \url{-log\_summary} for a program that employs
...
Recall that the option \url{-log\_all} produces extensive profile
...
\url{-log\_all} when running programs with a relatively small
...
Section~\ref{sec:matsparse}, \url{-log\_info} activates the
...
\url{object\_cookie} is one of \url{MAT\_COOKIE}, \url{SNES\_COOKIE}, etc.
...
events at runtime for matrix and linear solver components via \url{-log\_info [no\_mat, no\_sles]}.
...
option \url{-log\_history [filename]}. \findex{-log_history}
...
noticeable in the the log files generated by \url{-log\_mpe}, which is
...
\url{\$\{PETSC\_DIR\}/src/sles/examples/tutorials/ex10.c} to
...
In particular, the macros \url{PreLoadBegin(PetscTruth,char* stagename)}, {\tt
...
\url{-preload true} and \url{-preload false} may be used to turn on and off
...
of the optimized versions of code (\url{BOPT=O}, \url{BOPT=O\_c++}, or
\url{BOPT=O\_complex}) when evaluating performance.
...
(\url{PETSC\_ARCH}) and \url{BOPT} by editing the file
\url{\$\{PETSC\_DIR\}/bmake/\$\{PETSC\_ARCH\}/base.\$\{BOPT\}}.
...
\begin{itemize}
\item Run the code with the option \url{-log\_summary} to print a performance
   summary for various phases of the code. \findex{-log_summary}

\item Run the code with the option \url{-log\_mpe [logfilename]}, which creates a
...
\item When using \url{MatSetValues()}, if the column indices of the values being
   ...
   the routine \url{MatSetOption(mat,MAT\_COLUMNS\_SORTED)} before setting the values
...
determine a good value for \url{f} is to run a program with the option \url{-log\_info}.
...
is \url{-pc\_lu\_fill <fill\_amount>}.
...
to PETSc objects. See \url{\$\{PETSC\_DIR\}/snes/examples/tutorials/ex5.c} and 
\url{\$\{PETSC\_DIR\}/snes/examples/tutorials/ex5f.F} for examples of user-defined application
...
\begin{itemize}
\item Use the options \url{-snes\_view}, \url{-sles\_view}, etc. (or the routines 
...
\item Run the code with the option \url{-help} for a list of the available 
     runtime commands.
\item Use the option \url{-log\_info} to print details about the solvers' operation.
...
parameter with the option \url{-ksp\_gmres\_restart <restart>} or by
...
debug mode (\url{BOPT=g}, \url{BOPT=g\_c++}, \url{BOPT=g\_complex}).
PETSc memory allocation can be activated for other other cases, such
as \url{BOPT=O}, with the option \url{-trmalloc}, while {\tt
-trmalloc\_off} forces the use of conventional memory allocation for the
\url{BOPT=g}, \url{BOPT=g\_c++}, and \url{BOPT=g\_complex} versions.
...
routines is \url{-trmalloc\_log}, which activates logging of all calls
...
\url{-trmalloc\_log} activates calls to \url{PetscTrLog()}, {\tt
PetscTrLogDump()}, and \url{PetscGetResidentSetSize()} during {\tt
...
\findex{-trmalloc_nan}
\item The option \url{-trmalloc\_nan} is useful for tracking down the
...
\item The utility \url{\$\{PETSC\_DIR\}/bin/petscview} can illustrate graphically
...
\url{-DPETSC\_LOG} flag (which is the default for all versions).
...
\item {\bf Problem too large for physical memory size}: When timing a program, one
      ...
      Also, the PETSc option \url{-log\_summary} prints the amount of 
      memory used by the basic PETSc objects, thus providing a lower
      bound on the memory used.  Another useful option is \url{-trmalloc\_log}
...
\item {\bf Overhead of timing routines on certain machines}: On certain machines,
      ...
      \url{\$\{PETSC\_DIR\}/src/benchmarks/PetscTime.c} contains a
...
the usual command line arguments, while the \url{file\_name} is a name of
...
environmental variable \url{PETSC\_OPTIONS}.  \findex{PETSC_OPTIONS} 
...
PETSc, we employ the format \url{-package\_option} (for instance, 
\url{-ksp\_type} and \url{-mat\_view\_info}).
...
\begin{verbatim}
   ...
   ierr = OptionsGetString(char *pre,char *name,char *value,int maxlen,PetscTruth *flg);
   ierr = OptionsGetStringArray(char *pre,char *name,char **values,int *maxlen,PetscTruth *flg);
   ierr = OptionsGetIntArray(char *pre,char *name,int *value,int *nmax,PetscTruth *flg);
   ierr = OptionsGetDoubleArray(char *pre,char *name,double *value, int *nmax,PetscTruth *flg);
\end{verbatim}
All \findex{OptionsGetDouble()}
of \findex{OptionsGetString()} \findex{OptionsGetIntArray()}
these \findex{OptionsGetDoubleArray()}
routines set \url{flg=PETSC\_TRUE} if the corresponding option was found, \url{flg=PETSC\_FALSE} if it
...
Usually \url{pre} should be set to \url{PETSC\_NULL} (or \url{PETSC\_NULL\_CHARACTER}
...
blocks can be controlled via the options \url{-sub\_ksp\_type} and {\tt
...
use of \url{-optionstable}, which prints to \url{stdout} during {\tt
...
\item Passing in a zero for the viewer causes the object to be printed 
      ...
      a debugger.
\item \url{VIEWER\_STDOUT\_SELF} \findex{VIEWER_STDOUT_SELF}
...
\item \url{VIEWER\_STDOUT\_WORLD} \findex{VIEWER_STDOUT_WORLD}
      is similar to \url{VIEWER\_STDOUT\_SELF}, except that 
      ...
      this viewer uses the communicator \url{PETSC\_COMM\_WORLD}.
\item \url{VIEWER\_DRAW\_WORLD} \findex{VIEWER_DRAW_WORLD} causes the 
      object to be drawn in a default X window.
\item \url{VIEWER\_DRAW\_SELF} \findex{VIEWER_DRAW_SELF} causes the 
      object to be drawn in a default X window.
...
\item To save an object to a file in ASCII format, the user creates
      ...
      \url{ViewerASCIIOpen(MPI\_Comm comm, char* file, Viewer *viewer)}.  
      This object is \findex{ViewerASCIIOpen()}
      analogous to \url{VIEWER\_STDOUT\_SELF} (for a communicator of
      \url{MPI\_COMM\_SELF}) and 
      \url{VIEWER\_STDOUT\_WORLD} (for a parallel communicator).
\item To save an object to a file in binary format, the user creates
      the viewer object with the command
      \url{ViewerBinaryOpen(MPI\_Comm comm,char* file,ViewerBinaryType type,
...
\item Vector and matrix objects can be passed to a running Matlab process
      with a viewer created by \url{ViewerMatlabOpen(MPI\_Comm comm,
      ...
      ViewerDestroy()}. The corresponding Matlab \url{mex}
      files are located in \url{\$\{PETSC\_DIR\}/src/viewer/impls/matlab}.
\end{itemize}
...
\url{VIEWER\_FORMAT\_ASCII\_DEFAULT}, \url{VIEWER\_FORMAT\_ASCII\_MATLAB}, and
\break \url{VIEWER\_FORMAT\_ASCII\_IMPL}.  The implementation-specific format, 
\url{VIEWER\_FORMAT\_ASCII\_IMPL}, displays the object in the most natural way
...
\url{VIEWER\_FORMAT\_ASCII\_IMPL} prints by diagonals, while \url{VIEWER\_FORMAT\_ASCII\_DEFAULT}
...
\begin{itemize}
\item \url{-start\_in\_debugger [noxterm,dbx,xxgdb] [-display name]} 
     - start all processes in debugger
\item \url{-on\_error\_attach\_debugger [noxterm,dbx,xxgdb]
...
By default the GNU debugger {\em gdb} is used when \url{-start\_in\_debugger}
or \url{-on\_error\_attach\_debugger} is specified. \sindex{debugging}
...
with the options \url{-on\_error\_abort} or {\tt
...
at runtime with the 
option \url{-no\_signal\_handler} \findex{-no_signal_handler}.
...
The option \url{-fp\_trap} turns on the floating-point trap at runtime,
and the routine \findex{-fp_trap} \findex{PetscSetFPTrap()} \findex{PETSC_FP_TRAP_ON}
\begin{verbatim}
   ierr = PetscSetFPTrap(int flag);
\end{verbatim}
can be used in-line.
A \url{flag} of \url{PETSC\_FP\_TRAP\_ON} \findex{PETSC_FP_TRAP_OFF}
indicates that floating-point exceptions should be trapped,
while a value of \url{PETSC\_FP\_TRAP\_OFF} (the default) indicates that they 
...
\url{SETERRA(),CHKERRA()}, and \url{CHKPTRA()} should be used instead 
...
in the main program, since they ensure that \url{MPI\_Abort()} is called 
before the process ends; otherwise, other MPI processes that 
did not generate errors may remain unterminated.

Note that the macro \url{\_\_FUNC\_\_} is used to keep track of
...
that may call \url{SETERRQ()}, \url{SETERRA()}, \url{CHKERRQ()},
or \url{CHKERRA()}.  A simple example of usage is given below.
\begin{verbatim}
    #undef __FUNC__  
...
options \url{-start\_in\_debugger} and \url{-on\_error\_attach\_debugger} may 
...
PETSc codes by \url{Scalar}, is defined as \url{complex} (or {\tt
...
\url{BOPT=[g\_complex,O\_complex]} for debugging, optimized,
and profiling versions, respectively.  The file \url{\$\{PETSC\_DIR\}/docs/installation.html}
...
Numbers'' section of the file \url{\$\{PETSC\_DIR\}/docs/installation.html} for
...
\break \url{\$\{PETSC\_DIR\}/lib/lib\$\{BOPT\}/\$\{PETSC\_ARCH\}}, according to the
...
The program \url{\$\{PETSC\_DIR\}/src/sles/examples/tutorials/ex11.c}
solves a linear system with a complex
coefficient matrix.  Its Fortran counterpart is
\url{\$\{PETSC\_DIR\}/src/sles/examples/tutorials/ex11f.F}.
...
\url{\$\{PETSC\_DIR\}/TAGS} exists.  If this file is
not present, it should be generated by
running \url{make etags} from the PETSc home directory. 
...
command ``\url{M-x tags-search}''. To find repeated occurrences, 
...
\url{PETSC\_ARCH} can also be set as environmental
...
with the C++ compiler, one should use the option \url{BOPT=g\_c++} or 
\url{BOPT=O\_c++}.  \sindex{C++}
The options \url{BOPT=g\_complex} and \url{BOPT=O\_complex}
...
The directory \url{\$\{PETSC\_DIR\}/bmake} contains virtually all
...
PETSc system are defined in the file \url{\$\{PETSC\_DIR\}/bmake/common}.  
...
\end{verbatim}
were executed from the directory \url{\$\{PETSC\_DIR\}/src/sles/ksp},
the debugging library for all Krylov subspace solvers would be built.

\subsection{Customized Makefiles}
\label{sec:custom}

The directory \url{\$\{PETSC\_DIR\}/bmake} contains a subdirectory for each 
...
\item \url{base\_variables} - definitions of the compilers, linkers, etc.
\end{itemize}
Each architecture base file, denoted by
\url{\$\{PETSC\_DIR\}/bmake/\$\{PETSC\_ARCH\}/base},
includes the file \break \url{\$\{PETSC\_DIR\}/bmake/common},
...
the variable \url{PETSCFLAGS} within the base files of {\tt
\$\{PETSC\_DIR\}/bmake/\$\{PETSC\_ARCH\}}, discussed in Section
~\ref{sec:custom}.  The flags include
\begin{itemize}
\item PETSC\_USE\_DEBUG - The PETSc debugging options are activated. We 
      recommend always using this. \findex{PETSC_USE_DEBUG}
\item PETSC\_USE\_COMPLEX - The version with scalars represented 
      as complex numbers is used. \findex{PETSC_USE_COMPLEX}
\item PETSC\_USE\_LOG - Various monitoring statistics on floating-point operations,
...
\url{\$\{PETSC\_DIR\}}) and architecture (specified by
\url{\$\{PETSC\_ARCH\}}).  (See \ref{sec:running} for information on
...
Note that the variable \url{\$\{PETSC\_LIB\}} (as listed on the link
...
like  \url{\$\{PETSC\_SYS\_LIB\}}, \url{\$\{PETSC\_VEC\_LIB\}}, 
\url{\$\{PETSC\_MAT\_LIB\}}, \url{\$\{PETSC\_DM\_LIB\}},
\url{\$\{PETSC\_SLES\_LIB\}}, \url{\$\{PETSC\_SNES\_LIB\}} or
\url{\$\{PETSC\_TS\_LIB\}}.
...
\item \url{EXAMPLES\_1} - examples that will be built with
             \url{make BOPT=[g,O] examples} (see Section~\ref{sec:common})
\item \url{RUNEXAMPLES\_1} - examples that will be run with
...
\item \url{EXAMPLESF} - all Fortran examples that will be checked in/out of
             ...
             needed by users).
\item \url{PETSC\_LIB} - all of the base PETSc libraries.
             \findex{PETSC_LIB}
\item \url{PETSC\_FORTRAN\_LIB} - the PETSc Fortran interface 
...
\begin{tabbing}
1234\= EXAMPLESx\= \kill
\> \url{EXAMPLES\_1} \> - basic C suite used in installation tests\\
\> \url{EXAMPLES\_2} \> - additional C suite including graphics\\
\> \url{EXAMPLES\_3} \> - basic Fortran .F suite\\
\> \url{EXAMPLES\_4} \> - subset of 1 and 2 that runs on only a single processor\\
\> \url{EXAMPLES\_5} \> - examples that require complex numbers\\
\> \url{EXAMPLES\_6} \> - C examples that do not work with complex numbers\\
\> \url{EXAMPLES\_7} \> - C examples that require BlockSolve\\
\> \url{EXAMPLES\_8} \> - Fortran .F examples that do not work with complex numbers\\
\> \url{EXAMPLES\_9} \> - uniprocessor version of 3\\
\> \url{EXAMPLES\_10} \> - Fortran .F examples that require complex numbers\\
\end{tabbing}
...
call is  \url{MAT\_INITIAL\_MATRIX} \findex{MAT_INITIAL_MATRIX} it will create the matrix
\url{B}. If call is \url{MAT\_REUSE\_MATRIX} \findex{MAT_REUSE_MATRIX} it will reuse the \url{B}
...
format supports this concept. Often one can use the default by passing in \url{PETSC\_DECIDE}.
...
command line option \url{-mat\_ordering\_type sname}.  If reordering directly,
...
\begin{verbatim}
   ...
   ierr = MatLUFactor(Mat matrix,IS rowpermutation,IS columnpermutation,MatLUInfo *info); 
\end{verbatim}
...
\begin{verbatim}
   ...
   ierr = MatLUFactorSymbolic(Mat matrix,IS rowperm,IS colperm,MatLUInfo *info,Mat *result);
...
the options \url{-pc\_ilu\_nonzeros\_along\_diagonal <tol>} \findex{-pc_ilu_nonzeros_along_diagonal}
and \findex{-pc_lu_nonzeros_along_diagonal}
\url{-pc\_lu\_nonzeros\_along\_diagonal <tol>} may be used.  Here, \url{tol}
is an optional tolerance to decide if a value is nonzero; by default it
is $ 1.e-10.$ 
...
{\em PC\_LEFT} or {\em PC\_RIGHT}. The final routine applies \url{its} iterations of
