...
\begin{tabbing}
  SNESSetLineSearch(SNES snes,int (*ls)(SNES,Vec,Vec,Vec,Vec,double,double*,double*),void *lsctx);
\end{tabbing}
...
``TSSetInitialTimeStep'' function was not in use, so I used it to set
the tolerance.

Setting the tolerance defines the global tolerance, for the whole time
periode. The tolerance for each timestep is calculated relatively to
the size of the timestep.

The error in each timestep is calculated using the two solutions given
from Dormand-Prince 5(4). The local error is calculated finding the
...
\begin{tabbing}
  ...
  \>int (*localjacobian)(DALocalInfo *,void *x,Mat J,void *appctx),\\
