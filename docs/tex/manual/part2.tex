% $Id: part2.tex,v 1.229 1997/08/01 19:30:19 curfman Exp bsmith $ 
%
% NOTES:  
...
the user is responsible for providing enough space in the array; {\tt n*sizeof(Scalar)}.
...
and then one can call \findex{VecSetValuesLocal}
\begin{verbatim}
...
Above we discussed a very general method for managing the communication of 
required ghost values for unstructured grid computations. One scatters
the global vector into the local ``ghosted'' work vector and performs the computation
...
to copy the local vector values (they are already their). In addition the user does not
