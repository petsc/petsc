% $Id: part2.tex,v 1.208 1997/04/03 17:50:15 curfman Exp curfman $ 
%
% NOTES:  
...
of the Fortran77 convention. To allow the insertion of values in column 
...
zero-based indexing.  For compatibility with standard Fortran77 storage,
...
usual Fortran77 style.  To create a sequential, dense PETSc matrix,
...
In Fortran77, this situation is forbidden by the language standard, 
...
\subsection{Fortran90 Pointers}

PETSc includes limited support for direct use of Fortran90 pointers.
Current routines include:
\begin{itemize}
\item VecGetArrayF90(), VecRestoreArrayF90()
\item MatGetArrayF90(), MatRestoreArrayF90()
\item ISGetIndicesF90(), ISRestoreIndicesF90()
\item DAGetGlobalIndicesF90()
\item VecDuplicateVecsF90(), VecDestroyVecsF90()
\end{itemize}
See the man pages for details.  Unfortunately, these routines
currently work only with the NAG F90 compiler.  We hope to support
other compilers as well, but we will need assistance from the vendors
...
environmental {\tt \$(PETSC\_DIR)}) and architecture (specified by
