% $Id: part2.tex,v 1.161 1996/10/24 22:53:20 curfman Exp balay $ 
%
% NOTES:  
...
numbers are maintained completely separately from those for real
...
can be used to browse PETSc 
files quickly and efficiently.  To use this feature, one should 
first check if the file,
{\tt \$(PETSC\_DIR)/tags} exists.  If this file is
not present, it should be generated by
running {\tt make tags} from the PETSc home directory. 
Once the file exists, from VI the user should issue
the command ``{\tt :set tags=\$(PETSC\_DIR)/tags}'' or add to
his ``{\tt \~/.exrc }'' the line ``{\tt set tags=\$(PETSC\_DIR)/tags}''.
Then the command ``{\tt :tag FunctionName}'' will cause Emacs 
to find the file and line number where a desired PETSc function 
is defined. 
