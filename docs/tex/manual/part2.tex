...
different global (cross-process) numbering schemes and the \trl{
...
\begin{tabbing}
  MatSetValues(Mat A,int m,const int idxm[],int n,const int idxn[],const PetscScalar values[],INSERT\_VALUES);
\end{tabbing}
or 
\begin{tabbing}
  MatSetValues(Mat A,int m,const int idxm[],int n,const int idxn[],const PetscScalar values[],ADD\_VALUES);
\end{tabbing}
...
types for the uniprocess case.  \sindex{ILU, parallel}
...
is stored with a fixed number of rows on each process, given by \trl{
...
single block per process is supported. However, the \trl{MATMPIAIJ} and 
...
\begin{tabbing}
  ...
  ierr = DMMGSetSLES(dmmg,ComputeRHS,ComputeMatrix);CHKERRQ(ierr);\\
...
\begin{tabbing}\sindex{PetscObjectSetName}\sindex{PetscObjectName}
PetscObjectSetName((PetscObject)A,''name'');
\end{tabbing}
If no name is specified, the object is given a default name using\trl{PetscObjectName}.
...
\> \trl{EXAMPLES_4 } \> -- subset of 1 and 2 that runs on only a single process\\
...
\> \trl{EXAMPLES_9 } \> -- uniprocess version of 3\\
