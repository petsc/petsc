% $Id: part2.tex,v 1.307 1999/02/18 23:04:32 bsmith Exp bsmith $ 
%
% NOTES:  
...
{\tt VecxxxBegin()}; however if you mistakenly make the calls in the wrong order PETSc will generate an error,
informing you of this. These routines where suggested by Victor Eijkhout.
...
vectors can be accessed \findex{DACreateLocalVector()}
with the routines \findex{DACreateGlobalVector}
\begin{verbatim}
...
Partitioning in PETSc is organized around the {\tt Partitioning} object. \findex{Partitioning}
...
\end{verbatim}
The \findex{PartitioningCreate()} \findex{PartitioningSetAdjacency()} 
\findex{PartitioningSetFromOptions()} \findex{PartitioningApply()} 
\findex{PartitioningDestroy()}  resulting {\tt isg} contains for each local node the new global
...
{\tt -ksp\_gmres\__modifiedgramschmidt}. \findex{-ksp_gmres_modifiedgramschmidt}
...
\end{verbatim}
\findex{MatSNESFDMFSetFunctionError()}
...
see the manual page for MatSNESFDMFSetType() and MatSNESFDMFRegister(). \findex{MatSNESFDMFSetType()}
\findex{MatSNESFDMFRegister()}. We currently provide two default routines accessible via
\begin{verbatim}
...
\end{verbatim}
\findex{MatSNESFDMFDefaultSetUmin()}
...
The second approach, taken from Walker and Pernice, \cite{?}, computes $ h $ via
\begin{eqnarray*}
...
\item Finally, one tells the nonlinear solvers of SNES to use the
      ...
      routine to compute the Jacobians. \findex{SNESDefaultComputeJacobianWithColoring()}
\end{itemize}
...
\findex{TSDefaultComputeJacobianWithColoring()} routine 
{\tt TSSetRHSJacobianDefault()} that \findex{TSSetRHSJacobianDefault()}
allows easy setting of the coloring based callback from Fortran.
\sindex{coloring with TS}
