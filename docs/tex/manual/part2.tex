% $Id: part2.tex,v 1.365 2001/08/07 03:05:45 balay Exp bsmith $ 
%
% NOTES:  
...
\begin{verbatim}
  VecCreateSeqWithArray(PETSC_COMM_SELF,int m,PetscScalar *array,Vec *x);
\end{verbatim}
and  \findex{VecCreateMPIWithArray()} \findex{VecCreateSeqWithArray()}
\begin{verbatim}
  VecCreateMPIWithArray(MPI_Comm comm,int m,int M,,PetscScalar *array,Vec *x);
\end{verbatim}
...
\begin{verbatim}
  VecSetValuesLocal(Vec x,int n,int *indices,PetscScalar *values,INSERT_VALUES);
\end{verbatim}
...
\begin{figure}[tb]
\centerline{\immediate \pdfximage  {ghost.pdf} \pdfrefximage \pdflastximage}
...
\begin{verbatim}
  MatSetValues(Mat A,int m,int *im,int n,int *in,PetscScalar *values,
                       INSERT_VALUES);
\end{verbatim}
or 
\begin{verbatim}
...
\begin{verbatim}
  MatCreateMPIDense(MPI_Comm comm,int m,int n,int M,int N,PetscScalar *data,
                            Mat *A)
\end{verbatim}
...
{\bf Function Name} & {\bf Operation} \\
\hline
MatAXPY(PetscScalar *a,Mat X, Mat Y); & $ Y = Y + a*X $ \\
...
\begin{verbatim}
  MatZeroRows(Mat A,IS rows,PetscScalar *diag_value);
\end{verbatim}
...
\begin{verbatim}
  PetscMatlabEnginePutArray(PetscMatlabEngine e,int m,int n,PetscScalar *array,
                            char *name);
\end{verbatim}
and get them back via
\begin{verbatim}
  PetscMatlabEngineGetArray(PetscMatlabEngine e,int m,int n,PetscScalar *array,
                            char *name);
\end{verbatim}
...
%\centerline{\immediate \pdfximage width 7in {perflog1.pdf} \pdfrefximage \pdflastximage}
...
%\centerline{\immediate \pdfximage height 7in {perflog2.pdf} \pdfrefximage \pdflastximage}
...
\begin{verbatim}
  ...
  OptionsGetReal(char *pre,char *name,double *value,PetscTruth *flg);
  ...
  OptionsGetIntArray(char *pre,char *name,int *value,int *nmax,
                             PetscTruth *flg);
  OptionsGetRealArray(char *pre,char *name,double *value, int *nmax,
                             PetscTruth *flg);
\end{verbatim}
All \findex{OptionsGetReal()}
of \findex{OptionsGetString()} \findex{OptionsGetIntArray()}
these \findex{OptionsGetRealArray()}
...
\item Passing in a zero for the viewer causes the object to be printed 
      ...
      a debugger.
\item \trl{VIEWER_STDOUT_SELF} \findex{VIEWER_STDOUT_SELF} and 
      \trl{VIEWER_STDOUT_WORLD} \findex{VIEWER_STDOUT_WORLD}
      cause the object to be printed to the screen.

\item \trl{VIEWER_DRAW_SELF} \findex{VIEWER_DRAW_SELF} and 
      \trl{VIEWER_DRAW_WORLD} \findex{VIEWER_DRAW_WORLD} causes the 
...
\item To save an object to a file in binary format, the user creates
      the viewer object with the command
      \trl{ViewerBinaryOpen(MPI_Comm} \trl{comm,char* file,PetscViewerBinaryType} \trl{type,
