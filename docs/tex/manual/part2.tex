% $Id: part2.tex,v 1.341 2000/07/25 19:47:59 bsmith Exp bsmith $ 
%
% NOTES:  
...
and \url{-pc_mg_cycles cycles}. \findex{-pc_mg_type} \findex{-pc_mg_cycles}
...
\url{-pc_mg_smoothup~m} and \url{-pc_mg_smoothdown~n} or
...
the viewer set the output type with \url{ViewerSetFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name)} or
\url{ViewerPushFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name)}. \findex{VIEWER_FORMAT_ASCII_MATLAB}
\begin{verbatim}
...
One creates a viewer to Matlab via \url{ViewerSocketOpen(MPI_Comm,char *machine,int port,Viewer *v)}
...
\item To save an object to a file in ASCII format, the user creates
      ...
      \url{ViewerASCIIOpen(MPI_Comm comm, char* file, Viewer *viewer)}.  
...
\item To save an object to a file in binary format, the user creates
      the viewer object with the command
      \url{ViewerBinaryOpen(MPI_Comm comm,char* file,ViewerBinaryType type,
...
\url{VIEWER_FORMAT_ASCII_DEFAULT}, \url{VIEWER_FORMAT_ASCII_MATLAB}, and
\break \url{VIEWER_FORMAT_ASCII_IMPL}.  The implementation-specific format, 
\url{VIEWER_FORMAT_ASCII_IMPL}, displays the object in the most natural way
...
\url{VIEWER_FORMAT_ASCII_IMPL} prints by diagonals, while \url{VIEWER_FORMAT_ASCII_DEFAULT}
...
in the main program, since they ensure that \url{MPI_Abort()} is called 
