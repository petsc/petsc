...
See \trl{${PETSC_DIR}/src/ksp/examples/tutorials/ex5.c} for a simple example of
...
To solve a linear system, one merely executes the command \findex{KSPSolve()}
\begin{tabbing}
  KSPSolve(KSP ksp);
\end{tabbing}
...
Note that multiple linear solves can be performed by the same KSP context.
Once the KSP context is no longer needed, it should be destroyed with the 
command \findex{KSPDestroy()}
\begin{tabbing}
  KSPDestroy(KSP ksp);
\end{tabbing}
...
KSPSolve()} only; such operations will {\em not} be repeated for
successive solves.
...
example \trl{${PETSC_DIR}/src/ksp/examples/tutorials/ex7.c}.
...
achieved with the \trl{PCKSP} type. \findex{PCKSP}
\begin{tabbing}
...
KSPSetOperators()}, discussed in Section~\ref{sec_usingsles}.  
...
For linear problems the examples \trl{src/ksp/examples/tutorials/ex22.c} and \trl{ex25.c} can be used
...
\begin{tabbing}
   call KSPSolve(KSP ksp,int ierr)\\
  KSPSolve(KSP ksp);
\end{tabbing}
...
\trl{${PETSC_DIR}/src/ksp/examples/ex21.c}.  The code loads a
...
events at runtime for matrix and linear solver libraries via \trl{-log_info [no_mat, no_ksp]}.
...
\trl{${PETSC_DIR}/src/ksp/examples/tutorials/ex10.c} to
...
The program \trl{${PETSC_DIR}/src/ksp/examples/tutorials/ex11.c}
solves a linear system with a complex
coefficient matrix.  Its Fortran counterpart is
\trl{${PETSC_DIR}/src/ksp/examples/tutorials/ex11f.F}.
...
KSP interface, as discussed in Chapter \ref{ch_sles}, but the underlying 
