% $Id: part2.tex,v 1.253 1998/03/18 15:38:32 curfman Exp curfman $ 
%
% NOTES:  
...
Here {\tt N} denotes the number of local indices, {\tt globalnum} represents the
...
processes.  In many cases, several different arrays could share the
same communication information.  The design of the distributed array
object makes this relatively easy.  Each {\tt DA} operation may
...
DAGetLocalVector()} and {\tt DAGetDistributedVector()}, but, as
...
should use the option {\tt -mat\_aij\_no\_inode} to avoid I-node
overhead.
...
do not work well due to very poor conditioning.  The approximation requires several steps:
\begin{itemize}
...
\sindex{petsc-maint} \sindex{BLAS} \sindex{LAPACK} \sindex{LINPACK}
\sindex{MINPACK} \sindex{SPARSPAK} \sindex{BlockSolve95} \sindex{ESSL} \sindex{Matlab}
\sindex{ParMeTis} \sindex{PVODE} \sindex{SPAI}
To simplify the use of certain packages, we have integrated into PETSc
an interface to them.  The current collection includes parts of
LAPACK, BLAS, LINPACK, MINPACK, SPARSPAK, BlockSolve95, ESSL, Matlab,
ParMeTis, PVODE, and SPAI.  We invite users to let us know (by writing
to {\tt petsc-maint@mcs.anl.gov}) whether they need particular
libraries that are related to PETSc for an application; we may try to
provide easy-to-use PETSc interfaces for them.
