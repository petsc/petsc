% $Id: part2.tex,v 1.356 2001/03/28 19:43:26 balay Exp bsmith $ 
%
% NOTES:  
...
that parallel vectors are printed correctly to \url{stdout}. To display the vector in an X-window,
...
\end{verbatim}
See \url{$PETSC_DIR/src/snes/examples/tutorials/ex5.c} for a 
complete example and See \url{$PETSC_DIR/src/snes/examples/tutorials/ex19.c} for an
...
\url{-pc_ilu_ordering_type order}) computed in the first factorization to be reused
...
\url{docs/installation/index.htm} for details on installing PETSc to allow 
...
one of the following:  \url{-pc_lu_ordering_type natural, -pc_lu_ordering_type nd, 
...
See also \url{$PETSC_DIR/src/snes/examples/tutorials/ex19.c} for a higher level
interface to the multigrid solvers using the \url{DMMG} object.
...
(To install PETSc to use PVODE, see the installation guide, \url{docs/installation/index.htm}.) 
...
\url{-PETSC_DHAVE_X11} from the CONF variable definition.  Then (re)compile
the PETSc libraries.
...
the \url{-DPETSC_USE_LOG} flag (which is the default for all versions),
...
activated at runtime.  Note that the flag \url{-DPETSC_USE_LOG} can be
...
can turn on MPE logging by specifying \url{-DPETSC_HAVE_MPE} in the \url{
...
Note that the macro \url{__FUNCT__} is used to keep track of
...
and profiling versions, respectively.  The file \url{${PETSC_DIR}/docs/installation/index.htm}
...
Numbers'' section of the file \url{${PETSC_DIR}/docs/installation/index.htm} for
...
\end{itemize}
These orderings can also be set through the options database by specifying 
one of the following:  \url{-pc_lu_ordering_type} \url{natural}, \url{-pc_lu_ordering_type} \url{nd}, 
\url{-pc_lu_ordering_type} \url{1wd}, \url{-pc_lu_ordering_type} \url{rcm}, \url{-pc_lu_ordering_type}
\url{ qmd}.
\findex{MATORDERING_NATURAL} \findex{MATORDERING_ND} \findex{MATORDERING_1WD}
\findex{MATORDERING_RCM} \findex{MATORDERING_QMD} \sindex{nested dissection}
\sindex{one-way dissection} \sindex{reverse Cuthill-McKee} 
...
command line option \url{-pc_lu_ordering_type sname}.  If reordering directly,
