% $Id: part2.tex,v 1.240 1997/11/10 04:51:08 curfman Exp bsmith $ 
%
% NOTES:  
...
\begin{figure}[tb]
\centerline{\psfig{file=../pictures/danumbering.eps,angle=270,width=4.7in}}
...
If you do not use {\tt PETSC\_DECIDE} for
{\tt m} and {\tt n} then you  must ensure that they are chosen to be
...
PVODE is a parallel ODE solver developed by Hindmarsh et al. at LLNL \cite{?}.
...
PVODE does not use SNES for its nonlinear solver so one cannot change
the nonlinear solver options via SNES. Rather, PVODE uses PC, you can 
obtain the PC object via
\begin{verbatim}
  ierr = TSPvodeGetPC(TS ts,PC *pc);
\end{verbatim}
and thus set preconditioner options; or use the command line interface
...
Since you are accessing the array directly from Fortran you will use indexing
starting with 1, not 0 (unless you declare the array as {\tt xx\_v(0:1)}).
This is different than the use of {\tt VecSetValues()} where you always index 
starting with 0.
...
Numbers'' section of the file {\tt \$(PETSC\_DIR)/Installation} for
...
\begin{figure}
\hspace{0.8in}\psfig{file=treep.ps,height=5.0in,width=5.0in}
...
Improving Sequential Performance: Matrix Blocking Strategies
...
of techniques for blocking, including
 ...
 - storing the matrices using a variable block size, that can be 
...
the absolute best performance, since the code can be hardwired with
that particular size (for example, in some problems the size may be 3,
in others 5, etc.), so that the compiler will then optimize for that
...
The table below presents the floating point performance
...
second matrix (CFD), which comes from a three-dimensional compressible flow
simulation, has 15,360 degrees of freedom, 496,000 nonzeros, and a
...
--------------------------------------------------------------------
  Basic Kernel Flop Rates (MFlops/second) on 1 Node of an IBM SP2
--------------------------------------------------------------------
Problem  Basic    I-nodes    Fixed block size   Variable block size
...
Problems with a Single DoF Per Node

  Use the option -mat\_no\_unroll
     (e.g., Bratu problem)
