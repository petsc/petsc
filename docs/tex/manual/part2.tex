...
\trl{${PETSC_DIR}/src/vec/examples/tutorials/ex2.c} or \trl{ex2f.F}
...
There is no routine called VecGetValues(), since we provide 
...
\end{tabbing}
Note that here one must provide the value \trl{m}, it cannot be PETSC\_DECIDE and
...
VecTDotEnd(). \findex{VecTDotBegin()} \findex{VecTDotEnd()}
...
\ref{fig_daao}.  In addition, local numbering (on a single processor) 
...
AO mappings, it is crucial that all processors that
...
The AO context should be destroyed with \trl{AODestroy(AO ao)}
...
\trl{ISGlobalToLocalMappingApply()} a second time to set the values.
...
% See Section \ref{sec_bdiag} for sparse matrix
...
that are owned by neighboring processors).  Figure~\ref{fig_ghosts}
...
Note that the Fortran interface differs slightly; see Section~\ref{sec_fortranarrays}
for details.
\item
...
processors that will be using the IS.
...
\findex{VecSetValues()}, that is, VecGetValues(). \findex{VecGetValues()}
...
placed in the array 
values. In this example each processor now has the 100th and 
...
column (\trl{in}) indices set with MatSetValues() are sorted (in increasing order),
...
than the global index of the last local row) will be stored on the local processor.
...
three processors to indicate how this may be done for the MATMPIAIJ
...
\end{tabbing}
where \trl{viewer} was obtained with PetscViewerDrawOpenX().
...
interface routine (for example, MatMult() $ \rightarrow $ \trl{MATOP_MULT}).  All
...
The object SLES is the heart of PETSc, because it provides uniform and efficient access 
...
breakdown was detected.  Section~\ref{section_convergencetests} gives
...
\end{tabbing}
before calling SLESSolve() to perform any setup required for 
...
right-hand-side vectors, the user should simply call SLESSolve()
...
SLESSetOperators() and SLESSolve() for each solve.  See
Section~\ref{sec_usingsles} for a description of various flags for
SLESSetOperators() that can save work for such cases.
...
call to SLESSolve() (or KSPSolve()). To use a nonzero 
...
KSPTrueMonitor() prints \findex{KSPTrueMonitor()}
...
\trl{-pc_ilu_ordering_type} \trl{order}) computed in the first factorization to be reused
...
\begin{tabbing}
   \trl{-pc_ilu_levels <levels>}\\
   \trl{-pc_ilu_reuse_ordering}\\
   \trl{-pc_ilu_use_drop_tolerance <dt>,<dtcol>,<dtcount>}\\
   \trl{-pc_ilu_reuse_fill}\\
   \trl{-pc_ilu_in_place}\\
   \trl{-pc_ilu_nonzeros_along_diagonal}\\
   \trl{-pc_ilu_diagonal_fill
}
\end{tabbing}
...
MatCreate() with the option \trl{-mat_mpirowbs}, or by directly
...
m}, or determined by PETSc if \trl{m} is PETSC\_DECIDE.  The
...
allocation.  
%The argument \trl{proci} is an optional BlockSolve95
...
processor before doing a ghost point update from the other processors)
...
\begin{tabbing}
   \trl{-pc_lu_ordering_type <ordering>}
\end{tabbing}
...
user {\em must} explicitly call SLESSetUp() \findex{SLESSetUp()} 
before calling PCBJacobiGetSubSLES() or
PCASMGetSubSLES().
...
\trl{-pc_asm_type} \trl{[basic,interpolate,restrict,none]}.
...
\trl{PC_ASM_NONE} (or \trl{-pc_asm_type} \trl{none}) ignores off-processor valies
...
For example, \trl{-sub_0_pc_ilu_fill} \trl{0}.
...
By default the inner SLES preconditioner uses the outer preconditioner matrix 
...
\end{tabbing}
or at the command line with \trl{-pc_sles_true}. \findex{-pc_sles_true}
...
\begin{tabbing}
MatNullSpaceCreate(MPI\_Comm,PetscTruth hasconstants,int dim,Vec *basis,\\
MatNullSpace *nsp);
\end{tabbing}
...
One then tells the PC object you are using what the null space is with the call
\begin{tabbing}
PCNullSpaceAttach(PC pc,MatNullSpace nsp);
\end{tabbing}
The PETSc solvers will now handle the null space during the solution processor.

\emph{But} if one chooses a direct solver (or an incomplete factorization)
it may still detect a zero pivot. \sindex{zero pivot}
You can run with the additional options \trl{-pc_lu_damping <dampingfactor>} 
\findex{-pc_lu_damping} or \trl{-pc_ilu_damping <dampingfactor>} \findex{-pc_ilu_damping} to 
prevent the zero pivot. A good choice for the \trl{damping} factor is 1.e-10.
\sindex{damping}

% ---------------------------------------------------------------
\section{Using PETSc interface to external linear solvers}
...
\item Enable using the external software from PETSc by editing \trl{${PETSC_DIR}/bmake/${PETSC_ARCH}/packages}.
...
\item Use the runtime option: \trl{-mat_<mattype>_<package>} \trl{-ksp_type preonly} \trl{-pc_type <pctype>}.
\end{enumerate}

\begin{table}[H]
\begin{center}
\begin{tabular}{llll}
{\bf Package}    &{\bf MatType}  & {\bf PCType} & {\bf Runtime Options}\\
\hline
DSCPACK          & baij         & cholesky     &  \trl{-mat_baij_dscpack}\\
MUMPS            & aij          & lu           &  \trl{-mat_aij_mumps}\\
                 & sbaij        & cholesky     &  \trl{-mat_sbaij_mumps}\\
SPOOLES          & aij          & lu           &  \trl{-mat_aij_spooles}\\
                 & sbaij        & cholesky     &  \trl{-mat_sbaij_spooles}\\
SUPERLU          & aij          & lu           &  \trl{-mat_aij_superlu} \\
SUPERLU\_DIST    & aij          & lu           &  \trl{-mat_aij_superlu_dist}\\
UMFPACK          & aij          & lu           &  \trl{-mat_aij_umfpack}\\
\hline
\end{tabular}
\end{center}
...
SLESSetOperators()}, discussed in Section~\ref{sec_usingsles}.  
...
discussion of SLESSetOperators() in Section~\ref{sec_usingsles} for
details.
...
The method \trl{SNESLS} (\trl{-snes_type} \trl{ls}) provides a line
...
\trl{-snes_ls [cubic, quadratic, basic, basicnonorms]}. \findex{-snes_ls}
...
equations, \trl{SNESTR} (\trl{-snes_type} \trl{tr}), is taken from
...
SNES monitor routine, SNESDefaultMonitor(), \findex{SNESDefaultMonitor()}
...
options discussed in Chapter~\ref{ch_sles}, such as 
...
in combination with no preconditioning (\trl{PCNONE} or \trl{-pc_type} \trl{none}),
a user-provided preconditioner matrix, or a user-provided preconditioner
shell (\trl{PCSHELL}, discussed in Section~\ref{sec_pc}); that is,
...
or 
\begin{tabbing}
   -snes\_mf\_compute\_norma\trl{ <true or false>}\\
   -snes\_mf\_compute\_normu \trl{<true or false>}
\end{tabbing}
...
\begin{tabbing}
 ...
 int (*f)(TS,double,Mat*,Mat*,MatStructure*,void*),void *fP);
\end{tabbing}
...
\begin{tabbing}
 TSSetRHSJacobian(TS ts,Mat A, Mat B,int (*fjac)(TS,double,Vec,Mat*,Mat*,\\
...
\end{tabbing}
or use the command line option \trl{-ts_type} \trl{pvode}. \findex{TS_PVODE} 
...
PVODE does not use the SNES library within PETSc for its nonlinear
...
database with \trl{-ts_pvode_gmres_restart} \trl{restart}. \findex{-ts_pvode_gmres_restart}
...
\begin{tabbing}
  ...
  /*  Tell the DMMG we are solving a linear problem (hence SLES) and provide the \\
...
For nonlinear problems one replaces the DMMGSetSLES() with 
\begin{tabbing}
...
\begin{tabbing}
  ...
  int (*localfunction)(DALocalInfo *info,void *x,void *f,void* appctx),\\
  int (*localjacobian)(DALocalInfo *,void *x,Mat J,void *appctx),ad\_function,ad\_mf\_function);
\end{tabbing}
...
your call to PetscViewerSocketOpen()), then \trl{a = receive(p);} returns the object you have passed from PETSc.
...
PETSc also provides mechanisms to load ESI classes directly from dynamic libraries to be
used with PETSc.
\begin{tabbing}
...
\end{tabbing}
where \trl{name} is the full name of the class; for example, \trl{esi::petsc::Vector}
or \trl{Petra_ESI_Vector}. These are implemented by loading a factory which then creates the
...
\end{tabbing}
Note that the \trl{is} in \trl{-is_esi_type} is short for IndexSpace, {\bf not} PETSc IS objects.
...
as KSPSetConvergenceTest(), that routine is assumed to be a
...
\begin{tabbing}
   ex3: ex3.o \\
   \trl{      -${FLINKER}} -o ex3 ex3.o  \${PETSC\_FORTRAN\_LIB} \${PETSC\_LIB}\\
   \trl{       ${RM}} ex3.o
\end{tabbing}
...
PetscInitialize()} and PetscFinalize() can be \findex{PETSC_USE_LOG}
...
SLES module are composed of lower-level operations in these
packages.  Note also that the nonlinear solvers library, SNES, 
is build on top of the SLES module, and the timestepping
...
\> \bu SLESSetUp - Set up linear solver\\
...
\> \bu SLESSolve - Solve linear system\\
...
\findex{-log_summary} as shown within Figures~\ref{fig_exparprof} and
~\ref{fig_exparprof2}, which present the combined output generated by
...
operations, as given by the routines VecScatterBegin() and \trl{
VecScatterEnd()}. 
%
%  Temp removed
%
...
Section~\ref{sec_matsparse}, \trl{-log_info} activates the
...
\begin{tabbing}
   PetscLogInfo(snes,\trl{"Cubically determined step, lambda '%'g\n"},lambda);
\end{tabbing}
...
events at runtime for matrix and linear solver libraries via \trl{-log_info [no_mat, no_sles]}.
...
In addition, as discussed in Section~\ref{sec_profileuser},
...
MatCreateSeqAIJ()} and MatCreateMPIAIJ() for compressed, sparse
...
determine a good value for f is to run a program with the option \trl{-log_info}.
...
is \trl{-pc_lu_fill} \trl{<fill_amount\trl{>}}.
...
As discussed in Chapter~\ref{ch_sles}, the default linear solvers are
\begin{itemize}
...
option \trl{-trdump} merely activates a call to PetscTrDump() during
...
\trl{-trmalloc_log} activates calls to PetscTrLog(), \trl{
PetscTrLogDump()}, and PetscGetResidentSetSize() during \trl{
...
\section{PETSc on a processor subset}
...
\end{tabbing}
{\em before} calling \trl{PetscInitialize()}, but, obviously, after
calling \trl{MPI_Init()}. \trl{PetscSetCommWorld()} can be called
at most once per process. Most users will never need to use the routine
\trl{PetscSetCommWorld()}.
...
\begin{tabbing}
    -ksp\_type tfqmr
\end{tabbing}
to select the Transpose-Free QMR algorithm. (See Chapter~\ref{ch_sles} for details.)
...
specified.  \findex{-optionstable} A related option is \trl{-options_left},
...
\begin{tabbing}
     PetscViewerMatlabOpen(MPI\_Comm comm,char *machine,int port,PetscViewer *viewer). \findex{PetscViewerMatlabOpen()}
\end{tabbing} 
...
% Section \ref{sec_bdiag} gives details.
...
than \trl{<tolerance\trl{>}}, PETSc will generate an error. The usual runtime
...
complex\trl{<}double\trl{>}} for machines using templated complex class
...
command \findex{PetscDrawOpenX()} \sindex{X windows}
\begin{tabbing}
...
\end{tabbing}
{\em all} processors must call \findex{PetscDrawSynchronizedFlush()}
\begin{tabbing}
...
\findex{-snes_xmonitor} See also Sections \ref{sec_kspmonitor} and
\ref{sec_snesmonitor}. 

The two functions used are KSPLGMonitor() \findex{KSPLGMonitor()}
and KSPLGMonitorCreate()  \findex{KSPLGMonitorCreate()}. These 
...
\begin{tabbing}
   include \$\trl{{PETSC_DIR}/bmake/common/base}
\end{tabbing}
...
\item \trl{EXAMPLES_1} - examples that will be built with
             \trl{make BOPT=[g,O] examples} (see Section~\ref{sec_common})
\item \trl{RUNEXAMPLES_1} - examples that will be run with
             \trl{make} \trl{runexamples} (see Section~\ref{sec_common})
\item \trl{EXAMPLESC} - all C examples that will be checked in/out of RCS
             with \trl{make} \trl{ci} and \trl{make} \trl{co} (not generally
             needed by users).
\item \trl{EXAMPLESF} - all Fortran examples that will be checked in/out of
             RCS with \trl{make} \trl{ci} and \trl{make} \trl{co} (not generally
             needed by users).
...
SLES interface, as discussed in Chapter \ref{ch_sles}, but the underlying 
...
command line option \trl{-pc_lu_ordering_type} \trl{sname}.  If reordering directly,
