...
of Equation~(\ref{eq_F=0}) or (\ref{eq_minF}).
...
and (\ref{eq_n2}) at each iteration can be costly, modifications 
...
one does not call the DrawOpenX() command discussed below.
...
to pause \trl{n} seconds at each DrawPause(). A time of \trl{-1}
