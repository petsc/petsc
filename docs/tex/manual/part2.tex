% $Id: part2.tex,v 1.169 1996/11/16 00:45:57 bsmith Exp bsmith $ 
%
% NOTES:  
...
\findex{INSERT_VALUES} \findex{SCATTER_ALL}
...
the the {\tt AO} \findex{AO} routines. In the former case they provide a mapping 
...
In addition to all of the conventional vector operations, the support
...
routine for viewing discrete function vectors is \findex{DFVecView}
\begin{verbatim}
...
One routine for visualization of vector fields is \findex{DFVecDrawTensorContoursX}
\begin{verbatim}
...
vector.
Another routine for viewing vector fields is \findex{DFVecDrawTensorSurfaceContoursVRML}
\begin{verbatim}
...
Once can cancel all hardwired monitoring routines for KSP at runtime with the 
option {\tt -ksp\_cancelmonitors}. \findex{-ksp_cancelmonitors}
...
\end{verbatim}
indicates \findex{MGSetType()} \findex{Multiplicative} which
\findex{Additive} \findex{Full Multigrid} \findex{Kaskade} form of
...
Once can cancel all hardwired monitoring routines for SNES at runtime with the 
option {\tt -snes\_cancelmonitors}. \findex{-snes_cancelmonitors}
...
There are two parameters one may set to control how the Jacobian matrix vector
...
the \findex{SNESSetMatrixFreeParameters()}
...
In certain situations, where, for example, one is unable to compute the Jacobians
...
One need not use the routine {\tt MatGetColoring()} to provide the coloring, if, for example,
...
{\tt COLORING\_LF}. \findex{-mat_coloring} \findex{COLORING_SL} \findex{COLORING_ID}
\findex{COLORING_LF}
...
\end{verbatim}
The parameter {\tt rerror} is the square root of 
the \findex{MatFDColoringSetParameters()}
relative error in the function evaluations; the default is $ 1.0e-8 $ , which assumes
that the functions are evaluated to full double precision accuracy. The 
second parameter, {\tt umin}, is a bit more involved, it's default is 
$ 1.0e-8 $ . The Jacobian matrix vector product is via the formula
...
Users can add their own reordering routines for 
...
The preconditioner can be used in one of five ways: 
\findex{PCApply()} \findex{PCApplyTrans()} \findex{PCApplyBAorAB()}
...
If users develop \sindex{Emacs} \sindex{etags, in Emacs}
...
If users develop \sindex{VI} \sindex{ctags, in VI}
