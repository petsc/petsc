% $Id: part2.tex,v 1.339 2000/06/23 15:47:52 bsmith Exp bsmith $ 
%
% NOTES:  
...
by {\tt M} and {\tt N} and the local dimensions, given by {\tt m} and 
{\tt n} while PETSc completely controls memory allocation.  This routine
...
{\tt \$\{PETSC\_DIR\}/include/petscmat.h} provides a complete list of matrix
...
{\tt \$\{PETSC\_DIR\}/include/petscmat.h} for other available matrix types;
...
command
\begin{verbatim}
  ierr = MatCreateMPIAdj(MPI_Comm comm,int mlocal,int n,int *ia,int *ja, MatScalar *weights,Mat *Adj);
\end{verbatim}
...
\begin{itemize}
\item {\tt MATORDERING\_NATURAL} - Natural
\item {\tt MATORDERING\_ND} - Nested Dissection
\item {\tt MATORDERING\_1WD} - One-way Dissection
\item {\tt MATORDERING\_RCM} - Reverse Cuthill-McKee
\item {\tt MATORDERING\_QMD} - Quotient Minimum Degree
\end{itemize}
...
one of the following:  {\tt -mat\_ordering\_type natural, -mat\_ordering\_type nd, 
...
{\tt -pc\_type sles -sles\_pc\_type ilu -sles\_ksp\_max\_it 5 -sles\_ksp\_type bcgs}. 
...
when calling {\tt MatGetColoring()}. \findex{-mat_coloring} \findex{MATCOLORING_SL} \findex{MATCOLORING_ID}
...
of the line. A list of predefined colors may be found in {\tt include/petscdraw.h}
...
MatView() functions with a viewer type of ASCII (see {\tt ViewerASCIIOpen()}, 
{\tt VIEWER\_STDOUT\_WORLD}, {\tt VIEWER\_STDOUT\_SELF}, or {\tt VIEWER\_STDOUT\_(MPI_Comm)}). Before calling
the viewer set the output type with {\tt ViewerSetFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name)} or
{\tt ViewerPushFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name)}. \findex{VIEWER_FORMAT_ASCII_MATLAB}
\begin{verbatim}
...
One creates a viewer to Matlab via {\tt ViewerSocketOpen(MPI_Comm,char *machine,int port,Viewer *v)}
(port is usally set to {\tt PETSC\_DEFAULT}) and then sends matrices or vectors via
\begin{verbatim}
  VecView(Vec A,v);
  MatView(Mat B,v);
\end{verbatim}
...
{\tt PetscStartMatlab(MPI_Comm,char *machine,char *script,FILE **fp);} where machine and script may be {\tt PETSC\_NULL}.

To receive the objects in Matlab you must first make sure that {\tt \${PETSC\_DIR}/src/sys/src/viewer/impls/socket/matlab}
is in your Matlab path. Use {\tt p = openport;} (or {\tt p = openport(portnumber) if you provided a port number in
your call to {\tt ViewerSocketOpen()}), then {\tt a = receive(p);} returns the object you have passed from PETSc.
{\tt receive()} may be called any number of times. You many call {\tt closeport()} to close the connection from Matlab.
It is also possible to start your PETSc program from Matlab via {\tt launch()}.

\section{Using the Matlab Compute Engine}

One creates access to the Matlab engine via {\tt PetscMatlabEngineCreate(MPI_Comm comm,char *machine,PetscMatlabEngine *e);}
({\tt machine} may be {\tt PETSC\_NULL}).
One can send objects to Matlab via {\tt PetscMatlabEnginePut(PetscMatlabEngine e,PetscObject obj);} One can get objects
via {\tt PetscMatlabEngineGet(PetscMatlabEngine e,PetscObject obj);}. Similarly one can send arrays via
{\tt PetscMatlabEnginePutArray(PetscMatlabEngine e,int m,int n,Scalar *array,char *name);} and get them back via
{\tt PetscMatlabEngineGetArray(PetscMatlabEngine e,int m,int n,Scalar *array,char *name);} One cannot use Matlab
interactively in this mode but you can send Matlab commands via
{\tt PetscMatlabEngineEvaluate(PetscMatlabEngine,''format'',...);} where {\tt format} has the usual {\tt printf()} format.
Responses can be returned from Matlab via {\tt PetscMatlabEngineGetOutput(PetscMatlabEngine,char **);} or 
{\tt PetscMatlabEnginedPrintOutput(PetscMatlabEngine,FILE*)}. There is a short-cut to starting the Matlab engine
with {\tt MATLAB\_ENGINE\_(MPI\_Comm)}.
...
{\tt PETSC\_HAVE\_FORTRAN\_UNDERSCORE} and {\tt PETSC\_HAVE\_FORTRAN\_CAPS}
...
\end{verbatim}
Analogous include files for other components are {\tt petscda.h90},
{\tt petscmat.h90}, and {\tt petscis.h90}; the conventional Fortran style
...
PetscInitialize()} and {\tt PetscFinalize()} can be \findex{PETSC_USE_LOG}
...
In particular, the macros {\tt PreLoadBegin(PetscTruth,char* stagename)}, {\tt
PreLoadStage(char *stagename)}, and {\tt PreLoadEnd()} can be used to easily 
convert a regular PETSc program to one that uses preloading. The command line options
{\tt -preload true} and {\tt -preload false} may be used to turn on and off
...
routines set {\tt flg=PETSC\_TRUE} if the corresponding option was found, {\tt flg=PETSC\_FALSE} if it
...
{\tt BOPT=[g\_complex,O\_complex]} for debugging, optimized,
...
architecture, {\tt arch}, is one of {\tt solaris, rs6000, IRIX,
hpux}, etc. Note
...
{\tt BOPT=O\_c++}.  \sindex{C++}
The options {\tt BOPT=g\_complex} and {\tt BOPT=O\_complex}
...
directory is called {\tt solaris}.  Each architecture directory contains
two base makefiles:
\begin{itemize}
...
\item {\tt base\_variables} - definitions of the compilers, linkers, etc.
\end{itemize}
...
like  {\tt \$\{PETSC\_SYS\_LIB\}}, {\tt \$\{PETSC\_VEC\_LIB\}}, 
{\tt \$\{PETSC\_MAT\_LIB\}}, {\tt \$\{PETSC\_DM\_LIB\}},
{\tt \$\{PETSC\_SLES\_LIB\}}, {\tt \$\{PETSC\_SNES\_LIB\}} or
{\tt \$\{PETSC\_TS\_LIB\}}.
...
as follows:
\begin{itemize}
\item {\tt CFLAGS, FFLAGS} - User specified additional options for the C compiler and
        fortran compiler.
\item {\tt CPPFLAGS, FPPFLAGS} - User specified additional flags for the C preprocessor
...
\item {\tt EXAMPLES\_1} - examples that will be built with
             {\tt make BOPT=[g,O] examples} (see Section~\ref{sec:common})
...
input argument {\tt inname} is a string of the user's choice, {\tt iname} is either 
an ordering defined in {\tt petscmat.h} or a users string,
...
command line option {\tt -mat\_ordering\_type sname}.  If reordering directly,
...
\end{verbatim}
The argument {\tt info->fill $ \ge $ 1} is the predicted fill
expected in the factored matrix, as a ratio of the original fill. 
For example, {\tt info->fill=2.0} would indicate that one expects the factored
...
on whether the \sindex{Richardson's method} {\tt right} is
{\em PC\_LEFT} or {\em PC\_RIGHT}. The final routine applies {\tt its} iterations of
