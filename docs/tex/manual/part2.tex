% $Id: part2.tex,v 1.338 2000/05/08 15:10:06 balay Exp bsmith $ 
%
% NOTES:  
...
MatView() functions with a viewer type of ASCII (see ViewerASCIIOpen(), 
VIEWER\_STDOUT\_WORLD, VIEWER\_STDOUT\_SELF, or VIEWER\_STDOUT\_(MPI_Comm)). Before calling
the viewer set the output type with ViewerSetFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name) or
ViewerPushFormat(viewer,VIEWER_FORMAT_ASCII_MATLAB,char *name). \findex{VIEWER_FORMAT_ASCII_MATLAB}
\begin{verbatim}
...
(port is usally set to PETSC\_DEFAULT) and then sends matrices or vectors via
\begin{verbatim}
  VecView(Vec A,v);
  MatView(Mat B,v);
\end{verbatim}
