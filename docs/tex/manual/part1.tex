...
and version 3.2 of PETSc, availalble at \url{http://www.mcs.anl.gov/petsc}.
PETSc must be configured without complex variables, but it is now possible 
for TAO to work when PETSc is configured without a C++ compiler or with
...
\]
where $n = 2m$ is the number of variables.  
...
Section~\ref{chapter:fortran} discusses additional issues concerning
Fortran usage.
...
handling, error handling, compiling system and compiling system.  We provide
here a quick overview of some of these PETSc features, please reference
the PETSc manual for a more in-depth discussion of PETSc.
...
runtime throught the PETSc options database. This gives the application
