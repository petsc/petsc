% $Id: part1.tex,v 1.136 2000/05/08 15:10:06 balay Exp bsmith $ 
...
\url{PETSC_DIR}, \findex{PETSC_DIR} indicating the full path of the PETSc home
...
\findex{-h} or \url{-help} option as well as the \url{-v} \findex{-v}
...
\item \url{-fp_trap} - stop on floating-point exceptions; \findex{-fp_trap}
...
\item \url{-on_error_attach_debugger}  \url{[noxterm,gdb,dbx,xxgdb]}
      \url{[-display name]} - \findex{-on_error_attach_debugger}start debugger only on encountering an error
\end{itemize}
...
programs. \sindex{command line arguments} The argument \url{file_name}
...
As illustrated by the \url{PetscInitialize()} statements above,
...
All PETSc programs should call \url{PetscFinalize()} \findex{PetscFinalize()}
...
the program, and calls \url{MPI_Finalize()} \findex{MPI_Finalize()}
...
\url{VecSetType()} or \url{VecSetFromOptions()}. \findex{VecSetType()} \findex{VecSetFromOptions}
...
macro, \url{CHKERRA(ierr)}, should be used in the main program to
...
As shown in Figure \ref{fig:exprof}, the option \url{
...
% \item \url{petscview} (GUI utility for high-level visualization of program activity),
% \item \url{petscopts} (GUI utility for setting runtime options), and
% \item \url{petscdemo} (GUI utility the provides a brief PETSc demo).
 \end{itemize}
% Note that \url{petscview}, \url{petscopts}, and \url{petscdemo} use Tcl/Tk.
