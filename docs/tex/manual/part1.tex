% $Id: part1.tex,v 1.141 2001/04/06 02:59:34 bsmith Exp bsmith $ 
...
\trl{${PETSC_DIR}/docs} contains all documentation.
...
The file \trl{manual.ps} contains the PostScript form of
the {\em PETSc 2.0 Users Manual} in its entirety, while \trl{intro.ps} 
...
\trl{http://www.mcs.anl.gov/petsc}.  The PETSc home page also
...
All PETSc 2.0-compliant programs support the use of the \trl{-h}
...
\begin{verbatim}
  PetscInitialize(int *argc,char ***argv,char *file_name,char *help_mess);
\end{verbatim} 
which initializes PETSc and MPI.  The arguments \trl{argc} and 
\trl{argv} are the command line arguments delivered in all C and C++
programs. \sindex{command line arguments} The argument \trl{file_name}
...
customization. The final argument, \trl{help_mess}, is an optional
...
As illustrated by the \trl{PetscInitialize()} statements above,
...
\trl{${PETSC_DIR}/src/sles/examples/tutorials/ex1.c},
...
\begin{verbatim}
   #include "petscsles.h"
\end{verbatim}
where \trl{petscsles.h} is the include file for the SLES component.
...
example, \trl{petscsles.h} includes \trl{petscmat.h} (matrices),
\trl{petscvec.h} (vectors), and \trl{petsc.h} (base PETSc file).  
The PETSc include files are located in the directory 
\trl{${PETSC_DIR}/include}.  See Section \ref{sec:fortran_includes}
...
\trl{OptionsGetInt(PETSC_NULL,"-n",&n,&flg);} checks whether the user has
...
\end{verbatim}
where \trl{comm} denotes the MPI communicator. The type of storage
...
Note the use of the PETSc variable type \trl{Scalar} in this example.
The \trl{Scalar} is simply defined to be \trl{double} in C/C++
...
The \trl{Scalar} data type enables
...
\trl{Ax = b}, the user can then use SLES to solve the system 
...
the SLES context.  We emphasize the command \trl{SLESSetFromOptions()}, 
...
\ref{ch:snes} describes the nonlinear solvers in detail. We recommend 
...
\trl{ex3.c} on line 49.  See Section \ref{sec:fortran_errors} for
...
\trl{${PETSC_DIR}/src/sles/examples/tutorials/ex2.c}, handles the
...
\trl{http://www.mcs.anl.gov/petsc} or
given in the file \trl{${PETSC_DIR}/docs/troubleshooting.html}.
...
As shown in Figure \ref{fig:exprof}, the option \trl{
...
section), such as \trl{snes} or \trl{sles}.  
...
\item Copy one of the many PETSc examples in the component directory
      ...
      for linear solvers, see \trl{${PETSC_DIR}/src/sles/examples/tutorials}).
...
\begin{itemize}
\item \trl{docs} - All documentation for PETSc. The files \trl{manual.ps}
                   and \trl{manual.html} contain the users manual in
...
\item \trl{include/foldinclude} - PETSc include files for Fortran programmers using 
                                  the .f suffix.
\item \trl{include/pinclude}    - Private PETSc include files that should {\em not} 
                                  be used by application programmers.
\item \trl{src} - The source code for all PETSc components, which
                  currently includes
 \begin{itemize}
 \item \trl{vec} - vectors,
   \begin{itemize}
 ...
 \item \trl{dm}
   ...
   \end{itemize}
 \item \trl{sles} - complete linear equations solvers,
 \begin{itemize}
