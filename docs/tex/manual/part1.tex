% $Id: part1.tex,v 1.90 1996/10/28 15:54:14 balay Exp bsmith $ 
...
UNIX man pages for all PETSc functions can be
accessed with the command {\tt \$(PETSC\_DIR)/bin/petscman
[-xman, -netscape, -xmosaic]}.  The options {\tt -xman} (the default),
{\tt -netscape}, and {\tt -xmosaic} respectively indicate viewing man pages
via xman, Netscape, or Mosaic.  We recommend using one of the HTML
versions, since this format provides hyperlinked indices (organized by
...
manual and the man pages in UNIX, PostScript, and HTML
...
\end{verbatim}
can be placed in a .cshrc file.  Thus, even if several machines of different
types share the same filesystem, {\tt PETSC\_ARCH} will be set correctly
when logging into any of them. 

You must also set the environmental variable {\tt PETSC\_DIR} to 
indicate the location of the PETSc root directory, for example, \findex{PETSC_DIR}
\begin{verbatim}
...
or {\tt -version} option. For C and C++ programs these options can be
placed on the \findex{-help} \findex{-version} command line.  For most
machines, options for Fortran programs can also be specified on the
command line; otherwise, they can be set in the environmental
variable {\tt PETSC\_OPTIONS} or placed in a file called {\tt
.petscrc} in the user's home directory.  Under the UNIX C shell the
environmental variable can be set \findex{PETSC_OPTIONS}
...
calling {\tt MPI\_Init()}. {\tt PetscSetCommWorld()} can only be called
...
{\small
\begin{verbatim}
   eagle>mpirun ex3 -m 100000000000
...
\begin{verbatim}
   ...
   rm -f ex1.o
   eagle> mpirun ex1
   Norm of error 8.40696e-15 Iterations 5
   eagle>
   eagle> mpirun -np 2 ex2
   Norm of error 1.08779e-15 Number of iterations 4
\end{verbatim}
}
\nobreak
...
\item {\tt docs} - All documentation for PETSc. The files {\tt manual.ps}
                   ...
                   the subdirectories
 \subitem - {\tt man} (UNIX man pages), and
