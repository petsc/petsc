% $Id: developers.tex,v 1.9 1998/06/01 21:38:07 bsmith Exp bsmith $ 
%
...
\ANLTitle{ALICE Component Developers Manual}{\em Satish Balay\\
William Gropp\\ Lois Curfman McInnes\\ Barry Smith\\
\hspace{0.3in}\\
Mathematics and Computer Science Division \\
http://www.mcs.anl.gov/petsc
\vspace{0.5in} \\
{\rm This document is intended for use with PETSc 2.0.22}}
...
PETSc error handling uses a ``call error handler; then (depending on result) return
...
\end{verbatim}
it simply calls the error handler with the current function name and location: line number,
file and directory.
{\tt CHKERRQ()} is defined by
\begin{verbatim}
...
generic error code {\tt PETSC_ERR_MEM} is used whenever requested memory allocation
...
composed with any PETSc object.
