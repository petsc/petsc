% $Id: developers.tex,v 1.5 1998/04/28 04:43:42 curfman Exp bsmith $ 
%
...
to (as painlessly as possible) contribute their own libraries to.
...
these initiatives progress. Here should list specific individuals and groups
...
{\tt <component>Operation\_Implementation}, for instance, 
...
\item {\tt composefunction(PetscObject,char *name,char *fname,void *)} associates a function
...
\item {\tt queryfunction(PetscObject,char *name,void **)} retreives a function pointer that
...
Since this model only allows one to compose PETSc objects with PETSc objects
rather than any arbitrary pointer, PETsc provides the convenience object {\tt PetscObjectContainer},
created with the routine {\tt PetscObjectContainerCreate(MPI_Comm,PetscObjectContainer)} to 
allow one to wrap any kind of data into a PETSc object that can then be composed with 
any PETSc object.

The introduction of the seperate {\tt composefunction()} routine is to easily support
the loading of functions when they are actually used as opposed to when they are 
first referred to.
...
{\tt \$\{PETSC\_DIR\}/include/petsclog.h}.
