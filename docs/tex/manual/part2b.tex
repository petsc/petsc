...
\section{Linearly Constrained Lagrangian}
\label{sec:lcl}
...
\texttt{xkin} above, the following inputs are required:
\begin{description}
  \item[\texttt{n}] the problem dimension/number of
  variables ($\geq 1$, integer);
  
  \item[\texttt{m}] the number of residual components ($\geq 1$, integer);

  \item[\texttt{delta}] the initial trust region radius ($>0$, real).
  This is  used to determine the size of the initial neighborhood within
  which the algorithm should look. In particular, if \texttt{nfs}=0, the
  algorithm will evaluate $F$ at $n$ points within a distance
  \texttt{delta} of \texttt{X0};
  
  \item[\texttt{nfmax}]  the maximum number of function evaluations
  allowed. This is the typical termination criteria;
  
  \item[\texttt{gtol}] The termination tolerance for the 2-norm of the
  ...
  \item[\texttt{npmax}] The maximum number of interpolation points used
  ($n+2\leq$ \texttt{npmax} $\leq 0.5(n+1)(n+2)$). This is input is made
  available to advanced users. We recommend other users to use the value 
  \texttt{npmax}$=2n+1$;
   
  \item[\texttt{L}] Vector of lower bounds on $x$ (can be arbitrarily
  large if the problem is unconstrained);

  \item[\texttt{U}] Vector of upper bounds on $x$ (can be arbitrarily
  large if the problem is unconstrained);
  
  \item[\texttt{printf}]  Controls the level of output to the screen.
  \textbf{****Need more on the options here!******}
\end{description}

In addition the user provides a function for evaluating the residual
vector $F$.  \textbf{****Describe this in detail or say******} This is
described in detail in Section~\ref{sec:}.
...
and solve (\ref{eq:poundersp}) with the XXXx method described in
Section~\ref{sec:}. \textbf{***Jason, the new version uses $p=\infty$,
your version may use $p=2$.}
...
\texttt{Jason, is this roughly what you had in mind? I could also write a
sentence on termination but this is described both in the inputs section
and in the algorithm outline.}
