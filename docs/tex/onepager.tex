% $Id: onepager.tex,v 1.20 1996/02/12 20:46:31 bsmith Exp bsmith $ 
...
\vsp
\item \code{TS} - time-stepping code for scalably solving 
      nonlinear ODEs arising from applying general ``methods of lines'' 
      techniques to time dependent PDEs. Includes support for pseudo 
      time-stepping. 
\end{itemize}
\vsp
...
RS/6000 workstations; IBM PCs running FreeBSD or Linux; the Cray T3D/T3E,
the Intel Paragon; and the IBM SP, SGI Origin, Convex Exemplar, Windows NT.

\section*{Availability}

The \code{PETSc} package is freely available by anonymous ftp from 
\URL{ftp://info.mcs.anl.gov/pub/petsc}.
The \code{PETSc} distribution contains all source code, 
a users guide, {\tt man pages}, and a
collection of examples. 

\vspace{-.1cm}
\contact{Satish Balay, William Gropp, Lois Curfman McInnes, Barry Smith}
\email{petsc-maint@mcs.anl.gov}
\phone{http://www.mcs.anl.gov/petsc/petsc.html}
\makeinfo
\end{document}
