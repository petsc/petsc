...
In many cases, each iteration involve calculating a \emph{search direction}, 
then function values and gradients along that direction are calculated until
...
\item Model-based methods \remp{In beta!}
...
\begin{minipage}[b]{.45\linewidth}
%\ifpdf
\includegraphics[height=1.2in]{../images/pjb_e}
%\else
%\fi
\end{minipage}
...
\centerline {\includegraphics[height=3.5in]{../images/tao_pic4}}
}
...
\frametitle{TAO Examples}
TAO has some example applications (in C and Fortran) included in the source distribution
for you to test the TAO installation, learn about TAO features, and reference
for creating your own applications
\begin{table}
\begin{tabular}{ll}
unconstrained  & bound  \\
\hline
eptorsion1.c &  jbearing2.c\\
eptorsion2.c & plate2.c\\
eptorsion2f.F & plate2f.F\\
minsurf1.c\\
minsurf2.c\\
rosenbrock1.c\\
rosenbrock1f.F\\
...
A \emph{driver} program is used to hook up the user's application to the
TAO library.
...
\item TAO 2.0 Beta (including POUNDerS algorithm) can be downloaded from
\url{https://www.mcs.anl.gov/research/projects/tao//download/tao-2.0-beta3.tar.gz}. It requires the petsc
development branch, see PETSc download page for more information on
development version.  TAO 2.0 Beta3 has been tested with petsc revision 1f64e5dc80ba (8/15/11).
