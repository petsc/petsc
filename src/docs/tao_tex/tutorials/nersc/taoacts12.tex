...
\providecommand\thispdfpagelabel[1]{} % fix beamer bug when pdfpagelabels==false
\usepackage{graphicx}
...
\item \textcolor{blue}{Routines to
...
algorithmic options and tolerances and sets up user evaluation routines.}
\end{itemize}

TAO uses \textcolor{red}{Matrix}, \textcolor{red}{Vector}, and \textcolor{red}{KSP} objects from PETSc.
...
\frame{
\frametitle{TAO Applications}
\begin{center}What do you need to do for the \textcolor{blue}{User Routines}?\end{center}
You need to write C, C++, or Fortran functions that:
\begin{itemize}
\item Set the initial variable vector (optional)
...
\begin{table}
\begin{tabular}{lll}
unconstrained  & bound & least-squares  \\
\hline
eptorsion1.c &  jbearing2.c & chwirut1.c\\
eptorsion2.c & plate2.c     & chwirut2.c\\
eptorsion2f.F & plate2f.F   & chwirut1f.F\\
...
\begin{verbatim}
   PetscErrorCode MyFunction(Tao tao, Vec x, 
                       PetscReal *fcnval, void *userCtx){
...
\begin{verbatim}
  PetscErrorCode MyGradient(Tao tao, Vec x, Vec g, 
                       void *userCtx){
     UserContext *user = (UserContext *)userCtx;
     ...
  }
  PetscErrorCode MyHessian(Tao tao, Vec x, Mat *H, 
                  Mat *Hpre, MatStructure *flag, void *userCtx){
     UserContext *user = (UserContext *)userCtx;
     ...
  }
\end{verbatim}
...
\begin{itemize}
\item Create the TAO Solver
\item Create the variable vector and Hessian matrix
\item Hook up the user evaluation routines to the TAO solver
\item Solve the application
\item Analyze the solution
\end{itemize} 
}

\frame[containsverbatim]{
\frametitle{TAO Applications}
\begin{center}Create the TAO Solver\end{center}
\begin{verbatim}
...
\begin{verbatim}
 ...
 TaoGradientRoutine(tao,MyGradient,(void *)&user);
...
\begin{itemize}
\item You can download TAO 2.1 from the webpage \url{https://www.mcs.anl.gov/research/projects/tao/}
