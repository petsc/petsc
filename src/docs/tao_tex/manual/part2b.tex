...
\subsection{Projected Gradient Descent Method}
\label{sec:pgd}

PGD is a steepest descent algorithm with projected gradients and a 
bounded More-Thuente line search. The method can be set by using the 
TAO solver {\tt TAO\_PGD} or the {\tt -tao\_type pgd} flag, and 
serves as a useful worst-case benchmarking tool when developing 
more advanced bound-constrained algorithms.
...
Polak-Ribi\'ere, Polak-Ribi\'ere-Plus, Hestenes-Stiefel, and the Dai-Yuan. These 
conjugate gradient methods can be specified by using the command line argument 
{\tt -tao\_bncg\_type <fr,pr,prp,hs,dy>}, respectively. The default value is {\tt dy}.
...
{\tt -tao\_bnk\_as\_step} flags, respectively.
...
and recommended for the BNLS algorithm. A BFGS approximation is used both to precondition the Krylov 
solution and to construct the step direction for the active-set estimation. The preconditioner 
type can be changed using the {\tt -tao\_bnk\_pc\_type} {\tt none}/{\tt ahess}/{\tt bfgs}/{\tt petsc}.
The {\tt ahess} option results in a Jacobi preconditioner, while {\tt petsc} option allows the 
user to specify a built-in PETSc preconditioner with {\tt -pc\_type}. When the BFGS preconditioner 
is not available, the active-set estimation falls back to using the inverse diagonal of the Hessian.
