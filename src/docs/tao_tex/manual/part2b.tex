...
and the gradient of the objective as $g_k \approx -J_k r(x_k)$.
...
{\tt -tao\_brgn\_lambda} command line option, while the smooth approximation parameter can be set 
with either {\tt TaoBRGNSetL1SmoothEpsilon()} or {\tt -tao\_brgn\_epsilon}. For the L1-norm term, 
...
The regularization selection can be made using the command line option \texttt{-tao\_brgn\_reg\_type <l2prox, l1dict, user>}
where the ``user'' option allows the user to define a custom $\mathcal{C}2$-continuous regularization 
term. This custom term can be defined by using the iterface functions:
\begin{itemize} 
