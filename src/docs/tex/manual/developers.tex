...
\end{lstlisting}
\item The PETSc ASCI output routines, such as \lstinline{PetscPrintf}, format strings take a \%D for all PETSc variables of
type \lstinline{PetscInt}, not a \%d.
\item All argumements of type \lstinline{PetscReal} to PETSc ASCI output routines, such as \lstinline{PetscPrintf}, must be cast to \lstinline{double}, for example
\begin{lstlisting}
...
on different systems thus making portability testing difficult. Instead use the PetscRandom which produces the exact same results regardless of
...
\item There shall be no PETSc routines introduced that provide essentially the same functionality as an available MPI routine.
  ...
  One should use the MPI routine \lstinline{MPI_Allreduce()} directly in the code.
...
Note that PETSc can be configured to build a separate shared library for each top-level class (\lstinline{Mat}, \lstinline{Vec}, \lstinline{KSP}, etc.) and that plugin implementations of these classes can be included as separate shared libraries; thus, private functions may need to be marked \lstinline{PETSC_EXTERN}.
For example,
\begin{itemize}
...
\begin{itemize}
    \item
      \lstinline{/*@}\\      indicates a formatted comment of a function that will be used for
...
The Fortran interface files supplied by user go into the two directories  \trl{ftn-custom} and \trl{f90-custom} while those generated by Sowing go into \trl{ftn-auto}.
...
(see previous section) to generate complete manual pages and Fortran interfaces with Sowing.
...
\begin{itemize}
  ...
  \item \trl{Level: } (no newline) followed by \trl{beginner}, \trl{intermediate}, \trl{advanced}, or \trl{developer}.
...
% -------------------------------------------------------------------------------
\section{PETSc Types}
For maximum flexibility, the basic data types \lstinline{int}, \lstinline{double}, etc. are
...
Normally \lstinline{comm} is \lstinline{PETSC_COMM_SELF}; it can only be another communicator if
...
\begin{tightitemize}
  ...
  \item \lstinline{PetscAbortErrorHandler()}, called with \trl{-on_error_abort}, useful for when running in the debugger,
...
\item \lstinline{view(PetscObject,PetscViewer)} allows one to store or visualize the data inside
...
example \lstinline{KSPInitializePackage()}, which registers all the classes
...
\end{lstlisting}
The routine \lstinline{DMGetDMXXX(DM,DMXXX*)} has the form
\begin{lstlisting}
...
harness, this is explained further below.  The input language
   ...
   \item[]  \textbf{output\_file}: (\emph{Optional}; \emph{Default:}
    ...
    \item[]  \textbf{requires}: (\emph{Optional}; \emph{Default:} \lstinline{""})
      \begin{itemize}
        ...
        DATAFILES path.  \lstinline{requires: datafilespath} can be specifed for these
        tests.
...
\item for loops:  Specifying \lstinline!{{ ... }shared output}!
      ...
      shared (in short hand notation, \lstinline!{{ ... }}!) syntax must be used.
...
Here is a similar example, but the permutation of arguments create
different output:
