...
\item Often, it pays to not ask too many questions or give to many suggestions in the same email. The user often only responds to the first of them.
\end{tightitemize}
...
\item Functions that PETSc provides as defaults for user provideable functions end with \lstinline{Default} (for example, \lstinline{KSPMonitorDefault()} or \lstinline{PetscSignalHandlerDefault()})
...
\item Subroutines that would normally have a \lstinline{void**} argument to return a pointer to some data should actually be protyped as as \lstinline{void*}. This prevents the caller from having to put a \lstinline{(void**)} caste in each function call. See, for example, \lstinline{DMDAVecGetArray()}.
...
\begin{tightitemize}
  ...
  \item \lstinline{PetscBT} - bit storate of logical true and false
\end{tightitemize}
...
\item \lstinline{queryfunction(PetscObject,const char *name,void **func)} retreives a function pointer that
...
\item storing the matrices using a fixed (problem dependent) block size
  (via the \lstinline{MATSEQBAIJ} and \lstinline{MATMPIBAIJ} formats); and
\end{itemize}
