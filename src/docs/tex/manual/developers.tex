...
Before contributing code to PETSc please read Chapter 4, which contains the source code style guide.
...
\\
  PetscFunctionBegin;\\
  ierr = PetscFunctionListFind(obj-$>$qlist,obj-$>$comm,name,PETSC\_FALSE,( int(**)(void *)) ptr);CHKERRQ(ierr);\\
  PetscFunctionReturn(0);\\
\}
\end{tabbing}
...
The \trl{PetscFunctionListAdd\_Private()} and all related routines can be found in the directory
\trl{src/sys/dll}.
...
the KSP package is \trl{-ksp_atol}. In addition, operations in different
...
user can set \trl{nz=0} and \trl{nzz=PETSC_NULL} for PETSc to dynamically
