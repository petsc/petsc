...
In addition to SETERRQ() are the macros SETERRQ1(PETSC_COMM_SELF,), SETERRQ2(), SETERRQ3()
...
\item A data structure defined in  the file
      \trl{include/private/$<$class$>$impl.h}.
...
#endif
\end{verbatim}
\item There shall be no PETSc routines introduced that provide essentially the same functionality as an available MPI routine. For example, one should not write a routine PetscGlobalSum() that takes a scalar value and performs an MPI_Allreduce() on it. One should use MPI_Allreduce() directly in the code. (Note: PetscGlobalSum() does exist but is ONLY to be used in code that is to be run through ADIC (since ADIC cannot handle MPI calls)).
