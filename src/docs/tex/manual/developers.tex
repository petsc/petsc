...
\begin{itemize}
    ...
    \item
      \trl{/*@C}\\    a formatted comment of a function that will be used only for
        documentation, not to generate a Fortran interface, in general such labeled C functions should have a custom Fortran interface provided. Functions that take \trl{char*} or function pointer arguments must have the \trl{C} symbol. 
    \item
    ...
    \item
    \item
      \trl{/*MC}\\    a formatted comment of a CPP macro or enum value for documentation.
\end{itemize}

The Fortran interface files supplied by user go into the two directories  \trl{ftn-custom} and \trl{ftn-f90} while those generated by Sowing go into \trl{ftn-auto},.
...
\begin{itemize}
  ...
  \item (Optional) a \trl{Notes: } section, with a newline after \trl{Notes:}. In-depth discussion, technical caveats, special cases, and so on should be listed here. 
  ...
  \item (If applicable) a \trl{Fortran Notes: } section, with a newline after \trl{Fortran Notes:}, detailing any relevant differences in calling or using the item from Fortran.
  ...
  \item The \trl{.seealso: } A list of related manual pages. These manual pages should usually also point back to this manual page in their seealso section.
\end{itemize}
...
One can direct all error messages to \lstinline{stderr}, instead of the default \lstinline{stdout}, with the command line options \trl{-error_output_stderr}.
...
\item Its own directory, \trl{src/<class>} or \trl{src/<package>/<class>} (see Chapter \ref{ch:packages}).
...
library level packages: \trl{sys}, \trl{Vec}, \trl{Mat}, \trl{DM}, \trl{KSP}, \trl{SNES}, \trl{TS}, \trl{TAO}. Each
...
package defines one or more classes, for example the \trl{KSP} package
defines the \trl{KSP} and \trl{PC} classes, as well as several utility
...
example, \trl{KSPInitializePackage()} which registers all the classes
...
The solver objects in PETSc, KSP (optionally), SNES, and TS require
user provided callback functions (and contexts for the functions) that define the problem to be
solved. These functions are supplied by the user with calls such as
SNESSetFunction(SNES,...) and TSSetRHSFunction(TS,...). One would naturally think
...
SNES object and TS callbacks to the TS object. This is not the
case. Or possibly one might think the callbacks would be attached to
the DM object associated with the solver object. This is also not the
case. Rather the callback functions are attached to an inner nonpublic DMXXX object
(XXX is KSP, SNES, or TS) that is attached to the DM that is attached
to the XXX solver object. This convoluted design is to support
...
back context. One can control exactly what XXX/DM objects share a common DMXXX object.
...
In Figure \ref{fig_dmksp} we depict how three levels of KSP objects share a common DMKSP object. The code to access the inner DMKSP object is
\begin{lstlisting}
  DM    dm_2;
  DMKSP dmksp;
  KSPGetDM(ksp_2,&dm_2);
...
The DMKSP object is essentially the list of callback functions and their contexts.
\begin{lstlisting}
...
We now explore in more detail exactly how the solver calls set by the user are passed down to the inner DMKSP object. For each user
level solver routine for setting a callback there exists a similar routine at the DM level, thus XXXSetY(XXX,...) has a routine DMXXXSetY(DM,...).
\begin{lstlisting}
...
\end{lstlisting}
The implementation of DMXXXSetY(DM,...) gets a ``writable'' version of the DMXXX object via DMGetDMXXXWrite(DM,DMXXX*) and sets the function callback
and its context into the DMXXX object.
\begin{lstlisting}
...
\end{lstlisting}
The routine for DMGetDMXXXWrite(DM,DMXXX*) entails a duplication of the object unless the DM associated with the DMXXX object is the original DM that
the DMXXX object was created with. This can be seen in the following code.
\begin{lstlisting}
...
\end{lstlisting}
The routine DMGetDMXXX(DM,DMXXX*) has the form
\begin{lstlisting}
...
\end{lstlisting}
This routine uses DMCoarsenHookAdd and DMCoarsenHookAdd to attach to the DM object two functions that are automatically called when the object is coarsened or refined.
The hooks DMCoarsenHook\_DMXXX() and DMCoarsenHook\_DMXXX() have the form.
\begin{lstlisting}
...
\end{lstlisting}
insures that the new DM shares the same DMXXX as the parent DM and also inherits the hooks if it is refined or coarsened.

If you provide callbacks to a solver {\em after} the DM associated with a solver has been refined or coarsened those child DM's will not share a common DMXXX.

The TS object manages its callback functions in a way similar to KSP
and SNES though there are no multi-level TS implementations so in
theory the DMTS object is currently unneeded.
...
\end{outputlisting}
\item Examine the scripts and output files there. In this example, these are \trl{runex4_4.sh} and \trl{runex4_4/}, which contains \trl{sys_classes_viewer_tests-ex4_4.sh} along with various output files.
\item By examining these files, one can determine that
\begin{tightitemize}
\item The command to run the test was 
\begin{outputlisting}
/Users/patrick/petsc-master/arch-darwin-master-double-debug/bin/mpiexec  -n 1 ../ex4 -myviewer ascii:ex4a1.tmp::append
\end{outputlisting}
\item The reference output is in
\begin{outputlisting}
