...
\end{itemize}
PetscInt can be set using config/configure.py to be either \trl{int} (32 bit) or \trl{long long}
(64 bit)
...
\item PetscStopErrorHandler(), and
...
\item A data structure defined in  the file
      ...
      private include files in \trl{include/private}, along
      with \trl{petscimpl.h} so that other classes can have direct access to 
...
(in \trl{include/private/petscimpl.h})
...
the convenience object PetscObjectContainer, created with the
routine PetscObjectContainerCreate(MPI\_Comm,PetscObjectContainer)
...
The base PETSc PC object is defined in the \trl{include/private/pcimpl.h} include file. 
...
The base PETSc KSP object is defined in the \trl{include/private/kspimpl.h} include file. 
...
\begin{tabbing}
   ierr = MatCreateMPIBAIJ(MPI\_Comm comm,int nb,int m,int n,int M,int N,int d\_nz,\\
...
$y = A x$. The \trl{m} that one uses in MatCreateMPIBAIJ()
...
nb} $\times$ \trl{nb}, where conventional diagonal storage results for
\trl{nb}=1.  Such storage is particularly useful for multicomponent PDEs
discretized on regular grids.

The routine for creating a uniprocessor block diagonal matrix with \trl{m} 
rows, \trl{n} columns, and a block size of \trl{nb} is
\begin{tabbing}
   ierr = MatCreateSeqBDiag(PETSC\_COMM\_SELF,int m,int n,int nd,int nb,int *diag,\\
                                   Scalar **diagv,Mat *A);
\end{tabbing}
The \findex{MatCreateSeqBDiag} argument \trl{nd} is the number of 
block diagonals, and \trl{diag} is
...
\[ {\tt diag} = i/nb - j/nb. \]
If matrix storage space is allocated by the user, the argument \trl{diagv} 
is a pointer to the actual diagonals (in the same order as the \trl{diag} 
array).  For PETSc to control memory allocation, the user should merely
set \trl{diagv=PETSC\_NULL}.

A simple example of this storage format is illustrated below for block
size \trl{nb}=1. 
Here \trl{nd} = 4 and \trl{diag} = [2, 1, 0, -3]. The
diagonals need not be listed in any particular order, so that
\trl{diag} = [-3, 0, 1, 2] or \trl{diag} = [0, 2, -3, 1] would also
be valid values for the \trl{diag} array. 
...
parallel block diagonal matrix with \trl{m} local rows, \trl{M} global
rows, \trl{n} global columns, and a block size of \trl{nb} is
\begin{tabbing}
   ierr = MatCreateMPIBDiag(PETSC\_COMM\_SELF,int m,int M,int N,int nd,int nb,int *diag,\\
                            Scalar **diagv,Mat *A);
\end{tabbing}
Either the \trl{m} or \trl{M} can be set to PETSC\_DECIDE for PETSc
