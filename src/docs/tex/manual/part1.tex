...
All PETSc programs should call PetscFinalize() \findex{PetscFinalize()}
...
PetscOptionsGetInt\trl{(PETSC_NULL,"-n",&n,&flg);} checks whether the user has
...
VecSetType() or VecSetFromOptions(). \findex{VecSetType()} \findex{VecSetFromOptions()}
...
using \trl{m} and \trl{n}.
Generally one then sets the "type" of the matrix, \findex{MatSetType()} with, for example,
\begin{tabbing}
