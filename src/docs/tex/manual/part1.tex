...
\begin{bashlisting}
   make -f gmakefile.examples test PETSC_ARCH=<PETSC_ARCH>
\end{bashlisting}

For testing installs, the general invocation from the installation (prefix)
directory is:
\begin{bashlisting}
   make -f share/petsc/examples/gmakefile.examples test
\end{bashlisting}

For a full list of options, use
\begin{bashlisting}
   make -f gmakefile.examples help
\end{bashlisting}

\subsection{Understanding test output}

As discussed in Section~\ref{sec_running}, 
the users should set \trl{PETSC_DIR} and \trl{PETSC_ARCH} before running
the tests.
...
\item not ok
  \subitem test failed
\item not ok \#SKIP
  \subitem Test was skipped, usually because build requirements were not
...
(See ~\ref{sec_directory}).  
For testing installs, the default location is:
\trl{${PREFIX_DIR}/tests} but this can be changed with the TESTDIR location.
...
\trl{%pattern%} search.
Finally, the default examples have default arguments, and we often wish
to test examples with various arguments.  We use the \trl{argsearch}
argument for those search.  Like \trl{searchin}, it does not use
wildcards, but rather whether the string is within the arguments.

Some examples are:
\begin{bashlisting}
      make -f gmakefile.examples search='ts\%' test  # Run all ts examples
\end{bashlisting}
\begin{bashlisting}
      make -f gmakefile.examples searchin='tutorials' test  # Run all tutorials
\end{bashlisting}
\begin{bashlisting}
      make -f gmakefile.examples argsearch='cuda' test  # Run examples with cuda in arguments
\end{bashlisting}

Finally, it is useful before invoking the tests to see what targets will
be run.  The \trl{print-test} is useful for this:
\begin{bashlisting}
      make -f gmakefile.examples argsearch='cuda' print-test
\end{bashlisting}
Finally, to see all of the test targets, this command can be used:
\begin{bashlisting}
      make -f gmakefile.examples print-test
\end{bashlisting}

For testing in install directories, some examples are:
\begin{bashlisting}
  cd ${PREFIX_DIR}; make -f gmakefile.examples test TESTDIR=mytests
\end{bashlisting}
or
\begin{bashlisting}
  cd ${PREFIX_DIR}/share/petsc/examples; make -f gmakefile.examples test TESTDIR=$PWD/mytests
\end{bashlisting}
