...
To run the tests, the general invocation is:
\begin{bashlisting}
...
Some examples are:
\begin{bashlisting}
      make -f gmakefile search='ts\%' testsearch  # Run all ts examples
\end{bashlisting}
\begin{bashlisting}
      make -f gmakefile searchin='tutorials' testsearch  # Run all tutorials
\end{bashlisting}
\begin{bashlisting}
      make -f gmakefile argsearch='cuda' testsearch  # Run examples with cuda in arguments
\end{bashlisting}

Finally, it is useful before invoking the tests to see what targets will
be run.  The \trl{echo_testsearch} is useful for this:
\begin{bashlisting}
      make -f gmakefile argsearch='cuda' echo_testsearch
\end{bashlisting}
Finally, to see all of the test targets, this command can be used:
\begin{bashlisting}
      make -f gmakefile echo_test
\end{bashlisting}
