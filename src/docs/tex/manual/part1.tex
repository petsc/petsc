...
\item timesteppers for solving time-dependent (nonlinear) PDEs, including support for differential algebraic equations, and the computation of adjoints (sensitivities/gradients of the solutions) (TS)
\end{itemize}
...
.bashrc} or other startup file.  In addition, the user may need to set the environmental
...
machine type. Using different {\trl{PETSC_ARCH}} allows one to switch between
...
PetscOptionsGetInt(NULL,NULL,"-n",\&n,\&flg); checks whether the user has
...
\> \>  and William~D. Gropp and Karl Rupp and Barry~F. Smith and Stefano Zampini and Hong Zhang",\\
