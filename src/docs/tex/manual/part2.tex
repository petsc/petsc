...
PETSc provides a native AMG PC -- {\it gamg} -- and interfaces to two
external AMG packages: {\it hypre} and {\it ML}.
Note, ML has be depreciated, as a replacement is being developed, and
is not recommended currently.
Run any test problem with -help and the appropriate PC to see a listing of all parameters for
each PC.
%These packages are monolithic in that they implement one class of
%algorithms, ``classic" AMG in the case of  {\it hypre}  and smoothed
%aggregation for {\it ML}.
...
triple product, etc.
PETSc has instantiated one method, (smoothed) aggregation AMG, (\trl{-pc_type gamg -pc_gamg_type
  agg} or just \trl{-pc_type gamg}) that is fully supported.
There is an unsupported implementations of a classical AMG method (\trl{-pc_gamg_type classical}  and a hybrid geometric AMG method (\trl{-pc_gamg_type geo} or PCGAMGType().
GAMG provides unsmoothed aggregation (\trl{-pc_gamg_agg_nsmooths 0}) and smoothed aggregation (\trl{-pc_gamg_agg_nsmooths 1} or PCGAMGSetNSmooths()).
...
AMG methods benefit greatly from knowledge of the number of degrees of
freedom per vertex, and is all but required for GAMG.
The default is one (a scalar problem) but vector problems like elasticity should set the block size of the matrix appropriately: 
\trl{-mat_block_size} $bs$ (e.g., $bs=3$ for 3D) or manually in the code with MatSetBlockSize().

Smoothed aggregation relies an explicit representation of the (near) null space of the operator.
One can provide this null space with KSPSetNullSpace().
Elasticity is a common case where rotational rigid body modes must be explicitly provided; we provide a syntactic sugar method PCSetCoordinates() that will create the approbate rotational modes in 2D or 3D.

 
AMG solvers applied to the broad spectrum of
matrices that we see at PETSc, in terms of equations, discretizations, and parameter
regimes (eg, stretched grids), not to mention possible user code bugs,
can lead to a suboptimal initial experiences with AMG solvers in
general and with PETSc in particular.
%Hypre has been hardened from over 15 years of use; GAMG is much younger but we will fix any code bugs and appreciate users that find them and are willing to work with us to fix them.

{\bf Trouble shooting:}  If GAMG or hypre is not satisfactory, the
first thing to try is the other one.
For GAMG we recommend \trl{-pc_gamg_agg_nsmooths 1} for elliptic
operators because it often improves convergence rates significantly.
This smoothing requires estimates of the highest eigenvalues of the
matrix (as does Chebyshev smoothers \trl{-mg_levels_ksp_type
  chebyshev}).
These highest eigen estimates are usually robust but not provably so,
thus \trl{-pc_gamg_agg_nsmooths 0} is recommended first if poor convergence rates are observed.
Likewise,  \trl{-mg_levels_ksp_type richardson -mg_levels_pc_type sor}
removes any dependance on eigen estimates.
...
Finally, there are several legitimate sources of poor performance of
AMG solvers and often special purpose methods must be developed to
achieve the full potential of multigrid.
To name just a few legitimate sources of performance degradation that
