...
\begin{lstlisting}
DMNetworkGetComponentKeyOffset(DM dm,PetscInt p, PetscInt compnum, PetscInt *compkey, PetscInt *offset);
\end{lstlisting}
...
optional user-defined event color (for use with {\em Jumpshot} logging; see Section \ref{sec_mpelog});\findex{-log_mpe}
...
This chapter provides hints on how to get to achieve best performance with PETSc, particularly distributed memory machines with multiple CPU sockets per node.
We focus on machine-related performance optimization here; algorithmic aspects like preconditioner selection are not covered here.

\section{Maximizing Memory Bandwidth}

Most operations in PETSc deal with large datasets (typically vectors and sparse matrices) and perform relatively few arithmetic operations for each byte processed.
...
Since modern CPUs are able to execute on the order of 10 floating point operations for each byte loaded or stored, PETSc operations are thus mostly limited by the rate at which data can be loaded or stored (\emph{memory bandwidth limited} rather than by the rate of floating point operations.

This section discusses ways to maximize the memory bandwidth achieved by applications based on PETSc.
Where appropriate, we include benchmark results in order to provide quantitative results on typically performance gains one can achieve through parallelization, both on a single compute node and across nodes.
...
Because a smaller number of MPI ranks usually implies better preconditioners and better performance for smaller problems, the best performance for PETSc applications is often obtained with less ranks than there are physical CPU cores available.
...
Since the mechanisms for doing so vary across MPI implementations, consult the manual of your MPI implementation.
...
\end{lstlisting}
In this configuration, process 0 is bound to the first physical core on the first socket (with IDs 0 and 12), process 1 is bound to the first core on the second socket (IDs 6 and 18), an similarly for the remaining processes.
