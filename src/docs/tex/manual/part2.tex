...
iterative solvers than just treating the matrices as black box. In
...
the underlying physics or discretization of the problem. Under a
...
etc. On a process the blocks may be stored with rows/columns
interlaced or one block followed by another. For example,
noninterlaced,
\[
...
The PETSc PCFIELDSPLIT preconditioner is used to implement that
``block matrix'' solvers. There are three ways to provide the
...
of the matrix belong to a particular block. With these two approaches
...
There are several variants of the Schur complement preconditioner obtained by dropping some of the terms, these can be obtained with \trl{-pc_fieldsplit_schur_factorization_type <diag,lower,upper,full>} or NOTE WE NEED TO ADD A FUNCTIONAL INTERFACE. Note that the \trl{diag} form uses the preconditioner
\[
\left( \begin{array}{cc}
ksp(A_{00},Ap_{00})   & 0 \\
0 & -ksp(\hat{S},\hat{S}p) \\
...
If you provide names for the partions you have created the options prefix for the internal KSPs are given by \trl{-fieldsplit_name_} if you did not provide names then they are given by \trl{-fieldsplit_0_}, \trl{-fieldsplit_1_} etc.
