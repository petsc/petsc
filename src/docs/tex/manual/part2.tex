...
An example of this for 2D distributed arrays is given below that uses the utility routine DMGetColoring().
...
pass in PETSC\_NULL.  The variable \trl{ctx} allows 
...
\trl{A}. If \trl{B} is an identity matrix, the user should pass in PETSC\_NULL. 
...
\end{tabbing}
The vector \trl{R} is an optional location to store the result.
...
To configure TS to use coloring, create a MatFDColoring, register the function
\begin{tabbing}
  MatFDColoringSetFunction(matfdcoloring,(PetscErrorCode(*)(void))SNESTSFormFunction,ts);
\end{tabbing}
obtain the snes using TSGetSNES(), and set the Jacobian routine
\begin{tabbing}
  SNESSetJacobian(snes,A,B,SNESDefaultComputeJacobianColor,matfdcoloring);
\end{tabbing}
...
\begin{equation}\label{eq:imex}
  G(t,u,\dot u) = F(t,u),\quad u(t_0) = u_0
\end{equation}
where $G$ will be treated implicitly using a method suitable for stiff problems and $F$ will be treated explicitly when using an IMEX method like TSARKIMEX.
$G$ is typically linear or weakly nonlinear while $F$ may have very strong nonlinearities such as arise in non-oscillatory methods for hyperbolic PDE.
The user provides three pieces of information, the APIs for which have been described above.
\begin{itemize}
\item ``Slow'' part $F(t,u)$ using TSSetRHSFunction().
\item ``Stiff'' part $G(t,u,\dot u)$ using TSSetIFunction().
\item Jacobian $J(t,u,\dot u) = G_u + a G_{\dot u}$ using TSSetIJacobian().
  An assembled Jacobian is optional and can be approximated using finite differencing.
\end{itemize}
To solve Equation~\ref{eq:imex} using fully implicit methods, the user may also provide the Jacobian $F_u$.
...
provided, (indicated by passing in a PETSC\_NULL) the DMMG will compute the
