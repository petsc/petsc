...
techniques and trust region methods.  Also provided are several iterative nonlinear methods, as well as nonlinear
...
\hline
Line search Newton           & SNESNEWTONLS    & newtonls    & SNESConverged\_NEWTONLS()\\
Trust region Newton          & SNESNEWTONTR    & newtontr    & SNESConverged\_NEWTONTR()\\
Test Jacobian                & SNESTEST        & test        & \\
Nonlinear Richardson         & SNESNRICHARDSON & nrichardson & \\
Nonlinear Conjugate Gradient & SNESNCG         & ncg         & \\
Nonlinear GMRES              & SNESNGMRES      & ngmres      & \\
Quasi-Newton                 & SNESQN          & qn          & \\
Full Approximation Scheme    & SNESFAS         & fas         & \\
Nonlinear Additive Schwarz   & SNESNASM        & nasm        & \\
...
Every SNES has a line search context of type SNESLineSearch that may be retrieved using SNESGetLineSearch().  Custom
line search types may either be defined using SNESLineSearchShell, or by creating a custom user line search type in the
model of the preexisting ones and register it using SNESLineSearchRegister.

There are several default options for the line searches.  The order of polynomial approximation may be set
with \trl{-snes_linesearch_order}; for instance, 2 for quadratic or 3 for cubic.  \trl{-snes_linesearch_norms} may be
used to turn off norm computation in the case where monitoring is not necessary.

The default SNESLINESEARCHBT involves several parameters, which are set to defaults that are reasonable for many
applications.  The user can override the defaults by using the options
...
default).  Four variants (Fletcher-Reed, Polak-Ribiere-Polyak,Dai-Yuan,Conjugate Descent) are implemented in PETSc.
...
One may also control the form of the initial Jacobian approximation and the conditions for restart.

\subsection{The Full Approximation Scheme}

The Full Approximation Scheme is a nonlinear multigrid correction.  At each level, there is a recursive cycle control
SNES instance, and either one or two nonlinear solvers as smoothers (up and down).  Problems set up using the SNES DMDA
interface are automatically coarsened.

\subsection{Nonlinear Additive Schwarz}

Nonlinear Additive Schwarz methods take a number of local nonlinear subproblems, solves them independently in parallel,
and combines those solutions into a new approximate solution.  SNESNASMSetSubdomains() allows for the user to create
these local subdomains.  Problems set up using the SNES DMDA interface are automatically decomposed.

SNESASPIN is a helper type that sets up a nonlinearly preconditioned Newton's method using NASM as the preconditioner.
...
instance.  The inner instance may be extracted using SNESGetNPC() and passed run-time options using the \trl{-npc_}
...
There are several patterns of nonlinear preconditioning that may be used with great success for particular problems.
...
comma separated list argument to \trl{-snes_composite_sneses}.
