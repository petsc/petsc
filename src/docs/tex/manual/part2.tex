...
It supports automatic makefile generation using cross-platform \texttt{qmake} and \texttt{cmake}
build systems as well as allowing one to import projects based on existing, possibly hand-written, makefiles.
Qt Creator has a visual debugger using GDB and LLDB (on Linux and OSX) or Microsoft's
CDB (on Windows) as backends. It also has interface to Valgrind's ``memcheck'' and ``callgrind''
tools to detect memory leaks and profile your code. It has built-in support for a variety of version
control using git, mercurial, subversion, etc. Finally, Qt Creator comes fully equipped with
auto-completion, function look-up, and code refactoring tools. This enables one to easily browse source
files, easily find relevant functions, and refactor them across the entire project.

\subsection*{Creating a Project}
When using Qt Creator with \texttt{qmake}, one needs a \texttt{.pro} file. This is a configuration file that
tells Qt Creator about all build/compile options and locations of source files. One may start with
a blank \texttt{.pro} file and fill in the configuration options as needed. Here's is an example:
\begin{lstlisting}
# This is the name of your application executable
TARGET = ex1

# Set any include path and libs you need to link
...
# set appropriate complier and its flags
...
# Here we add all files that must be compiled.
...
\item \lstinline{INCLUDEPATH}: Used at compile time to point to required include files. Essentially, it is used as a \trl{ -I \$\$INCLUDEPATH} flag for the compiler. This should include all your application-specific header files and those related to PETSc (which may be found via \trl{make getincludedirs}).
\item \lstinline{LIBS}: Defines all required external libraries to link with your application. To get PETSc's linking libraries, use \trl{make getlinklibs}.
\item \lstinline{QMAKE_CC} and \lstinline{QMAKE_CXX}: Define which C/C++ compilers use.
\item \lstinline{QMAKE_LINK}: Defines the proper linker to be used. Relavant if compiling C++ projects
...
There are more options available that one can feed into the \trl{.pro} file; see e.g. \url{http://doc.qt.io/qt-5/qmake-project-files.html}. 
...
The variable \trl{PETSC_ARCH} can also be set as environment
