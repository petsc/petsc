...
\begin{lstlisting}
VecScatterCreateWithData(Vec x,IS ix,Vec y,IS iy,VecScatter *ctx);
...
their communicator must be on the same set of processes, but their process order can be different.
...
used in the call to \lstinline{VecScatterCreateWithData()} so long as they have the same
...
\lstinline{VecScatterCreateWithData()}.
...
VecScatterCreateWithData(p,from,x,to,&scatter);
