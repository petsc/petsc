...
Because a smaller number of MPI ranks usually implies better preconditioners and better performance for smaller problems, the best performance for PETSc applications may be obtained with less ranks than there are physical CPU cores available.
...
PETSc code builds in debug mode per default.
...
Only once the new code is thoroughly tested, one should disable debugging facilities by passing \lstinline|--with-debugging=no| to configure.

PETSc has no reliable way of knowing whether a particular run is a production or debug run.
