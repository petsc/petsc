...
\end{lstlisting}
  Here, \lstinline{name} is the component name, \lstinline{size} is the size of component data type, and \lstinline{compkey} is an integer key that can be used for setting/getting the component at a vertex or an edge.
...
The \lstinline{edgelist} and \lstinline{edgelistCouple} for this network is \\
...
\begin{lstlisting}
DMNetworkAddComponent(DM dm, PetscInt p, PetscInt compkey, void* component);
\end{lstlisting}
  %DMNetwork supports setting multiple components at a vertex/edge.
%\begin{lstlisting}
...
The components set at vertices/edges be accessed by
\begin{lstlisting}
