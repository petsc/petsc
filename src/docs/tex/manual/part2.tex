...
control systems including git, mercurial, subversion, etc. Finally, Qt Creator comes fully equipped with
auto-completion, function look-up, and code refactoring tools. This enables one to easily browse source
files, easily find relevant functions, and refactor them across an entire project.

\subsection*{Creating a Project}
When using Qt Creator with \texttt{qmake}, one needs a \texttt{.pro} file. This configuration file
tells Qt Creator about all build/compile options and locations of source files. One may start with
a blank \texttt{.pro} file and fill in configuration options as needed. For example:
\begin{lstlisting}
...
\item \lstinline{CONFIG}: Configuration options to be used by \texttt{qmake}. Here, the option \texttt{link_pkgconfig} instructs \textt{qmake} to internally use \texttt{pkgconfig} to resolve \lstinline{INCLUDEPATH} and \lstinline{LIBS} variables.
\item \lstinline{PKGCONFIG}: Name of the configuration file (the \texttt{.pc} file -- here \texttt{PETSc.pc}) to be passed to \texttt{pkgconfig}. Note that for this functionality to work, \texttt{PETSc.pc} must be in path which might require adjusting the \texttt{PKG_CONFIG_PATH} enviroment variable. For mor einformation see \url{http://doc.qt.io/qtcreator/creator-build-settings.html#build-environment}.
...
More options can be included in a \trl{.pro} file; see e.g. \url{http://doc.qt.io/qt-5/qmake-project-files.html}. 
