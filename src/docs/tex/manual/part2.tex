...
\begin{enumerate}
  ...
  In the next two panels, you can either add your exisiting local repository or download PETSc from bitbucket by providing the URI.
  Most Eclipse disctributions come with Git support. If not, install the EGit plugin. When importing the project, select the wizard ``Import as general project''.
...
If you lunch Eclipse from Dock on Mac OS X, the .bashrc would not be loaded (a known behavior of Mac OS X for security reasons).
This would be a probem if you set the environment variables \trl{PETSC_DIR} and \trl{PETSC_ARCH} in .bashrc.
A solution which involves replacing the executable can be found at
\href{http://stackoverflow.com/questions/829749/launch-mac-eclipse-with-environment-variables-set}
{\trl{http://stackoverflow.com/questions/829749/launch-mac-eclipse-with-environment-variables-set}}.
