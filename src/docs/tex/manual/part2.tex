...
keys are \trl{-mat_no_inode} (do not use inodes) and 
\trl{-mat_inode_limit <limit>} (set inode limit (max limit=5)).
...
\end{lstlisting}
This
routine accepts the options \trl{-help} as well as
...
Section~\ref{sec_convergencetests} gives more details regarding
...
\end{lstlisting}
The default parameter values are \trl{damping_factor=1.0, emax=0.01, emin=100.0}, 
and \trl{max_steps=30}. The GMRES
...
%----------------------------------------------------------------------
\subsection{Combining Preconditioners\label{sec_combining-pcs}} \sindex{combining preconditioners}
...
\trl{mode} to be \findex{PC_MG_MULTIPLICATIVE} \lstinline{PC_MG_MULTIPLICATIVE}; for the
...
and not out of \trl{Amat} (i.e., $A$ itself). As discussed above in Sec.~\ref{sec_combining-pcs}, however, it is possible to use \lstinline{Amat}
...
by specifying \trl{-help} at runtime.
...
\end{lstlisting}
The arguments of the routine \trl{FormJacobian()} are the current iterate, 
\trl{x}; the (approximate) Jacobian matrix,
...
elements of the two matrices by using the option 
\trl{-snes_test_jacobian_view} \findex{-snes_test_jacobian_view}, which causes the two
...
\ref{sec_convergencetests}), one can easily create one's own,
...
For example, we can encode the doublet mesh in Figure \ref{fig_doubletMesh},
\begin{figure}
\centering
...
\label{fig_doubletMesh}
\end{figure}
which can also be represented as the DAG in Figure~\ref{fig_doubletDAG}.
\begin{figure}
\centering
\begin{tikzpicture}[scale=2.0]
  \tikzstyle{vertex}  = [text=blue]
...
then vertices, then faces, and then edges. In 2 dimensions the convention is to number faces, vertices, and then edges. The user is free to violate these conventions. In terms of the labels in Figure~\ref{fig_doubletMesh}, these numberings are
\begin{equation*}
...
\end{enumerate}
For example, using the mesh from Figure~\ref{fig_doubletMesh}, we can lay out data for a continuous Galerkin $P_3$ finite
element method,
\begin{lstlisting}
...
algorithms, data structures, etc. to the screen by using the option 
\trl{-info} \findex{-info} or by calling \lstinline{PetscInfoAllow(PETSC_TRUE)}.
...
\trl{-help} at runtime, e.g., \findex{-help}
\begin{lstlisting}
...
blocks can be controlled via the options \trl{-sub_ksp_type} and 
\trl{-sub_pc_type}. \sindex{block Jacobi}
