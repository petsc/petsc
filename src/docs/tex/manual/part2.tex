...
\trl{MatMult(Mat,Vec,Vec)} $ \to$ \trl{UserMult(Mat,Vec,Vec)}.
...
\findex{PCFactorSetUseDropTolerance()} \findex{PCFactorSetReuseFill()}
...
\trl{-pc_factor_ordering_type} \trl{order}) computed in the first factorization to be reused
...
\begin{tabbing}
   ...
   \trl{-pc_factor_use_drop_tolerance <dt>,<dtcol>,<dtcount>}\\
...
\findex{-pc_factor_use_drop_tolerance} \findex{-pc_factor_reuse_fill}
...
In addition, for parallel matrices
...
details provided within the BlockSolve95 users manual. To 
use BlockSolve you Consult the file
\trl{docs/installation/index.htm} for details on installing PETSc to allow 
the use of BlockSolve95. Note that when using parallel preconditioners such as
block Jacobi and the additive Schwarz method with factorizations on each block
(which are sequential) the regular PETSc sequential ILU and ICC preconditioners are used.
...
\begin{tabbing}
  MatSetOption(Mat mat,MAT\_SYMMETRIC,PETSC\_TRUE);
\end{tabbing}
...
option \trl{-ksp_monitor_true_residual} \findex{-ksp_monitor_true_residual} to print both the
...
of equations with the command \findex{-pc_factor_ordering_type} \sindex{orderings}
\begin{tabbing}
   \trl{-pc_factor_ordering_type <ordering>}
\end{tabbing}
...
\sindex{quotient minimum degree} \findex{-pc_factor_ordering_type}
These orderings can also be set through the options database by specifying 
one of the following:  \trl{-pc_factor_ordering_type} \trl{natural}, \trl{-pc_factor_ordering_type}
\trl{nd}, \trl{-pc_factor_ordering_type} \trl{1wd}, \trl{-pc_factor_ordering_type} \trl{rcm},
\trl{-pc_factor_ordering_type} \trl{qmd}.
...
\begin{enumerate}
\item Run config/configure.py with the additional options \trl{--download-packagename=1}. 
For eg: \trl{--download-superlu_dist=1}
\item Build the PETSc libraries.
\item Use the runtime option: \trl{-mat_type <mattype>} \trl{-ksp_type preonly} \trl{-pc_type <pctype>}. 
For eg: \trl{-mat_type superlu_dist} \trl{-ksp_type preonly} \trl{-pc_type lu}
\end{enumerate}

\begin{table}[H]
\begin{center}
\begin{tabular}{lllll}
{\bf Package}    &{\bf MatType}  & {\bf PCType} & {\bf MatSolverPackage} & {\bf Runtime Options}\\
\hline
DSCPACK          & baij         & cholesky     &  \trl{MAT_SOLVER_DSCPACK}         & \trl{-pc_factor_mat_solver_package dscpack}\\
ESSL             & seqaij       & lu           &  \trl{MAT_SOLVER_ESSL}            & \trl{-pc_factor_mat_solver_package essl}\\
LUSOL            & seqaij       & lu           &  \trl{MAT_SOLVER_LUSOL}           & \trl{-pc_factor_mat_solver_package lusol}\\
MATLAB           & seqaij       & lu           &  \trl{MAT_SOLVER_MATLAB}          & \trl{-pc_factor_mat_solver_package matlab}\\
MUMPS            & aij          & lu           &  \trl{MAT_SOLVER_MUMPS}        & \trl{-pc_factor_mat_solver_package aijmumps}\\
                 & sbaij        & cholesky     & \\
PLAPACK          & plapack      & lu           &  \trl{MAT_SOVLER_PLAPACK}         & \trl{-pc_factor_mat_solver_package plapack}\\
                 & plapack      & cholesky     &  \\
SPOOLES          & aij          & lu           &  \trl{MAT_SOLVER_SPOOLES}      & \trl{-pc_factor_mat_solver_package aijspooles}\\                
                 & sbaij        & cholesky     &  \\
SUPERLU          & seqaij       & lu           &  \trl{MAT_SOVLER_SUPERLU}         & \trl{-pc_factor_mat_solver_package superlu} \\
SUPERLU\_DIST    & aij          & lu           &  \trl{MAT_SOLVER_SUPERLU_DIST}   & \trl{-pc_factor_mat_solver_package superlu_dist}\\
UMFPACK          & seqaij       & lu           &  \trl{MAT_SOLVER_UMFPACK}         & \trl{-pc_factor_mat_solver_package umfpack}\\
...
or 
\begin{tabbing}
   -mat\_mffd\_compute\_norma\trl{ <true or false>}\\
...
\trl{TS_BEULER},  \trl{TS_PSEUDO}, \trl{TS_CRANK_NICHOLSON},
\trl{TS_SUNDIALS} (only if the Sundials package is installed),
and \trl{TS_RK}
\findex{TS_EULER} \findex{TS_BEULER} \findex{TS_PSEUDO}
\findex{TS_CRANK_NICHOLSON} \findex{TS_SUNDIALS} \findex{TS_RUNGE_KUTTA}
or the command line option
\trl{-ts_type euler, beuler, pseudo, crank-nicholson, sundials, runge-kutta}.
...
function, the user must construct a \trl{MatShell} matrix. Note that this is 
...
\trl{PetscBinaryRead()} may be called any number of times. Each call should correspond on the PETSc side with
...
-info} \findex{-info} or by calling \trl{
PetscInfoAllow(PETSC_TRUE)}. \findex{PetscInfoAllow()}
...
\findex{-malloc_log} \findex{PetscGetResidentSetSize()}
...
\trl{-malloc_log} activates calls to PetscTrLog(), \trl{
...
\trl{MATMPIBAIJ}). The diagonal information 
...
Numbers'' section of the file \trl{${PETSC_DIR}/src/docs/website/documentation/installation.html} for
...
\begin{tabbing}
   include \$\trl{{PETSC_DIR}/conf/base}
\end{tabbing}
...
\item[EXAMPLES\_1] examples that will be built with
             \trl{make examples} (see Section~\ref{sec_common})
...
\end{itemize}
These orderings can also be set through the options database by specifying 
one of the following:  \trl{-pc_factor_ordering_type} \trl{natural}, \trl{-pc_factor_ordering_type} \trl{nd}, 
\trl{-pc_factor_ordering_type} \trl{1wd}, \trl{-pc_factor_ordering_type} \trl{rcm}, \trl{-pc_factor_ordering_type}
\trl{qmd}.
\findex{MATORDERING_NATURAL} \findex{MATORDERING_ND} \findex{MATORDERING_1WD}
\findex{MATORDERING_RCM} \findex{MATORDERING_QMD} \sindex{nested dissection}
\sindex{one-way dissection} \sindex{reverse Cuthill-McKee} 
...
command line option \trl{-pc_factor_ordering_type} \trl{sname}.  If reordering directly,
...
\begin{tabbing}
  MatGetFactor(Mat matrix,MatFactorType,char *packagename,Mat *factor);
\end{tabbing}
and then performs the factorization
\begin{tabbing}
  MatCholeskyFactorSymbolic(Mat factor,Mat matrix,IS perm, double pf);\\
  MatLUFactorSymbolic(Mat factor,Mat matrix,IS rowperm,IS colperm,MatLUInfo *info);\\
  MatCholeskyFactorNumeric(Mat factor,Mat matrix);\\
  MatLUFactorNumeric(Mat factor,Mat matrix);
\end{tabbing}
...
\begin{tabbing}
  ...
  PCApplyRichardson(PC pc,Vec x,Vec y,Vec work,PetscReal rtol,PetscReal atol, PetscReal dtol,PetscInt maxits,PetscInt *its,PCRichardsonConvergedReason*);
\end{tabbing}
