...
PETSc has instantiated one method, (smoothed) aggregation AMG, (\trl{-pc_type gamg -pc_gamg_type
  agg} or just \trl{-pc_type gamg}) that is supported.
There is an unsupported implementations of a classical AMG method and a
hybrid geometric AMG method.
GAMG provides unsmoothed aggregation (\trl{-pc_gamg_agg_nsmooths 0}) and smoothed aggregation (\trl{-pc_gamg_agg_nsmooths 1}).
...
Hypre has been hardened from over 15 years of use; GAMG is much
younger but we will fix any code bugs and appreciate users that find
them and are willing to work with us to fix them.
...
\trl{-pc_gamg_threshold x.y} to regulate coarsening rates.
A high threshold (eg, 0.08) will result in an expensive but potentially
powerful preconditioner; a low threshold (eg, 0.0) will result in
...
The default is one (a scalar problem) but vector problems like 2D
elasticity should set the block size of the matrix appropriately (eg
\trl{-mat_block_size 2} or manually in the code).
