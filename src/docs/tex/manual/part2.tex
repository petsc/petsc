...
\begin{tabbing}
TSSetReverseMode(TS ts,PetscBool mode);
\end{tabbing} 
...
\begin{tabbing}
TSSetSensitivity(TS ts,Vec *lambda,PetscInt np);  
\end{tabbing}
and 
\begin{tabbing}
TSSetSensitivityP(TS ts,Vec *mu,PetscInt np);  
\end{tabbing}
...
Currently supported method types are TSRK (Runge-Kutta) methods and the problems to be solved are assumed not to have mass matrices.

The procedure is similar to that for using implicit methods. 
