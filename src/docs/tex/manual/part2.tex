...
\begin{tabbing}
  MatSetOption(Mat A,MAT\_ROWS\_SORTED,PETSC\_TRUE);
\end{tabbing}
or 
\begin{tabbing}
  MatSetOption(Mat A,MAT\_COLUMNS\_SORTED,PETSC\_TRUE);
\end{tabbing}
...
\item When using MatSetValues(), if the column indices of the values being
   ...
   the routine \trl{MatSetOption(mat,MAT_COLUMNS_SORTED,PETSC\_TRUE)} before setting the values
