...
This chapter introduces the DMComplex subclass of DM, which allows the user to handle unstructured grids using the generic
DM interface for hierarchy and multi-physics. DMComplex was created to remedy a huge problem in all current PDE
...
\section{Representing Unstructured Grids} \sindex{DMComplex}

The main advantage of DMComplex in representing topology is that it treats all the different pieces of a mesh,
...
\begin{tabbing}
  DMComplexSetConeSize(dm, 0, 3);\\
  DMComplexSetConeSize(dm, 1, 3);\\
  DMComplexSetConeSize(dm, 6, 2);\\
  DMComplexSetConeSize(dm, 7, 2);\\
  DMComplexSetConeSize(dm, 8, 2);\\
  DMComplexSetConeSize(dm, 9, 2);\\
  DMComplexSetConeSize(dm, 10, 2);\\
  DMSetUp(dm);
\end{tabbing}
and then point values,
\begin{tabbing}
  DMComplexSetCone(dm, 0, [6, 7, 8]);\\
  DMComplexSetCone(dm, 1, [8, 9, 10]);\\
  DMComplexSetCone(dm, 6, [2, 3]);\\
  DMComplexSetCone(dm, 7, [3, 4]);\\
  DMComplexSetCone(dm, 8, [4, 2]);\\
  DMComplexSetCone(dm, 9, [4, 5]);\\
  DMComplexSetCone(dm, 10, [5, 3]);
\end{tabbing}

There is also an API for the dual relation, using DMComplexSetSupportSize() and DMComplexSetSupport(), but this can be
calculated automatically by calling
\begin{tabbing}
  DMComplexSymmetrize(dm);
\end{tabbing}
...
\\
  DMComplexGetChart(dm, \&pStart, \&pEnd);\\
  DMComplexGetHeightStratum(dm, 0, \&cStart, \&cEnd);\\
  DMComplexGetHeightStratum(dm, 1, \&eStart, \&eEnd);\\
  DMComplexGetDepthStratum(dm, 0, \&vStart, \&vEnd);\\
...
\begin{tabbing}
  ...
  DMGetLocalVector(s, \&localVec);\\
  DMGetGlobalVector(s, \&globalVec);
\end{tabbing}
...
\end{itemize}
DMComplex separates these different concerns by passing sets of points, which are just integers, from mesh traversal
routines to data extraction routines and back. In this way, the PetscSection which structures the data inside a Vec does
not need to know anything about the mesh inside a DMComplex.
...
\\
  VecGetArray(u, \&a);\\
  DMComplexGetTransitiveClosure(dm, cell, PETSC\_TRUE, \&numPoints, \&points);\\
...
\\
    ...
    \>\}\\
  \}\\
  DMComplexRestoreTransitiveClosure(dm, cell, PETSC\_TRUE, \&numPoints, \&points);\\
  VecRestoreArray(u, \&a);
\end{tabbing}
...
\begin{tabbing}
  ...
  PetscInt           csize;\\
\\
  DMComplexVecGetClosure(dm, section, u, cell, \&csize, \&values);\\
  /* Do integral in quadrature loop */\\
  DMComplexVecRestoreClosure(dm, section, u, cell, \&csize, \&values);\\
  DMComplexVecSetClosure(dm, section, residual, cell, \&r, ADD\_VALUES);
\end{tabbing}
A simple example of this kind of calculation is in DMComplexComputeL2Diff(). Note that there is no restriction on the
...
\\
  VecGetArray(u, \&a);\\
  DMComplexGetTransitiveClosure(dm, cell, PETSC\_FALSE, \&numPoints, \&points);\\
  assert(numPoints == 2);\\
