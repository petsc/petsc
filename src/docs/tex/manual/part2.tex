...
\end{tabbing}
Note that here one must provide the value \trl{n}, it cannot be PETSC_DECIDE and
...
PETSC_DECIDE for \trl{m} and \trl{n} 
...
\trl{m} and the dimension of \trl{ly} is \trl{n}; or PETSC_NULL 
may be passed in.
...
global entries (or PETSC_NULL) and \trl{nghost} is the number of 
...
If PETSC_DECIDE is not used for the arguments
...
set \trl{data} to PETSC_NULL for PETSc to control matrix memory allocation.
...
of the matrix. If the pointer \trl{diag_value} is PETSC_NULL, it 
...
can use PETSC_NULL for the \trl{vals} argument. Similarly,
one can use PETSC_NULL for the \trl{cols} argument.
...
\trl{w} is PETSC_NULL, space allocated by PETSc in the KSP context is 
...
\end{array} \right).
\]
In a parallel computer the matrices are not stored explicitly this way however, each process will own some of the rows of $A_{0*}$, $A_{1*}$ etc. On a process the blocks may be stored with rows/columns interlaced or one block followed by another. For example, noninterlaced,
\[
...
\end{array} \right).
\]
Note that for interlaced storage the number of rows/columns of each block must be the same size. Matrices obtained with DMGetMatrix() where the DM is a a DMDA are always stored interlaced. 

The PETSc PCFIELDSPLIT preconditioner is used to implement that ``block matrix'' solvers. There are three ways to provide the information  that defines the blocks.  If the matrices are stored as interlaced then  PCFieldSplitSetFields() can be called repeatedly to indicate which fields belong to each block. More generally PCFieldSplitSetIS() can be used to indicate exactly which rows/columns of the matrix belong to a particular block. With these two approaches the blocks may overlap (though generally they will not). If only one block is defined then the complement of the matrices is used to define the other block. Finally the option  \trl{-pc_fieldsplit_detect_saddle_point} 
\findex{-pc_fieldsplit_detect_saddle_point} causes two blocks to be found, one associated with all rows/columns that have zeros on the diagonals and the rest.
...
\end{array} \right).
\]
These can be access with \trl{-pc_fieldsplit_type <additive,multiplicative,symmetric_multiplicative} \findex{-pc_fieldsplit_type} or the function PCFieldSplitSetType().

For two by two blocks only are another family of solvers, based on Schur complements. These can be accessed with \trl{-pc_fieldsplit_type schur}. 
\[
\left( \begin{array}{cc}
ksp(A_{00},Ap_{00})   & 0 \\
0 & I \\
...
There are several variants of the Schur complement preconditioner obtained by dropping some of the terms ZZZZ
...
% ---------------------------------------------------------------
\cleardoublepage
\chapter{TS: Scalable ODE and DMDAE Solvers}
\label{chapter_ts}
\sindex{ODE solvers}

The TS library provides a framework for the scalable solution of ODEs and DMDAEs
...
More generally, this can be applied to implicit ODE and DMDAE for which
...
pass in PETSC_NULL.  The variable \trl{ctx} allows 
...
\trl{A}. If \trl{B} is an identity matrix, the user should pass in PETSC_NULL. 
...
\end{tabbing}
The arguments for the function \trl{fjac()} are
...
equation (DMDAE), but if the matrix $F_{\dot{u}}(t) = \partial F
...
To solve a DMDAE, instead of TSSetRHSFunction and TSSetRHSJacobian, one
uses:
...
\end{tabbing}
The \trl{ad_function} and \trl{ad_mf_function} are described in the next chapter. 
...
\end{tabbing}
(port is usally set to \trl{PETSC_DEFAULT}, use PETSC_NULL for the machine if the 
...
PetscStartMATLAB\trl{(MPI_Comm,char *machine,char *script,FILE **fp)}; where machine and script may be PETSC_NULL.
...
\end{tabbing}
where \trl{machine} is the name of the machine hosting MATLAB (PETSC_NULL may be used for localhost).
...
Usually \trl{pre} should be set to PETSC_NULL (or \trl{PETSC_NULL_CHARACTER}
...
In the \trl{projects} subdirectory of PETSc are several Developers Studio projects that use PETSc.

\section{XCode Users (The Apple GUI Development System} \sindex{xcode}

Please let us know how you have set things up at \trl{petsc-dev@mcs.anl.gov}.
