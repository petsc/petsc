...
into a new approximate solution.  SNESNASMSetSubdomains() allows for the user to
create these local subdomains.  Problems set up using the SNES DMDA interface
are automatically decomposed.  To begin, the type of subdomain updates to the
whole solution are limited to two types borrowed from PCASM: \trl{PC_ASM_BASIC},
in which the overlapping updates added.  \trl{PC_ASM_RESTRICT} updates in a
