...
\end{tabbing}
The argument to the ``setup'' routine is the same \trl{PC} object which
...
\end{tabbing}
\findex{TSSetType()} Currently supported types are \trl{TSEULER}, \trl{TSRUNGE_KUTTA},
\trl{TSBEULER}, \trl{TSCRANK_NICHOLSON}, \trl{TSTHETA}, \trl{TSGL}, \trl{TSPSEUDO}, and
\trl{TSSUNDIALS} (only if the Sundials package is installed),
...
Note: only the general linear \trl{TSGL} and $\theta$ method
\trl{TSTHETA} have support for solving DAE.
...
eis  = new esi::petsc::IndexSpace\trl{<}int\trl{>}::IndexSpace(PetscLayout map);\\
...
option \trl{-malloc_dump} merely activates a call to \trl{PetscMallocDump()} during
\trl{PetscFinalize()} the user can also call \trl{PetscMallocDump()} elsewhere
in a program.
...
\trl{-malloc_log} activates logging by calling \trl{PetscMallocSetDumpLog()} in
\trl{PetscInitialize()} and then prints the log by calling \trl{PetscMallocDumpLog()}
in \trl{PetscFinalize()}.  The user can also call these routines elsewhere in a
program.  When finer granularity is desired, the user should call
\trl{PetscMallocGetCurrentUsage()} and \trl{PetscMallocGetMaximumUsage()} for memory
allocated by PETSc, or \trl{PetscMemoryGetCurrentUsage()} and
\trl{PetscMemoryGetMaximumUsage()} for the total memory used by the program.
Note that \trl{PetscMemorySetGetMaximumUsage()} must be called before
\trl{PetscMemoryGetMaximumUsage()} (typically at the beginning of the program).
...
\begin{tabbing}
  ...
  PCApplyRichardson(PC pc,Vec x,Vec y,Vec work,PetscReal rtol,PetscReal atol, PetscReal dtol,PetscInt maxits,PetscTruth zeroguess,PetscInt *its,PCRichardsonConvergedReason*);
\end{tabbing}
