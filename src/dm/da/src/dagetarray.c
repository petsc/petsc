...
.  array - the array
  ...
  Fortran Notes: From Fortran use DAVecGetArrayF90() and pass for the array type PetscScalar,pointer :: array(:,:,:) of the appropriate number of dimensions. For a DA with a dof of 1 use the dimension of the DA, for a dof greater than 1 use one more than the dimension of the DA. The order of the indices is array(1:dof,xs+1:xs+xm,ys+1:ys+ym,zs+1:zs+zm) where the values are obtained from DAGetCorners() for a global array or DAGetGhostCorners() for a local array. Include finclude/petscda.h90 to access this routine.
