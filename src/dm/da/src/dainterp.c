...
#undef __FUNC__  
#define __FUNC__ "DAGetInterpolation_1D_1"
int DAGetInterpolation_1D_1(DA dac, DA daf, Mat *A)
{
  int      ierr,i,i_start,m_f,Mx,*idx;
  int      m_ghost,*idx_c,m_ghost_c;
  int      row,col,i_start_ghost,mx, m_c, nc,ratio;
  int      i_c,i_start_c,i_start_ghost_c,cols[2];
  Scalar   v[2],x;
  Mat      mat;

  PetscFunctionBegin;
  ierr = DAGetInfo(dac,0,&Mx,0,0,0,0,0,0,0,0,0);CHKERRQ(ierr);
  ierr = DAGetInfo(daf,0,&mx,0,0,0,0,0,0,0,0,0);CHKERRQ(ierr);
  ratio = (mx-1)/(Mx-1);
  if (ratio*(Mx-1) != mx-1) SETERRQ1(1,1,"Ratio between levels: (mx - 1)/(Mx - 1) must be integer: mx %d Mx %d",mx,Mx);
...
/*   1 degree of freedom per node, nonperiodic */
#undef __FUNC__  
#define __FUNC__ "DAGetInterpolation_2D_1"
int DAGetInterpolation_2D_1(DA dac, DA daf, Mat *A)
{
...
int DAGetInterpolation_2D_dof(DA dac, DA daf, Mat *A)
{
  ...
  int      m_ghost,n_ghost,*idx_c,m_ghost_c,n_ghost_c,l;
  int      row,col,i_start_ghost,j_start_ghost,cols[4],mx, m_c,my, nc,ratio;
  int      i_c,j_c,i_start_c,j_start_c,n_c,i_start_ghost_c,j_start_ghost_c;
  Scalar   v[4],x,y;
  Mat      mat;
    ...
    for ( j=j_start; j<j_start+n_f; j++ ) {
      for ( i=i_start; i<i_start+m_f; i++ ) {
        ...
        */
        ...
        nc = 0;
        /* one left and below; or we are right on it */
        if (l_c < l_start_ghost_c || l_c > l_start_ghost_c+p_ghost_c) {
          SETERRQ3(1,1,"Sorry l %d %d %d",l_c,l_start_ghost_c,l_start_ghost_c+p_ghost_c);
        }
        if (j_c < j_start_ghost_c || j_c > j_start_ghost_c+n_ghost_c) {
          SETERRQ3(1,1,"Sorry j %d %d %d",j_c,j_start_ghost_c,j_start_ghost_c+n_ghost_c);
        }
...
int DAGetInterpolation(DA dac,DA daf, Mat *A,Vec *scale)
{
  ...
  if (dimc == 1 && dofc == 1 && wrapc == DA_NONPERIODIC) {
    ierr = DAGetInterpolation_1D_1(dac,daf,A);CHKERRQ(ierr);
  } else if (dimc == 2 && dofc == 1 && wrapc == DA_NONPERIODIC) {
    ierr = DAGetInterpolation_2D_1(dac,daf,A);CHKERRQ(ierr);
