...
void PETSC_STDCALL davecgetarrayf902_(DA *da,Vec *v,F90Array1d *a,PetscErrorCode *ierr PETSC_F90_2PTR_PROTO(ptrd))
{
  PetscInt    xs,ys,zs,xm,ym,zm,gxs,gys,gzs,gxm,gym,gzm,N,dim,dof;
  PetscScalar *aa;
  ...
  *ierr = VecGetLocalSize(*v,&N);if (*ierr) return;
  ...
  gys++;
  *ierr = VecGetArray(*v,&aa);if (*ierr) return;
  *ierr = F90Array2dCreate(aa,PETSC_SCALAR,gxs,gxm,gys,gxm,a PETSC_F90_2PTR_PARAM(ptrd));if (*ierr) return;
}

void PETSC_STDCALL davecrestorearrayf902_(DA *da,Vec *v,F90Array1d *a,PetscErrorCode *ierr PETSC_F90_2PTR_PROTO(ptrd))
{
  *ierr = VecRestoreArray(*v,0);if (*ierr) return;
