...
PetscErrorCode VecView_MPI_Draw_DA2d(Vec xin,PetscViewer viewer)
{
  ...
  if (!xlocal) {
    if (periodic != DA_NONPERIODIC || s != 1 || st != DA_STENCIL_BOX) {
      ...
      */
      ...
      ierr = PetscInfo((da,"VecView_MPI_Draw_DA2d:Creating auxilary DA for managing graphics ghost points\n"));CHKERRQ(ierr);
    } else {
  ...
  */
  ...
  ierr = PetscInfo((da,"VecView_MPI_Draw_DA2d:Preparing DA 2d contour plot coordinates %G %G %G %G\n",coors[0],coors[1],coors[2],coors[3]));CHKERRQ(ierr);
  ...
  */
  ...
  if (!xcoorl) {
    ...
    ierr = PetscInfo((dag,"VecView_MPI_Draw_DA2d:Creating auxilary DA for managing graphics coordinates ghost points\n"));CHKERRQ(ierr);
  ...
  */
  for (zctx.k=0; zctx.k<zctx.step; zctx.k++) {
    ...
    */
    ...
    ierr = PetscInfo((da,"VecView_MPI_Draw_DA2d:DA 2d contour plot min %G max %G\n",zctx.min,zctx.max));CHKERRQ(ierr);
...
PetscErrorCode PETSCDM_DLLEXPORT VecLoadIntoVector_Binary_DA(PetscViewer viewer,Vec xin)
{
  ...
  ierr = VecDestroy(natural);CHKERRQ(ierr);
  ierr = PetscInfo((xin,"VecLoadIntoVector_Binary_DA:Loading vector from natural ordering into DA\n"));CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
EXTERN_C_END
