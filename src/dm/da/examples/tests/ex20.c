...
int main(int argc,char **argv)
{
  ...
  flg  = PETSC_FALSE;
  ierr = PetscOptionsGetBool(PETSC_NULL,"-same_array",&flg,PETSC_NULL);CHKERRQ(ierr);
  if (flg) {
  ...
  if (norm != 0.0) {
    ...
    ierr = PetscPrintf(PETSC_COMM_WORLD,"ex20: Norm of difference %G should be zero\n",norm);CHKERRQ(ierr);
