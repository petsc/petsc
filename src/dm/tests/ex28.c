...
int main(int argc,char **args)
{
  ...
  for (ii = 0; ii < 3; ++ii) {
    CHKERRQ(DMGetGlobalVector(da1, &xxsplit[ii]));
    CHKERRQ(PetscObjectSetName((PetscObject) xxsplit[ii], "Real space split vector"));
    CHKERRQ(DMGetGlobalVector(da1, &yysplit[ii]));
    CHKERRQ(PetscObjectSetName((PetscObject) yysplit[ii], "FFTW frequency space split vector"));
    CHKERRQ(DMGetGlobalVector(da1, &zzsplit[ii]));
    CHKERRQ(PetscObjectSetName((PetscObject) zzsplit[ii], "FFTW reconstructed split vector"));
  }
  ...
  /* Split xx */
  CHKERRQ(VecStrideGatherAll(xx,xxsplit, INSERT_VALUES)); /*YES! 'Gather' means 'split' (or maybe 'scatter'?)! */

  CHKERRQ(VecNorm(x,NORM_2,&norm));
  CHKERRQ(PetscPrintf(PETSC_COMM_SELF, "|x|_2 = %g\n",norm));

  /* create USFFT object */
  CHKERRQ(MatCreateSeqUSFFT(da,da,&A));
  /* create FFTW object */
  ...
  for (ii = 0; ii < 3; ++ii) {
    CHKERRQ(MatMult(AA,xxsplit[ii],zzsplit[ii]));
  }
  /* Now apply USFFT and FFTW forward several (3) times */
  for (i=0; i<3; ++i) {
    CHKERRQ(MatMult(A,x,y));
    for (ii = 0; ii < 3; ++ii) {
      CHKERRQ(MatMult(AA,xxsplit[ii],yysplit[ii]));
    }
    CHKERRQ(MatMultTranspose(A,y,z));
    for (ii = 0; ii < 3; ++ii) {
      CHKERRQ(MatMult(AA,yysplit[ii],zzsplit[ii]));
    }
  }
  /* Unsplit yy */
  CHKERRQ(VecStrideScatterAll(yysplit, yy, INSERT_VALUES)); /*YES! 'Scatter' means 'collect' (or maybe 'gather'?)! */
  /* Unsplit zz */
  CHKERRQ(VecStrideScatterAll(zzsplit, zz, INSERT_VALUES)); /*YES! 'Scatter' means 'collect' (or maybe 'gather'?)! */
