...
PetscErrorCode  AOSetType(AO ao, AOType method)
{
  ...
  ierr = PetscFListFind(((PetscObject)ao)->comm, AOList, method,PETSC_TRUE,(void (**)(void)) &r);CHKERRQ(ierr);
...
PetscErrorCode  AORegister(const char sname[], const char path[], const char name[], PetscErrorCode (*function)(AO))
{
  ...
  ierr = PetscFListAdd(PETSC_COMM_WORLD,&AOList, sname, fullname, (void (*)(void)) function);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
