...
PetscErrorCode PetscP4estInitialize(void)
{
  ...
  if (sc_package_id == -1) {
    ...
    PetscAssertFalse(sc_package_id == -1,PETSC_COMM_WORLD,PETSC_ERR_LIB,"Could not initialize libsc package used by p4est");
  ...
  if (p4est_package_id == -1) {
    ...
    PetscAssertFalse(p4est_package_id == -1,PETSC_COMM_WORLD,PETSC_ERR_LIB,"Could not initialize p4est");
  }
