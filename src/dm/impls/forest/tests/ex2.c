...
int main(int argc, char **argv)
{
  ...
  for (step = 0; step < adaptSteps; ++step) {
    ...
    if (!transfer_from_base[1]) {
      ...
      PetscAssertFalse(postCount != preCount,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Adaptation not memory neutral: reference count increase from %d to %d",preCount,postCount);
    }
