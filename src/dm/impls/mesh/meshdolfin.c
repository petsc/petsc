...
bool XMLObject::parseBool(const xmlChar* name, const xmlChar** attrs, const char* attribute)
{
  ...
  for (uint i = 0; attrs[i]; i++) {
    ...
    if (xmlStrcasecmp(attrs[i], (xmlChar*) attribute) == 0) {
      ...
      if (strcmp(value.c_str(), "true") == 0 or strcmp(value.c_str(), "1") == 0) return true;
      if (strcmp(value.c_str(), "false") == 0 or strcmp(value.c_str(), "0") == 0) return false;
...
void XMLMesh::endElement(const xmlChar *name)
{
  ...
  case INSIDE_MESH:
    ...
    break;
  case INSIDE_VERTICES:
    if (xmlStrcasecmp(name, (xmlChar*) "vertices") == 0) state = INSIDE_MESH;
    break;
  case INSIDE_CELLS:
    if (xmlStrcasecmp(name, (xmlChar*) "cells") == 0) state = INSIDE_MESH;
    break;
  default:
    break;
  }
...
{
  // Check dimension
  if (mesh->getDimension() != 1) error("Mesh entity (interval) does not match dimension of mesh (%d).", mesh->getDimension());
  // Parse values
...
{
  // Check dimension
  if (mesh->getDimension() != 2) error("Mesh entity (triangle) does not match dimension of mesh (%d).", mesh->getDimension());
  // Parse values
  unsigned int c  = parseUnsignedInt(name, attrs, "index");
  unsigned int v0 = parseUnsignedInt(name, attrs, "v0") + this->numCells;
  unsigned int v1 = parseUnsignedInt(name, attrs, "v1") + this->numCells;
...
{
  // Check dimension
  if (mesh->getDimension() != 3) error("Mesh entity (tetrahedron) does not match dimension of mesh (%d).", mesh->getDimension());
  // Parse values
  unsigned int c  = parseUnsignedInt(name, attrs, "index");
  unsigned int v0 = parseUnsignedInt(name, attrs, "v0") + this->numCells;
  unsigned int v1 = parseUnsignedInt(name, attrs, "v1") + this->numCells;
