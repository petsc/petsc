...
PetscErrorCode ProcessOptions(MPI_Comm comm, Options *options)
{
  ...
  ierr = PetscOptionsBegin(comm, "", "Options for the Sieve package tests", "Sieve");CHKERRQ(ierr);
    ierr = PetscOptionsBool("-function", "Run functionality tests", "unitTests", options->function, &options->function, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-stress", "Run stress tests", "unitTests", options->stress, &options->stress, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-memory", "Run memory tests", "unitTests", options->memory, &options->memory, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-stl", "Run STL tests", "unitTests", options->stl, &options->stl, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-sifter", "Run Sifter tests", "unitTests", options->sifter, &options->sifter, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-sieve", "Run Sieve tests", "unitTests", options->sieve, &options->sieve, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-isieve", "Run ISieve tests", "unitTests", options->isieve, &options->isieve, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-section", "Run Section tests", "unitTests", options->section, &options->section, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-isection", "Run ISection tests", "unitTests", options->isection, &options->isection, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-imesh", "Run IMesh tests", "unitTests", options->imesh, &options->imesh, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-distribution", "Run Distribution tests", "unitTests", options->distribution, &options->distribution, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsBool("-idistribution", "Run IDistribution tests", "unitTests", options->idistribution, &options->idistribution, NULL);CHKERRQ(ierr);
