...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  if (flag) {
    ...
    if (flag) {
      ...
      options->func = x2_x4;
      if (!flag) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Unknown function %s",fstring);
    }
  }
...
static PetscErrorCode PerturbVertices(DM dm, AppCtx *user)
{
  ...
  if (bs != cdim) SETERRQ(PetscObjectComm((PetscObject) dm), PETSC_ERR_ARG_SIZ, "Coordinate vector has wrong block size %D != %D", bs, cdim);
...
static PetscErrorCode TestL2ProjectionParticlesToField(DM dm, DM sw, AppCtx *user)
{
  ...
  for (m = 0; m < 3; ++m) {
    if (PetscAbsReal((fmoments[m] - pmoments[m])/fmoments[m]) > user->momentTol) SETERRQ(comm, PETSC_ERR_ARG_WRONG, "Moment %D error too large %g > %g", m, PetscAbsReal((fmoments[m] - pmoments[m])/fmoments[m]), user->momentTol);
  }
