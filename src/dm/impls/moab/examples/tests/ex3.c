...
int main(int argc,char **argv)
{
  ...
  ierr = DMDASetBlockFills(dm,NULL,ofill);CHKERRQ(ierr);
...
PetscErrorCode InitializeVariableSystem(DM dm)
{
  ...
  const PetscInt nvars=3*N+N*N;
  const PetscInt maxwidth=16;
  char           field[maxwidth];

  PetscFunctionBeginUser;
  ierr = DMMoabSetFields(dm, nvars, NULL);CHKERRQ(ierr);
  ierr = DMMoabSetBlockSize(dm, nvars);CHKERRQ(ierr);
...
PetscErrorCode MyMonitorSetUp(TS ts)
{
  ...
  ierr = PetscViewerDrawOpen(PetscObjectComm((PetscObject)dm),NULL,"",PETSC_DECIDE,PETSC_DECIDE,600,400,&ctx->viewer);CHKERRQ(ierr);
