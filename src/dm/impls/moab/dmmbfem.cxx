                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  if (nverts == 4) { /* Linear Quadrangle */
    ...
    for (j = 0; j < npts; j++)
    {
      ...
      if (*volume < 1e-12) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Quadrangular element has zero volume: %g. Degenerate element or invalid connectivity", *volume);
    ...
    /* invert the jacobian */
    ierr = DMatrix_Invert_2x2_Internal(jacobian, ijacobian, volume);CHKERRQ(ierr);
    if (*volume < 1e-12) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Triangular element has zero volume: %g. Degenerate element or invalid connectivity", (double)*volume);
                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  if (nverts == 8) { /* Linear Hexahedra */
    ...
    for (j = 0; j < npts; j++) {
      ...
      if (*volume < 1e-8) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Hexahedral element has zero volume: %g. Degenerate element or invalid connectivity", *volume);
    ...
    /* invert the jacobian */
    ierr = DMatrix_Invert_3x3_Internal(jacobian, ijacobian, volume);CHKERRQ(ierr);
    if (*volume < 1e-8) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Tetrahedral element has zero volume: %g. Degenerate element or invalid connectivity", (double)*volume);
                                     ...
                                     PetscReal *fe_basis, PetscReal **fe_basis_derivatives)
{
  ...
  if (idim != dim) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Dimension mismatch: provided (%D) vs quadrature (%D)",idim,dim);
  ...
  PetscReal *jacobian, PetscReal *ijacobian, PetscReal* dvolume)
{
  ...
  if (volume < 1e-12) SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Element has zero volume: %g. Degenerate element or invalid connectivity", volume);
