                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  if (phypts) {
    PetscCall(PetscArrayzero(phypts, npts * 3));
  }
                                     ...
                                     PetscReal *fe_basis, PetscReal **fe_basis_derivatives)
{
  ...
  switch (dim) {
  case 1:
    PetscCall(Compute_Lagrange_Basis_1D_Internal(nverts, coordinates, npoints, quadpts, phypts,
                                               jacobian_quadrature_weight_product, fe_basis,
                                               (compute_der ? fe_basis_derivatives[0] : NULL),
                                               jacobian, ijacobian, &volume));
    break;
  case 2:
    PetscCall(Compute_Lagrange_Basis_2D_Internal(nverts, coordinates, npoints, quadpts, phypts,
                                               jacobian_quadrature_weight_product, fe_basis,
                                               (compute_der ? fe_basis_derivatives[0] : NULL),
    ...
    break;
  case 3:
    PetscCall(Compute_Lagrange_Basis_3D_Internal(nverts, coordinates, npoints, quadpts, phypts,
                                               jacobian_quadrature_weight_product, fe_basis,
                                               (compute_der ? fe_basis_derivatives[0] : NULL),
                                               (compute_der ? fe_basis_derivatives[1] : NULL),
  ...
  PetscReal *jacobian, PetscReal *ijacobian, PetscReal* dvolume)
{
  ...
  if (phypts) {
    PetscCall(PetscArrayzero(phypts, /*npts=1 * */ 3));
  }
...
PetscErrorCode DMMoabPToRMapping(const PetscInt dim, const PetscInt nverts, const PetscReal *coordinates, const PetscReal* xphy, PetscReal* natparam, PetscReal* phi)
{
  ...
  if (phi) {
    PetscCall(PetscArraycpy(phi, phibasis, nverts));
  }
  PetscFunctionReturn(0);
}
