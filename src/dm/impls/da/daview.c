...
PetscErrorCode DMView_DA_Matlab(DM da,PetscViewer viewer)
{
  ...
  if (rank == 0) {
    ...
    PetscAssertFalse(!mx,PETSC_COMM_SELF,PETSC_ERR_LIB,"Unable to generate MATLAB struct array to hold DMDA information");
...
PetscErrorCode DMView_DA_VTK(DM da, PetscViewer viewer)
{
  ...
  PetscAssertFalse(!da->coordinates,PetscObjectComm((PetscObject)da),PETSC_ERR_SUP, "VTK output requires DMDA coordinates.");
  /* Write Header */
