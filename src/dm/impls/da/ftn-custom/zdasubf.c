...
PETSC_EXTERN void PETSC_STDCALL  dmdagetprocessorsubset_(DM *da,DMDirection *dir,PetscInt *gp,MPI_Fint *fcomm, int *__ierr )
{
  MPI_Comm comm;
  *__ierr = DMDAGetProcessorSubset(*da,*dir,*gp,&comm);
  *fcomm = MPI_Comm_c2f(comm);
}
PETSC_EXTERN void PETSC_STDCALL  dmdagetprocessorsubsets_(DM *da,DMDirection *dir,MPI_Fint *subfcomm, int *__ierr )
{
  MPI_Comm subcomm;
