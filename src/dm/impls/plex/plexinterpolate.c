...
static PetscErrorCode DMPlexInterpolateFaces_Internal(DM dm, PetscInt cellDepth, DM idm)
{
  ...
  for (c = cStart; c < cEnd; ++c) {
    ...
    for (cf = 0; cf < numFaces; foff += faceSizes[cf], ++cf) {
      ...
      PetscAssertFalse(faceSize > 4,PETSC_COMM_SELF, PETSC_ERR_SUP, "Do not support faces of size %D > 4", faceSize);
      key.i = face[0];
  ...
  /* We need to number faces contiguously among types */
  {
    ...
    if (numFT > 1) {
      ...
      for (ct = 1; ct < DM_NUM_POLYTOPES; ++ct) {
        PetscAssertFalse(faceTypeStart[ct] != faceTypeStart[ct-1] + faceTypeNum[ct],PETSC_COMM_SELF, PETSC_ERR_PLIB, "Inconsistent numbering for cell type %s, %D != %D + %D", DMPolytopeTypes[ct], faceTypeStart[ct], faceTypeStart[ct-1], faceTypeNum[ct]);
      }
    }
  }
  ...
  for (c = cStart; c < cEnd; ++c) {
    ...
    for (cf = 0; cf < numFaces; foff += faceSizes[cf], ++cf) {
      ...
      ierr = PetscSortInt(faceSize, (PetscInt *) &key);CHKERRQ(ierr);
      ierr = PetscHashIJKLPut(faceTable, key, &iter, &missing);CHKERRQ(ierr);
      PetscAssertFalse(missing,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Missing face (cell %D, lf %D)", c, cf);
  ...
  for (c = cStart; c < cEnd; ++c) {
    ...
    for (cf = 0; cf < numFaces; foff += faceSizes[cf], ++cf) {
      ...
      if (fcone[0] < 0) {ierr = DMPlexSetCone(idm, f, face);CHKERRQ(ierr);}
      {
        ...
        PetscAssertFalse(coneSize != faceSize,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Invalid number of face vertices %D for face %D should be %D", coneSize, f, faceSize);
...
PetscErrorCode DMPlexOrientInterface_Internal(DM dm)
{
  ...
  PetscAssertFalse(maxConeSize > 4,comm, PETSC_ERR_SUP, "This method does not support cones of size %D", maxConeSize);
...
PetscErrorCode DMPlexInterpolatePointSF(DM dm, PetscSF pointSF)
{
  ...
  PetscAssertFalse(ov,comm, PETSC_ERR_SUP, "Interpolation of overlapped DMPlex not implemented yet");
  ...
  PetscAssertFalse(Nr < 0,comm, PETSC_ERR_ARG_WRONGSTATE, "This DMPlex is distributed but input PointSF has no graph set");
  ...
  */
  ...
  /*   Note that this section is indexed by offsets into leaves, not by point number */
  {
    ...
    ierr = PetscSFBcastBegin(sfCandidates, MPIU_2INT, candidates, candidatesRemote,MPI_REPLACE);CHKERRQ(ierr);
    ierr = PetscSFBcastEnd(sfCandidates, MPIU_2INT, candidates, candidatesRemote,MPI_REPLACE);CHKERRQ(ierr);
    ...
    for (r = 0, idx = 0, idx2 = 0; r < Nr; ++r) {
      ...
      for (deg = 0; deg < rootdegree[r]; ++deg, ++idx) {
        ...
        for (d = 0; d < dof; ++d) {
          ...
          PetscAssertFalse(Np > 4,PETSC_COMM_SELF, PETSC_ERR_SUP, "Cannot handle face (%D, %D) at (%D, %D, %D) with %D cone points", rface.rank, rface.index, r, idx, d, Np);
          fcp0.rank  = rank;
          fcp0.index = r;
          d += Np;
      ...
      for (deg = 0; deg < rootdegree[r]; ++deg, ++idx2) {
        ...
        for (d = 0; d < dof; ++d) {
          ...
          PetscAssertFalse(Np > 4,PETSC_COMM_SELF, PETSC_ERR_SUP, "Cannot handle faces with %D cone points", Np);
          fcp0.rank  = rank;
          fcp0.index = r;
          d += Np;
          ...
          ierr = PetscHashIJKLRemotePut(faceTable, key, &iter, &missing);CHKERRQ(ierr);
          PetscAssertFalse(missing,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Root %D Idx %D ought to have an associated face", r, idx2);
  ...
  /* Step 4: Push back owned faces */
  {
    ...
    ierr = PetscSFBcastBegin(sfClaims, MPIU_2INT, candidatesRemote, claims,MPI_REPLACE);CHKERRQ(ierr);
    ierr = PetscSFBcastEnd(sfClaims, MPIU_2INT, candidatesRemote, claims,MPI_REPLACE);CHKERRQ(ierr);
    ...
    for (p = Nl; p < Nl + NlNew; ++p) {
      ...
      PetscAssertFalse(claims[off].rank < 0 || claims[off].index < 0,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Invalid claim for local point %D, (%D, %D)", localPointsNew[p], claims[off].rank, claims[off].index);
...
PetscErrorCode DMPlexInterpolate(DM dm, DM *dmInt)
{
  ...
  PetscAssertFalse(interpolated == DMPLEX_INTERPOLATED_PARTIAL,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Not for partially interpolated meshes");
...
PetscErrorCode DMPlexCopyCoordinates(DM dmA, DM dmB)
{
  ...
  PetscAssertFalse((vEndA-vStartA) != (vEndB-vStartB),PETSC_COMM_SELF, PETSC_ERR_ARG_SIZ, "The number of vertices in first DM %d != %d in the second DM", vEndA-vStartA, vEndB-vStartB);
  ...
  PetscAssertFalse(Nf > 1,PETSC_COMM_SELF, PETSC_ERR_ARG_SIZ, "The number of coordinate fields must be 1, not %D", Nf);
  if (!coordSectionB) {
    PetscInt dim;
  ...
  if (cStartA <= cS && cS < cEndA) { /* localized coordinates */
    PetscAssertFalse((cEndA-cStartA) != (cEndB-cStartB),PETSC_COMM_SELF, PETSC_ERR_ARG_SIZ, "The number of cells in first DM %D != %D in the second DM", cEndA-cStartA, cEndB-cStartB);
...
PetscErrorCode DMPlexUninterpolate(DM dm, DM *dmUnint)
{
  ...
  /* Reduce SF */
  {
    ...
    if (vEnd >= 0) {
      ...
      PetscAssertFalse(n != numLeavesUn,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Inconsistent number of leaves %d != %d", n, numLeavesUn);
...
PetscErrorCode DMPlexIsInterpolated(DM dm, DMPlexInterpolatedFlag *interpolated)
{
  ...
  if (plex->interpolated < 0) {
    ...
    PetscAssertFalse(flg != plex->interpolated,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Stashed DMPlexInterpolatedFlag %s is inconsistent with current %s", DMPlexInterpolatedFlags[plex->interpolated], DMPlexInterpolatedFlags[flg]);
  }
