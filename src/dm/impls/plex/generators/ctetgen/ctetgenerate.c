    ...
    if (interpolate) {
      ...
      for (f = 0; f < out->numberoftrifaces; f++) {
        if (out->trifacemarkerlist[f]) {
          ...
          if (numFaces != 1) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_PLIB, "Three vertices must cover only one face, not %D", numFaces);
...
PETSC_EXTERN PetscErrorCode DMPlexRefine_CTetgen(DM dm, PetscReal *maxVolumes, DM *dmRefined)
{
  ...
  if (isInterpolated == DMPLEX_INTERPOLATED_FULL && in->numberoftrifaces > 0) {
    ...
    for (f = fStart; f < fEnd; ++f) {
      ...
      if (Nv != 3) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Face %D has %D vertices, not 3", f, Nv);
  ...
  if (in->numberoftetrahedra > 0) {
    ...
    for (c = cStart; c < cEnd; ++c) {
      ...
      if ((closureSize != 5) && (closureSize != 15)) SETERRQ(comm, PETSC_ERR_ARG_WRONG, "Mesh has cell which is not a tetrahedron, %D vertices in closure", closureSize);
