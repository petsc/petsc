...
PetscErrorCode DMComputeL2Diff_Plex(DM dm, PetscReal time, PetscErrorCode (**funcs)(PetscInt, PetscReal, const PetscReal [], PetscInt, PetscScalar *, void *), void **ctxs, Vec X, PetscReal *diff)
{
  ...
  if (qNc != numComponents) SETERRQ2(PetscObjectComm((PetscObject) dm), PETSC_ERR_ARG_SIZ, "Quadrature components %D != %D field components", qNc, numComponents);
    ...
    for (field = 0, fieldOffset = 0; field < numFields; ++field) {
      ...
      for (q = 0; q < Nq; ++q) {
        ...
        for (fc = 0; fc < Nc; ++fc) {
          ...
          elemDiff += PetscSqr(PetscRealPart(interpolant[fc] - funcVal[fc]))*quadWeights[q*qNc+qc+fc]*detJ[q];
        }
      }
      fieldOffset += Nb;
      fc += Nc;
    ...
    for (field = 0, fieldOffset = 0; field < numFields; ++field) {
      ...
      for (q = 0; q < Nq; ++q) {
        ...
        for (fc = 0; fc < Nc; ++fc) {
          ...
          elemDiff += PetscSqr(PetscRealPart(interpolant[fc] - funcVal[fc]))*quadWeights[q*qNc+fc+qc]*detJ[q];
        }
      }
      fieldOffset += Nb;
      qc          += Nc;
