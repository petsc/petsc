...
static PetscErrorCode DMPlexCreateFluent_ReadValues(PetscViewer viewer, void *data, PetscInt count, PetscDataType dtype, PetscBool binary)
{
  ...
  if (!binary && dtype == PETSC_INT) {
    ...
    for (i = 0; i < count; i++) {
      ...
      PetscAssertFalse(snum != 1,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "File is not a valid Fluent file");
...
static PetscErrorCode DMPlexCreateFluent_ReadSection(PetscViewer viewer, FluentSection *s)
{
  ...
  if (s->index == 0) {           /* Comment */
    ...
    PetscAssertFalse(snum != 5,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "File is not a valid Fluent file");
    if (s->zoneID > 0) {
    ...
    if (s->zoneID == 0) {  /* Header section */
      ...
      PetscAssertFalse(snum != 4,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "File is not a valid Fluent file");
    } else {               /* Data section */
      snum = sscanf(buffer, "(%x %x %x %d %d)", &(s->zoneID), &(s->first), &(s->last), &(s->type), &(s->nd));
      PetscAssertFalse(snum != 5,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "File is not a valid Fluent file");
      if (s->nd == 0) {
      ...
      for (f = 0; f < numFaces; f++) {
        if (s->nd == 0) {
          ...
          if (numEntries == PETSC_DETERMINE) {
            ...
            PetscAssertFalse(numEntries != numFaceVert + 2,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "No support for mixed faces in Fluent files");
          }
        }
...
PetscErrorCode DMPlexCreateFluent(MPI_Comm comm, PetscViewer viewer, PetscBool interpolate, DM *dm)
{
    ...
    do {
      ...
      if (s.index == 2) {                 /* Dimension */
        ...
        if (s.zoneID == 0) numVertices = s.last;
          ...
          PetscAssertFalse(coordsIn,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Currently no support for multiple coordinate sets in Fluent files");
        ...
        if (s.zoneID == 0) {  /* Header section */
          ...
          PetscAssertFalse(numFaceVertices != PETSC_DETERMINE && s.nd != numFaceVertices,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Mixed facets in Fluent files are not supported");
          PetscAssertFalse(numFaces < 0,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "No header section for facets in Fluent file");
  ...
  PetscAssertFalse(dim < 0,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Fluent file does not include dimension");
  ...
  if (rank == 0) {
    PetscAssertFalse(numCells < 0,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Unknown number of cells in Fluent file");
  ...
  if (rank == 0 && faces) {
    ...
    for (f = 0; f < numFaces; f++) {
      ...
      if (cl > 0 && cr > 0) {
        ...
        PetscAssertFalse(meetSize != 1,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Could not determine Plex facet for Fluent face %d", f);
        ...
        PetscAssertFalse(joinSize != 1,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Could not determine Plex facet for Fluent face %d", f);
