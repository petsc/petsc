...
!
    PetscInt, parameter :: numVertices = 3, numCorners = 3
...
! Jacobian (constant, repeated tria_qpts times)
    ...
    PetscReal, parameter :: vertexCoords(6) = [1.0, 1.0, 5.0, 5.0, 3.0, 6.0]
    PetscInt, parameter :: cells(3) = [0, 1, 2]
    ...
    PetscCallA(DMPlexGetHeightStratum(dm, 0_PETSC_INT_KIND, cStart, PETSC_NULL_INTEGER, ierr))
...
!
    PetscInt, parameter :: numVertices = 4, numCorners = 4

! Number of quadrature points
    PetscInt, parameter :: quad_qpts = 4

    PetscReal :: J(quad_qpts*numDim**2), invJ(quad_qpts*numDim**2), v(quad_qpts*numDim), detJ(quad_qpts)
    PetscReal, parameter :: vertexCoords(8) = [-3.0, -2.0, 3.0, -1.0, 2.0, 4.0, -2.0, 3.0]
    PetscInt, parameter  :: cells(4) = [0, 1, 2, 3]
    PetscReal, parameter :: a = -1.0, b = 1.0
    PetscInt, parameter :: nc = 1, npoints = 2
    ...
    PetscCallA(DMPlexComputeCellGeometryFEM(dm, cStart, q, v, J, invJ, detJ, ierr))
    do i = 1, quad_qpts
      PetscCheckA(all(abs(v((i - 1)*numDim + 1:i*numDim) - quad_v(Gauss_rs(i))) < eps), PETSC_COMM_WORLD, PETSC_ERR_PLIB, 'Wrong mapped quadrature points (quadrilateral)')
      PetscCheckA(all(abs(J((i - 1)*numDim**2 + 1:i*numDim**2) - quad_J(Gauss_rs(i))) < eps), PETSC_COMM_WORLD, PETSC_ERR_PLIB, 'Wrong jacobian (quadrilateral)')
    end do
    PetscCallA(PetscQuadratureDestroy(q, ierr))
    PetscCallA(DMDestroy(dm, ierr))
