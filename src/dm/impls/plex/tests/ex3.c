    ...
    if (aSec) {
      ...
      for (c = cStart; c < cEnd; c++) {
        ...
        if (cDof) {
          ...
          for (j = 0; j < cDof; j++) {
            PetscCall(MatSetValue(cMat,cOff+j,aOff+j,1.,INSERT_VALUES));
          }
        }
      }
...
static PetscErrorCode TestInjector(DM dm, AppCtx *user)
{
  ...
  if (refTree) {
    ...
    if (rank == 0) {
      PetscCall(MatView(inj,PETSC_VIEWER_STDOUT_SELF));
    }
...
static PetscErrorCode TestFVGrad(DM dm, AppCtx *user)
{
  ...
  dmRedist = NULL;
  if (size > 1) {
    PetscCall(DMPlexDistributeOverlap(dm,1,NULL,&dmRedist));
  }
  if (!dmRedist) {
    dmRedist = dm;
...
static PetscErrorCode CheckInterpolation(DM dm, PetscBool checkRestrict, PetscInt order, AppCtx *user)
{
  ...
  if (checkRestrict) {PetscCall(MatRestrict(Interp, iu, fu));PetscCall(VecPointwiseMult(fu, scaling, fu));}
