...
PetscErrorCode TestLookup(DMLabel label, AppCtx *user)
{
  ...
  for (p = pStart; p < pEnd; ++p) {
    ...
    if (((val >= 0) && !has) || ((val < 0) && has)) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_PLIB, "Label value %D does not match contains check %D for point %D", val, (PetscInt) has, p);
    if (has) ++n;
  }
  if (n != user->size) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_PLIB, "Invalid number of label points detected %D does not match number set %D", n, user->size);
...
PetscErrorCode TestClear(DMLabel label, AppCtx *user)
{
  ...
  for (p = pStart; p < pEnd; p++) {
    ...
    if (val != defaultValue) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Expected default value %D after clearing point %D, got %D",defaultValue,p,val);
    if (hasPoint) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Label contains %D after clearing",p);
  }
  PetscFunctionReturn(0);
}
