...
static PetscErrorCode TransferWrite(MPI_Comm comm, PetscViewer viewer,FILE *fp,PetscMPIInt srank,PetscMPIInt root,const void *send,void *recv,PetscMPIInt count,MPI_Datatype mpidatatype,PetscMPIInt tag)
{
  ...
  if (rank == srank && rank != root) {
    ...
    if (root == srank) {        /* self */
      ...
      PetscAssertFalse(count != nrecv,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Array size mismatch");
      buffer = recv;
    }
...
static PetscErrorCode DMPlexGetVTKConnectivity(DM dm, PetscBool localized, PieceInfo *piece,PetscVTKInt **oconn,PetscVTKInt **ooffsets,PetscVTKType **otypes)
{
  ...
  PetscAssertFalse(countcell != piece->ncells,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Inconsistent cell count");
  PetscAssertFalse(countconn != piece->nconn,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Inconsistent connectivity count");
  *oconn    = conn;
  *ooffsets = offsets;
  *otypes   = types;
  PetscFunctionReturn(0);
      ...
      for (link=vtk->link; link; link=link->next) {
        ...
        for (i=0; field<(nfields?nfields:1); field++) {
          ...
          if (link->ft == PETSC_VTK_CELL_VECTOR_FIELD) {
            vector = PETSC_TRUE;
            PetscAssertFalse(fbs > 3,PetscObjectComm((PetscObject)viewer),PETSC_ERR_ARG_SIZ,"Cell vector fields can have at most 3 components, %D given", fbs);
            for (j = 0; j < fbs; j++) {
        ...
        //PetscAssertFalse(i != bs,comm,PETSC_ERR_PLIB,"Total number of field components %D != block size %D",i,bs);
      }
      ierr = PetscFPrintf(PETSC_COMM_SELF,fp,"      </CellData>\n");CHKERRQ(ierr);
      ...
      for (link=vtk->link; link; link=link->next) {
        ...
        for (i=0; field<(nfields?nfields:1); field++) {
          ...
          if (link->ft == PETSC_VTK_POINT_VECTOR_FIELD) {
            PetscAssertFalse(fbs > 3,PetscObjectComm((PetscObject)viewer),PETSC_ERR_ARG_SIZ,"Point vector fields can have at most 3 components, %D given", fbs);
#if defined(PETSC_USE_COMPLEX)
            ierr = PetscFPrintf(comm,fp,"        <DataArray type=\"%s\" Name=\"%s%s.Re\" NumberOfComponents=\"3\" format=\"appended\" offset=\"%D\" />\n",precision,vecname,fieldname,boffset);CHKERRQ(ierr);
            boffset += gpiece[r].nvertices*3*sizeof(PetscVTUReal) + sizeof(int);
            ierr = PetscFPrintf(comm,fp,"        <DataArray type=\"%s\" Name=\"%s%s.Im\" NumberOfComponents=\"3\" format=\"appended\" offset=\"%D\" />\n",precision,vecname,fieldname,boffset);CHKERRQ(ierr);
      ...
      {                         /* Position */
        ...
        if (copy) {
          ...
          if (localized) {
            ...
            PetscAssertFalse(cnt != piece.nvertices,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Count does not match");
          } else {
        ...
        for (i=0; field<(nfields?nfields:1); field++) {
          ...
          if (vector) {
            ...
            for (l = 0; l < loops_per_scalar; l++) {
              ...
              PetscAssertFalse(cnt != piece.ncells*3,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Count does not match");
            ...
            for (i=0; i<fbs; i++) {
              ...
              for (l = 0; l < loops_per_scalar; l++) {
                ...
                PetscAssertFalse(cnt != piece.ncells,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Count does not match");
        ...
        for (i=0; field<(nfields?nfields:1); field++) {
          ...
          if (link->ft == PETSC_VTK_POINT_VECTOR_FIELD) {
            ...
            for (l = 0; l < loops_per_scalar; l++) {
              ...
              PetscAssertFalse(cnt != piece.nvertices*3,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Count does not match");
