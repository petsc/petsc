...
PETSC_INTERN PetscErrorCode DMStagSetUniformCoordinatesExplicit_2d(DM dm,PetscReal xmin,PetscReal xmax,PetscReal ymin,PetscReal ymax)
{
  ...
  for (s=0; s<3; ++s) {
    PetscAssertFalse(stagCoord->dof[s] !=0 && stagCoord->dof[s] != 2,PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Coordinate DM in 2 dimensions must have 0 or 2 dof on each stratum, but stratum %d has %d dof",s,stagCoord->dof[s]);
  }
...
static PetscErrorCode DMStagSetUpBuildRankGrid_2d(DM dm)
{
  ...
  n = stag->nRanks[1];
  if (m != PETSC_DECIDE) {
    PetscAssertFalse(m < 1,PetscObjectComm((PetscObject)dm),PETSC_ERR_ARG_OUTOFRANGE,"Non-positive number of ranks in X direction: %D",m);
    else PetscAssertFalse(m > size,PetscObjectComm((PetscObject)dm),PETSC_ERR_ARG_OUTOFRANGE,"Too many ranks in X direction: %D %d",m,size);
  }
  if (n != PETSC_DECIDE) {
    PetscAssertFalse(n < 1,PetscObjectComm((PetscObject)dm),PETSC_ERR_ARG_OUTOFRANGE,"Non-positive number of ranks in Y direction: %D",n);
    else PetscAssertFalse(n > size,PetscObjectComm((PetscObject)dm),PETSC_ERR_ARG_OUTOFRANGE,"Too many ranks in Y direction: %D %d",n,size);
  }
  if (m == PETSC_DECIDE || n == PETSC_DECIDE) {
    ...
    PetscAssertFalse(m*n != size,PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Unable to create partition, check the size of the communicator and input m and n ");
  } else PetscAssertFalse(m*n != size,PetscObjectComm((PetscObject)dm),PETSC_ERR_ARG_OUTOFRANGE,"Given Bad partition. Product of sizes (%D) does not equal communicator size (%d)",m*n,size);
...
PETSC_INTERN PetscErrorCode DMCreateMatrix_Stag_2D_AIJ_Assemble(DM dm,Mat A)
{
  ...
  if (stencil_type == DMSTAG_STENCIL_NONE) {
    ...
    ierr = PetscMalloc1(dof[1],&row_edge_down);CHKERRQ(ierr);
    for (PetscInt c=0; c<dof[1]; ++c) {
      row_edge_down[c].loc = DMSTAG_DOWN;
      row_edge_down[c].c = c;
    }

    ierr = PetscMalloc1(dof[1],&row_edge_left);CHKERRQ(ierr);
    for (PetscInt c=0; c<dof[1]; ++c) {
      row_edge_left[c].loc = DMSTAG_LEFT;
      row_edge_left[c].c = c;
    }

    ierr = PetscMalloc1(dof[2],&row_element);CHKERRQ(ierr);
    ...
    for (PetscInt ey=start[1]; ey<start[1]+n[1]+n_extra[1]; ++ey) {
      for (PetscInt ex=start[0]; ex<start[0]+n[0]+n_extra[0]; ++ex) {
        ...
        if (ex < N[0]) {
          ...
          ierr = DMStagMatSetValuesStencil(dm,A,dof[1],row_edge_down,dof[1],row_edge_down,NULL,INSERT_VALUES);CHKERRQ(ierr);
        }
        if (ey < N[1]) {
          for (PetscInt c=0; c<dof[1]; ++c){
          ...
          ierr = DMStagMatSetValuesStencil(dm,A,dof[1],row_edge_left,dof[1],row_edge_left,NULL,INSERT_VALUES);CHKERRQ(ierr);
        }
        if (ex < N[0] && ey < N[1]) {
          for (PetscInt c=0; c<dof[2]; ++c) {
            row_element[c].i = ex;
          ...
          ierr = DMStagMatSetValuesStencil(dm,A,dof[2],row_element,dof[2],row_element,NULL,INSERT_VALUES);CHKERRQ(ierr);
        }
      }
    }
    ierr = PetscFree(row_vertex);CHKERRQ(ierr);
    ierr = PetscFree(row_edge_left);CHKERRQ(ierr);
    ierr = PetscFree(row_edge_down);CHKERRQ(ierr);
