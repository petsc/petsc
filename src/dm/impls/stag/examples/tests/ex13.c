...
int main(int argc,char **argv)
{
  ...
  switch (dim) {
    ...
    default: SETERRQ1(PetscObjectComm((PetscObject)dm),PETSC_ERR_SUP,"Not implemented for dimension %D",dim);
  }
...
static PetscErrorCode Test2_1d(DM dm)
{
  ...
  for (i=startx; i<startx+nx+nExtrax; ++i) {
    for (c=0; c<dof0; ++c) {
      const PetscScalar val = TEST_FUNCTION(i,0,0,idxLeft,c);
      arr[i][idxLeft+c] = val;
    }
    if (i < startx+nx) {
      for (c=0; c<dof1; ++c) {
        const PetscScalar val = TEST_FUNCTION(i,0,0,idxElement,c);
        arr[i][idxElement+c] = val;
      }
    }
  }
  ...
  for (i=startx; i<startx+nx+nExtrax; ++i) {
    for (c=0; c<dof0; ++c) {
      const PetscScalar val = TEST_FUNCTION(i,0,0,idxLeft,c);
      if (arr[i][idxLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
    }
    if (i < startx+nx) {
      for (c=0; c<dof1; ++c) {
        const PetscScalar val = TEST_FUNCTION(i,0,0,idxElement,c);
...
static PetscErrorCode Test2_2d(DM dm)
{
  ...
  for (j=starty; j<starty+ny+nExtray; ++j) {
    for (i=startx; i<startx+nx+nExtrax; ++i) {
      for (c=0; c<dof0; ++c) {
        const PetscScalar val = TEST_FUNCTION(i,j,0,idxDownLeft,c);
        arr[j][i][idxDownLeft+c] = val;
      }
      if (j < starty+ny) {
        for (c=0; c<dof1; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxLeft,c);
          arr[j][i][idxLeft+c] = val;
        }
      }
      if (i < startx+nx) {
        for (c=0; c<dof1; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxDown,c);
          arr[j][i][idxDown+c] = val;
        }
      }
      if (i < startx+nx && j < starty+ny) {
        for (c=0; c<dof2; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxElement,c);
          arr[j][i][idxElement+c] = val;
        }
      }
    }
  }
  ...
  for (j=starty; j<starty+ny+nExtray; ++j) {
    for (i=startx; i<startx+nx+nExtrax; ++i) {
      for (c=0; c<dof0; ++c) {
        const PetscScalar val = TEST_FUNCTION(i,j,0,idxDownLeft,c);
        if (arr[j][i][idxDownLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
      }
      if (j < starty+ny) {
        for (c=0; c<dof1; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxLeft,c);
          if (arr[j][i][idxLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      } else {
        for (c=0; c<dof1; ++c) {
          if (arr[j][i][idxLeft+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      }
      if (i < startx+nx) {
        for (c=0; c<dof1; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxDown,c);
          if (arr[j][i][idxDown+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      } else {
        for (c=0; c<dof1; ++c) {
          if (arr[j][i][idxDown+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      }
      if (i < startx+nx && j < starty+ny) {
        for (c=0; c<dof2; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,0,idxElement,c);
          if (arr[j][i][idxElement+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      } else {
        for (c=0; c<dof2; ++c) {
          if (arr[j][i][idxElement+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
      }
    }
  }
...
static PetscErrorCode Test2_3d(DM dm)
{
  ...
  for (k=startz; k<startz+nz+nExtraz; ++k) {
    for (j=starty; j<starty+ny+nExtray; ++j) {
      for (i=startx; i<startx+nx+nExtrax; ++i) {
        for (c=0; c<dof0; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackDownLeft,c);
          arr[k][j][i][idxBackDownLeft+c] = val;
        }
        if (k < startz+nz) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxDownLeft,c);
            arr[k][j][i][idxDownLeft+c] = val;
          }
        }
        if (j < starty+ny) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackLeft,c);
            arr[k][j][i][idxBackLeft+c] = val;
          }
        }
        if (i < startx+nx) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackDown,c);
            arr[k][j][i][idxBackDown+c] = val;
          }
        }
        if (j < starty+ny && k < startz+nz) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxLeft,c);
            arr[k][j][i][idxLeft+c] = val;
          }
        }
        if (i < startx+nx && k < startz+nz) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxDown,c);
            arr[k][j][i][idxDown+c] = val;
          }
        }
        if (i < startx+nx && j < starty+ny) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBack,c);
            arr[k][j][i][idxBack+c] = val;
          }
        }
        if (i < startx+nx && j < starty+ny && k < startz+nz) {
          for (c=0; c<dof3; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxElement,c);
            arr[k][j][i][idxElement+c] = val;
          }
        }
      }
    }
  ...
  for (k=startz; k<startz+nz+nExtraz; ++k) {
    for (j=starty; j<starty+ny+nExtray; ++j) {
      for (i=startx; i<startx+nx+nExtrax; ++i) {
        for (c=0; c<dof0; ++c) {
          const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackDownLeft,c);
          if (arr[k][j][i][idxBackDownLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
        }
        if (k < startz+nz) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxDownLeft,c);
            if (arr[k][j][i][idxDownLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof1; ++c) {
            if (arr[k][j][i][idxDownLeft+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (j < starty+ny) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackLeft,c);
            if (arr[k][j][i][idxBackLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof1; ++c) {
            if (arr[k][j][i][idxBackLeft+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (i < startx+nx) {
          for (c=0; c<dof1; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBackDown,c);
            if (arr[k][j][i][idxBackDown+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof1; ++c) {
            if (arr[k][j][i][idxBackDown+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (j < starty+ny && k < startz+nz) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxLeft,c);
            if (arr[k][j][i][idxLeft+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof2; ++c) {
            if (arr[k][j][i][idxLeft+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (i < startx+nx && k < startz+nz) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxDown,c);
            if (arr[k][j][i][idxDown+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof2; ++c) {
            if (arr[k][j][i][idxDown+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (i < startx+nx && j < starty+ny) {
          for (c=0; c<dof2; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxBack,c);
            if (arr[k][j][i][idxBack+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof2; ++c) {
            if (arr[k][j][i][idxBack+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
        if (i < startx+nx && j < starty+ny && k < startz+nz) {
          for (c=0; c<dof3; ++c) {
            const PetscScalar val = TEST_FUNCTION(i,j,k,idxElement,c);
            if (arr[k][j][i][idxElement+c] != val) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        } else {
          for (c=0; c<dof3; ++c) {
            if (arr[k][j][i][idxElement+c] != -1.0) SETERRQ(PetscObjectComm((PetscObject)dm),PETSC_ERR_PLIB,"Invalid value");
          }
        }
      }
    }
