...
PetscErrorCode DMComplexVTKWriteCells(DM dm, PetscSection globalConeSection, PetscViewer viewer, PetscInt *totalCells)
{
  MPI_Comm       comm = ((PetscObject) dm)->comm;
  PetscInt       dim;
  PetscInt       numCorners = 0, maxCorners;
  PetscInt       numCells   = 0, totCells, cellType, cStart, cEnd, c, v;
  ...
  if (!rank) {
    for(c = cStart; c < cEnd; ++c) {
      ...
      ierr = DMComplexVTKCellTypeValid(dm, cellType, &valid);CHKERRQ(ierr);
      if (!valid) continue;
      ierr = PetscViewerASCIIPrintf(viewer, "%d ", maxCorners);CHKERRQ(ierr);
      ierr = DMComplexGetCone(dm, c, &cone);CHKERRQ(ierr);
      /* TODO Need global vertex numbering */
      for(v = 0; v < coneSize; ++v) {
        ierr = PetscViewerASCIIPrintf(viewer, " %d", cone[v]);CHKERRQ(ierr);
      }
    ...
    for(proc = 1; proc < numProcs; ++proc) {
      ...
      for(c = 0; c < numCells; ++c) {
        ...
        for(v = 0; v < maxCorners; ++v) {
          ierr = PetscViewerASCIIPrintf(viewer, " %d", remoteVertices[c*maxCorners+v]);CHKERRQ(ierr);
        }
        ierr = PetscViewerASCIIPrintf(viewer, "\n");CHKERRQ(ierr);
      }
...
PetscErrorCode DMComplexVTKWriteSection(DM dm, PetscSection section, Vec v, PetscViewer viewer, PetscInt enforceDim, PetscInt precision) {
  ...
  enforceDim = PetscMax(enforceDim, maxDof);
  ierr = VecGetArray(v, &array);CHKERRQ(ierr);
  if (!rank) {
    char formatString[8];

    ierr = PetscSNPrintf(formatString, 8, "%%.%dg", precision);CHKERRQ(ierr);
    for(p = pStart; p < pEnd; ++p) {
      ...
      if (dof && off >= 0) {
        for(d = 0; d < dof; d++) {
          ...
          ierr = PetscViewerASCIIPrintf(viewer, formatString, array[off+d]);CHKERRQ(ierr);
        }
        for(d = dof; d < enforceDim; d++) {
    ...
    for(proc = 1; proc < numProcs; ++proc) {
      ...
      for(p = 0; p < size/maxDof; ++p) {
        for(d = 0; d < maxDof; ++d) {
          ...
          ierr = PetscViewerASCIIPrintf(viewer, formatString, remoteValues[p*maxDof+d]);CHKERRQ(ierr);
        }
        for(d = maxDof; d < enforceDim; ++d) {
          ierr = PetscViewerASCIIPrintf(viewer, " 0.0");CHKERRQ(ierr);
        }
        ierr = PetscViewerASCIIPrintf(viewer, "\n");CHKERRQ(ierr);
      }
...
PetscErrorCode DMComplexVTKWriteField(DM dm, PetscSection section, Vec field, const char name[], PetscViewer viewer, PetscInt enforceDim, PetscInt precision)
{
  PetscInt       numDof = 0, maxDof;
  PetscInt       pStart, pEnd, p;
  PetscErrorCode ierr;
  ...
  ierr = DMComplexVTKWriteSection(dm, section, field, viewer, enforceDim, precision);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
static PetscErrorCode DMComplexVTKWriteAll_ASCII(DM dm, PetscViewer viewer)
{
  ...
  ierr = DMComplexVTKWriteSection(dm, globalCoordSection, coordinates, viewer, 3, PETSC_DETERMINE);CHKERRQ(ierr);
  /* Cells */
  ...
  ierr = DMComplexVTKWriteCells(dm, globalConeSection, viewer, &totCells);CHKERRQ(ierr);
  /* Vertex fields */
  for(link = vtk->link; link; link = link->next) {
    if (link->ft == PETSC_VTK_POINT_FIELD) hasPoint = PETSC_TRUE;
    if (link->ft == PETSC_VTK_CELL_FIELD)  hasCell  = PETSC_TRUE;
  }
  if (hasPoint) {
    ...
    for(link = vtk->link; link; link = link->next) {
      ...
      if (link->ft != PETSC_VTK_POINT_FIELD) continue;
      ...
      ierr = DMComplexVTKWriteField(dm, globalSection, X, name, viewer, PETSC_DETERMINE, PETSC_DETERMINE);CHKERRQ(ierr);
  ...
  if (hasCell) {
    ...
    for(link = vtk->link; link; link = link->next) {
      ...
      if (link->ft != PETSC_VTK_CELL_FIELD) continue;
      ierr = PetscObjectGetName(link->vec, &name);CHKERRQ(ierr);
      ierr = PetscObjectQuery(link->vec, "section", (PetscObject *) &section);CHKERRQ(ierr);
      if (!section) SETERRQ1(((PetscObject) dm)->comm, PETSC_ERR_ARG_WRONG, "Vector %s had no PetscSection composed with it", name);
      ierr = PetscSectionCreateGlobalSection(section, mesh->sf, &globalSection);CHKERRQ(ierr);
