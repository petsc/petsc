...
PetscErrorCode CheckFunctions(DM dm, PetscInt order, Vec u, AppCtx *user)
{
  ...
  ierr = DMComplexProjectFunction(dm, q[0].numComponents, exactFuncs, INSERT_ALL_VALUES, u);CHKERRQ(ierr);
  /* Compare approximation to exact in L_2 */
  ierr = DMComplexComputeL2Diff(dm, &q, exactFuncs, u, &error);CHKERRQ(ierr);
  if (error > tol) {
