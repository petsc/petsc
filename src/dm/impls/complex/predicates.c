...
/*****************************************************************************/
#include <petsc.h>
/* TODO Change to configure check */
#ifdef CPU86
#include <float.h>
#endif /* CPU86 */
/* TODO Change to configure check */
...
static PetscReal splitter;
static PetscReal epsilon;         /* = 2^(-p).  Used to estimate roundoff errors. */
/* A set of coefficients used to calculate maximum roundoff errors.          */
static PetscReal resulterrbound;
static PetscReal ccwerrboundA, ccwerrboundB, ccwerrboundC;
static PetscReal o3derrboundA, o3derrboundB, o3derrboundC;
static PetscReal iccerrboundA, iccerrboundB, iccerrboundC;
static PetscReal isperrboundA, isperrboundB, isperrboundC;
...
{
  PetscReal Q;
  INEXACT PetscReal Qnew;
  int eindex;
  PetscReal enow;
  INEXACT PetscReal bvirt;
  PetscReal avirt, bround, around;
...
{
  INEXACT PetscReal Q;
  INEXACT PetscReal sum;
  INEXACT PetscReal product1;
  PetscReal product0;
  int eindex, hindex;
  PetscReal enow;
  INEXACT PetscReal bvirt;
  PetscReal avirt, bround, around;
  INEXACT PetscReal c;
  INEXACT PetscReal abig;
  PetscReal ahi, alo, bhi, blo;
  PetscReal err1, err2, err3;
...
PetscReal TetGenOrient2DExact(PetscReal *pa, PetscReal *pb, PetscReal *pc)
{
  ...
  PetscReal aterms[4], bterms[4], cterms[4];
  INEXACT PetscReal aterms3, bterms3, cterms3;
  PetscReal v[8], w[12];
  int vlength, wlength;
...
PetscReal TetGenOrient2DSlow(PetscReal *pa, PetscReal *pb, PetscReal *pc)
{
  ...
  PetscReal bcxtail, bcytail;
  PetscReal negate, negatetail;
  PetscReal axby[8], bxay[8];
  INEXACT PetscReal axby7, bxay7;
  PetscReal deter[16];
  int deterlen;
  ...
  INEXACT PetscReal c;
  INEXACT PetscReal abig;
  PetscReal a0hi, a0lo, a1hi, a1lo, bhi, blo;
  PetscReal err1, err2, err3;
  INEXACT PetscReal _i, _j, _k, _l, _m, _n;
  PetscReal _0, _1, _2;
...
PetscReal TetGenOrient2DAdapt(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal detsum)
{
  ...
  INEXACT PetscReal detleft, detright;
  PetscReal detlefttail, detrighttail;
  PetscReal det, errbound;
  PetscReal B[4], C1[8], C2[12], D[16];
  INEXACT PetscReal B3;
  int C1length, C2length, Dlength;
  PetscReal u[4];
  INEXACT PetscReal u3;
  INEXACT PetscReal s1, t1;
  PetscReal s0, t0;
  ...
  acx = (PetscReal) (pa[0] - pc[0]);
  bcx = (PetscReal) (pb[0] - pc[0]);
  acy = (PetscReal) (pa[1] - pc[1]);
  bcy = (PetscReal) (pb[1] - pc[1]);
...
PetscReal TetGenOrient3DExact(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd)
{
  ...
  PetscReal axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
  PetscReal bxay0, cxby0, dxcy0, axdy0, cxay0, dxby0;
  PetscReal ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
  PetscReal temp8[8];
  int templen;
  PetscReal abc[12], bcd[12], cda[12], dab[12];
  int abclen, bcdlen, cdalen, dablen;
  PetscReal adet[24], bdet[24], cdet[24], ddet[24];
  int alen, blen, clen, dlen;
  PetscReal abdet[48], cddet[48];
  int ablen, cdlen;
  PetscReal deter[96];
  int deterlen;
  int i;
...
PetscReal TetGenOrient3DSlow(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd)
{
  ...
  PetscReal bdxtail, bdytail, bdztail;
  PetscReal cdxtail, cdytail, cdztail;
  PetscReal negate, negatetail;
  INEXACT PetscReal axby7, bxcy7, axcy7, bxay7, cxby7, cxay7;
  PetscReal axby[8], bxcy[8], axcy[8], bxay[8], cxby[8], cxay[8];
  PetscReal temp16[16], temp32[32], temp32t[32];
  int temp16len, temp32len, temp32tlen;
  PetscReal adet[64], bdet[64], cdet[64];
  int alen, blen, clen;
  PetscReal abdet[128];
  int ablen;
  PetscReal deter[192];
...
PetscReal TetGenOrient3DAdapt(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal permanent)
{
  ...
  INEXACT PetscReal bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  PetscReal bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  PetscReal bc[4], ca[4], ab[4];
  INEXACT PetscReal bc3, ca3, ab3;
  PetscReal adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  PetscReal abdet[16];
  int ablen;
  PetscReal *finnow, *finother, *finswap;
  PetscReal fin1[192], fin2[192];
  int finlength;

  PetscReal adxtail, bdxtail, cdxtail;
  PetscReal adytail, bdytail, cdytail;
  PetscReal adztail, bdztail, cdztail;
  INEXACT PetscReal at_blarge, at_clarge;
  INEXACT PetscReal bt_clarge, bt_alarge;
  INEXACT PetscReal ct_alarge, ct_blarge;
  PetscReal at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
  INEXACT PetscReal bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
  INEXACT PetscReal adxt_cdy1, adxt_bdy1, bdxt_ady1;
  PetscReal bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  PetscReal adxt_cdy0, adxt_bdy0, bdxt_ady0;
  INEXACT PetscReal bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
  INEXACT PetscReal adyt_cdx1, adyt_bdx1, bdyt_adx1;
  PetscReal bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  PetscReal adyt_cdx0, adyt_bdx0, bdyt_adx0;
  PetscReal bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
  INEXACT PetscReal bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
  INEXACT PetscReal adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  PetscReal bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  PetscReal adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  PetscReal u[4], v[12], w[16];
  INEXACT PetscReal u3;
  int vlength, wlength;
  PetscReal negate;
  ...
  cdx = (PetscReal) (pc[0] - pd[0]);
  ady = (PetscReal) (pa[1] - pd[1]);
  bdy = (PetscReal) (pb[1] - pd[1]);
  cdy = (PetscReal) (pc[1] - pd[1]);
  adz = (PetscReal) (pa[2] - pd[2]);
  bdz = (PetscReal) (pb[2] - pd[2]);
  cdz = (PetscReal) (pc[2] - pd[2]);
...
PetscReal TetGenIncircleExact(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd)
{
  ...
  PetscReal abc[12], bcd[12], cda[12], dab[12];
  int abclen, bcdlen, cdalen, dablen;
  PetscReal det24x[24], det24y[24], det48x[48], det48y[48];
  int xlen, ylen;
  PetscReal adet[96], bdet[96], cdet[96], ddet[96];
  int alen, blen, clen, dlen;
  PetscReal abdet[192], cddet[192];
  int ablen, cdlen;
  PetscReal deter[384];
  int deterlen;
  int i;
...
PetscReal TetGenIncircleSlow(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd)
{
  ...
  PetscReal temp16[16];
  int temp16len;
  PetscReal detx[32], detxx[64], detxt[32], detxxt[64], detxtxt[64];
  int xlen, xxlen, xtlen, xxtlen, xtxtlen;
  PetscReal x1[128], x2[192];
  int x1len, x2len;
  PetscReal dety[32], detyy[64], detyt[32], detyyt[64], detytyt[64];
  int ylen, yylen, ytlen, yytlen, ytytlen;
  PetscReal y1[128], y2[192];
  int y1len, y2len;
  PetscReal adet[384], bdet[384], cdet[384], abdet[768], deter[1152];
...
PetscReal TetGenIncircleAdapt(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal permanent)
{
  ...
  PetscReal bc[4], ca[4], ab[4];
  INEXACT PetscReal bc3, ca3, ab3;
  PetscReal axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  PetscReal bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  PetscReal cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  PetscReal abdet[64];
  int ablen;
  PetscReal fin1[1152], fin2[1152];
  PetscReal *finnow, *finother, *finswap;
  int finlength;

  PetscReal adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT PetscReal adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  PetscReal adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  PetscReal aa[4], bb[4], cc[4];
  INEXACT PetscReal aa3, bb3, cc3;
  INEXACT PetscReal ti1, tj1;
  PetscReal ti0, tj0;
  PetscReal u[4], v[4];
  INEXACT PetscReal u3, v3;
  PetscReal temp8[8], temp16a[16], temp16b[16], temp16c[16];
  PetscReal temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  PetscReal axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  PetscReal bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  PetscReal cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  PetscReal axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen = 0, aytbclen = 0, bxtcalen = 0, bytcalen = 0, cxtablen = 0, cytablen = 0;
  PetscReal axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  PetscReal axtbctt[8], aytbctt[8], bxtcatt[8];
  PetscReal bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  PetscReal abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  PetscReal abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT PetscReal abtt3, bctt3, catt3;
  PetscReal negate;
...
PetscReal TetGenInsphereFast(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal *pe)
{
  ...
  PetscReal aez, bez, cez, dez;
  PetscReal alift, blift, clift, dlift;
  PetscReal ab, bc, cd, da, ac, bd;
  PetscReal abc, bcd, cda, dab;
...
PetscReal TetGenInsphereExact(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal *pe)
{
  ...
  INEXACT PetscReal axcy1, bxdy1, cxey1, dxay1, exby1;
  INEXACT PetscReal cxay1, dxby1, excy1, axdy1, bxey1;
  PetscReal axby0, bxcy0, cxdy0, dxey0, exay0;
  PetscReal bxay0, cxby0, dxcy0, exdy0, axey0;
  PetscReal axcy0, bxdy0, cxey0, dxay0, exby0;
  PetscReal cxay0, dxby0, excy0, axdy0, bxey0;
  PetscReal ab[4], bc[4], cd[4], de[4], ea[4];
  PetscReal ac[4], bd[4], ce[4], da[4], eb[4];
  PetscReal temp8a[8], temp8b[8], temp16[16];
  int temp8alen, temp8blen, temp16len;
  PetscReal abc[24], bcd[24], cde[24], dea[24], eab[24];
  PetscReal abd[24], bce[24], cda[24], deb[24], eac[24];
  int abclen, bcdlen, cdelen, dealen, eablen;
  int abdlen, bcelen, cdalen, deblen, eaclen;
  PetscReal temp48a[48], temp48b[48];
  int temp48alen, temp48blen;
  PetscReal abcd[96], bcde[96], cdea[96], deab[96], eabc[96];
  int abcdlen, bcdelen, cdealen, deablen, eabclen;
  PetscReal temp192[192];
  PetscReal det384x[384], det384y[384], det384z[384];
  int xlen, ylen, zlen;
  PetscReal detxy[768];
  int xylen;
  PetscReal adet[1152], bdet[1152], cdet[1152], ddet[1152], edet[1152];
  int alen, blen, clen, dlen, elen;
  PetscReal abdet[2304], cddet[2304], cdedet[3456];
  int ablen, cdlen;
  PetscReal deter[5760];
  int deterlen;
  int i;
...
PetscReal TetGenInsphereSlow(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal *pe)
{
  ...
  PetscReal aeytail, beytail, ceytail, deytail;
  PetscReal aeztail, beztail, ceztail, deztail;
  PetscReal negate, negatetail;
  INEXACT PetscReal axby7, bxcy7, cxdy7, dxay7, axcy7, bxdy7;
  INEXACT PetscReal bxay7, cxby7, dxcy7, axdy7, cxay7, dxby7;
  PetscReal axby[8], bxcy[8], cxdy[8], dxay[8], axcy[8], bxdy[8];
  PetscReal bxay[8], cxby[8], dxcy[8], axdy[8], cxay[8], dxby[8];
  PetscReal ab[16], bc[16], cd[16], da[16], ac[16], bd[16];
  int ablen, bclen, cdlen, dalen, aclen, bdlen;
  PetscReal temp32a[32], temp32b[32], temp64a[64], temp64b[64], temp64c[64];
  int temp32alen, temp32blen, temp64alen, temp64blen, temp64clen;
  PetscReal temp128[128], temp192[192];
  int temp128len, temp192len;
  PetscReal detx[384], detxx[768], detxt[384], detxxt[768], detxtxt[768];
  int xlen, xxlen, xtlen, xxtlen, xtxtlen;
  PetscReal x1[1536], x2[2304];
  int x1len, x2len;
  PetscReal dety[384], detyy[768], detyt[384], detyyt[768], detytyt[768];
  int ylen, yylen, ytlen, yytlen, ytytlen;
  PetscReal y1[1536], y2[2304];
  int y1len, y2len;
  PetscReal detz[384], detzz[768], detzt[384], detzzt[768], detztzt[768];
  int zlen, zzlen, ztlen, zztlen, ztztlen;
  PetscReal z1[1536], z2[2304];
  int z1len, z2len;
  PetscReal detxy[4608];
  int xylen;
  PetscReal adet[6912], bdet[6912], cdet[6912], ddet[6912];
  int alen, blen, clen, dlen;
  PetscReal abdet[13824], cddet[13824], deter[27648];
  int deterlen;
  int i;
                   ...
                   PetscReal permanent)
{
  ...
  INEXACT PetscReal aexbey1, bexaey1, bexcey1, cexbey1;
  INEXACT PetscReal cexdey1, dexcey1, dexaey1, aexdey1;
  INEXACT PetscReal aexcey1, cexaey1, bexdey1, dexbey1;
  PetscReal aexbey0, bexaey0, bexcey0, cexbey0;
  PetscReal cexdey0, dexcey0, dexaey0, aexdey0;
  PetscReal aexcey0, cexaey0, bexdey0, dexbey0;
  PetscReal ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
  INEXACT PetscReal ab3, bc3, cd3, da3, ac3, bd3;
  PetscReal abeps, bceps, cdeps, daeps, aceps, bdeps;
  PetscReal temp8a[8], temp8b[8], temp8c[8], temp16[16], temp24[24], temp48[48];
  int temp8alen, temp8blen, temp8clen, temp16len, temp24len, temp48len;
  PetscReal xdet[96], ydet[96], zdet[96], xydet[192];
  int xlen, ylen, zlen, xylen;
  PetscReal adet[288], bdet[288], cdet[288], ddet[288];
  int alen, blen, clen, dlen;
  PetscReal abdet[576], cddet[576];
  int ablen, cdlen;
  PetscReal fin1[1152];
  int finlength;
  ...
  aex = (PetscReal) (pa[0] - pe[0]);
  bex = (PetscReal) (pb[0] - pe[0]);
  cex = (PetscReal) (pc[0] - pe[0]);
  dex = (PetscReal) (pd[0] - pe[0]);
  aey = (PetscReal) (pa[1] - pe[1]);
  bey = (PetscReal) (pb[1] - pe[1]);
  cey = (PetscReal) (pc[1] - pe[1]);
  dey = (PetscReal) (pd[1] - pe[1]);
  aez = (PetscReal) (pa[2] - pe[2]);
  bez = (PetscReal) (pb[2] - pe[2]);
  cez = (PetscReal) (pc[2] - pe[2]);
  dez = (PetscReal) (pd[2] - pe[2]);
...
PetscReal TetGenInsphere(PetscReal *pa, PetscReal *pb, PetscReal *pc, PetscReal *pd, PetscReal *pe)
{
  ...
  PetscReal aey, bey, cey, dey;
  PetscReal aez, bez, cez, dez;
  PetscReal aexbey, bexaey, bexcey, cexbey, cexdey, dexcey, dexaey, aexdey;
  PetscReal aexcey, cexaey, bexdey, dexbey;
  PetscReal alift, blift, clift, dlift;
  PetscReal ab, bc, cd, da, ac, bd;
  PetscReal abc, bcd, cda, dab;
  PetscReal aezplus, bezplus, cezplus, dezplus;
  PetscReal aexbeyplus, bexaeyplus, bexceyplus, cexbeyplus;
  PetscReal cexdeyplus, dexceyplus, dexaeyplus, aexdeyplus;
  PetscReal aexceyplus, cexaeyplus, bexdeyplus, dexbeyplus;
  PetscReal det;
  PetscReal permanent, errbound;
