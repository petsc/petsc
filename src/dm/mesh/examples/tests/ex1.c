...
int main(int argc, char *argv[])
{
  ...
  ierr = PetscOptionsBegin(comm, "", "Options for Sieve testing", "Mesh");
    ...
    ierr = PetscOptionsBool("-presieve_tests", "Perform the PreSieve test", "ex1.c", PETSC_TRUE, &presieveTests, PETSC_NULL);CHKERRQ(ierr);
    sieveTests = PETSC_TRUE;
    ierr = PetscOptionsBool("-sieve_tests", "Perform the Sieve test", "ex1.c", PETSC_TRUE, &sieveTests, PETSC_NULL);CHKERRQ(ierr);
    coneTests = PETSC_TRUE;
    ierr = PetscOptionsBool("-cone_tests", "Perform the cone test", "ex1.c", PETSC_TRUE, &coneTests, PETSC_NULL);CHKERRQ(ierr);
