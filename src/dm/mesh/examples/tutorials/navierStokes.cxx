static char help[] = "This example solves the time-independent Navier-Stokes problem.\n\n";
...
double rhs_v_2d(const double x[]) {
  return 2.0*x[1]*x[1]*x[1] - 2*x[0]*x[1]*x[1] + 2.0*x[0]*x[0]*x[1] - 3.0;
}
  ...
  if (options->dim == 2) {
    if (options->bcType == DIRICHLET) {
      if (options->solnType == SOLN_SIMPLE) {
        ...
        options->funcs[1]  = rhs_u_2d;
        options->funcs[2]  = rhs_v_2d;
      }
    } else {
