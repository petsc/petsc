...
PetscErrorCode CreateProblem(DM dm, Options *options)
{
  ...
  if (options->structured) {
    ...
    ierr = MeshGetMesh(mesh, m);CHKERRQ(ierr);
    if (options->dim == 1) {
      ierr = CreateProblem_gen_0(dm, "u", 1, options->exactFunc, options->exactFunc);CHKERRQ(ierr);
      options->integrate = IntegrateDualBasis_gen_0;
    } else if (options->dim == 2) {
      ierr = CreateProblem_gen_1(dm, "u", 1, options->exactFunc, options->exactFunc);CHKERRQ(ierr);
      options->integrate = IntegrateDualBasis_gen_1;
    } else if (options->dim == 3) {
      ierr = CreateProblem_gen_2(dm, "u", 1, options->exactFunc, options->exactFunc);CHKERRQ(ierr);
