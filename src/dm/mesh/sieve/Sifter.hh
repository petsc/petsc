...
namespace ALE {
  ...
  namespace def {
    ...
    // Creation
    class PyLithBuilder {
      inline void ignoreCommentsPyLith(char *buf, PetscInt bufSize, FILE *f) {
        while((fgets(buf, bufSize, f) != NULL) && (buf[0] == '#')) {}
      };

      void readConnectivityPyLith(MPI_Comm comm, const std::string& filename, int dim, bool useZeroBase, int& numElements, int *vertices[]);
      void readCoordinatesPyLith(MPI_Comm comm, const std::string& filename, int dim, int& numVertices, double *coordinates[]);
    public:
      PyLithBuilder() {};
      virtual ~PyLithBuilder() {};

      Obj<Mesh> createPyLith(MPI_Comm comm, const std::string& baseFilename);
    };
  } // namespace def
} // namespace ALE
