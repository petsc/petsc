...
namespace ALECompat {
  namespace PyLith {
    ...
    void Builder::buildTractions(const Obj<real_section_type>& tractionField, const Obj<topology_type>& boundaryTopology, int numCells, int numTractions, int vertsPerFace, int tractionVertices[], double tractionValues[]) {
      ...
      ALECompat::New::SieveBuilder<Mesh>::buildTopology(boundarySieve, 2, numTractions, tractionVertices, 0, false, vertsPerFace, numCells);
    ...
    Obj<Mesh> Builder::readMesh(MPI_Comm comm, const int dim, const std::string& basename, const bool useZeroBase = false, const bool interpolate = false, const int debug = 0) {
      ...
      ALECompat::PyLith::Builder::readConnectivity(comm, basename+".connect", numCorners, useZeroBase, numCells, &cells, &materials);
      ALECompat::PyLith::Builder::readCoordinates(comm, basename+".coord", dim, numVertices, &coordinates);
      ALECompat::New::SieveBuilder<Mesh>::buildTopology(sieve, dim, numCells, cells, numVertices, interpolate, numCorners);
      sieve->stratify();
      topology->setPatch(0, sieve);
      topology->stratify();
      mesh->setTopology(topology);
      ALECompat::New::SieveBuilder<Mesh>::buildCoordinates(mesh->getRealSection("coordinates"), dim, coordinates);
      Obj<int_section_type> material = mesh->getIntSection("material");
      buildMaterials(material, materials);
      Obj<ALECompat::Mesh::pair_section_type> split = createSplit(mesh, basename, useZeroBase);
      if (!split.isNull()) {mesh->setPairSection("split", split);}
      Obj<ALECompat::Mesh::real_section_type> traction = createTraction(mesh, basename, useZeroBase);
    ...
    Obj<Builder::pair_section_type> Builder::createSplit(const Obj<Mesh>& mesh, const std::string& basename, const bool useZeroBase = false) {
      ...
      ALECompat::PyLith::Builder::readSplit(comm, basename+".split", dim, useZeroBase, numSplit, &splitInd, &loadHistory, &splitValues);
      MPI_Allreduce(&numSplit, &hasSplit, 1, MPI_INT, MPI_MAX, comm);
      if (hasSplit) {
        split = new pair_section_type(mesh->getTopology());
        Obj<int_section_type> loadField = mesh->getIntSection("loadHistory");
    ...
    Obj<Mesh::real_section_type> Builder::createTraction(const Obj<Mesh>& mesh, const std::string& basename, const bool useZeroBase = false) {
      ...
      ALECompat::PyLith::Builder::readTractions(comm, basename+".traction", dim, numCorners, useZeroBase, numTractions, vertsPerFace, &tractionVertices, &tractionValues);
      MPI_Allreduce(&numTractions, &hasTractions, 1, MPI_INT, MPI_MAX, comm);
      if (hasTractions) {
    ...
    void Builder::createCohesiveElements(const Obj<Mesh>& mesh, const std::set<Mesh::point_type>& faultVertices) {
      ...
      for(std::set<int>::const_iterator fv_iter = fvBegin; fv_iter != fvEnd; ++fv_iter) {
        ...
        if (debug) {std::cout << "Checking fault vertex " << *fv_iter << std::endl;}
        for(ALECompat::Mesh::sieve_type::traits::supportSequence::iterator c_iter = cBegin; c_iter != cEnd; ++c_iter) {
          if (debug) {std::cout << "  Checking cell " << *c_iter << std::endl;}
          if (faultCells.find(*c_iter) != faultCells.end()) continue;
          const Obj<ALECompat::Mesh::sieve_type::traits::coneSequence>& cone   = sieve->cone(*c_iter);
          const Mesh::sieve_type::traits::coneSequence::iterator  vBegin = cone->begin();
          const Mesh::sieve_type::traits::coneSequence::iterator  vEnd   = cone->end();

          face->clear();
          for(ALECompat::Mesh::sieve_type::traits::coneSequence::iterator v_iter = vBegin; v_iter != vEnd; ++v_iter) {
            if (faultVertices.find(*v_iter) != fvEnd) {
              if (debug) {std::cout << "    contains fault vertex " << *v_iter << std::endl;}
              face->insert(face->end(), *v_iter);
            }
      ...
      for(Mesh::topology_type::label_sequence::iterator f_iter = faces->begin(); f_iter != faces->end(); ++f_iter) {
        ...
        if (debug) {std::cout << "  Replacing cell " << cell << std::endl;}
        newVertices.clear();
        for(ALECompat::Mesh::sieve_type::traits::coneSequence::iterator v_iter = cone->begin(); v_iter != cone->end(); ++v_iter) {
          if (vertexRenumber.find(*v_iter) != vertexRenumber.end()) {
            if (debug) {std::cout << "    vertex " << vertexRenumber[*v_iter] << std::endl;}
            newVertices.insert(newVertices.end(), vertexRenumber[*v_iter]);
          } else {
      ...
      // Fix coordinates
      const Obj<Mesh::real_section_type>&             coordinates = mesh->getRealSection("coordinates");
      const Obj<Mesh::topology_type::label_sequence>& fVertices2  = fault->depthStratum(patch, 0);

      for(Mesh::topology_type::label_sequence::iterator v_iter = fVertices2->begin(); v_iter != fVertices2->end(); ++v_iter) {
        coordinates->addPoint(patch, vertexRenumber[*v_iter], coordinates->getFiberDimension(patch, *v_iter));
      }
      coordinates->reallocate();
      for(Mesh::topology_type::label_sequence::iterator v_iter = fVertices2->begin(); v_iter != fVertices2->end(); ++v_iter) {
    ...
    PetscErrorCode Viewer::writeSplitLocal(const Obj<Mesh>& mesh, const Obj<Builder::pair_section_type>& splitField, PetscViewer viewer) {
      ...
      for(Builder::pair_section_type::atlas_type::chart_type::const_iterator c_iter = chart.begin(); c_iter != chart.end(); ++c_iter) {
        ...
        for(int i = 0; i < size; i++) {
          const Builder::pair_section_type::point_type& v     = values[i].first;
          const ALECompat::Mesh::base_type::split_value&      split = values[i].second;
