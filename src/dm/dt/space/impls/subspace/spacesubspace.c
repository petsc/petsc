  ...
  if (iascii) {
    ...
    if (subsp->Jx) {
      ...
      for (o = 0; o < origDim; o++) {
        ...
        for (s = 1; s < subDim; s++) {
          PetscCall(PetscViewerASCIIPrintf(viewer," %g", (double)subsp->Jx[o * subDim + s]));
        }
    ...
    if (subsp->Ju) {
      ...
      for (o = 0; o < origNc; o++) {
        PetscCall(PetscViewerASCIIUseTabs(viewer,PETSC_FALSE));
        for (s = 0; s < subNc; s++) {
          PetscCall(PetscViewerASCIIPrintf(viewer," %g", (double)subsp->Ju[o * subNc + s]));
        }
        PetscCall(PetscViewerASCIIUseTabs(viewer,PETSC_TRUE));
      }
      PetscCall(PetscViewerASCIIPrintf(viewer,"\n"));
...
static PetscErrorCode PetscSpaceEvaluate_Subspace(PetscSpace sp, PetscInt npoints, const PetscReal points[], PetscReal B[], PetscReal D[], PetscReal H[])
{
  ...
  if (B) {
    PetscCall(DMGetWorkArray(sp->dm,npoints*origNb*origNc,MPIU_REAL,&inB));
  }
  if (D) {
    PetscCall(DMGetWorkArray(sp->dm,npoints*origNb*origNc*origDim,MPIU_REAL,&inD));
  }
  if (H) {
    PetscCall(DMGetWorkArray(sp->dm,npoints*origNb*origNc*origDim*origDim,MPIU_REAL,&inH));
  }
