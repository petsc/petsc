...
static PetscErrorCode testOrthogonality(PetscInt dim, PetscInt deg)
{
  ...
  for (i = 0; i < Npoly; i++) {
    for (j = i; j < Npoly; j++) {
      ...
      PetscAssertFalse(PetscAbsReal(integral - exact) > PETSC_SMALL,PETSC_COMM_SELF, PETSC_ERR_PLIB, "<P[%D], P[%D]> = %g != delta_{%D,%D}", i, j, (double) integral, i, j);
    }
  }
...
static PetscErrorCode testDerivativesLegendre(PetscInt dim, PetscInt deg, PetscInt k)
{
  ...
  for (i = 0; i < Nk; i++) {
    ...
    PetscAssertFalse(PetscAbsReal(diff) > tol,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Jet mismatch between PKD and tensor Legendre bases: error %g at tolerance %g", (double) diff, (double) tol);
  }
