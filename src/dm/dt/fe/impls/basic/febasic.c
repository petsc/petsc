    ...
    if (debug > 1) {
      ...
      for (f = fS; f < fE; ++f) {
        ...
        for (g = gS; g < gE; ++g) {
          ...
          PetscCheck(i < totDim && j < totDim, PETSC_COMM_SELF, PETSC_ERR_PLIB, "Fuck up %" PetscInt_FMT " %" PetscInt_FMT " %" PetscInt_FMT " %" PetscInt_FMT "\n", f, i, g, j);
