...
static PetscErrorCode PetscFESetUp_Composite(PetscFE fem)
{
  ...
  for (s = 0; s < cmp->numSubelements; ++s) {
    ...
    PetscAssertFalse(sd != spdim,PetscObjectComm((PetscObject) fem), PETSC_ERR_PLIB, "Subelement %d has %d dual basis vectors != %d", s, sd, spdim);
  }
...
static PetscErrorCode PetscFECreateTabulation_Composite(PetscFE fem, PetscInt npoints, const PetscReal points[], PetscInt K, PetscTabulation T)
{
  ...
  for (p = 0; p < npoints; ++p) {
    ...
    PetscAssertFalse(s >= cmp->numSubelements,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Point %d was not found in any subelement", p);
  }
