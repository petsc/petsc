...
PetscErrorCode DMCreateGlobalVector_Section_Private(DM dm,Vec *vec)
{
  ...
  PetscAssertFalse(localSize%blockSize,PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Mismatch between blocksize %d and local storage size %d", blockSize, localSize);
...
PetscErrorCode DMCreateSectionSubDM(DM dm, PetscInt numFields, const PetscInt fields[], IS *is, DM *subdm)
{
  ...
  PetscAssertFalse(!section,PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Must set default section for DM before splitting fields");
  PetscAssertFalse(!sectionGlobal,PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Must set default global section for DM before splitting fields");
  ierr = PetscSectionGetNumFields(section, &Nf);CHKERRQ(ierr);
  PetscAssertFalse(numFields > Nf,PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Number of requested fields %d greater than number of DM fields %d", numFields, Nf);
  if (is) {
      ...
      if (dm->probs[0].fields) {
        ...
        for (d = 0, e = 0; d < dm->Nds && e < (*subdm)->Nds; ++d) {
          ...
          /* Translate DM fields to DS fields */
          {
            ...
            PetscAssertFalse(!nf,PETSC_COMM_SELF, PETSC_ERR_PLIB, "DS cannot be supported on 0 fields");
...
PetscErrorCode DMCreateSectionSuperDM(DM dms[], PetscInt len, IS **is, DM *superdm)
{
  ...
  for (i = 0 ; i < len; ++i) {
    ...
    PetscAssertFalse(!sections[i],comm, PETSC_ERR_ARG_WRONG, "Must set default section for DM before splitting fields");
    PetscAssertFalse(!sectionGlobals[i],comm, PETSC_ERR_ARG_WRONG, "Must set default global section for DM before splitting fields");
    ...
    for (i = 0 ; i < len; startf += Nfs[i], ++i) {
      ...
      for (p = pStart, subOff = 0; p < pEnd; ++p) {
        ...
        if (gdof > 0 && gtdof) {
          ...
          PetscAssertFalse(end-start != gtdof,PETSC_COMM_SELF, PETSC_ERR_ARG_SIZ, "Invalid number of global dofs %D != %D for dm %D on point %D", end-start, gtdof, i, p);
