  ...
  for (i=xs; i<xs+xm+1; i++) {
    ...
    if(i<len_slow/2 || i>len_slow/2+len_fast+1){
      ...
      for (j=0; j<dof; j++) {
        ...
        ierr =  (*ctx->physics.flux)(ctx->physics.user,u,ctx->flux,&maxspeed);CHKERRQ(ierr);
        for (j=0; j<dof; j++) {
          f[(i-1)*dof+j] -= ctx->flux[j]/hs;
          f[i*dof+j] += ctx->flux[j]/hf;
        }
  ...
  for (i=xs; i<xs+xm+1; i++) {
    ...
    if(i<len_slow/2){
      ...
      for (j=0; j<dof; j++) f[i*dof+j] += ctx->flux[j]/hs;
        ...
        ierr =  (*ctx->physics.flux)(ctx->physics.user,u,ctx->flux,&maxspeed);CHKERRQ(ierr);
        for (j=0; j<dof; j++){
          f[(i-len_fast-1)*dof+j] -= ctx->flux[j]/hs;
          f[(i-len_fast)*dof+j] += ctx->flux[j]/hs;
        }
    } else if (i>len_slow/2+len_fast+1){
      u = &ctx->u[0];
      alpha[0] = 1.0/6.0;
      ...
      if (i < xs+xm) {for (j=0; j<dof; j++) f[(i-len_fast)*dof+j] += ctx->flux[j]/hs; }
    }
  }
  ierr = DMDAVecRestoreArray(da,Xloc,&x);CHKERRQ(ierr);
  ...
  for (i=xs; i<xs+xm+1; i++) {
    ...
    if(i==len_slow/2){
       ...
       for (j=0; j<dof; j++) {
        ...
        ierr =  (*ctx->physics.flux)(ctx->physics.user,u,ctx->flux,&maxspeed);CHKERRQ(ierr);
        for (j=0; j<dof; j++) {
          f[(i-len_slow/2-1)*dof+j] -= ctx->flux[j]/hf;
          f[(i-len_slow/2)*dof+j] += ctx->flux[j]/hf;
        }
      } else if(i>len_slow/2+1 && i<len_slow/2+len_fast) {
        u = &ctx->u[0];
        alpha[0] = 1.0/6.0;
...
/*TEST
    ...
    test:
      args: -da_grid_x 60 -initial 7 -xmin -1 -xmax 1 -hratio 2 -ts_dt 0.01 -ts_max_steps 20 -ts_type rk -ts_rk_type 2a -ts_rk_multirate_type partitioned -ts_rk_dtratio 2 -simulation -f "yourfilename.bin"
    ...
    test:
      args: -da_grid_x 60 -initial 7 -xmin -1 -xmax 1 -hratio 2 -simulation -f "yourfilename.bin" -ts_dt 0.01 -ts_max_steps 20 -ts_type prk -ts_prk_type pm2 -ts_prk_multirate_type combined
      note: yourfilename.bin need to be generated by userselves and this is only for the convergent study
            -hratio is the ratio between mesh size of corse grids and fine grids, this one must be provided
