...
static PetscErrorCode TestMonitor(DM dm, const char *filename, Vec X, PetscReal time, PetscInt ns, Vec *Y, PetscInt stepnum)
{
  Vec            odesol,stagesol;
  PetscInt       Nr,i;
  PetscBool      equal;
...
static PetscErrorCode MonitorBIN(TS ts,PetscInt stepnum,PetscReal time,Vec X,void *ctx)
{
  ...
  ierr = PetscViewerDestroy(&viewer);CHKERRQ(ierr); 
  ierr = TestMonitor(dm,filename,X,time,ns,Y,stepnum);
  PetscFunctionReturn(0);
}
