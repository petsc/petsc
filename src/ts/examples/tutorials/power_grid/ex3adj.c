...
int main(int argc,char **argv)
{
  ...
  ierr = VecGetArray(lambda[1],&y_ptr);CHKERRQ(ierr);
  y_ptr[0] = 0.0; y_ptr[1] = 1.0;
  ierr = VecRestoreArray(lambda[1],&y_ptr);CHKERRQ(ierr);
  ...
  ierr = MatGetVecs(Jacp,&lambdap[1],NULL);CHKERRQ(ierr);
  ierr = VecGetArray(lambdap[1],&x_ptr);CHKERRQ(ierr);
  x_ptr[0] = 0.0;
  ...
  */
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"\n sensitivity wrt initial conditions: d[phi(tf)]/d[phi0]  d[phi(tf)]/d[omega0]\n");CHKERRQ(ierr);
  ierr = VecView(lambda[0],PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"\n sensitivity wrt initial conditions: d[omega(tf)]/d[phi0]  d[omega(tf)]/d[omega0]\n");CHKERRQ(ierr);
...
/*
  ...
  ierr = VecView(lambdap[1],PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
*/
...
/*
  ...
  ierr = VecDestroy(&lambdap[1]);CHKERRQ(ierr);
*/
