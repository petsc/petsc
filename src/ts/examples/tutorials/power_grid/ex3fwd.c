...
int main(int argc,char **argv)
{
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"\n sensitivity wrt initial conditions: d[phi(tf)]/d[pm]  d[omega(tf)]/d[pm]\n");CHKERRQ(ierr);
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,"\n Integral sensitivity wrt parameter pm:\n");CHKERRQ(ierr);
  ierr = VecView(qgrad[0],PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
