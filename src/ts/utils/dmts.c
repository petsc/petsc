...
#undef __FUNCT__
#define __FUNCT__ "TSDefaultComputeFunction_DMLegacy"
static PetscErrorCode TSDefaultComputeFunction_DMLegacy(TS ts,Vec X,Vec F,void *ctx)
{
  PetscErrorCode ierr;
  DM             dm;
...
#undef __FUNCT__
#define __FUNCT__ "TSDefaultComputeJacobian_DMLegacy"
static PetscErrorCode TSDefaultComputeJacobian_DMLegacy(TS ts,Vec X,Mat *A,Mat *B,MatStructure *mstr,void *ctx)
{
  PetscErrorCode ierr;
  DM             dm;
...
#undef __FUNCT__
#define __FUNCT__ "DMTSSetUpLegacy"
/* Sets up calling of legacy DM routines */
PetscErrorCode DMTSSetUpLegacy(DM dm)
{
  PetscErrorCode ierr;
  TSDM         sdm;
  ...
  PetscFunctionBegin;
  ierr = DMTSGetContext(dm,&sdm);CHKERRQ(ierr);
  if (!sdm->computefunction) {ierr = DMTSSetFunction(dm,TSDefaultComputeFunction_DMLegacy,PETSC_NULL);CHKERRQ(ierr);}
  ierr = DMTSGetContext(dm,&sdm);CHKERRQ(ierr);
  if (!sdm->computejacobian) {
    if (dm->ops->functionj) {
      ierr = DMTSSetJacobian(dm,TSDefaultComputeJacobian_DMLegacy,PETSC_NULL);CHKERRQ(ierr);
    } else {
      ierr = DMTSSetJacobian(dm,TSDefaultComputeJacobianColor,PETSC_NULL);CHKERRQ(ierr);
    }
  }
  PetscFunctionReturn(0);
}
