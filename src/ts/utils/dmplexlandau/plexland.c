...
static PetscErrorCode LandauFormJacobian_Internal(Vec a_X, Mat JacP, const PetscInt dim, PetscReal shift, void *a_ctx)
{
  ...
  PetscAssertFalse(ctx->plex[0] == NULL,ctx->comm,PETSC_ERR_ARG_WRONG,"Plex not created");
  ...
  if (container) {
    PetscAssert(ctx->gpu_assembly,ctx->comm,PETSC_ERR_ARG_WRONG,"maps but no GPU assembly");
    ierr = PetscContainerGetPointer(container, (void **) &maps);CHKERRQ(ierr);
    PetscAssertFalse(!maps,ctx->comm,PETSC_ERR_ARG_WRONG,"empty GPU matrix container");
    for (PetscInt i=0;i<ctx->num_grids*ctx->batch_sz;i++) subJ[i] = NULL;
  } else {
    PetscAssert(!ctx->gpu_assembly,ctx->comm,PETSC_ERR_ARG_WRONG,"No maps but GPU assembly");
  ...
  PetscAssertFalse(Nq >LANDAU_MAX_NQ,ctx->comm,PETSC_ERR_ARG_WRONG,"Order too high. Nq = %D > LANDAU_MAX_NQ (%D)",Nq,LANDAU_MAX_NQ);
  // get metadata for collecting dynamic data
  for (PetscInt grid=0;grid<ctx->num_grids;grid++) {
    PetscInt cStart, cEnd;
    PetscAssertFalse(ctx->plex[grid] == NULL,ctx->comm,PETSC_ERR_ARG_WRONG,"Plex not created");
  ...
  if (shift==0) { /* create dynamic point data: f_alpha for closure of each cell (cellClosure[nbatch,ngrids,ncells[g],f[Nb,ns[g]]]) or xdata */
    ...
    PetscAssertFalse(!pack,PETSC_COMM_SELF, PETSC_ERR_PLIB, "pack has no DM");
    ...
    if (!ctx->gpu_assembly) {
      ...
      PetscAssertFalse(cellClosure_it-cellClosure != cellClosure_sz*ctx->batch_sz,PETSC_COMM_SELF, PETSC_ERR_PLIB, "iteration wrong %D != cellClosure_sz = %D",cellClosure_it-cellClosure,cellClosure_sz*ctx->batch_sz);
      ...
      while (b_elem_idx >= elem_offset[grid+1]) grid++;
      {
        ...
        for (PetscInt qj = 0; qj < Nq; ++qj) {
          ...
          if (shift==0.0) { // Jacobian
            ...
            PetscAssertFalse(IPf_idx != IPf_sz_glb,PETSC_COMM_SELF, PETSC_ERR_PLIB, "IPf_idx != IPf_sz %D %D",IPf_idx,IPf_sz_glb);
      ...
      for (PetscInt b_id = 0 ; b_id < ctx->batch_sz ; b_id++) { // OpenMP
        for (PetscInt grid=0 ; grid<ctx->num_grids ; grid++) {
          ...
          for (int i=0 ; i<nloc ; i++) {
            ...
            PetscAssert(nzl<=1024,PetscObjectComm((PetscObject) B), PETSC_ERR_PLIB, "Row too big: %D",nzl);
...
static PetscErrorCode adaptToleranceFEM(PetscFE fem, Vec sol, PetscInt type, PetscInt grid, LandauCtx *ctx, DM *newForest)
{
  ...
  PetscAssertFalse(!isForest,ctx->comm,PETSC_ERR_ARG_WRONG,"! Forest");
...
static PetscErrorCode ProcessOptions(LandauCtx *ctx, const char prefix[])
{
  ...
  PetscAssertFalse(LANDAU_MAX_BATCH_SZ < ctx->batch_sz,ctx->comm,PETSC_ERR_ARG_WRONG,"LANDAU_MAX_BATCH_SZ %D < ctx->batch_sz %D",LANDAU_MAX_BATCH_SZ,ctx->batch_sz);
  ...
  PetscAssertFalse(ctx->batch_view_idx >= ctx->batch_sz,ctx->comm,PETSC_ERR_ARG_WRONG,"-ctx->batch_view_idx %D > ctx->batch_sz %D",ctx->batch_view_idx,ctx->batch_sz);
  ...
  PetscAssertFalse(flg && nm != ctx->num_species,ctx->comm,PETSC_ERR_ARG_WRONG,"wrong num n: %D != num species %D",nm,ctx->num_species);
  ...
  PetscAssertFalse(flg && nc != ctx->num_species-1,ctx->comm,PETSC_ERR_ARG_WRONG,"num charges %D != num species %D",nc,ctx->num_species-1);
  ...
  if (flg) {
    ...
    PetscAssertFalse(ctx->num_species != nt,ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_num_species_grid: sum %D != num_species = %D. %D grids (check that number of grids <= LANDAU_MAX_GRIDS = %D)",nt,ctx->num_species,ctx->num_grids,LANDAU_MAX_GRIDS);
  } else {
  ...
  PetscAssertFalse(ctx->species_offset[ctx->num_grids] != ctx->num_species,ctx->comm,PETSC_ERR_ARG_WRONG,"ctx->species_offset[ctx->num_grids] %D != ctx->num_species = %D ???????????",ctx->species_offset[ctx->num_grids],ctx->num_species);
  for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) {
  ...
  PetscAssertFalse(flg && nt < ctx->num_grids,ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_domain_radius: given %D radius != number grids %D",nt,ctx->num_grids);
  for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) {
  ...
  PetscAssertFalse(flg && nt < ctx->num_grids,ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_amr_levels_max: given %D != number grids %D",nt,ctx->num_grids);
  nt = LANDAU_MAX_GRIDS;
  ...
  PetscAssertFalse(flg && ctx->num_grids != nt,ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_i_radius: %D != num_species = %D",nt,ctx->num_grids);
  PetscAssertFalse(ctx->sphere && ctx->e_radius <= ctx->i_radius[0],ctx->comm,PETSC_ERR_ARG_WRONG,"bad radii: %g < %g < %g",ctx->i_radius[0],ctx->e_radius,ctx->radius[0]);
  /* processing options */
            ...
            for (PetscInt f = 0 ; f < numindices ; ++f) { // look for a non-zero on the diagonal
              if (PetscAbs(PetscRealPart(elMat[f*numindices + f])) > PETSC_MACHINE_EPSILON) {
                ...
                if (PetscAbs(PetscRealPart(elMat[f*numindices + f] - 1.)) < PETSC_MACHINE_EPSILON) { // normal vertex 1.0
                  ...
                  PetscAssertFalse(maps[grid].num_reduced>=MAP_BF_SIZE,PETSC_COMM_SELF, PETSC_ERR_PLIB, "maps[grid].num_reduced %d > %d",maps[grid].num_reduced,MAP_BF_SIZE);
                }
                break;
              }
            }
...
static PetscErrorCode LandauCreateBatchOrdering(MPI_Comm comm, Vec X, IS grid_batch_is_inv[LANDAU_MAX_GRIDS], LandauCtx *ctx)
{
  ...
  for (PetscInt grid=0 ; grid<ctx->num_grids ; grid++) {
    ...
    for (PetscInt b_id = 0 ; b_id < ctx->batch_sz ; b_id++) {
      ...
      for (int i=0 ; i<nloc ; i++) {
        ierr = MatGetRow(B,i,&nzl,&cols,&vals);CHKERRQ(ierr);
        PetscAssert(nzl<=1024,comm, PETSC_ERR_PLIB, "Row too big: %D",nzl);
...
PetscErrorCode LandauCreateVelocitySpace(MPI_Comm comm, PetscInt dim, const char prefix[], Vec *X, Mat *J, DM *pack)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(dim!=2 && dim!=3,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Only 2D and 3D supported");
  PetscAssertFalse(LANDAU_DIM != dim,PETSC_COMM_SELF, PETSC_ERR_PLIB, "dim %D != LANDAU_DIM %d",dim,LANDAU_DIM);
  ...
  if (ctx->gpu_assembly) { /* we need GPU object with GPU assembly */
    ...
    if (ctx->deviceType == LANDAU_CUDA) {
      ...
      PetscAssertFalse(!flg,ctx->comm,PETSC_ERR_ARG_WRONG,"must use '-dm_mat_type aijcusparse -dm_vec_type cuda' for GPU assembly and Cuda or use '-dm_landau_device_type cpu'");
    } else if (ctx->deviceType == LANDAU_KOKKOS) {
      ierr = PetscObjectTypeCompareAny((PetscObject)ctx->J,&flg,MATSEQAIJKOKKOS,MATMPIAIJKOKKOS,MATAIJKOKKOS,"");CHKERRQ(ierr);
#if defined(PETSC_HAVE_KOKKOS_KERNELS)
      PetscAssertFalse(!flg,ctx->comm,PETSC_ERR_ARG_WRONG,"must use '-dm_mat_type aijkokkos -dm_vec_type kokkos' for GPU assembly and Kokkos or use '-dm_landau_device_type cpu'");
#else
      PetscAssertFalse(!flg,ctx->comm,PETSC_ERR_ARG_WRONG,"must configure with '--download-kokkos-kernels' for GPU assembly and Kokkos or use '-dm_landau_device_type cpu'");
#endif
    }
  }
...
PetscErrorCode LandauPrintNorms(Vec X, PetscInt stepi)
{
  ...
  PetscAssertFalse(!pack,PETSC_COMM_SELF, PETSC_ERR_PLIB, "Vector has no DM");
  ierr = DMGetDimension(pack, &dim);CHKERRQ(ierr);
  PetscAssertFalse(dim!=2 && dim!=3,PETSC_COMM_SELF, PETSC_ERR_PLIB, "dim= %D",dim);
  ...
  ierr = DMCompositeGetNumberDM(pack,&nDMs);CHKERRQ(ierr);
  PetscAssertFalse(nDMs != ctx->num_grids*ctx->batch_sz,PETSC_COMM_WORLD, PETSC_ERR_PLIB, "#DM wrong %D %D",nDMs,ctx->num_grids*ctx->batch_sz);
  ierr = PetscMalloc(sizeof(*globXArray)*nDMs, &globXArray);CHKERRQ(ierr);
        ...
        if (ctx->use_relativistic_corrections) {
          ...
          if (ii==0 && grid==0) { // do all at once
            ...
            for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) { // yes a grid loop in a grid loop to print nice
              ...
              PetscAssertFalse(bs != Nf,PETSC_COMM_SELF, PETSC_ERR_PLIB, "bs %D != num_species %D in Gsub",bs,Nf);
              ierr = VecGetBlockSize(Mfsub,&bs);CHKERRQ(ierr);
              PetscAssertFalse(bs != Nf,PETSC_COMM_SELF, PETSC_ERR_PLIB, "bs %D != num_species %D",bs,Nf);
...
PetscErrorCode LandauCreateMassMatrix(DM pack, Mat *Amat)
{
  ...
  ierr = DMGetApplicationContext(pack, &ctx);CHKERRQ(ierr);
  PetscAssert(ctx,PETSC_COMM_SELF, PETSC_ERR_PLIB, "no context");
  ...
  PetscAssertFalse(N1 != N2,PetscObjectComm((PetscObject) pack), PETSC_ERR_PLIB, "Incorrect matrix sizes: |Jacobian| = %D, |Mass|=%D",N1,N2);
  ...
  for (PetscInt grid=0 ; grid<ctx->num_grids ; grid++) {
    ...
    for (PetscInt b_id = 0 ; b_id < ctx->batch_sz ; b_id++) {
      ...
      for (int i=0 ; i<nloc ; i++) {
        ierr = MatGetRow(B,i,&nzl,&cols,&vals);CHKERRQ(ierr);
        PetscAssertFalse(nzl>1024,PetscObjectComm((PetscObject) pack), PETSC_ERR_PLIB, "Row too big: %D",nzl);
        for (int j=0; j<nzl; j++) colbuf[j] = cols[j] + moffset;
        row = i + moffset;
...
PetscErrorCode LandauIJacobian(TS ts, PetscReal time_dummy, Vec X, Vec U_tdummy, PetscReal shift, Mat Amat, Mat Pmat, void *actx)
{
  ...
  ierr = DMGetApplicationContext(pack, &ctx);CHKERRQ(ierr);
  PetscAssertFalse(!ctx,PETSC_COMM_SELF, PETSC_ERR_PLIB, "no context");
  PetscAssertFalse(Amat!=Pmat || Amat!=ctx->J,ctx->comm, PETSC_ERR_PLIB, "Amat!=Pmat || Amat!=ctx->J");
  ierr = DMGetDimension(pack, &dim);CHKERRQ(ierr);
  ...
  PetscAssertFalse(shift==0.0,ctx->comm, PETSC_ERR_PLIB, "zero shift");
  PetscAssertFalse(!ctx->aux_bool,ctx->comm, PETSC_ERR_PLIB, "wrong state");
