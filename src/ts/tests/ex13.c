...
int main(int argc,char **argv)
{
  ...
  for (i = 0; i < Nt; i++) {
    ...
    PetscAssertFalse(check && serr > tol,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error in state %g > %g",(double)serr,(double)tol);
    PetscAssertFalse(check && derr > tol,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error in state derivative %g > %g",(double)derr,(double)tol);
  }
  for (i = Nt-1; i >= 0; i--) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (i) {
      ...
      if (use2) {
        ...
        derr = PetscAbsScalar(dfunc(p,testtime)-aWdot[0]);
        ierr = VecRestoreArrayRead(hWdot,&aWdot);CHKERRQ(ierr);
        PetscAssertFalse(check && i >= p && derr > tol,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error in state derivative %g > %g",(double)derr,(double)tol);
      }
