...
static PetscErrorCode TestExplicitTS(TS ts,PetscInt order,const char subtype[])
{
  ...
  for (i = 0; i < order; i++) {
    ...
    if (error > eps) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bad solution, error %g, %s '%s'",(double)error,type,subtype);
  }
  ierr = VecRestoreArrayRead(X,&xx);CHKERRQ(ierr);
  ...
  for (i = 0; done && i < order-1; i++) {
    ...
    if (error > eps) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bad estimator, error %g, %s '%s'",(double)error,type,subtype);
  }
  ...
  if (step != 2) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bad step number %D, %s '%s'",step,type,subtype);
  if (reason != TS_DIVERGED_STEP_REJECTED) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bad reason %s, %s '%s'",TSConvergedReasons[reason],type,subtype);
  PetscFunctionReturn(0);
}
