...
PetscErrorCode CharacteristicView(Characteristic c, PetscViewer viewer)
{
  ...
  if (!viewer) {
    PetscCall(PetscViewerASCIIGetStdout(PetscObjectComm((PetscObject)c),&viewer));
  }
...
PetscErrorCode CharacteristicSetUp(Characteristic c)
{
  ...
  if (!((PetscObject)c)->type_name) {
    PetscCall(CharacteristicSetType(c, CHARACTERISTICDA));
  }
...
PetscErrorCode CharacteristicSolve(Characteristic c, PetscReal dt, Vec solution)
{
  ...
  PetscCall(PetscInfo(NULL, "Calculating %d remote velocities at t_{n - 1/2}\n", c->queueRemoteSize));
  ...
  PetscCall(PetscInfo(NULL, "Calculating %d remote field points at t_{n}\n", c->queueRemoteSize));
  for (n = 0; n < c->queueRemoteSize; n++) {
...
int CharacteristicSendCoordinatesBegin(Characteristic c)
{
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCallMPI(MPI_Irecv(&(c->fillCount[n]), 1, MPIU_INT, c->neighbors[n], tag, PetscObjectComm((PetscObject)c), &(c->request[n-1])));
  }
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCallMPI(MPI_Send(&(c->needCount[n]), 1, MPIU_INT, c->neighbors[n], tag, PetscObjectComm((PetscObject)c)));
  }
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCall(PetscInfo(NULL, "Receiving %d requests for values from proc %d\n", c->fillCount[n], c->neighbors[n]));
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCall(PetscInfo(NULL, "Sending %d requests for values from proc %d\n", c->needCount[n], c->neighbors[n]));
...
PetscErrorCode CharacteristicGetValuesBegin(Characteristic c)
{
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCallMPI(MPI_Irecv(&(c->queue[c->localOffsets[n]]), c->needCount[n], c->itemType, c->neighbors[n], tag, PetscObjectComm((PetscObject)c), &(c->request[n-1])));
  }
  for (n = 1; n < c->numNeighbors; n++) {
    PetscCallMPI(MPI_Send(&(c->queueRemote[c->remoteOffsets[n]]), c->fillCount[n], c->itemType, c->neighbors[n], tag, PetscObjectComm((PetscObject)c)));
  }
  PetscFunctionReturn(0);
}
...
/*---------------------------------------------------------------------*/
{
  ...
  if (0) { /* Check the order of the queue before sorting */
    ...
    for (n=0; n<size; n++) {
      PetscCall(PetscInfo(NULL,"%d %d\n",n,queue[n].proc));
    }
  }

  /* SORTING PHASE */
  for (n = (size / 2)-1; n >= 0; n--) {
    PetscCall(CharacteristicSiftDown(c, queue, n, size-1)); /* Rich had size-1 here, Matt had size*/
  }
  for (n = size-1; n >= 1; n--) {
    temp     = queue[0];
    queue[0] = queue[n];
