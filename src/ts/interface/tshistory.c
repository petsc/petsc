...
#define PetscValidLogicalCollectiveIntComm(a,b,c)                       \
  do {                                                                  \
    ...
    if (-b2[0] != b2[1]) SETERRQ(a,PETSC_ERR_ARG_WRONG,"Int value must be same on all processes, argument # %d",c); \
  } while (0)
...
#define PetscValidLogicalCollectiveBoolComm(a,b,c)                      \
  do {                                                                  \
    ...
    if (-b2[0] != b2[1]) SETERRQ(a,PETSC_ERR_ARG_WRONG,"Bool value must be same on all processes, argument # %d",c); \
  } while (0)
...
#define PetscValidLogicalCollectiveRealComm(a,b,c)                      \
  do {                                                                  \
    ...
    if (!(b2[2] > 0) && !PetscEqualReal(-b2[0],b2[1])) SETERRQ(a,PETSC_ERR_ARG_WRONG,"Real value must be same on all processes, argument # %d",c); \
  } while (0)
...
PetscErrorCode TSHistoryGetTime(TSHistory tsh, PetscBool backward, PetscInt step, PetscReal *t)
{
  ...
  if (step < 0 || step >= tsh->n) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Given time step %D does not match any in history [0,%D]",step,tsh->n);
