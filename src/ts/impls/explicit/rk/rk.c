...
/*
  This is interpolate formula for partitioned RHS multirate RK method
 */

static PetscErrorCode TSInterpolate_PARTITIONEDMRK(TS ts,PetscReal itime,Vec X)
{
  TS_RK            *rk = (TS_RK*)ts->data;
  PetscInt         s  = rk->tableau->s,p = rk->tableau->p,i,j;
  ...
  ierr = VecGetSubVector(rk->Y[0],ts->iss,&Yslow);CHKERRQ(ierr);
  ierr = VecCopy(Yslow,X);CHKERRQ(ierr);
  ierr = VecMAXPY(X,s,b,rk->YdotRHS_slow);CHKERRQ(ierr);
  ierr = VecRestoreSubVector(rk->Y[0],ts->iss,&Yslow);CHKERRQ(ierr);
  ierr = PetscFree(b);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

/*
 This is for combined RHS multirate RK method
 The step completion formula is

 x1 = x0 + h b^T YdotRHS

*/
static PetscErrorCode TSEvaluateStep_RKMULTIRATE(TS ts,PetscInt order,Vec X,PetscBool *done)
{
  TS_RK           *rk = (TS_RK*)ts->data;
  RKTableau       tab  = rk->tableau;
  Vec             Xtemp;                          /* temporary work vector for X                                   */
  PetscScalar     *w = rk->work;
  PetscScalar     *x_ptr,*xtemp_ptr;              /* location to put the pointer to X and Xtemp respectively       */
  PetscReal       h = ts->time_step;
  PetscInt        s = tab->s,j;
  PetscInt        len_slow,len_fast;              /* the number of slow components and fast components respectively */
  const PetscInt  *is_slow,*is_fast;              /* the index for slow components and fast components respectively */
  PetscErrorCode  ierr;

  PetscFunctionBegin;
  ierr = VecCopy(ts->vec_sol,X);CHKERRQ(ierr);
  ierr = VecDuplicate(ts->vec_sol,&Xtemp);CHKERRQ(ierr);
  ierr = VecCopy(ts->vec_sol,Xtemp);CHKERRQ(ierr);
  if (!rk->slow){
    for (j=0; j<s; j++) w[j] = h*tab->b[j];
    /* update the value of slow components, and discard the updating value of fast components */
    ierr = VecMAXPY(Xtemp,s,w,rk->YdotRHS);CHKERRQ(ierr);
    ierr = VecGetArray(X,&x_ptr);CHKERRQ(ierr);
    ierr = VecGetArray(Xtemp,&xtemp_ptr);CHKERRQ(ierr);
    ...
    for (j=0; j<len_slow; j++) x_ptr[is_slow[j]] = xtemp_ptr[is_slow[j]];
    ierr = VecRestoreArray(X,&x_ptr);CHKERRQ(ierr);
    ierr = VecRestoreArray(Xtemp,&xtemp_ptr);CHKERRQ(ierr);
  } else {
    /* update the value of fast components, and discard the updating value of slow components */
    ...
    for (j=0; j<len_fast; j++) x_ptr[is_fast[j]] = xtemp_ptr[is_fast[j]];
    ierr = VecRestoreArray(X,&x_ptr);CHKERRQ(ierr);
    ierr = VecRestoreArray(Xtemp,&xtemp_ptr);CHKERRQ(ierr);
  }
  /* free temporary vector space */
  ierr = VecDestroy(&Xtemp);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

static PetscErrorCode TSStep_RKMULTIRATE(TS ts)
{
  TS_RK             *rk = (TS_RK*)ts->data;
  RKTableau         tab  = rk->tableau;
  Vec               *Y = rk->Y,*YdotRHS = rk->YdotRHS;
  Vec               stage_fast,stage_slow;                              /* vectors store the stage value of fast and slow components respectively           */
  Vec               presol_fast,newslo_fast;                            /* vectors store the previous and new time solution of fast components respectively */
  Vec               YdotRHS_prev,prev_sol;                              /* vectors store the previous YdotRHS and solution respectively                     */
  const PetscInt    s = tab->s,*is_slow,*is_fast;                       /* is_slow: index of slow components; is_fast: index of fast components             */
  const PetscReal   *A = tab->A,*c = tab->c;
  PetscScalar       *w = rk->work;
  PetscScalar       *y_ptr,*faststage_ptr,*slowstage_ptr;               /* location to put the pointer to Y, stage_fast and stage_slow respectively         */
  PetscScalar       *ydotrhsprev_ptr,*ydotrhs_ptr;                      /* location to put the pointer to YdotRHS_prev and YdotRHS respectively             */
  PetscInt          i,j,k,len_slow,len_fast;                            /* len_slow: the number of slow comonents; len_fast: the number of fast components  */
  PetscReal         next_time_step = ts->time_step,t = ts->ptime,h = ts->time_step;
  PetscErrorCode    ierr;

  PetscFunctionBegin;
  rk->status = TS_STEP_INCOMPLETE;
  ierr = VecDuplicate(ts->vec_sol,&stage_fast);CHKERRQ(ierr);
  ierr = VecDuplicate(ts->vec_sol,&stage_slow);CHKERRQ(ierr);
  ierr = VecDuplicate(ts->vec_sol,&YdotRHS_prev);CHKERRQ(ierr);
  ierr = VecDuplicate(ts->vec_sol,&prev_sol);CHKERRQ(ierr);
  ierr = ISGetSize(ts->iss,&len_slow);CHKERRQ(ierr);
  ierr = ISGetSize(ts->isf,&len_fast);CHKERRQ(ierr);
  ierr = ISGetIndices(ts->iss,&is_slow);CHKERRQ(ierr);
  ierr = ISGetIndices(ts->isf,&is_fast);CHKERRQ(ierr);
  ...
  ierr = VecCopy(ts->vec_sol,prev_sol);CHKERRQ(ierr);
  rk->slow = 0;
  ierr = TSEvaluateStep_RKMULTIRATE(ts,tab->order,ts->vec_sol,NULL);CHKERRQ(ierr);
  for (k=0; k<rk->dtratio; k++){
    for (i=0; i<s; i++) {
      rk->stage_time = t + h/rk->dtratio*c[i];
      ierr = TSPreStage(ts,rk->stage_time);CHKERRQ(ierr);
      ierr = VecCopy(prev_sol,Y[i]);CHKERRQ(ierr);
      ierr = VecCopy(prev_sol,stage_slow);CHKERRQ(ierr);
      ierr = VecCopy(prev_sol,stage_fast);CHKERRQ(ierr);
      /* guarantee the stage RHS for slow components do not change while updating the stage RHS for fast components */
      ierr = VecCopy(YdotRHS[i],YdotRHS_prev);CHKERRQ(ierr);
      /* update the fast components stage value */
      for (j=0; j<i; j++) w[j] = h/rk->dtratio*A[i*s+j];
      ierr = VecMAXPY(stage_fast,i,w,YdotRHS);CHKERRQ(ierr);
      /* update the slow components stage value */
      ierr = VecCopy(prev_sol,Y[0]);CHKERRQ(ierr);
      ierr = TSInterpolate_RK(ts,t+k*h/rk->dtratio+h/rk->dtratio*c[i],stage_slow);CHKERRQ(ierr);
      /* combine the update fast components stage value and slow components stage value to Y[i] */
      ierr = VecGetArray(Y[i],&y_ptr);CHKERRQ(ierr);
      ierr = VecGetArray(stage_slow,&slowstage_ptr);CHKERRQ(ierr);
      ierr = VecGetArray(stage_fast,&faststage_ptr);CHKERRQ(ierr);
      for (j=0; j<len_slow; j++) y_ptr[is_slow[j]] = slowstage_ptr[is_slow[j]];
      for (j=0; j<len_fast; j++) y_ptr[is_fast[j]] = faststage_ptr[is_fast[j]];
      ierr = VecRestoreArray(Y[i],&y_ptr);CHKERRQ(ierr);
      ierr = VecRestoreArray(stage_slow,&slowstage_ptr);CHKERRQ(ierr);
      ierr = VecRestoreArray(stage_fast,&faststage_ptr);CHKERRQ(ierr);
      ierr = TSPostStage(ts,rk->stage_time,i,Y); CHKERRQ(ierr);
      /* compute the stage RHS */
      ierr = TSComputeRHSFunction(ts,t+k*h/rk->dtratio+h/rk->dtratio*c[i],Y[i],YdotRHS[i]);CHKERRQ(ierr);
      /* re-assign the value of slow components in YdotRHS[i] to the calculated value before working on smaller time step-size */
      ierr = VecGetArray(YdotRHS[i],&ydotrhs_ptr);CHKERRQ(ierr);
      ierr = VecGetArray(YdotRHS_prev,&ydotrhsprev_ptr);CHKERRQ(ierr);
      for (j=0; j<len_slow; j++) ydotrhs_ptr[is_slow[j]] = ydotrhsprev_ptr[is_slow[j]];
      ierr = VecRestoreArray(YdotRHS[i],&ydotrhs_ptr);CHKERRQ(ierr);
      ierr = VecRestoreArray(YdotRHS_prev,&ydotrhsprev_ptr);CHKERRQ(ierr);
    }
    rk->slow = 1;
    ierr = TSEvaluateStep_RKMULTIRATE(ts,tab->order,ts->vec_sol,NULL);CHKERRQ(ierr);
    /* update the intial value for fast components */
    ierr = VecGetSubVector(prev_sol,ts->isf,&presol_fast);CHKERRQ(ierr);
    ierr = VecGetSubVector(ts->vec_sol,ts->isf,&newslo_fast);CHKERRQ(ierr);
    ierr = VecCopy(newslo_fast,presol_fast);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(prev_sol,ts->isf,&presol_fast);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(ts->vec_sol,ts->isf,&newslo_fast);CHKERRQ(ierr);
  }
  ts->ptime += ts->time_step;
  ts->time_step = next_time_step;
  rk->slow = 0;
  ...
  ierr = VecDestroy(&stage_fast);CHKERRQ(ierr);
  ierr = VecDestroy(&stage_slow);CHKERRQ(ierr);
  ierr = VecDestroy(&YdotRHS_prev);CHKERRQ(ierr);
  ierr = VecDestroy(&prev_sol);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
*/
static PetscErrorCode TSEvaluateStep_RKPMULTIRATE(TS ts,PetscInt order,Vec X,PetscBool *done)
{
  TS_RK           *rk = (TS_RK*)ts->data;
  RKTableau       tab  = rk->tableau;
  ...
  if (!rk->slow){
    for (j=0; j<s; j++) w[j] = h*tab->b[j];
    ierr = VecGetSubVector(ts->vec_sol,ts->iss,&Xslow);CHKERRQ(ierr);
    ierr = VecMAXPY(Xslow,s,w,rk->YdotRHS_slow);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(ts->vec_sol,ts->iss,&Xslow);CHKERRQ(ierr);;
    } else {;
    for (j=0; j<s; j++) w[j] = h/rk->dtratio*tab->b[j];
    ierr = VecGetSubVector(X,ts->isf,&Xfast);CHKERRQ(ierr);
    ierr = VecMAXPY(Xfast,s,w,rk->YdotRHS_fast);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(X,ts->isf,&Xfast);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}

static PetscErrorCode TSStep_RKPMULTIRATE(TS ts)
{
  TS_RK             *rk = (TS_RK*)ts->data;
  RKTableau         tab  = rk->tableau;
  Vec               *Y = rk->Y;
  ...
  Vec               Yfast_prev,Yfast_new,prev_sol;       /* subvectors store the previous and new solution of fast components and vector store the previous solution */
  const PetscInt    s = tab->s;
  const PetscReal   *A = tab->A,*c = tab->c;
  PetscScalar       *w = rk->work;
  PetscInt          i,j,k;
  ...
  for (i=0; i<s; i++) {
    ...
    ierr = TSPostStage(ts,rk->stage_time,i,Y); CHKERRQ(ierr);
    /* compute the stage RHS for both slow and fast components */
    ierr = TSComputeRHSFunctionslow(ts,t+h*c[i],Y[i],YdotRHS_slow[i]);CHKERRQ(ierr);
    ierr = TSComputeRHSFunctionfast(ts,t+h*c[i],Y[i],YdotRHS_fast[i]);CHKERRQ(ierr);
  }
  /* store the value of slow components at previous time  */
  ierr = VecDuplicate(ts->vec_sol,&prev_sol);CHKERRQ(ierr);
  ierr = VecCopy(ts->vec_sol,prev_sol);CHKERRQ(ierr);
  rk->slow = 0;
  ierr = TSEvaluateStep_RKPMULTIRATE(ts,tab->order,ts->vec_sol,NULL);CHKERRQ(ierr);
  for (k=0; k<rk->dtratio; k++){
    for (i=0; i<s; i++) {
    rk->stage_time = t + h/rk->dtratio*c[i];
    ierr = TSPreStage(ts,rk->stage_time);CHKERRQ(ierr);
    ...
    ierr = VecCopy(prev_sol,Y[0]);CHKERRQ(ierr);
    ierr = VecGetSubVector(Y[i],ts->iss,&Yslow);CHKERRQ(ierr);
    ierr = TSInterpolate_PARTITIONEDMRK(ts,t+k*h/rk->dtratio+h/rk->dtratio*c[i],Yslow);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(Y[i],ts->iss,&Yslow);CHKERRQ(ierr);
    ierr = TSPostStage(ts,rk->stage_time,i,Y); CHKERRQ(ierr);
    /* compute the stage RHS for fast components */
    ierr = TSComputeRHSFunctionfast(ts,t+k*h/rk->dtratio+h/rk->dtratio*c[i],Y[i],YdotRHS_fast[i]);CHKERRQ(ierr);
   }
    /* update the value of fast components whenusing fast time step */
    rk->slow = 1;
    ierr = TSEvaluateStep_RKPMULTIRATE(ts,tab->order,ts->vec_sol,NULL);CHKERRQ(ierr);
    ierr = VecGetSubVector(prev_sol,ts->isf,&Yfast_prev);CHKERRQ(ierr);
    ierr = VecGetSubVector(ts->vec_sol,ts->isf,&Yfast_new);CHKERRQ(ierr);
    ierr = VecCopy(Yfast_new,Yfast_prev);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(prev_sol,ts->isf,&Yfast_prev);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(ts->vec_sol,ts->isf,&Yfast_new);CHKERRQ(ierr);
  }
  ts->ptime += ts->time_step;
  ts->time_step = next_time_step;
  rk->slow = 0;
...
static PetscErrorCode TSRKTableauReset(TS ts)
{
  ...
  ierr = VecDestroyVecs(tab->s,&rk->YdotRHS_fast);CHKERRQ(ierr);
...
static PetscErrorCode TSRKTableauSetUp(TS ts)
{
  ...
  ierr = VecDuplicateVecs(YdotRHS_fast,tab->s,&rk->YdotRHS_fast);CHKERRQ(ierr);
  ...
  ierr = VecDuplicateVecs(YdotRHS_slow,tab->s,&rk->YdotRHS_slow);CHKERRQ(ierr);
...
/*
  construct a database to choose single-step rk method, or combined rhs mutirate rk method or partitioned rhs rk method
*/
const char *const TSRKMultirateTypes[] = {"NONE","COMBINED","PARTITIONED","TSRKMultirateType","RKM_",0};
...
static PetscErrorCode TSSetFromOptions_RK(PetscOptionItems *PetscOptionsObject,TS ts)
{
  ...
  ierr = PetscOptionsHead(PetscOptionsObject,"RK ODE solver options");CHKERRQ(ierr);
  {
    ...
    ierr = PetscOptionsEList("-ts_rk_multirate_type","Use Multirate or partioned RHS Multirate or single rate RK method","TSRKSetMultirateType",TSRKMultirateTypes,3,TSRKMultirateTypes[0],&rkmtype,&flg);CHKERRQ(ierr);
    if (flg) {ierr = TSRKSetMultirateType(ts,rkmtype);CHKERRQ(ierr);}
...
.   -ts_rk_multiarte_type - <none,combined,partitioned>
  ...
  Level: intermediate
@*/
PetscErrorCode TSRKSetMultirateType(TS ts, TSRKMultirateType rkmtype)
{
  TS_RK *rk = (TS_RK*)ts->data;
  PetscErrorCode ierr;
  ...
  switch(rkmtype){
    case RKM_NONE:
      break;
    case RKM_COMBINED:
      ts->ops->step           = TSStep_RKMULTIRATE;
      ts->ops->evaluatestep   = TSEvaluateStep_RKMULTIRATE;
      rk->dtratio             = 2;
      ierr = TSRKSetType(ts,TSMRKDefault);CHKERRQ(ierr);
      break;
    case RKM_PARTITIONED:
      ts->ops->step           = TSStep_RKPMULTIRATE;
      ts->ops->evaluatestep   = TSEvaluateStep_RKPMULTIRATE;
      ts->ops->interpolate    = TSInterpolate_PARTITIONEDMRK;
      rk->dtratio             = 2;
      ierr = TSRKSetType(ts,TSMRKDefault);CHKERRQ(ierr);
      break;
    default :
      SETERRQ1(PetscObjectComm((PetscObject)ts),PETSC_ERR_ARG_UNKNOWN_TYPE,"Unknown type '%s'",rkmtype);
  }
  PetscFunctionReturn(0);
}
