#ifndef lint
static char vcid[] = "$Id: beuler.c,v 1.3 1996/03/05 01:21:16 bsmith Exp bsmith $";
#endif
/*
...
#else
    if (real(ts->ptime) > real(ts->max_time)) break;
#endif
...
static int TSDestroy_BEuler_No_Matrix(PetscObject obj )
{
  TS        ts = (TS) obj;
  TS_BEuler *beuler = (TS_BEuler*) ts->data;
  int       ierr;
...
int TSCreate_BEuler(TS ts )
{
  ...
  if (ts->problem_type == TS_LINEAR) {
    ...
    if (!ts->rhsmatrix) {
      if (mtype == MATSHELL) {
        ts->setup  = TSSetUp_BEuler_Linear_Constant_Matrix_Shell;
        ts->Ashell = PETSC_TRUE;
      } else {
      ...
      if (mtype == MATSHELL) {
        ts->setup  = TSSetUp_BEuler_Linear_Variable_Matrix_Shell;
        ts->Ashell = PETSC_TRUE;
      } else {
