...
static PetscErrorCode TSStep_Alpha(TS ts)
{
  ...
  for (reject=0; reject<ts->max_reject; reject++,ts->reject++) {
    ...
    ierr = TSFunctionDomainError(ts,th->stage_time,0,&(th->V1),&accept);CHKERRQ(ierr);
...
PETSC_EXTERN PetscErrorCode TSCreate_Alpha(TS ts)
{
  ...
  ierr = PetscObjectComposeFunction((PetscObject)ts,"TSAlphaSetAdapt_C",TSAlphaSetAdapt_Alpha);CHKERRQ(ierr);
