...
.seealso: [](ch_ts), `TSTrajectory`, `TSTrajectoryView()`, `PetscObjectViewFromOptions()`, `TSTrajectoryCreate()`, `PetscOptionsCreateViewer()`
@*/
...
- ts - the TS context
  ...
  Options Database Key:
. -ts_trajectory_type (basic|memory|singlefile|visualization) - the method of storing the trajectory
