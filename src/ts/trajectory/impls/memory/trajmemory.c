...
static PetscErrorCode ElementSet(TS ts, Stack *stack, StackElement *e, PetscInt stepnum, PetscReal time, Vec X)
{
  ...
  if (HaveSolution((*e)->cptype)) {
    PetscCall(VecCopy(X,(*e)->X));
  }
  if (HaveStages((*e)->cptype)) {
    PetscCall(TSGetStages(ts,&stack->numY,&Y));
    for (i=0;i<stack->numY;i++) {
      PetscCall(VecCopy(Y[i],(*e)->Y[i]));
    }
  }
  (*e)->stepnum = stepnum;
  (*e)->time    = time;
...
static PetscErrorCode StackInit(Stack *stack,PetscInt size,PetscInt ny)
{
  ...
  if (!stack->container) {
    PetscCall(PetscCalloc1(size,&stack->container));
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode UpdateTS(TS ts,Stack *stack,StackElement e,PetscInt stepnum,PetscBool adjoint_mode)
{
  ...
  if (!adjoint_mode || (HaveSolution(e->cptype) && e->stepnum!=stepnum)) {
    PetscCall(VecCopy(e->X,ts->vec_sol));
  }
  if (HaveStages(e->cptype)) {
    PetscCall(TSGetStages(ts,&stack->numY,&Y));
    if (e->stepnum && e->stepnum==stepnum) {
      for (i=0;i<stack->numY;i++) {
        PetscCall(VecCopy(e->Y[i],Y[i]));
      }
...
static PetscErrorCode TSTrajectoryMemorySet_N(TS ts,TJScheduler *tjsch,PetscInt stepnum,PetscReal time,Vec X)
{
  ...
  if (stack->top+1 == stack->stacksize) {
    PetscCall(StackResize(stack,2*stack->stacksize));
  }
...
{
  PetscFunctionBegin;
  PetscCall(PetscTryMethod(tj,"TSTrajectoryMemorySetType_C",(TSTrajectory,TSTrajectoryMemoryType),(tj,tj_memory_type)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(tj,"TSTrajectorySetMaxCpsRAM_C",(TSTrajectory,PetscInt),(tj,max_cps_ram)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(tj,"TSTrajectorySetMaxCpsDisk_C",(TSTrajectory,PetscInt),(tj,max_cps_disk)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(tj,"TSTrajectorySetMaxUnitsRAM_C",(TSTrajectory,PetscInt),(tj,max_units_ram)));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  PetscCall(PetscUseMethod(tj,"TSTrajectorySetMaxUnitsDisk_C",(TSTrajectory,PetscInt),(tj,max_units_disk)));
  PetscFunctionReturn(0);
}
