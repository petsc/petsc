...
int main(int argc, char **argv)
{
  ...
  PetscCall(TSSetRHSJacobian(ctx.ts, ctx.Jac, ctx.Jac, (TSRHSJacobian_Fn *)RHSJacobian, &ctx));
  ...
  if (ctx.sa == SA_ADJ) {
    ...
    PetscCall(TSSetRHSFunction(ctx.quadts, NULL, (TSRHSFunction_Fn *)CostIntegrand, &ctx));
    PetscCall(TSSetRHSJacobian(ctx.quadts, ctx.DRDU, ctx.DRDU, (TSRHSJacobian_Fn *)DRDUJacobianTranspose, &ctx));
