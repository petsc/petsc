...
PetscErrorCode PostEventFunction(TS ts,PetscInt nevents,PetscInt event_list[],PetscReal t,Vec U,PetscBool forwardsolve,void* ctx)
{
  ...
  PetscFunctionBegin;
  if (event_list[0] == 0) {
    CHKERRQ(PetscPrintf(PETSC_COMM_SELF,"Ball hit the ground at t = %5.2f seconds\n",(double)t));
    ...
    CHKERRQ(PetscPrintf(PETSC_COMM_SELF,"Ball bounced %D times\n",app->nbounces));
  }
  app->nbounces++;
  PetscFunctionReturn(0);
}
...
int main(int argc,char **argv)
{
  ...
  CHKERRQ(PetscOptionsInt("-maxbounces","","",app.maxbounces,&app.maxbounces,NULL));
  CHKERRQ(PetscOptionsBool("-test_adapthistory","","",hist,&hist,NULL));
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     Set solver options
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (hist) CHKERRQ(TSSetSaveTrajectory(ts));
  ...
  CHKERRQ(TSSetEventHandler(ts,2,direction,terminate,EventFunction,PostEventFunction,(void*)&app));
