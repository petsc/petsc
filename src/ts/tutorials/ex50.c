...
int main(int argc,char **argv)
{
  ...
  PetscAssertFalse(appctx.param.E % size,PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONG,"Number of elements must be divisible by number of processes");
...
PetscErrorCode RHSMatrixLaplaciangllDM(TS ts,PetscReal t,Vec X,Mat A,Mat BB,void *ctx)
{
  ...
  if (!flg) {
    ...
    PetscAssertFalse(appctx->param.N <= 1,PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONG,"Spectral element order should be > 1");
