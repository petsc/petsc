...
static PetscErrorCode PCSetUp_HYPRE(PC pc)
{
  ...
  if (jac->setup == HYPRE_AMSSetup) {
    ...
    if (jac->dim) {
      PetscCallExternal(HYPRE_AMSSetDimension,jac->hsolver,jac->dim);
    }
    if (jac->constants[0]) {
      ...
      if (jac->constants[2]) {
        PetscCallExternal(HYPRE_IJVectorGetObject,jac->constants[2]->ij,(void**)(&zzo));
      }
...
static PetscErrorCode PCApply_HYPRE(PC pc,Vec b,Vec x)
{
  ...
  if (jac->setup == HYPRE_AMSSetup && jac->ams_beta_is_zero_part) {
    PetscCallExternal(HYPRE_AMSProjectOutGradients,jac->hsolver,jxv);
  }
...
static PetscErrorCode PCSetFromOptions_HYPRE_BoomerAMG(PetscOptionItems *PetscOptionsObject,PC pc)
{
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetNumFunctions,jac->hsolver,bs);
  }
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetNodal,jac->hsolver,jac->nodal_coarsening);
  }
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetNodalDiag,jac->hsolver,jac->nodal_coarsening_diag);
  }
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetInterpVecVariant,jac->hsolver,jac->vec_interp_variant);
  }
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetSmoothInterpVectors,jac->hsolver,jac->vec_interp_smooth);
  }
  ...
  if (flg) {
    PetscCallExternal(HYPRE_BoomerAMGSetInterpRefine,jac->hsolver,jac->interp_refine);
  }
...
static PetscErrorCode PCSetFromOptions_HYPRE_AMS(PetscOptionItems *PetscOptionsObject,PC pc)
{
  ...
  if (flag || flag2 || flag3 || flag4) {
    PetscCallExternal(HYPRE_AMSSetSmoothingOptions,jac->hsolver,jac->as_relax_type,
                                                                      jac->as_relax_times,
                                                                      jac->as_relax_weight,
                                                                      jac->as_omega);
  }
...
static PetscErrorCode PCSetFromOptions_HYPRE_ADS(PetscOptionItems *PetscOptionsObject,PC pc)
{
  ...
  if (flag || flag2 || flag3 || flag4) {
    PetscCallExternal(HYPRE_ADSSetSmoothingOptions,jac->hsolver,jac->as_relax_type,
                                                                      jac->as_relax_times,
                                                                      jac->as_relax_weight,
                                                                      jac->as_omega);
  }
...
PetscErrorCode PCSetFromOptions_SysPFMG(PetscOptionItems *PetscOptionsObject,PC pc)
{
  ...
  if (flg) {
    PetscCallExternal(HYPRE_SStructSysPFMGSetPrintLevel,ex->ss_solver,3);
  }
