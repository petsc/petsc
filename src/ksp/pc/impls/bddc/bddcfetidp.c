...
PetscErrorCode PCBDDCSetupFETIDPMatContext(FETIDPMat_ctx fetidpmat_ctx)
{
  ...
  if (pP) { /* saddle point */
    ...
    PetscAssertFalse(!gP,PETSC_COMM_SELF,PETSC_ERR_PLIB,"gP not present");
    ...
    PetscAssertFalse(!fetidpmat_ctx->B_BI,PETSC_COMM_SELF,PETSC_ERR_PLIB,"B_BI not present");
    ...
    PetscAssertFalse(!fetidpmat_ctx->B_BB,PETSC_COMM_SELF,PETSC_ERR_PLIB,"B_BB not present");
    ...
    PetscAssertFalse(!fetidpmat_ctx->Bt_BI,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bt_BI not present");
    ...
    PetscAssertFalse(!fetidpmat_ctx->Bt_BB,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Bt_BB not present");
  ...
  if (PetscDefined(USE_DEBUG)) {
    ...
    PetscAssertFalse(i != fetidpmat_ctx->n_lambda,PETSC_COMM_WORLD,PETSC_ERR_PLIB,"Global number of multipliers mismatch! (%D != %D)",fetidpmat_ctx->n_lambda,i);
  }
  ...
  BD2 = NULL;
  if (fully_redundant) {
    PetscAssertFalse(pcbddc->use_deluxe_scaling,comm,PETSC_ERR_SUP,"Deluxe FETIDP with fully-redundant multipliers to be implemented");
    ...
    if (!pcbddc->use_deluxe_scaling || !pcbddc->sub_schurs) {
      ...
      PetscAssertFalse(!pcbddc->deluxe_singlemat,comm,PETSC_ERR_USER,"Cannot compute B_Ddelta! rerun with -pc_bddc_deluxe_singlemat");
      mss  = 0;
      ...
      if (mss) {
        ...
        PetscAssertFalse(B_ierr,PETSC_COMM_SELF,PETSC_ERR_LIB,"Error in query to GETRI Lapack routine %d",(int)B_ierr);
      ...
      for (i=0,cum=0;i<sub_schurs->n_subs;i++) {
        ...
        PetscAssertFalse(B_ierr,PETSC_COMM_SELF,PETSC_ERR_LIB,"Error in GETRF Lapack routine %d",(int)B_ierr);
        PetscStackCallBLAS("LAPACKgetri",LAPACKgetri_(&B_N,W,&B_N,pivots,Bwork,&B_lwork,&B_ierr));
        PetscAssertFalse(B_ierr,PETSC_COMM_SELF,PETSC_ERR_LIB,"Error in GETRI Lapack routine %d",(int)B_ierr);
        ierr = PetscFPTrapPop();CHKERRQ(ierr);
        /* silent static analyzer */
        PetscAssertFalse(!idxs,PETSC_COMM_SELF,PETSC_ERR_PLIB,"IDXS not present");
