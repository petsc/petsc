...
PetscErrorCode PCBDDCSetupFETIDPMatContext(FETIDPMat_ctx fetidpmat_ctx)
{
  ...
  if (!pcbddc->use_deluxe_scaling) {
    ...
    for (i = 1; i < pcis->n_neigh; i++) {
      ...
      PetscCallMPI(MPIU_Isend(&send_buffer[ptrs_buffer[i - 1]], buf_size, MPIU_SCALAR, pcis->neigh[i], 0, comm, &send_reqs[i - 1]));
      PetscCallMPI(MPIU_Irecv(&recv_buffer[ptrs_buffer[i - 1]], buf_size, MPIU_SCALAR, pcis->neigh[i], 0, comm, &recv_reqs[i - 1]));
    }
