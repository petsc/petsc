...
PetscErrorCode PCBDDCSubSchursSetUp(PCBDDCSubSchurs sub_schurs, Mat Ain, Mat Sin, PetscBool exact_schur, PetscInt xadj[], PetscInt adjncy[], PetscInt nlayers, Vec scaling, PetscBool compute_Stilda, PetscBool reuse_solvers, PetscBool benign_trick, PetscInt benign_n, PetscInt benign_p0_lidx[], IS benign_zerodiag_subs[], Mat change, IS change_primal)
{
  ...
  if (!sub_schurs->schur_explicit) {
        ...
        for (i=0;i<benign_n;i++) {
          ...
          for (k=0;k<benign_n;k++) {
            ...
            if (k == i) {
              ...
              if (matl_dbg_viewer) {
                PetscCallBLAS("BLASsyrk",BLASsyrk_("L","N",&B_n,&B_k,&sum,cs_AIB+i*size_schur,&B_n,&one,S3_data,&B_n));
              }
              ...
              if (matl_dbg_viewer) {
                PetscCallBLAS("BLASsyr2k",BLASsyr2k_("L","N",&B_n,&B_k,&sum,cs_AIB+k*size_schur,&B_n,cs_AIB+i*size_schur,&B_n,&one,S3_data,&B_n));
              }
            }
          }
          sum  = 1.;
          ...
          if (matl_dbg_viewer) {
            PetscCallBLAS("BLASsyr2k",BLASsyr2k_("L","N",&B_n,&B_k,&sum,array+n_I,&B_n,cs_AIB+i*size_schur,&B_n,&one,S2_data,&B_n));
          }
          PetscCall(VecRestoreArrayRead(v,(const PetscScalar**)&array));
    ...
    if (sub_schurs->gdsw) {
      PetscCall(MatCreateSubMatrices(sub_schurs->A,sub_schurs->n_subs,sub_schurs->is_subs,sub_schurs->is_subs,MAT_INITIAL_MATRIX,&gdswA));
    }
    for (i=0;i<sub_schurs->n_subs;i++) {
      PetscInt j;
    ...
    if (sub_schurs->gdsw) {
      PetscCall(MatDestroySubMatrices(sub_schurs->n_subs,&gdswA));
    }
