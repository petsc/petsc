...
static PetscErrorCode PCBDDCMatTransposeMatSolve_SeqDense(Mat A,Mat B,Mat X)
{
  ...
  if (A->factortype == MAT_FACTOR_LU) {
    ...
    PetscAssertFalse(info,PETSC_COMM_SELF,PETSC_ERR_LIB,"GETRS - Bad solve");
...
PetscErrorCode PCBDDCScalingExtension(PC pc, Vec local_interface_vector, Vec global_vector)
{
  ...
  PetscAssertFalse(local_interface_vector == pcbddc->work_scaling,PETSC_COMM_SELF,PETSC_ERR_SUP,"Local vector cannot be pcbddc->work_scaling!");
  ...
  if (sub_schurs->is_vertices || sub_schurs->is_dir) {
    ...
    if (!deluxe_ctx->n_simple) {
      ...
      if (sub_schurs->is_vertices) {
        ...
        PetscAssertFalse(nmap != n_com,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error when mapping simply scaled dofs (is_vertices)! %D != %D",nmap,n_com);
      ...
      if (sub_schurs->is_dir) {
        ...
        PetscAssertFalse(nmap != n_dir,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error when mapping simply scaled dofs (sub_schurs->is_dir)! %D != %D",nmap,n_dir);
      ...
      PetscAssertFalse(deluxe_ctx->n_simple != n_dir + n_com,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Number of simply scaled dofs %D is different from the previous one computed %D",n_dir + n_com,deluxe_ctx->n_simple);
    }
  } else {
    deluxe_ctx->n_simple = 0;
...
static PetscErrorCode PCBDDCScalingSetUp_Deluxe_Private(PC pc)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(!sub_schurs,PetscObjectComm((PetscObject)pc),PETSC_ERR_PLIB,"Missing PCBDDCSubSchurs");
  ...
  if (!deluxe_ctx->seq_n) {
    ...
    newsetup = PETSC_TRUE;
  } else PetscAssertFalse(deluxe_ctx->seq_n != sub_schurs->n_subs,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Number of deluxe subproblems %D is different from the sub_schurs %D",deluxe_ctx->seq_n,sub_schurs->n_subs);
  ...
  for (i=0;i<deluxe_ctx->seq_n;i++) {
    ...
    if (pcbddc->deluxe_singlemat) {
      ...
      if (deluxe_ctx->change) {
        Mat C,CY;
        PetscAssertFalse(!deluxe_ctx->change_with_qr,PETSC_COMM_SELF,PETSC_ERR_SUP,"Only QR based change of basis");
