...
static PetscErrorCode MatMult_ML(Mat A,Vec x,Vec y)
{
  ...
  PetscCallExternalNoErrorCode("ML_Operator_Apply",ML_Operator_Apply(shell->mlmat,x_length,(PetscScalar*)xarray,y_length,yarray));
...
static PetscErrorCode MatWrapML_SeqAIJ(ML_Operator *mlmat,MatReuse reuse,Mat *newmat)
{
  ...
  if (!reuse) {
    ...
    for (i=0; i<m; i++) {
      PetscCallExternalNoErrorCode("ML_Operator_Getrow",ML_Operator_Getrow(mlmat,1,&i,nz_max,aj,aa,&nnz[i]));
    }
  ...
  for (i=0; i<m; i++) {
    ...
    PetscCallExternalNoErrorCode("ML_Operator_Getrow",ML_Operator_Getrow(mlmat,1,&i,nz_max,aj,aa,&ncols));
...
static PetscErrorCode MatWrapML_MPIAIJ(ML_Operator *mlmat,MatReuse reuse,Mat *newmat)
{
  ...
  PetscCallExternalNoErrorCode("ML_build_global_numbering",ML_build_global_numbering(mlmat,&gordering,"rows"));
  ...
  PetscCallExternalNoErrorCode("ML_free",ML_free(gordering));
...
PetscErrorCode PCReset_ML(PC pc)
{
  ...
  if (dim) {
    for (level=0; level<=fine_level; level++) {
      PetscCall(VecDestroy(&pc_ml->gridctx[level].coords));
    }
    if (pc_ml->ml_object && pc_ml->ml_object->Grid) {
      ...
      grid_info->y = 0;
      grid_info->z = 0;
      PetscCallExternalNoErrorCode("ML_Operator_Getrow",ML_Aggregate_VizAndStats_Clean(pc_ml->ml_object));
    }
  }
  PetscCallExternalNoErrorCode("ML_Aggregate_Destroy",ML_Aggregate_Destroy(&pc_ml->agg_object));
  PetscCallExternalNoErrorCode("ML_Aggregate_Destroy",ML_Destroy(&pc_ml->ml_object));
...
PetscErrorCode PCSetUp_ML(PC pc)
{
  ...
  if (pc->setupcalled) {
    if (pc->flag == SAME_NONZERO_PATTERN && pc_ml->reuse_interpolation) {
      ...
      PetscCallExternalNoErrorCode("ML_Aggregate_Destroy",ML_Init_Amatrix(ml_object,0,m,m,PetscMLdata));
      PetscCallExternalNoErrorCode("ML_Set_Amatrix_Matvec",ML_Set_Amatrix_Matvec(ml_object,0,PetscML_matvec));
      ...
      while (ml_object->SingleLevel[mesh_level].Rmat->to) {
        ...
        PetscCallExternalNoErrorCode("ML_Operator_Clean",ML_Operator_Clean(mlmat));
        PetscCallExternalNoErrorCode("ML_Operator_Init",ML_Operator_Init(mlmat,ml_object->comm));
        PetscCallExternalNoErrorCode("ML_Gen_AmatrixRAP",ML_Gen_AmatrixRAP(ml_object, old_mesh_level, mesh_level));
      }
      ...
      for (level=0; level<fine_level; level++) {
        if (level > 0) {
          PetscCall(PCMGSetResidual(pc,level,PCMGResidualDefault,gridctx[level].A));
        }
  ...
  PetscCall(MatGetSize(Aloc,&m,&nlocal_allcols));
  PetscCall(MatGetBlockSize(A,&bs));
  PetscCallExternalNoErrorCode("ML_Create",ML_Create(&ml_object,pc_ml->MaxNlevels));
  PetscCallExternalNoErrorCode("ML_Comm_Set_UsrComm",ML_Comm_Set_UsrComm(ml_object->comm,PetscObjectComm((PetscObject)A)));
  pc_ml->ml_object = ml_object;
  PetscCallExternalNoErrorCode("ML_Init_Amatrix",ML_Init_Amatrix(ml_object,0,m,m,PetscMLdata));
  PetscCallExternalNoErrorCode("ML_Set_Amatrix_Getrow",ML_Set_Amatrix_Getrow(ml_object,0,PetscML_getrow,PetscML_comm,nlocal_allcols));
  PetscCallExternalNoErrorCode("ML_Set_Amatrix_Matvec",ML_Set_Amatrix_Matvec(ml_object,0,PetscML_matvec));

  PetscCallExternalNoErrorCode("ML_Set_Symmetrize",ML_Set_Symmetrize(ml_object,pc_ml->Symmetrize ? ML_YES : ML_NO));

  /* aggregation */
  PetscCallExternalNoErrorCode("ML_Aggregate_Create",ML_Aggregate_Create(&agg_object));
...

  {
    ...
    case PCML_NULLSPACE_USER: {
      ...
      PetscCallExternalNoErrorCode("ML_Aggregate_Create",PetscCall(ML_Aggregate_Set_NullSpace(agg_object,bs,nvec+!!has_const,nullvec,mlocal)));
      PetscCall(PetscFree(nullvec));
    } break;
    case PCML_NULLSPACE_BLOCK:
      PetscCallExternalNoErrorCode("ML_Aggregate_Set_NullSpace",PetscCall(ML_Aggregate_Set_NullSpace(agg_object,bs,bs,0,0)));
      break;
  ...
  PetscCallExternalNoErrorCode("ML_Aggregate_Set_MaxCoarseSize",ML_Aggregate_Set_MaxCoarseSize(agg_object,pc_ml->MaxCoarseSize));
  /* set options */
  switch (pc_ml->CoarsenScheme) {
  case 1:
    PetscCallExternalNoErrorCode("ML_Aggregate_Set_CoarsenScheme_Coupled",ML_Aggregate_Set_CoarsenScheme_Coupled(agg_object));break;
  case 2:
    PetscCallExternalNoErrorCode("ML_Aggregate_Set_CoarsenScheme_MIS",ML_Aggregate_Set_CoarsenScheme_MIS(agg_object));break;
  case 3:
    PetscCallExternalNoErrorCode("ML_Aggregate_Set_CoarsenScheme_METIS",ML_Aggregate_Set_CoarsenScheme_METIS(agg_object));break;
  }
  PetscCallExternalNoErrorCode("ML_Aggregate_Set_Threshold",ML_Aggregate_Set_Threshold(agg_object,pc_ml->Threshold));
  PetscCallExternalNoErrorCode("ML_Aggregate_Set_DampingFactor",ML_Aggregate_Set_DampingFactor(agg_object,pc_ml->DampingFactor));
  if (pc_ml->SpectralNormScheme_Anorm) {
    PetscCallExternalNoErrorCode("ML_Set_SpectralNormScheme_Anorm",ML_Set_SpectralNormScheme_Anorm(ml_object));
  }
  ...
  if (pc_ml->dim) {
    ...
    PetscCallExternalNoErrorCode("ML_Aggregate_VizAndStats_Setup(",ML_Aggregate_VizAndStats_Setup(ml_object)); /* create ml info for coords */
  ...
  if (pc_ml->Repartition) {
    PetscCallExternalNoErrorCode("ML_Repartition_Activate",ML_Repartition_Activate(ml_object));
    PetscCallExternalNoErrorCode("ML_Repartition_Set_LargestMinMaxRatio",ML_Repartition_Set_LargestMinMaxRatio(ml_object,pc_ml->MaxMinRatio));
    PetscCallExternalNoErrorCode("ML_Repartition_Set_MinPerProc",ML_Repartition_Set_MinPerProc(ml_object,pc_ml->MinPerProc));
    PetscCallExternalNoErrorCode("ML_Repartition_Set_PutOnSingleProc",ML_Repartition_Set_PutOnSingleProc(ml_object,pc_ml->PutOnSingleProc));
    ...
    PetscCallExternalNoErrorCode("ML_Repartition_Set_StartLevel",ML_Repartition_Set_StartLevel(ml_object,1));
#endif
    ...
    if (!pc_ml->RepartitionType) {
      ...
      PetscCallExternalNoErrorCode("ML_Repartition_Set_Partitioner",ML_Repartition_Set_Partitioner(ml_object,ML_USEZOLTAN));
      PetscCallExternalNoErrorCode("ML_Aggregate_Set_Dimensions",ML_Aggregate_Set_Dimensions(agg_object, pc_ml->dim));
      ...
      PetscCallExternalNoErrorCode("ML_Repartition_Set_Partitioner",ML_Repartition_Set_Partitioner(ml_object,ML_USEPARMETIS));
    }
  }

  if (pc_ml->OldHierarchy) {
    PetscCallExternalNoErrorCode("ML_Gen_MGHierarchy_UsingAggregation",Nlevels = ML_Gen_MGHierarchy_UsingAggregation(ml_object,0,ML_INCREASING,agg_object));
  } else {
    PetscCallExternalNoErrorCode("ML_Gen_MultiLevelHierarchy_UsingAggregation",Nlevels = ML_Gen_MultiLevelHierarchy_UsingAggregation(ml_object,0,ML_INCREASING,agg_object));
  }
...
PetscErrorCode PCSetFromOptions_ML(PetscOptionItems *PetscOptionsObject,PC pc)
{
  ...
  PetscCallExternalNoErrorCode("ML_Set_PrintLevel",ML_Set_PrintLevel(PrintLevel));
  ...
  */
  ...
  if (pc_ml->EnergyMinimization) {
    PetscCall(PetscOptionsReal("-pc_ml_EnergyMinimizationDropTol","Energy minimization drop tolerance","None",pc_ml->EnergyMinimizationDropTol,&pc_ml->EnergyMinimizationDropTol,NULL));
  }
