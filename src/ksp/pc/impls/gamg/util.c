...
PetscErrorCode PCGAMGCreateGraph(Mat Amat, Mat *a_Gmat)
{
  ...
  /* A solution consists in providing a new API, MatAIJGetCollapsedAIJ, and each class can provide a fast
     implementation */
  CHKERRQ(MatViewFromOptions(Amat, NULL, "-g_mat_view"));
  if (bs > 1 && (isseqaij || ((Mat_MPIAIJ*)Amat->data)->garray)) {
    PetscInt  *d_nnz, *o_nnz;
    Mat       a, b, c;
    MatScalar *aa,val,AA[4096];
    PetscInt  *aj,*ai,AJ[4096],nc;
    CHKERRQ(PetscInfo(Amat,"New bs>1 PCGAMGCreateGraph. nloc=%D\n",nloc));
    if (isseqaij) {
      a = Amat; b = NULL;
    }
    else {
    ...
    for (c=a, kk=0 ; c && kk<2 ; c=b, kk++){
      ...
      for (PetscInt brow=0,jj,ok=1,j0; brow < nloc*bs; brow += bs) { // block rows
        CHKERRQ(MatGetRow(c,brow,&jj,&cols,NULL));
        nnz[brow/bs] = jj/bs;
        if (jj%bs) ok = 0;
        if (cols) j0 = cols[0];
        else j0 = -1;
        CHKERRQ(MatRestoreRow(c,brow,&jj,&cols,NULL));
        if (nnz[brow/bs]>nmax) nmax = nnz[brow/bs];
        for (PetscInt ii=1; ii < bs && nnz[brow/bs] ; ii++) { // check for non-dense blocks
          CHKERRQ(MatGetRow(c,brow+ii,&jj,&cols,NULL));
          if (jj%bs) ok = 0;
          if (j0 != cols[0]) ok = 0;
          if (nnz[brow/bs] != jj/bs) ok = 0;
          CHKERRQ(MatRestoreRow(c,brow+11,&jj,&cols,NULL));
        }
        if (!ok) {
    ...
    for (PetscInt brow=0,n,grow; brow < nloc*bs; brow += bs) { // block rows
      ...
      CHKERRQ(MatSetValues(Gmat,1,&grow,n/bs,AJ,AA,INSERT_VALUES));
    }
    // off-diag
    if (ismpiaij) {
      ...
      for (PetscInt brow=0,grow; brow < nloc*bs; brow += bs) { // block rows
        CHKERRQ(MatGetRow(b,brow,&ncols,&cols,NULL));
        for (int k=0,cidx=0; k<ncols; k += bs,cidx++) {
          AA[k/bs] = 0;
          AJ[cidx] = garray[cols[k]]/bs;
        }
        nc = ncols/bs;
        CHKERRQ(MatRestoreRow(b,brow,&ncols,&cols,NULL));
        for (int ii=0; ii<bs; ii++) { // rows in block
          CHKERRQ(MatGetRow(b,brow+ii,&ncols,&cols,&vals));
          for (int k=0; k<ncols; k += bs) {
            for (int jj=0; jj<bs; jj++) { // cols in block
              AA[k/bs] += PetscAbs(PetscRealPart(vals[k+jj]));
            }
          }
          CHKERRQ(MatRestoreRow(b,brow+ii,&ncols,&cols,&vals));
        }
        grow = Istart/bs + brow/bs;
        CHKERRQ(MatSetValues(Gmat,1,&grow,nc,AJ,AA,INSERT_VALUES));
      }
    }
    CHKERRQ(MatAssemblyBegin(Gmat,MAT_FINAL_ASSEMBLY));
    CHKERRQ(MatAssemblyEnd(Gmat,MAT_FINAL_ASSEMBLY));
    CHKERRQ(MatViewFromOptions(Gmat, NULL, "-g_mat_view"));
  } else if (bs > 1) {
    const PetscScalar *vals;
    const PetscInt    *idx;
...
old_bs:
    ...
    */
    ...
    CHKERRQ(PetscInfo(Amat,"OLD bs>1 PCGAMGCreateGraph\n"));
    CHKERRQ(PetscObjectBaseTypeCompare((PetscObject)Amat,MATSEQAIJ,&isseqaij));
    CHKERRQ(PetscObjectBaseTypeCompare((PetscObject)Amat,MATMPIAIJ,&ismpiaij));
    CHKERRQ(PetscMalloc2(nloc, &d_nnz,isseqaij ? 0 : nloc, &o_nnz));
...
PetscErrorCode PCGAMGFilterGraph(Mat *a_Gmat,PetscReal vfilter,PetscBool symm)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscLogEventBegin(petsc_gamg_setup_events[SET16],0,0,0,0));
  ...
  if (vfilter < 0.0 && !symm) {
    ...
    CHKERRQ(PetscLogEventEnd(petsc_gamg_setup_events[SET16],0,0,0,0));
    PetscFunctionReturn(0);
  }
