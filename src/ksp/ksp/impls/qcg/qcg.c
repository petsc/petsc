  ...
  for (i=0; i<=maxit; i++) {
    ...
    if (ptasp <= dzero) {
      ...
      /* Scaled negative curvature direction:  Compute a step so that
       ...
       if (!i) {
         ierr = PetscVerboseInfo((ksp,"KSPSolve_QCG: negative curvature: delta=%g\n",pcgP->delta));CHKERRQ(ierr);
       } else {
         ierr = PetscVerboseInfo((ksp,"KSPSolve_QCG: negative curvature: step1=%g, step2=%g, delta=%g\n",step1,step2,pcgP->delta));CHKERRQ(ierr);
       }
       ...
       if (pcgP->ltsnrm > pcgP->delta) {
         ...
         if (!i) {
           ierr = PetscVerboseInfo((ksp,"KSPSolve_QCG: constrained step: delta=%g\n",pcgP->delta));CHKERRQ(ierr);
         } else {
           ierr = PetscVerboseInfo((ksp,"KSPSolve_QCG: constrained step: step1=%g, step2=%g, delta=%g\n",step1,step2,pcgP->delta));CHKERRQ(ierr);
         }
...
#if defined(PETSC_USE_COMPLEX)               
           ierr = PetscVerboseInfo((ksp,"KSPSolve_QCG: truncated step: step=%g, rnrm=%g, delta=%g\n",PetscRealPart(step),rnrm,pcgP->delta));CHKERRQ(ierr);
#else
