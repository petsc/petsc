...
static PetscErrorCode  KSPSolve_PIPECR(KSP ksp)
{
  ...
  if (!ksp->guess_zero) {
    CHKERRQ(KSP_MatMult(ksp,Amat,X,W));            /*     w <- b - Ax     */
    CHKERRQ(VecAYPX(W,-1.0,B));
  } else {
    CHKERRQ(VecCopy(B,W));                         /*     w <- b (x is 0) */
  }
  CHKERRQ(KSP_PCApply(ksp,W,U));                   /*     u <- Bw   */
  ...
  do {
    ...
    CHKERRQ(VecDotBegin(W,U,&gamma));
    CHKERRQ(VecDotBegin(M,W,&delta));
    CHKERRQ(PetscCommSplitReductionBegin(PetscObjectComm((PetscObject)U)));
    ...
    CHKERRQ(VecDotEnd(W,U,&gamma));
    CHKERRQ(VecDotEnd(M,W,&delta));
