...
PetscErrorCode KSPAGMRESRoddec(KSP ksp, PetscInt nvec)
{
  ...
  for (d = 0;d < nvec;d++) {
    ...
    if (rank == First) {
      ...
      ierr = MPI_Recv(& (wbufptr[d]), len, MPIU_SCALAR, rank - 1, tag , comm , &status);
...
PetscErrorCode KSPAGMRESRodvec(KSP ksp, PetscInt nvec, PetscScalar *In, Vec Out)
{
  ...
  if (agmres->size == 1) {
    BLAScopy_(&nvec, y, &pas, &(zloc[0]), &pas);
  } else {
    for (d = nvec - 1; d >= 0;d--) {
      if (rank == First) {
        ierr = MPI_Recv(&(zloc[d]), 1, MPIU_SCALAR, Iright, tag , comm , &status);
      } else {
        ...
        if (rank == Last) {
          ...
          y[d]    =      c * zp + s * zq;
          zloc[0] =   -s * zp + c * zq;
          ierr = MPI_Send(& (y[d]), 1, MPIU_SCALAR, Ileft, tag , comm);
        } else {
          ierr = MPI_Recv(&yd, 1, MPIU_SCALAR, Iright, tag , comm , &status);
          zp = yd;
          zq = zloc[0];
          yd       =      c * zp + s * zq;
          zloc[0] =   -s * zp + c * zq;
          ierr = MPI_Send(&yd, 1, MPIU_SCALAR, Ileft, tag , comm);
        }
      }
    }
  }
