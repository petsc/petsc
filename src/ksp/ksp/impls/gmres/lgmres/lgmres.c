  ...
  *GRS(0) = res_norm;
  ...
  while (!ksp->reason && loc_it < it_total && ksp->its < max_it) { /* LGMRES_MOD: changed to it_total */
    ...
    *HES(loc_it+1,loc_it) = tt;
    ...
    if (hapend) {
      if (!ksp->reason) {
        PetscAssertFalse(ksp->errorifnotconverged,PetscObjectComm((PetscObject)ksp),PETSC_ERR_NOT_CONVERGED,"You reached the happy break down, but convergence was not indicated. Residual norm = %g",(double)res);
        else {
  ...
  if (!ksp->reason && ksp->its < max_it && aug_dim > 0) {
    ...
    /* first do H+*y */
    avec = lgmres->hwork;
    ierr = PetscArrayzero(avec,it_total+1);CHKERRQ(ierr);
...
PetscErrorCode KSPSolve_LGMRES(KSP ksp)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(ksp->calc_sings && !lgmres->Rsvd,PetscObjectComm((PetscObject)ksp),PETSC_ERR_ORDER,"Must call KSPSetComputeSingularValues() before KSPSetUp() is called");
...
static PetscErrorCode KSPLGMRESBuildSoln(PetscScalar *nrs,Vec vguess,Vec vdest,KSP ksp,PetscInt it)
{
  ...
  PetscAssertFalse(*HH(it,it) == 0.0,PETSC_COMM_SELF,PETSC_ERR_CONV_FAILED,"HH(it,it) is identically zero; it = %D GRS(it) = %g",it,(double)PetscAbsScalar(*GRS(it)));
  if (*HH(it,it) != 0.0) {
...
PetscErrorCode KSPView_LGMRES(KSP ksp,PetscViewer viewer)
{
  ...
  if (iascii) {
    /*LGMRES_MOD */
    ierr = PetscViewerASCIIPrintf(viewer,"  aug. dimension=%D\n",lgmres->aug_dim);CHKERRQ(ierr);
    if (lgmres->approx_constant) {
      ierr = PetscViewerASCIIPrintf(viewer,"  approx. space size was kept constant.\n");CHKERRQ(ierr);
    }
...
static PetscErrorCode  KSPLGMRESSetAugDim_LGMRES(KSP ksp,PetscInt aug_dim)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(aug_dim < 0,PetscObjectComm((PetscObject)ksp),PETSC_ERR_ARG_OUTOFRANGE,"Augmentation dimension must be positive");
  PetscAssertFalse(aug_dim > (lgmres->max_k -1),PetscObjectComm((PetscObject)ksp),PETSC_ERR_ARG_OUTOFRANGE,"Augmentation dimension must be <= (restart size-1)");
