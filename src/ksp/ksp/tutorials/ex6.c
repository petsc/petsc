...
int main(int argc, char **args)
{
  ...
  for (i = rstart; i < rend; i++) {
    ...
    col[1] = i;
    col[2] = i + 1;
    if (i == 0) PetscCall(MatSetValues(A, 1, &i, 2, col + 1, value + 1, INSERT_VALUES));
    else if (i == N - 1) PetscCall(MatSetValues(A, 1, &i, 2, col, value, INSERT_VALUES));
    else PetscCall(MatSetValues(A, 1, &i, 3, col, value, INSERT_VALUES));
  }
