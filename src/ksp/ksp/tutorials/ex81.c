...
int main(int argc,char **args)
{
  ...
  if (flg) {
    PetscAssertFalse(found == 1,PETSC_COMM_WORLD,PETSC_ERR_USER,"Must supply more than one field");
    j = 0;
    for (i=0; i<found; ++i) j += outer[i];
    PetscAssertFalse(j != Q,PETSC_COMM_WORLD,PETSC_ERR_USER,"Sum of outer fieldsplit sizes (%D) greater than number of blocks in MatNest (%D)",j,Q);
  }
  ...
  if (flg) {
    ...
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatDense != MatNest");
    ...
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatDense != MatAIJ");
    ...
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatAIJ != MatNest");
    ...
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatHermitianTranspose != MatAIJ");
