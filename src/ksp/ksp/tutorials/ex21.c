...
static PetscScalar RBF(PetscInt sdim, PetscReal x[], PetscReal y[], PetscCtx ctx)
{
  ...
  for (PetscInt d = 0; d < sdim; d++) diff += (x[d] - y[d]) * (x[d] - y[d]) / (l[d] * l[d]);
...
int main(int argc, char **args)
{
  ...
  /* Populate dense matrix for comparisons */
  {
    ...
    for (PetscInt i = 0; i < n; i++) {
      for (PetscInt j = 0; j < n; j++) PetscCall(MatSetValue(Ad, i, j, RBF(sdim, coords + i * sdim, coords + j * sdim, &fctx), INSERT_VALUES));
    }
