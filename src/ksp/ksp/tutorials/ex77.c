...
int main(int argc,char **args)
{
  ...
  PetscAssertFalse(!flg,PETSC_COMM_WORLD,PETSC_ERR_SUP,"Must provide a binary file for the matrix");
  ...
  if (!flg) {
    if (!breakdown) {
      ...
      if (flg) {
        ...
        PetscAssertFalse(norm > 100*PETSC_MACHINE_EPSILON,PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"KSPMatSolve() and MatMatSolve() difference has nonzero norm %g",(double)norm);
      }
    } else {
      ierr = MatZeroEntries(B);CHKERRQ(ierr);
      ierr = KSPMatSolve(ksp,B,X);CHKERRQ(ierr);
      ierr = KSPGetConvergedReason(ksp,&reason);CHKERRQ(ierr);
      PetscAssertFalse(reason != KSP_CONVERGED_HAPPY_BREAKDOWN,PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"KSPConvergedReason() %s != KSP_CONVERGED_HAPPY_BREAKDOWN",KSPConvergedReasons[reason]);
      ierr = MatDenseGetArrayWrite(B,&x);CHKERRQ(ierr);
      for (i=0; i<m*N; ++i) x[i] = 1.0;
      ierr = MatDenseRestoreArrayWrite(B,&x);CHKERRQ(ierr);
      ierr = KSPMatSolve(ksp,B,X);CHKERRQ(ierr);
      ierr = KSPGetConvergedReason(ksp,&reason);CHKERRQ(ierr);
      PetscAssertFalse(reason != KSP_DIVERGED_BREAKDOWN && deflation < 0.0,PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"KSPConvergedReason() %s != KSP_DIVERGED_BREAKDOWN",KSPConvergedReasons[reason]);
    }
  } else {
