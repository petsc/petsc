...
int main(int argc,char **args)
{
  ...
  for (i=0; i<3; i++) {
    ...
    if (!flg) SETERRQ(PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"KSPType and KSPHPDDMType do not match: %s != %s", common[i], type);
    ierr = KSPSetFromOptions(ksp);CHKERRQ(ierr);
    ierr = KSPHPDDMGetType(ksp,&type);CHKERRQ(ierr);
    if (type != KSP_HPDDM_TYPE_GCRODR) SETERRQ(PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"-ksp_hpddm_type gcrodr and KSPHPDDMType do not match: gcrodr != %s", KSPHPDDMTypes[type]);
