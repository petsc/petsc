...
int main(int argc,char **args)
{
  ...
  for (i=0; i<3; i++) {
    ...
    PetscAssertFalse(!flg,PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"KSPType and KSPHPDDMType do not match: %s != %s", common[i], type);
    ierr = KSPSetFromOptions(ksp);CHKERRQ(ierr);
    ierr = KSPHPDDMGetType(ksp,&type);CHKERRQ(ierr);
    PetscAssertFalse(type != KSP_HPDDM_TYPE_GCRODR,PetscObjectComm((PetscObject)ksp),PETSC_ERR_PLIB,"-ksp_hpddm_type gcrodr and KSPHPDDMType do not match: gcrodr != %s", KSPHPDDMTypes[type]);
