...
static PetscErrorCode KSPGuessFormGuess_POD(KSPGuess guess, Vec b, Vec x)
{
  ...
  PetscCall(MPIU_Allreduce(pod->swork, pod->swork + pod->n, (PetscMPIInt)pod->n, MPIU_SCALAR, MPIU_SUM, PetscObjectComm((PetscObject)guess)));
...
#if !defined(PETSC_HAVE_MPI_NONBLOCKING_COLLECTIVES)
      PetscCall(MPIU_Allreduce(pod->swork, pod->swork + 3 * pod->n, (PetscMPIInt)(3 * pod->n), MPIU_SCALAR, MPIU_SUM, PetscObjectComm((PetscObject)guess)));
#else
...
#if !defined(PETSC_HAVE_MPI_NONBLOCKING_COLLECTIVES)
      PetscCall(MPIU_Allreduce(pod->swork, pod->swork + 3 * pod->n, (PetscMPIInt)(2 * pod->n), MPIU_SCALAR, MPIU_SUM, PetscObjectComm((PetscObject)guess)));
...
-  -ksp_guess_pod_Ainner <true> - Use the operator as inner product (must be SPD)
  ...
  Note:
  The initial guess is obtained by solving a small and dense linear system, obtained by Galerkin projection on a lower dimensional space generated by the previous solutions as presented in {cite}`volkwein2013proper`.

.seealso: [](ch_ksp), `KSPGuess`, `KSPGuessType`, `KSPGuessCreate()`, `KSPSetGuess()`, `KSPGetGuess()`
M*/
