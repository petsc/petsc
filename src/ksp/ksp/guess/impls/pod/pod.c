...
#else
      PetscCallMPI(MPI_Iallreduce(pod->swork, pod->dots_iallreduce, 2 * podn, MPIU_SCALAR, MPIU_SUM, PetscObjectComm((PetscObject)guess), &pod->req_iallreduce));
...
- -ksp_guess_pod_Ainner (true|false)  - Use the operator as inner product (must be SPD)
  ...
  Notes:
  See `KSPGuessSetFromOptions()` for the full set of options database keys for controlling the initial guess generator.

  The initial guess is obtained by solving a small and dense linear system, obtained by Galerkin projection on a lower dimensional space generated by the previous
  solutions as presented in {cite}`volkwein2013proper`.
