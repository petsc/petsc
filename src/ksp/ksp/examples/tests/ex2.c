...
We have petsc4py-3.3, petsc-3.3-p4, MUMPS 4.10.0_p3 from
Petsc. We force some partial pivoting to occur thanks to a pivot
threshold value equal to 0.99, and run the attached python code
(modified from yours) with:

mpirun -np 2 python ./testsolvetranspose.py -mat_mumps_icntl_4 2
-mat_mumps_icntl_3 6 -mat_mumps_icntl_2 6 -mat_mumps_icntl_1 6
-mat_mumps_icntl_7 6 -mat_mumps_cntl_1 .99 
...
int main(int argc,char **args)
{
  ...
  if (!rank) printf("solution of C*u = b\n");
  ...
  if (!rank) printf("solution of C^T*u = b\n");
  ierr = VecView(u,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
