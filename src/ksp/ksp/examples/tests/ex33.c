...
int main(int argc,char **args)
{
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = PetscOptionsGetString(NULL,"-fA",file[0],PETSC_MAX_PATH_LEN,&loadA);CHKERRQ(ierr);
  if (loadA) {
    ...
    ierr = PetscOptionsGetString(NULL,"-fB",file[1],PETSC_MAX_PATH_LEN,&loadB);CHKERRQ(ierr);
    if (loadB) {
  ...
  ierr = PetscOptionsGetScalar(NULL,"-sigma",&sigma,&flag);CHKERRQ(ierr);
  if (flag) {
    sigma = -1.0 * sigma;
