...
PetscErrorCode DMSNESCheckJacobian(SNES snes, DM dm, Vec u, PetscReal tol)
{
  ...
  /* Taylor test */
  {
    ...
    if (tol >= 0) {
      if (!isLinear && PetscAbsReal(2.0 - slope) > tol) SETERRQ1(comm, PETSC_ERR_ARG_WRONG, "Taylor approximation convergence rate should be 2, not %.2g", (double) slope);
    } else {
      if (!isLinear) {ierr = PetscPrintf(comm, "Taylor approximation converging at order %.2g\n", (double) slope);CHKERRQ(ierr);}
