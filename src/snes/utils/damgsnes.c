  ...
  if (jacobian == DMMGComputeJacobianWithFD) {
    ...
    for (i=0; i<nlevels; i++) {
      if (dmmg[0]->getcoloringfrommat) {
        ...
        ierr = DMGetColoring(dmmg[i]->dm,dmmg[0]->isctype,MATAIJ,&iscoloring);CHKERRQ(ierr);
      }
    ...
    for (i=0; i<nlevels; i++) {
      ISColoring iscoloring;
      ierr = DMGetColoring(dmmg[i]->dm,IS_COLORING_GHOSTED,MATAIJ,&iscoloring);CHKERRQ(ierr);
