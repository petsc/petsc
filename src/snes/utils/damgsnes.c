...
@*/
int DAMGSetSNES(DAMG *ctx,SNES snes,int (*computejacobian)(void*,DA,Vec,Mat*,Mat*,MatStructure*),void *user)
{
  ...
  ierr = DAMGSetSLES(ctx,sles,PETSC_NULL,PETSC_NULL);CHKERRQ(ierr);

  /* Create work vectors and matrix for each level */
  for (i=0; i<nlevels; i++) {
    ierr = DACreateLocalVector(ctx[i]->da,&ctx[i]->localX);CHKERRA(ierr);
    ierr = DACreateLocalVector(ctx[i]->da,&ctx[i]->localF);CHKERRA(ierr);
    if (!ctx[i]->user)            ctx[i]->user = user;
    if (!ctx[i]->computejacobian) ctx[i]->computejacobian = computejacobian;
  }
  PetscFunctionReturn(0);
}
...
int DAMGComputeJacobianJacobian(SNES snes,Vec X,Mat *J,Mat *B,MatStructure *flag,void *ptr)
{
  DAMG       *ctx = (DAMG*)ptr;
  int        ierr,i,nlevels = ctx[0]->nlevels;
  SLES       sles;
  PC         pc;
  DAMG       finectx = ctx[nlevels];
  PetscTruth ismg;

  PetscFunctionBegin;
  ierr = (*ctx[nlevels-1]->computejacobian)(ctx[nlevels-1]->user,ctx[nlevels-1]->da,X,J,B,flag);CHKERRQ(ierr);
  ...
  if (ismg) {
    ...
    ierr = SLESSetOperators(ctx[nlevels-1]->sles,ctx[nlevels-1]->J,ctx[nlevels-1]->J,*flag);CHKERRA(ierr);

    for (i=nlevels-1; i>0; i--) {

      /* restrict X to coarse grid */
      ierr = MatRestrict(ctx[i]->R,X,ctx[i-1]->x);CHKERRQ(ierr);
      X    = ctx[i-1]->x;      

      /* scale to "natural" scaling for that grid */
      ierr = VecPointwiseMult(ctx[i]->Rscale,X,X);CHKERRQ(ierr);

      /* form Jacobian on coarse grid */
      ierr = (*ctx[i]->computejacobian)(ctx[i]->user,ctx[i]->da,X,&ctx[i]->J,&ctx[i]->B,flag);CHKERRQ(ierr);

      ierr = SLESSetOperators(ctx[i-1]->sles,ctx[i-1]->J,ctx[i-1]->B,*flag);CHKERRA(ierr);
    }
  }
  PetscFunctionReturn(0);
}
