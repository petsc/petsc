...
!           boundary points
        if (i == 1 .or. j == 1 .or. i == mx .or. j == my) then
          PetscCallA(MatSetValues(jac_prec, 1_PETSC_INT_KIND, row, 1_PETSC_INT_KIND, row, [one], INSERT_VALUES, ierr))
!           interior grid points
          ...
          col(4) = row(1) + 1
          col(5) = row(1) + mx
          PetscCallA(MatSetValues(jac_prec, 1_PETSC_INT_KIND, row, 5_PETSC_INT_KIND, col, v, INSERT_VALUES, ierr))
        end if
      end do
    end do
...
!  techniques for preallocating matrix memory.
  ...
  if (.not. matrix_free) then
    PetscCallA(MatCreateSeqAIJ(PETSC_COMM_WORLD, N, N, 5_PETSC_INT_KIND, PETSC_NULL_INTEGER_ARRAY, J, ierr))
  end if
...
!
!   test:
!      args: -snes_monitor -nox -snes_type newtontr -ksp_orthogonalization_cgs_refinement_type refine_always
!
!   test:
!      suffix: 2
!      args: -snes_monitor -nox -snes_fd -ksp_orthogonalization_cgs_refinement_type refine_always
!
!   test:
!      suffix: 3
!      args: -snes_monitor -nox -snes_fd_coloring -mat_coloring_type sl -ksp_orthogonalization_cgs_refinement_type refine_always
!      filter: sort -b
