...
static PetscErrorCode SNESSolve_QN(SNES snes)
{
  ...
  if (snes->npc && snes->npcside== PC_LEFT && snes->functype == SNES_FUNCTION_UNPRECONDITIONED) {
    CHKERRQ(SNESApplyNPC(snes,X,F,D));
    CHKERRQ(SNESGetConvergedReason(snes->npc,&reason));
    if (reason < 0  && reason != SNES_DIVERGED_MAX_IT) {
      snes->reason = SNES_DIVERGED_INNER;
      PetscFunctionReturn(0);
  ...
  if (snes->npc && snes->npcside== PC_RIGHT) {
    CHKERRQ(PetscLogEventBegin(SNES_NPCSolve,snes->npc,X,0,0));
    CHKERRQ(SNESSolve(snes->npc,snes->vec_rhs,X));
    CHKERRQ(PetscLogEventEnd(SNES_NPCSolve,snes->npc,X,0,0));
    CHKERRQ(SNESGetConvergedReason(snes->npc,&reason));
    if (reason < 0 && reason != SNES_DIVERGED_MAX_IT) {
      snes->reason = SNES_DIVERGED_INNER;
      PetscFunctionReturn(0);
  ...
  if (qn->scale_type == SNES_QN_SCALE_JACOBIAN) {
    ...
    CHKERRQ(MatLMVMSetJ0KSP(qn->B, snes->ksp));
  }

  for (i = 0, i_r = 0; i < snes->max_its; i++, i_r++) {
    /* update QN approx and calculate step */
    CHKERRQ(MatLMVMUpdate(qn->B, X, D));
    CHKERRQ(MatSolve(qn->B, D, Y));

    /* line search for lambda */
    ynorm = 1; gnorm = fnorm;
    CHKERRQ(VecCopy(D, Dold));
    CHKERRQ(VecCopy(X, Xold));
    ...
    CHKERRQ(SNESSetIterationNumber(snes, i+1));
    snes->norm = fnorm;
    snes->xnorm = xnorm;
    snes->ynorm = ynorm;
    ...
    if (qn->restart_type == SNES_QN_RESTART_POWELL && i_r > 1) {
      ...
      if (qn->scale_type == SNES_QN_SCALE_JACOBIAN) {
        CHKERRQ(MatMult(snes->jacobian_pre,Dold,W));
      } else {
        CHKERRQ(VecCopy(Dold,W));
      }
      CHKERRQ(VecDotBegin(W, Dold, &DolddotDold));
      CHKERRQ(VecDotBegin(W, D, &DolddotD));
      CHKERRQ(VecDotEnd(W, Dold, &DolddotDold));
      CHKERRQ(VecDotEnd(W, D, &DolddotD));
    ...
    if (badstep || powell || periodic) {
      if (qn->monflg) {
        CHKERRQ(PetscViewerASCIIAddTab(qn->monitor,((PetscObject)snes)->tablevel+2));
        if (powell) {
          CHKERRQ(PetscViewerASCIIPrintf(qn->monitor, "Powell restart! |%14.12e| > %6.4f*|%14.12e| i_r = %D\n", (double)PetscRealPart(DolddotD), (double)qn->powell_gamma, (double)PetscRealPart(DolddotDold),i_r));
        } else {
          CHKERRQ(PetscViewerASCIIPrintf(qn->monitor, "Periodic restart! i_r = %D\n", i_r));
        }
        CHKERRQ(PetscViewerASCIISubtractTab(qn->monitor,((PetscObject)snes)->tablevel+2));
      }
      i_r = -1;
      if (qn->scale_type == SNES_QN_SCALE_JACOBIAN) {
        CHKERRQ(SNESComputeJacobian(snes,X,snes->jacobian,snes->jacobian_pre));
      ...
      CHKERRQ(MatLMVMReset(qn->B, PETSC_FALSE));
    }
  }
  if (i == snes->max_its) {
...
static PetscErrorCode SNESSetUp_QN(SNES snes)
{
  ...
  switch (qn->type) {
    case SNES_QN_BROYDEN:
      CHKERRQ(MatSetType(qn->B, MATLMVMBROYDEN));
      qn->scale_type = SNES_QN_SCALE_NONE;
      break;
    case SNES_QN_BADBROYDEN:
      CHKERRQ(MatSetType(qn->B, MATLMVMBADBROYDEN));
      qn->scale_type = SNES_QN_SCALE_NONE;
      break;
    default:
      CHKERRQ(MatSetType(qn->B, MATLMVMBFGS));
      switch (qn->scale_type) {
        case SNES_QN_SCALE_NONE:
          CHKERRQ(MatLMVMSymBroydenSetScaleType(qn->B, MAT_LMVM_SYMBROYDEN_SCALE_NONE));
          break;
        case SNES_QN_SCALE_SCALAR:
          CHKERRQ(MatLMVMSymBroydenSetScaleType(qn->B, MAT_LMVM_SYMBROYDEN_SCALE_SCALAR));
          break;
        case SNES_QN_SCALE_JACOBIAN:
          CHKERRQ(MatLMVMSymBroydenSetScaleType(qn->B, MAT_LMVM_SYMBROYDEN_SCALE_USER));
          break;
  ...
  CHKERRQ(MatSetSizes(qn->B, n, n, N, N));
  CHKERRQ(MatSetUp(qn->B));
  CHKERRQ(MatLMVMReset(qn->B, PETSC_TRUE));
  CHKERRQ(MatLMVMSetHistorySize(qn->B, qn->m));
  CHKERRQ(MatLMVMAllocate(qn->B, snes->vec_sol, snes->vec_func));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode SNESDestroy_QN(SNES snes)
{
  ...
  CHKERRQ(PetscObjectComposeFunction((PetscObject)snes,"",NULL));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode SNESSetFromOptions_QN(PetscOptionItems *PetscOptionsObject,SNES snes)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscOptionsHead(PetscOptionsObject,"SNES QN options"));
  CHKERRQ(PetscOptionsInt("-snes_qn_m","Number of past states saved for L-BFGS methods","SNESQN",qn->m,&qn->m,NULL));
  CHKERRQ(PetscOptionsReal("-snes_qn_powell_gamma","Powell angle tolerance",          "SNESQN", qn->powell_gamma, &qn->powell_gamma, NULL));
  CHKERRQ(PetscOptionsBool("-snes_qn_monitor",         "Monitor for the QN methods",      "SNESQN", qn->monflg, &qn->monflg, NULL));
  CHKERRQ(PetscOptionsEnum("-snes_qn_scale_type","Scaling type","SNESQNSetScaleType",SNESQNScaleTypes,(PetscEnum)stype,(PetscEnum*)&stype,&flg));
  if (flg) CHKERRQ(SNESQNSetScaleType(snes,stype));

  CHKERRQ(PetscOptionsEnum("-snes_qn_restart_type","Restart type","SNESQNSetRestartType",SNESQNRestartTypes,(PetscEnum)rtype,(PetscEnum*)&rtype,&flg));
  if (flg) CHKERRQ(SNESQNSetRestartType(snes,rtype));

  CHKERRQ(PetscOptionsEnum("-snes_qn_type","Quasi-Newton update type","",SNESQNTypes,(PetscEnum)qtype,(PetscEnum*)&qtype,&flg));
  if (flg) CHKERRQ(SNESQNSetType(snes,qtype));
  CHKERRQ(MatSetFromOptions(qn->B));
  ...
  if (qn->monflg) {
    CHKERRQ(PetscViewerASCIIGetStdout(PetscObjectComm((PetscObject)snes), &qn->monitor));
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode SNESView_QN(SNES snes, PetscViewer viewer)
{
  ...
  if (iascii) {
    CHKERRQ(PetscViewerASCIIPrintf(viewer,"  type is %s, restart type is %s, scale type is %s\n",SNESQNTypes[qn->type],SNESQNRestartTypes[qn->restart_type],SNESQNScaleTypes[qn->scale_type]));
    CHKERRQ(PetscViewerASCIIPrintf(viewer,"  Stored subspace size: %D\n", qn->m));
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode SNESQNSetRestartType(SNES snes, SNESQNRestartType rtype)
{
  ...
  CHKERRQ(PetscTryMethod(snes,"SNESQNSetRestartType_C",(SNES,SNESQNRestartType),(snes,rtype)));
  PetscFunctionReturn(0);
}
...
PetscErrorCode SNESQNSetScaleType(SNES snes, SNESQNScaleType stype)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(snes,SNES_CLASSID,1);
  CHKERRQ(PetscTryMethod(snes,"SNESQNSetScaleType_C",(SNES,SNESQNScaleType),(snes,stype)));
  PetscFunctionReturn(0);
}
...
PetscErrorCode SNESQNSetType(SNES snes, SNESQNType qtype)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(snes,SNES_CLASSID,1);
  CHKERRQ(PetscTryMethod(snes,"SNESQNSetType_C",(SNES,SNESQNType),(snes,qtype)));
  PetscFunctionReturn(0);
}
...
PETSC_EXTERN PetscErrorCode SNESCreate_QN(SNES snes)
{
  ...
  CHKERRQ(MatCreate(PetscObjectComm((PetscObject)snes), &qn->B));
  CHKERRQ(SNESGetOptionsPrefix(snes, &optionsprefix));
  CHKERRQ(MatSetOptionsPrefix(qn->B, optionsprefix));

  CHKERRQ(PetscObjectComposeFunction((PetscObject)snes,"SNESQNSetScaleType_C",SNESQNSetScaleType_QN));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)snes,"SNESQNSetRestartType_C",SNESQNSetRestartType_QN));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)snes,"SNESQNSetType_C",SNESQNSetType_QN));
  PetscFunctionReturn(0);
}
