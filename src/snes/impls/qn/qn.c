...
static PetscErrorCode SNESSolve_QN(SNES snes)
{
  ...
  Vec *dF = qn->dF;
  PetscScalar *rho = qn->rho;
  PetscScalar FolddotF, FolddotFold, FdotF, YdotF, a;
  ...
  for(i = 0, i_r = 0; i < snes->max_its; i++, i_r++) {
    ...
    ierr = VecDot(Fold, Fold, &FolddotFold);CHKERRQ(ierr);
    ierr = VecDot(Fold, F, &FolddotF);CHKERRQ(ierr);
    ierr = VecDot(F, F, &FdotF);CHKERRQ(ierr);
    ierr = VecDot(Y, F, &YdotF);CHKERRQ(ierr);
    if (PetscAbs(PetscRealPart(FolddotF)) > qn->powell_gamma*PetscAbs(PetscRealPart(FolddotFold))) {
      if (qn->monitor) {
        ierr = PetscViewerASCIIAddTab(qn->monitor,((PetscObject)snes)->tablevel+2);CHKERRQ(ierr);
      ...
      ierr = LBGFSApplyJinv_Private(snes, i_r+1, F, Y);CHKERRQ(ierr);
    }
}
  if (i == snes->max_its) {
