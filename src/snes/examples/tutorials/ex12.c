...
PetscErrorCode SetupDiscretization(DM dm, AppCtx *user)
{
  ...
  while (cdm) {
    ...
    ierr = DMPlexAddBoundary(cdm, user->bcType == DIRICHLET ? PETSC_TRUE : PETSC_FALSE, "wall", user->bcType == NEUMANN ? "boundary" : "marker", 0, user->exactFuncs[0], 1, &id, user);CHKERRQ(ierr);
