...
/*TEST
  ...
  test:
    ...
    requires: ctetgen mumps suitesparse chaco
    filter: grep -v "CG or CGNE: variant" | sed -e "s/linear solver iterations=10[0-9]/linear solver iterations=100/g" | sed -e "s/linear solver iterations=9[0-9]/linear solver iterations=100/g"
    nsize: 5
