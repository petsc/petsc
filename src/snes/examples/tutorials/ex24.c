/*$Id: ex23.c,v 1.1 2000/12/20 17:10:55 bsmith Exp bsmith $*/
...
int main(int argc,char **argv)
{
  ...
  ierr = OptionsSetValue("-mg_coarse_ksp_max_it","3");CHKERRQ(ierr);
  ...
  ierr = ViewerDrawOpen(PETSC_COMM_WORLD,0,"fu - discretization of function",-1,-1,-1,-1,&user.fu_viewer);CHKERRQ(ierr);
  ...
  ierr = ViewerDestroy(user.u_viewer);CHKERRQ(ierr);
  ierr = ViewerDestroy(user.fu_viewer);CHKERRQ(ierr);
