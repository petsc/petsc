...
PetscErrorCode MatMult_MFFD(Mat mat,Vec a,Vec y)
{
  ...
  */
  ...
  ierr = PetscInfo((mat,"MatMult_MFFD:Current differencing parameter: %G + %G i\n",PetscRealPart(h),PetscImaginaryPart(h)));CHKERRQ(ierr);
#else
  ierr = PetscInfo((mat,"MatMult_MFFD:Current differencing parameter: %15.12e\n",h));CHKERRQ(ierr);
#endif
...
PetscErrorCode PETSCSNES_DLLEXPORT MatSNESMFCheckPositivity(Vec U,Vec a,PetscScalar *h,void *dummy)
{
  ...
  if (val <= PetscAbsScalar(*h)) {
    ierr = PetscInfo((U,"MatSNESMFCheckPositivity: Scaling back h from %G to %G\n",PetscRealPart(*h),.99*val));CHKERRQ(ierr);
