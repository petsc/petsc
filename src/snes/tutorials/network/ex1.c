...
PetscErrorCode FormFunction(SNES snes,Vec X,Vec F,void *appctx)
{
  ...
  for (v=0; v<nv; v++) {
    ...
    for (k=0; k<ncomp; k++) {
      ...
      switch (k) {
      case 0:
        PetscAssertFalse(key != appctx_power.compkey_bus || nvar != 2,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"key %D not a power bus vertex or nvar %D != 2",key,nvar);
        break;
      case 1:
        PetscAssertFalse(key != appctx_power.compkey_load || nvar != 0 || goffset[1] != goffset[0]+2,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Not a power load vertex");
        break;
      case 2:
        PetscAssertFalse(key != appctx_water.compkey_vtx || nvar != 1 || goffset[2] != goffset[1],PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Not a water vertex");
        break;
    ...
    for (k=0; k<nconnedges; k++) {
      ...
      if (keye == appctx_water.compkey_edge) { /* water_compkey_edge */
        ...
        PetscAssertFalse(keye != appctx_power.compkey_branch,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Not a power branch");
      }
    }
  }
