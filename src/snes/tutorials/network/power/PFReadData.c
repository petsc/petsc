...
PetscErrorCode PFReadMatPowerData(PFDATA *pf,char *filename)
{
  ...
  PetscAssertFalse(!fp,PETSC_COMM_SELF,PETSC_ERR_FILE_OPEN,"Can't open Matpower data file %s",filename);
  pf->nload=0;
  ...
  for (i=0;i<line_counter;i++) {
    PetscAssertFalse(!fgets(line,MAXLINE,fp),PETSC_COMM_SELF,PETSC_ERR_SUP,"File is incorrectly formatted");
    ...
    if ((i >= bus_start_line) && (i < bus_end_line)) {
      ...
      if (!((Pd == 0.0) && (Qd == 0.0))) {
        ...
        PetscAssertFalse(Bus[busi].nload > NLOAD_AT_BUS_MAX,PETSC_COMM_SELF,PETSC_ERR_SUP,"Exceeded maximum number of loads allowed at bus");
        loadi++;
      }
      busi++;
    }
    ...
    if (i >= gen_start_line && i < gen_end_line) {
      ...
      PetscAssertFalse(Bus[intbusnum].ngen > NGEN_AT_BUS_MAX,PETSC_COMM_SELF,PETSC_ERR_SUP,"Exceeded maximum number of generators allowed at bus");
      geni++;
    }
