...
static PetscErrorCode THIDAGetPrm(DM da,PrmNode ***prm)
{
  ...
  PetscAssertFalse(!da2prm,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"No DMDA2Prm composed with given DMDA");
  ierr = PetscObjectQuery((PetscObject)da,"DMDA2Prm_Vec",(PetscObject*)&X);CHKERRQ(ierr);
  PetscAssertFalse(!X,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"No DMDA2Prm_Vec composed with given DMDA");
...
static PetscErrorCode THISurfaceStatistics(DM da,Vec X,PetscReal *min,PetscReal *max,PetscReal *mean)
{
  ...
  *min = *max = *mean = 0;
  ...
  ierr = DMDAGetCorners(da,&zs,&ys,&xs,&zm,&ym,&xm);CHKERRQ(ierr);
  PetscAssertFalse(zs != 0 || zm != mz,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Unexpected decomposition");
  ierr = DMDAVecGetArray(da,X,&x);CHKERRQ(ierr);
  for (i=xs; i<xs+xm; i++) {
    for (j=ys; j<ys+ym; j++) {
...
static PetscErrorCode DMRefineHierarchy_THI(DM dac0,PetscInt nlevels,DM hierarchy[])
{
  ...
  PetscAssertFalse(!thi,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Cannot refine this DMDA, missing composed THI instance");
  if (nlevels > 1) {
  ...
  PetscAssertFalse(dim != 2,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"This function can only refine 2D DMDAs");
...
static PetscErrorCode DMCreateInterpolation_DA_THI(DM dac,DM daf,Mat *A,Vec *scale)
{
  ...
  if (dim  == 2) {
    ...
    PetscAssertFalse(zs,PETSC_COMM_SELF,PETSC_ERR_PLIB,"unexpected");
...
static PetscErrorCode DMCreateMatrix_THI_Tridiagonal(DM da,Mat *J)
{
  ...
  PetscAssertFalse(dim != 3,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Expected DMDA to be 3D");
...
static PetscErrorCode THIDAVecView_VTK_XML(THI thi,DM da,Vec X,const char filename[])
{
  ...
  if (rank == 0) {
    ...
    for (r=0; r<size; r++) {
      ...
      PetscAssertFalse(xm*ym*zm*dof > nmax,PETSC_COMM_SELF,PETSC_ERR_PLIB,"should not happen");
      if (r) {
        ...
        PetscAssertFalse(nn != xm*ym*zm*dof,PETSC_COMM_SELF,PETSC_ERR_PLIB,"should not happen");
        ptr = array;
      } else ptr = x;
