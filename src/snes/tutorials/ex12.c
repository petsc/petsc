...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  PetscBool      rand = PETSC_FALSE, flg;
  PetscErrorCode ierr;
  ...
  ierr = PetscOptionsBool("-k_random", "Assign random k values to checkerboard", "ex12.c", rand, &rand, NULL);CHKERRQ(ierr);
  ...
  if (rand) {
    PetscRandom r;
    PetscReal   val;
    PetscInt    N = PetscPowInt(options->div, options->dim), i;

    ierr = PetscMalloc1(N, &options->kgrid);CHKERRQ(ierr);
    ierr = PetscRandomCreate(PETSC_COMM_SELF, &r);CHKERRQ(ierr);
    ierr = PetscRandomSetFromOptions(r);CHKERRQ(ierr);
    ierr = PetscRandomSetInterval(r, 0.0, options->k);CHKERRQ(ierr);
    ...
    for (i = 0; i < N; ++i) {
      ...
      options->kgrid[i] = 1 + (PetscInt) val;
    }
...
/*TEST
  ...
  testset:
    nsize: 4
    requires: hpddm slepc !single define(PETSC_HAVE_DYNAMIC_LIBRARIES) defined(PETSC_USE_SHARED_LIBRARIES)
  ...
  testset:
    nsize: 4
    requires: hpddm slepc triangle !single define(PETSC_HAVE_DYNAMIC_LIBRARIES) defined(PETSC_USE_SHARED_LIBRARIES)
  ...
  testset:
    nsize: 4
    requires: hpddm slepc parmetis !single define(PETSC_HAVE_DYNAMIC_LIBRARIES) defined(PETSC_USE_SHARED_LIBRARIES)
  ...
  test:
    ...
    args: -dm_refine_hierarchy 3 -cells 4,4 -interpolate -bc_type dirichlet -petscspace_degree 1 \
