...
PetscErrorCode ComputeFunction(SNES, Vec x, Vec f, PetscCtx ctx)
{
...
PetscErrorCode ComputeJacobian(SNES, Vec x, Mat J, Mat, PetscCtx ctx)
{
  DM          da = (DM)ctx;
  PetscInt    i, Mx, xm, xs;
  PetscScalar hx, *xx;
...
/*TEST
   ...
   testset:
      args: -snes_monitor -dm_mat_type aijcusparse -dm_vec_type cuda
