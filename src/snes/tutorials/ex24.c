...
{
  f0[0] = 0.0;
  for (PetscInt d = 0; d < dim; ++d) f0[0] += 2.0 * x[d] * (1.0 - x[d]);
  f0_u(dim, Nf, NfAux, uOff, uOff_x, u, u_t, u_x, aOff, aOff_x, a, a_t, a_x, t, x, numConstants, constants, f0);
}
...
{
  for (PetscInt c = 0; c < dim; ++c) f1[c * dim + c] = u[uOff[1]];
}
...
/*TEST
  ...
  test:
    nsize: 4
    suffix: 2d_bdm1_p0_bddc_multilevel
    requires: triangle !single
    args: -sol_type linear \
          -field_petscspace_degree 1 -field_petscdualspace_type bdm -dm_refine 1 \
          -dmsnes_check .001 -snes_error_if_not_converged \
          ...
          -pc_type bddc -pc_bddc_use_local_mat_graph 0 \
          -pc_bddc_benign_trick -pc_bddc_nonetflux -pc_bddc_detect_disconnected -pc_bddc_use_qr_single \
          -pc_bddc_levels 1 -pc_bddc_coarsening_ratio {{1 2}} -pc_bddc_coarse_eqs_limit 0 \
          -pc_bddc_neumann_pc_type svd -pc_bddc_dirichlet_pc_type svd \
          -pc_bddc_neumann_l1_pc_type svd -pc_bddc_dirichlet_l1_pc_type svd
