...
PetscErrorCode SNESDiffParameterCreate_More(SNES snes,Vec x,void **outneP)
{
  ...
  /* Create work vectors */
  CHKERRQ(VecDuplicateVecs(x,3,&neP->workv));
  w    = neP->workv[0];

  /* Set components of vector w to random numbers */
  CHKERRQ(PetscRandomCreate(PetscObjectComm((PetscObject)snes),&rctx));
  CHKERRQ(PetscRandomSetFromOptions(rctx));
  CHKERRQ(VecSetRandom(w,rctx));
  CHKERRQ(PetscRandomDestroy(&rctx));

  /* Open output file */
  CHKERRQ(PetscOptionsGetString(((PetscObject)snes)->options,((PetscObject)snes)->prefix,"-snes_mf_noise_file",noise_file,sizeof(noise_file),&flg));
  if (flg) neP->fp = fopen(noise_file,"w");
  else     neP->fp = fopen("noise.out","w");
  PetscCheckFalse(!neP->fp,PETSC_COMM_SELF,PETSC_ERR_FILE_OPEN,"Cannot open file");
  CHKERRQ(PetscInfo(snes,"Creating Jorge's differencing parameter context\n"));
...
PetscErrorCode SNESDiffParameterCompute_More(SNES snes,void *nePv,Vec x,Vec p,double *fnoise,double *hopt)
{
  ...
  if (!snes->setupcalled) CHKERRQ(SNESSetUp(snes));
  ...
  CHKERRQ(SNESGetIterationNumber(snes,&i));
  ...
  for (iter=0; iter<5; iter++) {
    ...
    for (k=0; k<nf; k++) {
      alpha = h * (k+1 - (nf+1)/2);
      CHKERRQ(VecWAXPY(xp,alpha,p,x));
      CHKERRQ(SNESComputeFunction(snes,xp,fvec));
      neP->function_count++;
      CHKERRQ(VecDot(fvec,w,&fval[k]));
    }
    f = fval[(nf+1)/2 - 1];
    ...
    CHKERRQ(PetscFPrintf(comm,fp,"Difference Table: iter = %D\n",iter));
    for (i=0; i<nf; i++) {
      for (j=0; j<nf-i; j++) {
        CHKERRQ(PetscFPrintf(comm,fp," %10.2e ",tab[i][j]));
      }
      CHKERRQ(PetscFPrintf(comm,fp,"\n"));
    }

    /* Call the noise estimator */
    CHKERRQ(SNESNoise_dnest_(&nf,fval,&h,fnoise,&fder2,hopt,&info,eps));

    /* Output statements */
    rerrf = *fnoise/PetscAbsScalar(f);
    if (info == 1) CHKERRQ(PetscFPrintf(comm,fp,"%s\n","Noise detected"));
    if (info == 2) {CHKERRQ(PetscFPrintf(comm,fp,"%s\n","Noise not detected; h is too small"));}
    if (info == 3) {CHKERRQ(PetscFPrintf(comm,fp,"%s\n","Noise not detected; h is too large"));}
    if (info == 4) CHKERRQ(PetscFPrintf(comm,fp,"%s\n","Noise detected, but unreliable hopt"));
    CHKERRQ(PetscFPrintf(comm,fp,"Approximate epsfcn %g  %g  %g  %g  %g  %g\n",(double)eps[0],(double)eps[1],(double)eps[2],(double)eps[3],(double)eps[4],(double)eps[5]));
    CHKERRQ(PetscFPrintf(comm,fp,"h = %g, fnoise = %g, fder2 = %g, rerrf = %g, hopt = %g\n\n",(double)h, (double)*fnoise, (double)fder2, (double)rerrf, (double)*hopt));
...
theend:
  ...
  if (*fnoise < neP->fnoise_min) {
    CHKERRQ(PetscFPrintf(comm,fp,"Resetting fnoise: fnoise1 = %g, fnoise_min = %g\n",(double)*fnoise,(double)neP->fnoise_min));
    *fnoise = neP->fnoise_min;
    neP->fnoise_resets++;
  }
  if (*hopt < neP->hopt_min) {
    CHKERRQ(PetscFPrintf(comm,fp,"Resetting hopt: hopt1 = %g, hopt_min = %g\n",(double)*hopt,(double)neP->hopt_min));
    *hopt = neP->hopt_min;
    neP->hopt_resets++;
  }

  CHKERRQ(PetscFPrintf(comm,fp,"Errors in derivative:\n"));
  CHKERRQ(PetscFPrintf(comm,fp,"f = %g, fnoise = %g, fder2 = %g, hopt = %g\n",(double)f,(double)*fnoise,(double)fder2,(double)*hopt));

  /* For now, compute h **each** MV Mult!! */
  /*
  CHKERRQ(PetscOptionsHasName(NULL,"-matrix_free_jorge_each_mvp",&flg));
  if (!flg) {
    Mat mat;
    CHKERRQ(SNESGetJacobian(snes,&mat,NULL,NULL));
    CHKERRQ(SNESDefaultMatrixFreeSetParameters2(mat,PETSC_DEFAULT,PETSC_DEFAULT,*hopt));
  }
  */
  fcount = neP->function_count - fcount;
  CHKERRQ(PetscInfo(snes,"fct_now = %D, fct_cum = %D, rerrf=%g, sqrt(noise)=%g, h_more=%g\n",fcount,neP->function_count,(double)rerrf,(double)PetscSqrtReal(*fnoise),(double)*hopt));

  CHKERRQ(PetscOptionsGetBool(NULL,NULL,"-noise_test",&noise_test,NULL));
  if (noise_test) {
    CHKERRQ(JacMatMultCompare(snes,x,p,*hopt));
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode JacMatMultCompare(SNES snes,Vec x,Vec p,double hopt)
{
  ...
  CHKERRQ(SNESGetJacobian(snes,&Jmf,&J,NULL,NULL));
  CHKERRQ(SNESComputeJacobian(snes,x,Jmf,J));
  CHKERRQ(SNESGetFunction(snes,&f,NULL,NULL));
  CHKERRQ(SNESComputeFunction(snes,x,f));

  /* Duplicate work vectors */
  CHKERRQ(VecDuplicate(x,&yy2));
  CHKERRQ(VecDuplicate(x,&yy1));

  /* Compute true matrix-vector product */
  CHKERRQ(MatMult(J,p,yy1));
  CHKERRQ(VecNorm(yy1,NORM_2,&yy1n));

  /* View product vector if desired */
  CHKERRQ(PetscOptionsGetBool(NULL,NULL,"-print_vecs",&printv,NULL));
  if (printv) {
    CHKERRQ(PetscViewerASCIIOpen(comm,"y1.out",&view2));
    CHKERRQ(PetscViewerPushFormat(view2,PETSC_VIEWER_ASCII_COMMON));
    CHKERRQ(VecView(yy1,view2));
    CHKERRQ(PetscViewerPopFormat(view2));
    CHKERRQ(PetscViewerDestroy(&view2));
  }
  ...
  for (i=0; i<5; i++) {
    ...
    CHKERRQ(SNESDefaultMatrixFreeSetParameters2(Jmf,PETSC_DEFAULT,PETSC_DEFAULT,h));

    /* Compute matrix-vector product via differencing approximation */
    CHKERRQ(MatMult(Jmf,p,yy2));
    CHKERRQ(VecNorm(yy2,NORM_2,&yy2n));

    /* View product vector if desired */
    if (printv) {
      sprintf(filename,"y2.%d.out",(int)i);
      CHKERRQ(PetscViewerASCIIOpen(comm,filename,&view2));
      CHKERRQ(PetscViewerPushFormat(view2,PETSC_VIEWER_ASCII_COMMON));
      CHKERRQ(VecView(yy2,view2));
      CHKERRQ(PetscViewerPopFormat(view2));
      CHKERRQ(PetscViewerDestroy(&view2));
    }

    /* Compute relative error */
    CHKERRQ(VecAXPY(yy2,alpha,yy1));
    CHKERRQ(VecNorm(yy2,NORM_2,&enorm));
    enorm = enorm/yy1n;
...
PetscErrorCode SNESNoiseMonitor(SNES snes,PetscInt its,double fnorm,void *dummy)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(SNESGetLinearSolveIterations(snes,&lin_its));
  ...
  CHKERRQ(SNESUnSetMatrixFreeParameter(snes));
  PetscFunctionReturn(0);
}
