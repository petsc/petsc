...
PetscErrorCode  PetscFOpen(MPI_Comm comm,const char name[],const char mode[],FILE **fp)
{
  ...
  if (rank == 0) {
    ...
    if (isstdout || !name) fd = PETSC_STDOUT;
      ...
      fd   = fopen(fname,mode);
      if (!fd) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_OPEN,"Unable to open file %s",fname);
    }
  } else fd = NULL;
  *fp = fd;
...
PetscErrorCode  PetscPOpen(MPI_Comm comm,const char machine[],const char program[],const char mode[],FILE **fp)
{
  ...
  if (rank == 0) {
    ...
    if (!(fd = popen(commandt,mode))) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"Cannot run command %s",commandt);
    if (fp) *fp = fd;
  }
  PetscFunctionReturn(0);
}
