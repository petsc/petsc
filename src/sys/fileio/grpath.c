  ...
  while (N) {
    ...
    if (n > 0) {
      ...
      if (tmp3[0] != '/') {
        ...
        ierr    = PetscStrncat(tmp4,"/",PETSC_MAX_PATH_LEN);CHKERRQ(ierr);
        ierr    = PetscStrncat(tmp4,tmp3,PETSC_MAX_PATH_LEN);CHKERRQ(ierr);
        ierr    = PetscGetRealPath(tmp4,rpath);CHKERRQ(ierr);
        ierr    = PetscStrncat(rpath,path+N,PETSC_MAX_PATH_LEN);CHKERRQ(ierr);
      } else {
