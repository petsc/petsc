...
#undef __FUNCT__
#define __FUNCT__ "PetscLogNestedEnd"
...
#undef __FUNCT__
#define __FUNCT__ "PetscLogEventBeginNested"
static PetscErrorCode PetscLogEventBeginNested(NestedEventId nstEvent, int t, PetscObject o1, PetscObject o2, PetscObject o3, PetscObject o4)
{
  PetscErrorCode  ierr;
...
/* End a nested event */
#undef __FUNCT__
#define __FUNCT__ "PetscLogEventEndNested"
static PetscErrorCode PetscLogEventEndNested(NestedEventId nstEvent, int t, PetscObject o1, PetscObject o2, PetscObject o3, PetscObject o4)
{
  PetscErrorCode  ierr;
 ...
 */
#undef __FUNCT__
#define __FUNCT__ "PetscLogSetThreshold"
PetscErrorCode PetscLogSetThreshold(const PetscLogDouble newThresh, PetscLogDouble *oldThresh)
{
  PetscFunctionBegin;
...
#undef __FUNCT__
#define __FUNCT__ "PetscPrintExeSpecs"
 ...
 */
#undef __FUNCT__
#define __FUNCT__ "PetscPrintXMLGlobalPerformanceElement"
 ...
 */
#undef __FUNCT__
#define __FUNCT__ "PetscPrintGlobalPerformance"
...
static PetscErrorCode  PetscCreateLogTreeNested(PetscViewer viewer, PetscNestedEventTree **p_tree, unsigned long long *p_nTimers)
{
  ...
  eventInfo = stageLog->eventLog->eventInfo;
  ...
  ierr = MPI_Allreduce(&maxDefaultTimer, &j, 1, MPI_INT, MPI_MAX, comm);CHKERRQ(ierr);
  maxDefaultTimer = j;
  ...
  depth = maxdepth;
  ierr = MPI_Allreduce(&depth, &maxdepth, 1, MPI_INT, MPI_MAX, comm);CHKERRQ(ierr);
  ...
  i = illegalEvent;
  ierr = MPI_Allreduce(&i, &illegalEvent, 1, MPI_INT, MPI_MAX, comm);CHKERRQ(ierr);
  ...
  for (yesno=0; yesno<=1; yesno++) {
    ...
    while (depth>0) {
      ...
      if (depth>0) {
        ...
        same = PETSC_TRUE;     
        for (j=0; same && j<=depth; j++) { same = (PetscBool) (same &&  nstMyPath[j] == nstPath[j]);}
...
static PetscErrorCode  PetscLogFreeNestedTree(PetscNestedEventTree *tree, const unsigned long long nTimers)
{
 ...
 */
#undef __FUNCT__
#define __FUNCT__ "PetscPrintXMLNestedLinePerfResults"
...
static PetscErrorCode  PetscLogPrintNestedLine(PetscViewer viewer,PetscEventPerfInfo perfInfo,PetscLogDouble countsPerCall,int parentCount,size_t depth,const char *name,PetscLogDouble totalTime,PetscBool *isPrinted)
{
  ...
  commarr_in[0] =  time; 
  commarr_in[1] = -time; 
  ierr = MPI_Allreduce(commarr_in, commarr_out,    2, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
  timeMx =  commarr_out[0];
  timeMn = -commarr_out[1];

  commarr_in[0] = time>0.0 ?  perfInfo.flops/time         : 0;
  commarr_in[1] = time>0.0 ?  perfInfo.numReductions/time : 0;
  commarr_in[2] = time>0.0 ?  perfInfo.messageLength/time : 0;
  commarr_in[3] = parentCount>0    ?  countsPerCall      : 0;

  commarr_in[4] = time>0.0 ? -perfInfo.flops/time         : -1e30;
  commarr_in[5] = time>0.0 ? -perfInfo.numReductions/time : -1e30;
  commarr_in[6] = time>0.0 ? -perfInfo.messageLength/time : -1e30;
  commarr_in[7] = parentCount>0    ? -countsPerCall      : -1e30;

  ierr = MPI_Allreduce(commarr_in, commarr_out,  n_comm, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
...
static PetscErrorCode  PetscLogNestedPrint(PetscViewer viewer, PetscNestedEventTree *tree,unsigned long long nTimers, const unsigned long long iStart, PetscLogDouble totalTime)
{
  size_t             depth   = tree[iStart].depth;
  const char         *name;
  unsigned long long nChildren;
  ...
  if (nChildren>0) {
    ...
    ierr = MPI_Allreduce(times, maxTimes, nChildren, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
  ...
  if (nChildren>0) {
    ...
    ierr = MPI_Allreduce(times, maxTimes, 2, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
...
static PetscErrorCode  PetscLogNestedPrintTop(PetscViewer viewer, PetscNestedEventTree *tree,const unsigned long long nTimers, PetscLogDouble totalTime)
{
  unsigned long long nChildren;
  PetscSortItem      *children;
  PetscErrorCode     ierr;
...
static PetscErrorCode  PetscCalcSelfTime(PetscViewer viewer, PetscSelfTimer **p_self, unsigned long long *p_nstMax)
{
  PetscErrorCode     ierr;
  PetscEventPerfInfo *eventPerfInfo;
  PetscEventRegInfo  *eventRegInfo; 
  ...
  unsigned long long i, maxDefaultTimer;
  NestedEventId      nst;
  PetscLogEvent      dft;
  ...
  ierr = MPI_Allreduce(&nstMax_local, &nstMax, 1, MPI_INT, MPI_MAX, comm);CHKERRQ(ierr);
...
static PetscErrorCode  PetscPrintSelfTime(PetscViewer viewer, const PetscSelfTimer *selftimes, const unsigned long long nstMax, PetscLogDouble totalTime) 
{
  PetscErrorCode     ierr;
  unsigned long long i;
  NestedEventId      nst;
  ...
  ierr = MPI_Allreduce(times, maxTimes, nstMax+1, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
  ierr = PetscFree(times);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "PetscLogView_Nested"
PetscErrorCode  PetscLogView_Nested(PetscViewer viewer)
{
  ...
  unsigned long long   nTimers, nstMax;
  ...
  ierr = MPI_Allreduce(&locTotalTime, &globTotalTime, 1, MPIU_PETSCLOGDOUBLE, MPI_MAX, comm);CHKERRQ(ierr);
