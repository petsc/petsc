  ...
  */
  if (PetscLogMalloc > -1 && PetscLogMalloc < PetscLogMallocMax && a >= PetscLogMallocThreshold) {
    if (!PetscLogMalloc) {
      ...
      PetscAssertFalse(!PetscLogMallocLength,PETSC_COMM_SELF,PETSC_ERR_MEM," ");

      PetscLogMallocFile = (const char**)malloc(PetscLogMallocMax*sizeof(char*));
      PetscAssertFalse(!PetscLogMallocFile,PETSC_COMM_SELF,PETSC_ERR_MEM," ");

      PetscLogMallocFunction = (const char**)malloc(PetscLogMallocMax*sizeof(char*));
      PetscAssertFalse(!PetscLogMallocFunction,PETSC_COMM_SELF,PETSC_ERR_MEM," ");
    }
...
PetscErrorCode  PetscTrFreeDefault(void *aa,int lineno,const char function[],const char filename[])
{
  ...
  *nend = ALREADY_FREED;
  ...
  PetscAssertFalse(TRallocated < asize,PETSC_COMM_SELF,PETSC_ERR_MEMC,"TRallocate is smaller than memory just freed");
  TRallocated -= asize;
  TRfrags--;
...
PetscErrorCode  PetscMallocPopMaximumUsage(int event,PetscLogDouble *mu)
{
  ...
  *mu = 0;
  if (NumTRMaxMems-- > MAXTRMAXMEMS) PetscFunctionReturn(0);
  PetscAssertFalse(TRMaxMemsEvents[NumTRMaxMems] != event,PETSC_COMM_SELF,PETSC_ERR_MEMC,"PetscMallocPush/PopMaximumUsage() are not nested");
...
PetscErrorCode  PetscMallocView(FILE *fp)
{
  ...
  PetscAssertFalse(PetscLogMalloc < 0,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"PetscMallocView() called without call to PetscMallocViewSet() this is often due to\n                      setting the option -malloc_view AFTER PetscInitialize() with PetscOptionsInsert() or PetscOptionsInsertFile()");
  ...
  shortcount    = (int*)malloc(PetscLogMalloc*sizeof(int));PetscAssertFalse(!shortcount,PETSC_COMM_SELF,PETSC_ERR_MEM,"Out of memory");
  shortlength   = (size_t*)malloc(PetscLogMalloc*sizeof(size_t));PetscAssertFalse(!shortlength,PETSC_COMM_SELF,PETSC_ERR_MEM,"Out of memory");
  shortfunction = (const char**)malloc(PetscLogMalloc*sizeof(char*));PetscAssertFalse(!shortfunction,PETSC_COMM_SELF,PETSC_ERR_MEM,"Out of memory");
...
foundit:;
  ...
  perm = (PetscInt*)malloc(n*sizeof(PetscInt));PetscAssertFalse(!perm,PETSC_COMM_SELF,PETSC_ERR_MEM,"Out of memory");
...
PetscErrorCode PetscMallocSetDebug(PetscBool eachcall, PetscBool initializenan)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(PetscTrMalloc == PetscTrMallocDefault,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Cannot call this routine more than once, it can only be called in PetscInitialize()");
