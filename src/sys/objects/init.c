...
PETSC_INTERN PetscErrorCode  PetscOptionsCheckInitial_Private(const char help[])
{
  ...
  */
  ...
  if (flg1) {
    PetscCallMPI(MPI_Comm_set_errhandler(comm,MPI_ERRORS_RETURN));
  }
  flg1 = PETSC_FALSE;
  ...
  if (flg1 || flg2) {
    ...
    */
    ...
    if (size > 2) {
      ...
      for (i=0; i<size; i++) {
        if (rank != i) {
          PetscCallMPI(MPI_Send(&dummy,1,MPI_INT,i,109,comm));
        }
      }
      for (i=0; i<size; i++) {
        if (rank != i) {
          PetscCallMPI(MPI_Recv(&dummy,1,MPI_INT,i,109,comm,&status));
        }
      }
    }
