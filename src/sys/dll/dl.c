...
@*/
PetscErrorCode PETSC_DLLEXPORT PetscDLLibraryOpen(MPI_Comm comm,const char libname[],PetscDLHandle *handle)
{
  PetscErrorCode ierr;
  ...
  */
  if (path && path[0] != '\0') {
    ...
    */
    ...
    ierr = PetscDLSym(handle,symbol,value);CHKERRQ(ierr);
    if (!*value) {
