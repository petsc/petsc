...
PETSC_HASH_MAP(HMapFunc, const char *, PetscVoid_Fn *, kh_str_hash_func, kh_str_hash_equal, NULL)
...
static PetscErrorCode PetscHMapFuncInsert_Private(PetscHMapFunc map, const char name[], PetscVoid_Fn *fnc)
{
  PetscHashIter it;
  PetscBool     found;
...
M*/
PetscErrorCode PetscFunctionListAdd_Private(PetscFunctionList *fl, const char name[], PetscVoid_Fn *fptr)
{
  PetscFunctionBegin;
...
\
    ...
    while (!PetscHashIterAtEnd(phmfi_map_, phmfi_iter_)) { \
      ...
      PetscVoid_Fn *PETSC_UNUSED __val_name__; \
\
...
M*/
PetscErrorCode PetscFunctionListFind_Private(PetscFunctionList fl, const char name[], PetscVoid_Fn **fptr)
{
  PetscFunctionBegin;
  PetscAssertPointer(name, 2);
