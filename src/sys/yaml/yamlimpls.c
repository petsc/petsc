...
int options_list_populate_yaml(char *str, options_list_t *options_list)
{
        ...
        if (event.type == YAML_SCALAR_EVENT) {
          ...
          if (strcmp((char*) event.data.scalar.value, "options") == 0
           || strcmp((char*) event.data.scalar.value, "Options") == 0) {
            ...
            for (i--; i>=0; i--) yaml_event_delete(&events[i]);
          }
        } else {
          for (i--; i>=0; i--) yaml_event_delete(&events[i]);
        }
    ...
    && events[i+1].type == YAML_SCALAR_EVENT) {
      ...
      && events[i+3].type == YAML_MAPPING_END_EVENT) {
        ...
        for (k=0; k<grouping_stack.count; k++) {
          if (grouping_stack.groups[k].name) ii+=strlen(grouping_stack.groups[k].name);
        }
        ...
        (*options_list).options[j].arguments.args    = (char**) calloc((*options_list).options[j].arguments.count+1, sizeof(char*));
        (*options_list).options[j].arguments.args[0] = (char*) calloc(events[i+2].data.scalar.length+1, sizeof(char));
        ...
        if (events[i+3].type == YAML_SCALAR_EVENT) {
          ...
          while (sequence_stack != 0) {
            ...
            case YAML_SCALAR_EVENT:
              if (sequence_stack == 1) (*options_list).options[j].arguments.count++;
              break;
            default: break;
            }
            k++;
          ...
          for (ii=i+2+1; ii < k; ii++) {
            if (events[ii].type == YAML_SCALAR_EVENT) {
              (*options_list).options[j].arguments.args[ii-i-2-1] = (char*)calloc(events[ii].data.scalar.length+1, sizeof(char));
              strcpy((*options_list).options[j].arguments.args[ii-i-2-1],(char*) events[ii].data.scalar.value);
            }
          }
          j++;
        } else if (events[i+3].type == YAML_MAPPING_START_EVENT) {
          /* We have a group of options coming up. */
          if (grouping_stack.count == 1 && strcmp(grouping_stack.groups[0].name, "default") == 0) grouping_stack.count--;
          ...
          grouping_stack.groups[grouping_stack.count].name = (char*) calloc(events[i+1].data.scalar.length+1, sizeof(char));
  ...
  for (i=0; i<events_length; i++) yaml_event_delete(&events[i]);
...
int yaml_event_initialize(yaml_event_t *out, yaml_event_t *in)
{
  ...
  case YAML_DOCUMENT_START_EVENT:
    if (!yaml_document_start_event_initialize(&(*out),(*in).data.document_start.version_directive,(*in).data.document_start.tag_directives.start,
                                                      (*in).data.document_start.tag_directives.end,(*in).data.document_start.implicit)) {
  ...
  case YAML_ALIAS_EVENT:
    ...
    break;
  case YAML_SCALAR_EVENT:
    if (!yaml_scalar_event_initialize(&(*out),(*in).data.scalar.anchor,(*in).data.scalar.tag,(*in).data.scalar.value, (*in).data.scalar.length,
                                              (*in).data.scalar.plain_implicit,(*in).data.scalar.quoted_implicit, (*in).data.scalar.style)) {
      fprintf(stderr, "error yaml_scalar_event_initialize (%s:%d)\n", __FILE__, __LINE__-1);
      return 0;
    }
    break;
  case YAML_SEQUENCE_START_EVENT:
    if (!yaml_sequence_start_event_initialize(&(*out),(*in).data.sequence_start.anchor,(*in).data.sequence_start.tag,(*in).data.sequence_start.implicit,
  ...
  case YAML_SEQUENCE_END_EVENT:
    ...
    break;
  case YAML_MAPPING_START_EVENT:
    if (!yaml_mapping_start_event_initialize(&(*out),(*in).data.mapping_start.anchor,(*in).data.mapping_start.tag,
                                                     (*in).data.mapping_start.implicit,(*in).data.mapping_start.style)) {
...
int alias_list_populate_yaml(char *str, alias_list_t *list)
{
  ...
  for (i=0; i<events_length; i++) {
    ...
    case YAML_SCALAR_EVENT:
      if (events[i].data.scalar.anchor != NULL) (*list).count++;
      break;
    case YAML_SEQUENCE_START_EVENT:
  ...
  for (i=0; i<events_length; i++) {
    ...
    case YAML_SCALAR_EVENT:
      if (events[i].data.scalar.anchor != NULL) {
        (*list).list[j].alias = (char*) calloc(strlen((char*) events[i].data.scalar.anchor)+1, sizeof(char));
    ...
    case YAML_SEQUENCE_START_EVENT:
      if (events[i].data.sequence_start.anchor != NULL) {
        (*list).list[j].alias = (char*) calloc(strlen((char*) events[i].data.sequence_start.anchor)+1, sizeof(char));
    ...
    case YAML_MAPPING_START_EVENT:
      if (events[i].data.mapping_start.anchor != NULL) {
        (*list).list[j].alias = (char*) calloc(strlen((char*) events[i].data.mapping_start.anchor)+1, sizeof(char));
