...
PetscErrorCode PetscAttachDebugger(void)
{
...
#if !PetscDefined(CANNOT_START_DEBUGGER) && PetscDefined(HAVE_FORK)
  int       child     = 0;
  ...
  */
  #if !PetscDefined(DO_NOT_SWAP_CHILD_FOR_DEBUGGER)
  ...
  if (child) { /* I am the parent, will run the debugger */
    ...
    */
  #if PetscDefined(NEED_KILL_FOR_DEBUGGER)
    kill(child, SIGCONT);
  #endif
    ...
    if (isxxgdb || isups || isddd) {
      ...
      if (isdbx) {
        j = jj;
  #if PetscDefined(USE_P_FOR_DEBUGGER)
        args[j++] = "-p";
        args[j++] = pid;
        args[j++] = program;
  #elif PetscDefined(USE_LARGEP_FOR_DEBUGGER)
        args[j++] = "-l";
        args[j++] = "ALL";
        args[j++] = "-P";
        args[j++] = pid;
        args[j++] = program;
  #elif PetscDefined(USE_A_FOR_DEBUGGER)
        args[j++] = "-a";
        args[j++] = pid;
  #elif PetscDefined(USE_PID_FOR_DEBUGGER)
        args[j++] = "-pid";
        args[j++] = pid;
        args[j++] = program;
  #else
  ...
  #if PetscDefined(NEED_DEBUGGER_NO_SLEEP)
    /*
  ...
  #elif PetscDefined(HAVE_SLEEP_RETURNS_EARLY)
    /*
...
@*/
PetscErrorCode PetscAttachDebuggerErrorHandler(MPI_Comm comm, int line, const char *fun, const char *file, PetscErrorCode num, PetscErrorType p, const char *mess, PetscCtx ctx)
{
  (void)comm;
  (void)num;
  (void)p;
...
PetscErrorCode PetscStopForDebugger(void)
{
  PetscErrorCode ierr;
  PetscReal      sleeptime = 0;
#if !PetscDefined(CANNOT_START_DEBUGGER)
  int         ppid;
  PetscMPIInt rank;
