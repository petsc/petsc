...
PetscErrorCode PetscSegBufferExtractTo(PetscSegBuffer seg,void *contig)
{
  ...
  PetscAssertFalse(ptr != contig,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Tail count does not match");
  s->used             = 0;
  s->tailused         = 0;
  s->tail             = NULL;
  PetscFunctionReturn(0);
...
PetscErrorCode PetscSegBufferUnuse(PetscSegBuffer seg,size_t unused)
{
  ...
  PetscFunctionBegin;
  head = seg->head;
  PetscAssertFalse(head->used < unused,PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Attempt to return more unused entries (%zu) than previously gotten (%zu)",unused,head->used);
  head->used -= unused;
  PetscFunctionReturn(0);
}
