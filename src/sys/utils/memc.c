...
PetscErrorCode PetscMemcmp(const void *str1,const void *str2,size_t len,PetscBool  *e)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(!str1,PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Trying to compare at a null pointer");
  PetscAssertFalse(!str2,PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Trying to compare at a null pointer");
...
PetscErrorCode PetscProcessPlacementView(PetscViewer viewer)
{
  ...
  PetscAssertFalse(!isascii,PetscObjectComm((PetscObject)viewer),PETSC_ERR_SUP,"Only ASCII viewer is supported");
  ...
  PetscAssertFalse(err,PETSC_COMM_SELF,PETSC_ERR_LIB,"Error %d from hwloc_get_proc_cpubind()",err);
