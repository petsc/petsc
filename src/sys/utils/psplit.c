...
PetscErrorCode  PetscSplitOwnershipBlock(MPI_Comm comm,PetscInt bs,PetscInt *n,PetscInt *N)
{
  ...
  if (*N == PETSC_DECIDE) {
    ...
    CHKERRMPI(MPIU_Allreduce(n,N,1,MPIU_INT,MPI_SUM,comm));
...
PetscErrorCode  PetscSplitOwnership(MPI_Comm comm,PetscInt *n,PetscInt *N)
{
  ...
  if (PetscDefined(USE_DEBUG)) {
    ...
    CHKERRMPI(MPI_Comm_size(comm,&size));
    CHKERRMPI(MPIU_Allreduce(l,g,2,MPI_INT,MPI_SUM,comm));
  ...
  if (*N == PETSC_DECIDE) {
    PetscInt64 m = *n, M;
    CHKERRMPI(MPIU_Allreduce(&m,&M,1,MPIU_INT64,MPI_SUM,comm));
    ...
    *n   = *N/size + ((*N % size) > rank);
    ...
    PetscInt tmp;
    CHKERRMPI(MPIU_Allreduce(n,&tmp,1,MPIU_INT,MPI_SUM,comm));
