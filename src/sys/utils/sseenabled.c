...
PetscErrorCode  PetscSSEIsEnabled(MPI_Comm comm,PetscBool  *lflag,PetscBool  *gflag)
{
  ...
  if (petsc_sse_local_is_untested && petsc_sse_global_is_untested) {
    ...
    ierr = PetscOptionsBool("-disable_sse","Disable use of hand tuned Intel SSE implementations <true,false>.","PetscSSEIsEnabled",disabled_option,&disabled_option,NULL);CHKERRQ(ierr);
    if (disabled_option) {
