...
PetscErrorCode PetscGlobalMinMaxInt(MPI_Comm comm, const PetscInt minMaxVal[2], PetscInt minMaxValGlobal[2])
{
  ...
  PetscFunctionBegin;
  buf[0] = hasminint ? PETSC_MIN_INT : -minMaxVal[0]; /* Note that -PETSC_INT_MIN = PETSC_INT_MIN: ternary to suppress sanitizer warnings */
  buf[1] = minMaxVal[1];
  buf[2] = hasminint ? 1 : 0; /* Are there PETSC_INT_MIN in minMaxVal[0]? */
  PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE, buf, 3, MPIU_INT, MPI_MAX, comm));
  minMaxValGlobal[0] = buf[2] ? PETSC_INT_MIN : -buf[0];
  minMaxValGlobal[1] = buf[1];
  PetscFunctionReturn(PETSC_SUCCESS);
}
