/*$Id: dtri.c,v 1.47 2001/01/15 21:43:22 bsmith Exp balay $*/
/*
...
#undef __FUNC__  
#define __FUNC__ "PetscDrawTriangle" 
/*@
...
#undef __FUNC__  
#define __FUNC__ "PetscDrawScalePopup" 
/*@
...
#undef __FUNC__  
#define __FUNC__ "PetscDrawTensorContour_Zoom" 
...
#undef __FUNC__  
#define __FUNC__ "PetscDrawTensorContour" 
/*@C
...
int PetscDrawTensorContour(PetscDraw win,int m,int n,const PetscReal xi[],const PetscReal yi[],Scalar *v)
{
  ...
  ierr = PetscOptionsHasName(PETSC_NULL,"-draw_contour_grid",&ctx.showgrid);CHKERRQ(ierr);
...
#undef __FUNC__  
#define __FUNC__ "PetscDrawTensorContourPatch" 
/*@
...
#else
      x1 = x[i];  y_1 = y[j];  c1 = (int)(PETSC_DRAW_BASIC_COLORS + scale*PetscRealPart(v[i+j*m]-min));
      x2 = x[i+1];y2 = y_1;    c2 = (int)(PETSC_DRAW_BASIC_COLORS + scale*PetscRealPart(v[i+j*m+1]-min));
      x3 = x2;    y3 = y[j+1];c3 = (int)(PETSC_DRAW_BASIC_COLORS + scale*PetscRealPart(v[i+j*m+1+m]-min));
      x4 = x1;    y4 = y3;    c4 = (int)(PETSC_DRAW_BASIC_COLORS + scale*PetscRealPart(v[i+j*m+m]-min));
#endif
