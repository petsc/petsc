...
/* Routines */
#define MPI_Send( buf, count, datatype, dest, tag, comm) MPI_SUCCESS
#define MPI_Recv( buf, count, datatype, source, tag, comm, status) \
                MPI_Abort(MPI_COMM_WORLD,0)
#define MPI_Get_count(status,  datatype, count) MPI_Abort(MPI_COMM_WORLD,0)
#define MPI_Bsend( buf, count, datatype, dest, tag, comm) MPI_SUCCESS
#define MPI_Ssend( buf, count,  datatype, dest, tag, comm) MPI_SUCCESS
#define MPI_Rsend( buf, count,  datatype, dest, tag, comm) MPI_SUCCESS
#define MPI_Buffer_attach( buffer, size)
#define MPI_Buffer_detach( buffer, size) 
#define MPI_Isend( buf, count,  datatype, dest, tag, comm, request) MPI_SUCCESS
...
#define MPI_Irecv( buf, count,  datatype, source, tag, comm, request) \
    MPI_Abort(MPI_COMM_WORLD,0)
#define MPI_Wait(request, status) MPI_SUCCESS
#define MPI_Test(request, flag, status) (*(flag) = 0, MPI_SUCCESS)
#define MPI_Request_free(request) MPI_SUCCESS
#define MPI_Waitany(a, b, c, d) MPI_SUCCESS
#define MPI_Testany(a, b, c, d, e) MPI_SUCCESS
#define MPI_Waitall(count, array_of_requests, array_of_statuses) MPI_SUCCESS
...
#define MPI_Pack_size(incount,  datatype, comm, size) \
    MPI_Abort(MPI_COMM_WORLD,0)
#define MPI_Barrier(comm ) MPI_SUCCESS
#define MPI_Bcast( buffer, count,  datatype, root, comm ) MPI_SUCCESS
...
#define MPI_Comm_compare(comm1, comm2, result) \
    (*(result)=MPI_IDENT,MPI_SUCCESS)
#define MPI_Comm_dup(comm, newcomm) MPI_SUCCESS
...
#define MPI_Get_processor_name(name, result_len) PetscStrcpy(name,"localhost"); \
     *(result_len) =10 
#define MPI_Errhandler_create(function, errhandler) MPI_SUCCESS
#define MPI_Errhandler_set(comm, errhandler) MPI_SUCCESS
