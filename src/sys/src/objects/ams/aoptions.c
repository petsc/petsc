...
int OptionsSelectBegin(MPI_Comm comm)
{
#if defined(PETSC_HAVE_AMS)
  AMS_Comm   acomm;
  int        ierr;
  ...
  ierr = AMS_Memory_create(acomm,"Options",&PetscOptionsamem);CHKERRQ(ierr);
  ierr = AMS_Memory_take_access(PetscOptionsamem);CHKERRQ(ierr); 
#endif
  PetscFunctionReturn(0);
}
...
int OptionsSelectEnd(MPI_Comm comm)
{
  ...
  PetscFunctionBegin;
#if defined(PETSC_HAVE_AMS)
  if (PetscOptionsamem < 0) SETERRQ(1,1,"Called without a call to OptionsSelectBegin()");
  ierr = AMS_Memory_publish(PetscOptionsamem);CHKERRQ(ierr);
  ierr = AMS_Memory_grant_access(PetscOptionsamem);CHKERRQ(ierr);
#endif
  PetscFunctionReturn(0);
}
...
int OptionsSelectInt(char *prefix,char *opt,char *text,int defaultv)
{
#if defined(PETSC_HAVE_AMS)
  int        ierr;
#endif
  ...
  PetscFunctionBegin;
#if defined(PETSC_HAVE_AMS)
  ierr = AMS_Memory_add_field(PetscOptionsamem,text,0,1,AMS_INT,AMS_WRITE,
