...
PetscErrorCode PETSC_DLLEXPORT PetscBagSetFromOptions(PetscBag *bag)
{
  PetscErrorCode ierr;
  PetscBagItem   nitem = bag->bagitems;
  char           name[PETSC_BAG_NAME_LENGTH+1];
  
  PetscFunctionBegin;

  ierr = PetscOptionsBegin(bag->bagcomm,PETSC_NULL,bag->baghelp,bag->bagname);
    while (nitem) {
      name[0] = '-';
      name[1] = 0;
      ierr    = PetscStrcat(name,nitem->name);CHKERRQ(ierr);
      if (nitem->dtype == PETSC_REAL) {
  ...
  if (isascii) {
    ...
    while (nitem) {
      if (nitem->dtype == PETSC_REAL) {
        ...
        PetscTruth value = *(PetscTruth*)(((char*)bag) + nitem->offset);
        ierr = PetscViewerASCIIPrintf(view,"  %s = %s; %s\n",nitem->name,value ? "True":"False",nitem->help);CHKERRQ(ierr);
      }
      nitem = nitem->next;
    }
  } else if (isbinary) {
    ...
    while (nitem) {
      ...
      ierr  = PetscViewerBinaryWrite(view,(((char*)bag) + nitem->offset),1,nitem->dtype,PETSC_FALSE);CHKERRQ(ierr);
      nitem = nitem->next;
    }
  } else {
...
/*@C
   PetscBagLoads - Loads a bag of values from a binary file
...
PetscErrorCode PETSC_DLLEXPORT PetscBagLoad(PetscViewer view,PetscBag **bag)
{
  PetscErrorCode ierr;
  PetscTruth     isbinary;
  PetscInt       cookie,bagsizecount[2],i,offsetdtype[2];
  ...
  for (i=0; i<bagsizecount[1]; i++) {
    ...
    if (offsetdtype[1] == (PetscInt) PETSC_REAL) {
      PetscReal mdefault;
