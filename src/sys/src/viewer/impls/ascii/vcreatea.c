/*$Id: vcreatea.c,v 1.19 2001/03/23 23:19:56 balay Exp bsmith $*/
...
#undef __FUNCT__  
#define __FUNCT__ "VIEWER_STDOUT_"  
/*@C
...
PetscViewer PETSC_VIEWER_STDOUT_(MPI_Comm comm)
{
  ...
  if (Petsc_Viewer_Stdout_keyval == MPI_KEYVAL_INVALID) {
    ...
    if (ierr) {PetscError(__LINE__,"VIEWER_STDOUT_",__FILE__,__SDIR__,1,1,0); PetscFunctionReturn(0);}
  }
...
#undef __FUNCT__  
#define __FUNCT__ "VIEWER_STDERR_" 
/*@C
...
PetscViewer PETSC_VIEWER_STDERR_(MPI_Comm comm)
{
  ...
  if (Petsc_Viewer_Stderr_keyval == MPI_KEYVAL_INVALID) {
    ...
    if (ierr) {PetscError(__LINE__,"VIEWER_STDERR_",__FILE__,__SDIR__,1,1,0); PetscFunctionReturn(0);}
  }
