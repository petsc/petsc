#ifndef lint
static char vcid[] = "$Id: send.c,v 1.45 1996/12/17 18:07:42 balay Exp balay $";
#endif
...
#undef __FUNCTION__  
#define __FUNCTION__ "ViewerDestroy_Matlab"
static int ViewerDestroy_Matlab(PetscObject obj)
{
  ...
  if (setsockopt(viewer->port,SOL_SOCKET,SO_LINGER,(char*)&locallinger,sizeof(Linger))) 
    SETERRQ(1,"System error setting linger");
  if (close(viewer->port)) 
    SETERRQ(1,"System error closing socket");
...
/*--------------------------------------------------------------*/
#undef __FUNCTION__  
#define __FUNCTION__ "SOCKCall_Private"
int SOCKCall_Private(char *hostname,int portnum)
{
  ...
  if ( (hp=gethostbyname(hostname)) == NULL ) {
    ...
    SETERRQ(1,"system error open connection");
  }
  ...
  while (flag) {
    if ( (s=socket(hp->h_addrtype,SOCK_STREAM,0)) < 0 ) {
      perror("SEND: error socket");  SETERRQ(-1,"system error");
    }
    if ( connect(s,(struct sockaddr *)&sa,sizeof(sa)) < 0 ) {
      if ( errno == EALREADY ) {
        ...
        perror(NULL); SETERRQ(-1,"system error");
      }
      flag = 1; close(s);
    } 
    else flag = 0;
...
#undef __FUNCTION__  
#define __FUNCTION__ "ViewerMatlabOpen"
/*@C
...
#undef __FUNCTION__  
#define __FUNCTION__ "ViewerInitializeMatlabWorld_Private"
...
#undef __FUNCTION__  
#define __FUNCTION__ "ViewerDestroyMatlab_Private"
