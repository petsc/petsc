...
int main(int argc, char **argv)
{
  ...
  for (PetscInt i = 0; i < 5; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %g\n", (double)x[i]));
  PetscCall(PetscPrintf(PETSC_COMM_SELF, "---------------\n"));
  PetscCall(PetscSortReal(5, x));
  for (PetscInt i = 0; i < 5; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %g\n", (double)x[i]));

  PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n2nd test\n"));
  for (PetscInt i = 0; i < 10; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %g\n", (double)x[i]));
  PetscCall(PetscPrintf(PETSC_COMM_SELF, "---------------\n"));
  PetscCall(PetscSortReal(10, x));
  for (PetscInt i = 0; i < 10; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %g\n", (double)x[i]));

  PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n3rd test\n"));
  for (PetscInt i = 0; i < 5; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %2" PetscInt_FMT "     %g\n", index2[i], (double)x2[i]));
  PetscCall(PetscPrintf(PETSC_COMM_SELF, "---------------\n"));
  PetscCall(PetscSortRealWithArrayInt(5, x2, index2));
  for (PetscInt i = 0; i < 5; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %2" PetscInt_FMT "     %g\n", index2[i], (double)x2[i]));

  PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n4th test\n"));
  for (PetscInt i = 0; i < 10; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %2" PetscInt_FMT "     %g\n", index3[i], (double)x3[i]));
  PetscCall(PetscPrintf(PETSC_COMM_SELF, "---------------\n"));
