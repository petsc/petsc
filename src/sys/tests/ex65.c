...
static PetscErrorCode TestPetscFunctionListFind(PetscViewer viewer, PetscFunctionList fl, PetscFunctionList fl_dup, size_t *fl_size, size_t *fl_dup_size)
{
  ...
  for (size_t i = 0; i < num_funcs; ++i) {
    PetscErrorCodeFn *func;

    PetscCall(PetscFunctionListAdd(&fl, all_names[i], all_funcs[i]));
    PetscCall(PetscFunctionListFind(fl, all_names[i], &func));
    PetscCheck(func == (PetscErrorCodeFn *)all_funcs[i], PETSC_COMM_SELF, PETSC_ERR_PLIB, "PetscFunctionListFind() failed to find %s() immediately after inserting it! returned %p != expected %p", all_names[i], (void *)(PETSC_UINTPTR_T)func, (void *)(PETSC_UINTPTR_T)all_funcs[i]);
  ...
  for (size_t i = 0; i < num_funcs; ++i) {
    ...
    PetscCall(PetscFunctionListFind(fl, all_names[i], &func));
    PetscCheck(func == (PetscErrorCodeFn *)all_funcs[i], PETSC_COMM_SELF, PETSC_ERR_PLIB, "PetscFunctionListFind() failed to find %s() after inserting all functions! returned %p != expected %p", all_names[i], (void *)(PETSC_UINTPTR_T)func, (void *)(PETSC_UINTPTR_T)all_funcs[i]);
    // make sure the pointer is good
    (void)func();
  }
  ...
  // ensure that none of them are missing
  for (size_t i = 0; i < num_funcs; ++i) {
    PetscErrorCodeFn *fl_func, *fl_dup_func;

    PetscCall(PetscFunctionListFind(fl, all_names[i], &fl_func));
    PetscCheck(fl_func == (PetscErrorCodeFn *)all_funcs[i], PETSC_COMM_SELF, PETSC_ERR_PLIB, "PetscFunctionListFind() failed to find %s() after inserting all functions! returned %p != expected %p", all_names[i], (void *)(PETSC_UINTPTR_T)fl_func, (void *)(PETSC_UINTPTR_T)all_funcs[i]);
    // make sure the pointer is good
    (void)fl_func();
  ...
  // ensure that none of them are missing
  for (size_t i = 0; i < num_funcs; ++i) {
    PetscErrorCodeFn *fl_dup_func;

    PetscCall(PetscFunctionListFind(fl_dup, all_names[i], &fl_dup_func));
    PetscCheck(fl_dup_func == (PetscErrorCodeFn *)all_funcs[i], PETSC_COMM_SELF, PETSC_ERR_PLIB, "PetscFunctionListFind() failed to find %s() in duplicated function list after clearing original list! returned %p != expected %p", all_names[i], (void *)(PETSC_UINTPTR_T)fl_dup_func, (void *)(PETSC_UINTPTR_T)all_funcs[i]);
    (void)fl_dup_func();
  }
  PetscCall(PetscFunctionListView(fl_dup, viewer));
  *fl_size     = 0;
