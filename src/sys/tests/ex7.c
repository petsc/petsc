...
int main(int argc, char **argv)
{
  ...
  for (PetscInt i = 0; i < 5; i++) index[i] = i;
  PetscCall(PetscSortIntWithPermutation(5, x, index));
  for (PetscInt i = 0; i < 5; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %" PetscInt_FMT "     %" PetscInt_FMT "     %" PetscInt_FMT "\n", x[i], index[i], x[index[i]]));

  PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n2nd test\n"));
  for (PetscInt i = 0; i < 8; i++) index2[i] = i;
  PetscCall(PetscSortIntWithPermutation(8, x2, index2));
  for (PetscInt i = 0; i < 8; i++) PetscCall(PetscPrintf(PETSC_COMM_SELF, " %" PetscInt_FMT "     %" PetscInt_FMT "     %" PetscInt_FMT "\n", x2[i], index2[i], x2[index2[i]]));
