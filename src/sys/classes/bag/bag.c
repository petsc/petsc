...
PetscErrorCode PetscBagRegisterBool(PetscBag bag,void *addr,PetscBool mdefault,const char *name,const char *help)
{
  ...
  PetscAssertFalse(mdefault != PETSC_FALSE && mdefault != PETSC_TRUE,PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Boolean %s %s must be boolean; integer value %d",name,help,(int)mdefault);
  nname[0] = '-';
  nname[1] = 0;
  ierr     = PetscStrlcat(nname,name,PETSC_BAG_NAME_LENGTH);CHKERRQ(ierr);
  ierr     = PetscOptionsHasHelp(NULL,&printhelp);CHKERRQ(ierr);
    ...
    while (nitem) {
      if (nitem->dtype == PETSC_CHAR) {
        ...
        for (i=0; i<nitem->msize; i++) {
          ...
          /* the checks here with != PETSC_FALSE and PETSC_TRUE is a special case; here we truly demand that the value be 0 or 1 */
          PetscAssertFalse(value[i] != PETSC_FALSE && value[i] != PETSC_TRUE,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Boolean value for %s %s is corrupt; integer value %" PetscInt_FMT,nitem->name,nitem->help,(PetscInt)(value[i]));
...
PetscErrorCode  PetscBagLoad(PetscViewer view,PetscBag bag)
{
  ...
  PetscAssertFalse(flag != MPI_CONGRUENT && flag != MPI_IDENT,PETSC_COMM_SELF,PETSC_ERR_ARG_NOTSAMECOMM,"Different communicators in the viewer and bag"); \
  ierr = PetscObjectTypeCompare((PetscObject)view,PETSCVIEWERBINARY,&isbinary);CHKERRQ(ierr);
  PetscAssertFalse(!isbinary,PETSC_COMM_SELF,PETSC_ERR_SUP,"No support for this viewer type");

  ierr = PetscViewerBinaryRead(view,&classid,1,NULL,PETSC_INT);CHKERRQ(ierr);
  PetscAssertFalse(classid != PETSC_BAG_FILE_CLASSID,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Not PetscBag next in binary file");
  ...
  PetscAssertFalse(bagcount != bag->count,comm,PETSC_ERR_ARG_INCOMP,"Bag in file has different number of entries %d then passed in bag %d",(int)bagcount,(int)bag->count);
...
PetscErrorCode PetscBagSetOptionsPrefix(PetscBag bag, const char pre[])
{
  ...
  if (!pre) {
    ...
    PetscAssertFalse(pre[0] == '-',PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Options prefix should not begin with a hyphen");
    ierr = PetscFree(bag->bagprefix);CHKERRQ(ierr);
