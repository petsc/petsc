...
PetscErrorCode  PetscDrawAxisCreate(PetscDraw draw,PetscDrawAxis *axis)
{
  ...
  CHKERRQ(PetscHeaderCreate(ad,PETSC_DRAWAXIS_CLASSID,"DrawAxis","Draw Axis","Draw",PetscObjectComm((PetscObject)draw),PetscDrawAxisDestroy,NULL));
  CHKERRQ(PetscLogObjectParent((PetscObject)draw,(PetscObject)ad));
...
PetscErrorCode  PetscDrawAxisDestroy(PetscDrawAxis *axis)
{
  ...
  CHKERRQ(PetscFree((*axis)->toplabel));
  CHKERRQ(PetscFree((*axis)->xlabel));
  CHKERRQ(PetscFree((*axis)->ylabel));
  CHKERRQ(PetscDrawDestroy(&(*axis)->win));
  CHKERRQ(PetscHeaderDestroy(axis));
  PetscFunctionReturn(0);
}
...
PetscErrorCode  PetscDrawAxisSetLabels(PetscDrawAxis axis,const char top[],const char xlabel[],const char ylabel[])
{
  ...
  CHKERRQ(PetscFree(axis->ylabel));
  CHKERRQ(PetscFree(axis->toplabel));
  CHKERRQ(PetscStrallocpy(xlabel,&axis->xlabel));
  CHKERRQ(PetscStrallocpy(ylabel,&axis->ylabel));
  CHKERRQ(PetscStrallocpy(top,&axis->toplabel));
  PetscFunctionReturn(0);
}
...
PetscErrorCode  PetscDrawAxisSetLimits(PetscDrawAxis axis,PetscReal xmin,PetscReal xmax,PetscReal ymin,PetscReal ymax)
{
  ...
  axis->ylow = ymin;
  axis->yhigh= ymax;
  CHKERRQ(PetscOptionsHasName(((PetscObject)axis)->options,((PetscObject)axis)->prefix,"-drawaxis_hold",&axis->hold));
  PetscFunctionReturn(0);
}
...
PetscErrorCode  PetscDrawAxisDraw(PetscDrawAxis axis)
{
  ...
  CHKERRMPI(MPI_Comm_rank(PetscObjectComm((PetscObject)axis),&rank));
  ...
  CHKERRQ(PetscDrawSetCoordinates(draw,0,0,1,1));
  ...
  CHKERRQ(PetscDrawLine(draw,axis->xlow,axis->ylow,axis->xhigh,axis->ylow,ac));
  CHKERRQ(PetscDrawLine(draw,axis->xlow,axis->ylow,axis->xlow,axis->yhigh,ac));
  CHKERRQ(PetscDrawLine(draw,axis->xlow,axis->yhigh,axis->xhigh,axis->yhigh,ac));
  CHKERRQ(PetscDrawLine(draw,axis->xhigh,axis->ylow,axis->xhigh,axis->yhigh,ac));

  /* PetscDraw the top label */
  if (axis->toplabel) {
    PetscReal x = (axis->xlow + axis->xhigh)/2, y = axis->yhigh + 0.5*th;
    CHKERRQ(PetscDrawStringCentered(draw,x,y,cc,axis->toplabel));
  }

  /* PetscDraw the X ticks and labels */
  if (axis->xticks) {
    numx = (int)(.15*(axis->xhigh-axis->xlow)/tw); numx = PetscClipInterval(numx,2,6);
    CHKERRQ((*axis->xticks)(axis->xlow,axis->xhigh,numx,&ntick,tickloc,PETSC_DRAW_AXIS_MAX_SEGMENTS));
    /* PetscDraw in tick marks */
    for (i=0; i<ntick; i++) {
      CHKERRQ(PetscDrawLine(draw,tickloc[i],axis->ylow,tickloc[i],axis->ylow+.5*th,tc));
      CHKERRQ(PetscDrawLine(draw,tickloc[i],axis->yhigh,tickloc[i],axis->yhigh-.5*th,tc));
    }
    /* label ticks */
    if (axis->xlabelstr) {
      for (i=0; i<ntick; i++) {
        ...
        CHKERRQ((*axis->xlabelstr)(tickloc[i],sep,&p));
        CHKERRQ(PetscDrawStringCentered(draw,tickloc[i],axis->ylow-1.5*th,cc,p));
      }
    }
  }
  if (axis->xlabel) {
    ...
    CHKERRQ(PetscDrawStringCentered(draw,x,y,cc,axis->xlabel));
  }

  /* PetscDraw the Y ticks and labels */
  if (axis->yticks) {
    numy = (int)(.50*(axis->yhigh-axis->ylow)/th); numy = PetscClipInterval(numy,2,6);
    CHKERRQ((*axis->yticks)(axis->ylow,axis->yhigh,numy,&ntick,tickloc,PETSC_DRAW_AXIS_MAX_SEGMENTS));
    /* PetscDraw in tick marks */
    for (i=0; i<ntick; i++) {
      CHKERRQ(PetscDrawLine(draw,axis->xlow,tickloc[i],axis->xlow+.5*tw,tickloc[i],tc));
      CHKERRQ(PetscDrawLine(draw,axis->xhigh,tickloc[i],axis->xhigh-.5*tw,tickloc[i],tc));
    }
    /* label ticks */
    if (axis->ylabelstr) {
      for (i=0; i<ntick; i++) {
        ...
        CHKERRQ((*axis->ylabelstr)(tickloc[i],sep,&p));
        CHKERRQ(PetscStrlen(p,&len)); ytlen = PetscMax(ytlen,len);
        CHKERRQ(PetscDrawString(draw,axis->xlow-(len+.5)*tw,tickloc[i]-.5*th,cc,p));
      }
    }
  }
  if (axis->ylabel) {
    ...
    CHKERRQ(PetscStrlen(axis->ylabel,&len));
    CHKERRQ(PetscDrawStringVertical(draw,x,y+len*th/2,cc,axis->ylabel));
  }
...
PetscErrorCode PetscStripe0(char *buf)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscStrlen(buf,&n));
  CHKERRQ(PetscStrendswith(buf,"e00",&flg));
  if (flg) buf[n-3] = 0;
  CHKERRQ(PetscStrstr(buf,"e0",&str));
  if (str) {
    buf[n-2] = buf[n-1];
    buf[n-1] = 0;
  }
  CHKERRQ(PetscStrstr(buf,"e-0",&str));
  if (str) {
    buf[n-2] = buf[n-1];
    buf[n-1] = 0;
  }
...
PetscErrorCode PetscStripTrailingZeros(char *buf)
{
  ...
  CHKERRQ(PetscStrchr(buf,'e',&found));
