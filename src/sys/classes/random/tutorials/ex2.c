...
int main(int argc, char *argv[])
{
  ...
  PetscAssertFalse(hinfo.n <1 || hinfo.n > 31,PETSC_COMM_SELF,PETSC_ERR_SUP,"Only 31 stocks listed in stock.txt. num_of_stocks %" PetscInt_FMT " must between 1 and 31",hinfo.n);
...
PetscErrorCode readData(MPI_Comm comm,himaInfo *hinfo)
{
  ...
  if (rank == 0) {
    ...
    for (i=0;i<num;i++) {
      double vv,tt;
      PetscAssertFalse(fscanf(fd,"%s%lf%lf",temp,&vv,&tt) != 3,PETSC_COMM_SELF,PETSC_ERR_FILE_UNEXPECTED,"Badly formatted input file");
      v[i] = vv;
      t[i] = tt;
    }
    fclose(fd);
