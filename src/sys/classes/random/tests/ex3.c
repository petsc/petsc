...
int main(int argc, char **argv)
{
  ...
  CHKERRQ(PetscOptionsInt("-t",     "t, the dimension of the sample space", "ex3.c", t, &t, NULL));
  CHKERRQ(PetscOptionsInt("-log2d", "The log of d, the number of bins per direction", "ex3.c", log2d, &log2d, NULL));
  CHKERRQ(PetscOptionsInt("-log2n", "The log of n, the number of samples per process", "ex3.c", log2n, &log2n, NULL));
  ...
  CHKERRQ(PetscPrintf(PETSC_COMM_WORLD,"Generating %zu samples (%g GB) per process in a %" PetscInt_FMT " dimensional space with %" PetscInt64_FMT " bins per dimension.\n", n, (n*1.e-9)*sizeof(PetscInt64), t, d));
  CHKERRQ(PetscPrintf(PETSC_COMM_WORLD,"Expected spacing collisions per process %g (%g total).\n", (double) lambda, (double)(N * lambda)));
  CHKERRQ(PetscRandomCreate(PETSC_COMM_WORLD,&random));
  CHKERRQ(PetscRandomSetFromOptions(random));
  CHKERRQ(PetscRandomSetInterval(random,0.0,1.0));
  CHKERRQ(PetscRandomView(random,PETSC_VIEWER_STDOUT_WORLD));
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < t; j++, mult *= d) {
      ...
      CHKERRQ(PetscRandomGetValueReal(random,&x));
  ...
  CHKERRMPI(MPI_Allreduce(MPI_IN_PLACE, &Y, 1, MPIU_INT, MPI_SUM, MPI_COMM_WORLD));
  CHKERRQ(PoissonTailProbability(N*lambda,Y,&p));
  CHKERRQ(PetscPrintf(PETSC_COMM_WORLD,"%" PetscInt_FMT " total collisions counted: that many or more should occur with probabilty %g.\n",Y,(double)p));
