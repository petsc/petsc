...
PETSC_EXTERN void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  if (dt == PETSC_DOUBLE) {
    ierr = PetscBinaryWrite(fd,mxGetPr(prhs[1]),cnt,(PetscDataType)dt);if (ierr) PETSC_MEX_ERROR("Unable to send double items.");
  } else if (dt == PETSC_INT) {
    int    *tmp = (int*) mxMalloc((cnt+5)*sizeof(int));
    double *t   = mxGetPr(prhs[1]);
    for (i=0; i<cnt; i++) tmp[i] = (int)t[i];
    ierr = PetscBinaryWrite(fd,tmp,cnt,(PetscDataType)dt);if (ierr) PETSC_MEX_ERROR("Unable to send int items.");
    mxFree(tmp);
  } else if (dt == PETSC_CHAR) {
    char *tmp = (char*) mxMalloc((cnt+5)*sizeof(char));
    mxGetNChars(prhs[1],tmp,cnt+1);
    ierr = PetscBinaryWrite(fd,tmp,cnt,(PetscDataType)dt);if (ierr) PETSC_MEX_ERROR("Unable to send char items.");
    mxFree(tmp);
