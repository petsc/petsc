...
PETSC_EXTERN void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  if (dt == PETSC_DOUBLE) {
    ...
    ierr    = PetscBinaryRead(fd,mxGetPr(plhs[0]),cnt,NULL,(PetscDataType)dt);if (ierr) PETSC_MEX_ERROR("Unable to receive int items.");
  } else if (dt == PETSC_CHAR) {
    char *tmp = (char*) mxMalloc(cnt*sizeof(char));
    ierr    = PetscBinaryRead(fd,tmp,cnt,NULL,(PetscDataType)dt);if (ierr) PETSC_MEX_ERROR("Unable to receive char items.");
