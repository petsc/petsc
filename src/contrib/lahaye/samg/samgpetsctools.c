...
{ 
   ...
   ierr = MatGetSize(Fact1, &rows1, &cols1); CHKERRQ(ierr); 
   ierr = MatGetSize(Fact2, &rows2, &cols2); CHKERRQ(ierr); 
   ...
   ierr =  MatCreateSeqAIJ(PETSC_COMM_WORLD,rows1,cols2,PETSC_NULL,
           PETSC_NULL,Prod); CHKERRQ(ierr);

   /*..Create vector to hold Fact2(:,i)..*/ 
   ierr = VecCreate(MPI_COMM_WORLD,&col_vec); CHKERRQ(ierr);
   ierr = VecSetSizes(col_vec,PETSC_DECIDE,rows2); CHKERRQ(ierr);
   ierr = VecSetType(col_vec,VECSEQ); CHKERRQ(ierr);

   /*..Create vector to hold Fact1 * Fact2(:,i)..*/ 
   ierr = VecCreate(MPI_COMM_WORLD,&mat_col_vec); CHKERRQ(ierr);
   ierr = VecSetSizes(mat_col_vec,PETSC_DECIDE,rows1); CHKERRQ(ierr);
   ierr = VecSetType(mat_col_vec,VECSEQ); CHKERRQ(ierr);

   for (col=0;col<cols2;col++){

       /*..Get Fact2(:,col)..*/ 
      ierr = MatGetColumnVector(Fact2,col_vec,col); CHKERRQ(ierr); 
      
      /*..Conpute Fact1 * Fact2(:,col)..*/
      ierr = MatMult(Fact1, col_vec, mat_col_vec); CHKERRQ(ierr);
      /*..Compute norm..*/ 
      ...
      ierr = VecGetArray(mat_col_vec, &vec_getvalues); CHKERRQ(ierr);
      ...
      ierr = VecRestoreArray(mat_col_vec, &vec_getvalues); CHKERRQ(ierr);
   }

   ierr = MatAssemblyBegin(*Prod,MAT_FINAL_ASSEMBLY); CHKERRQ(ierr);
   ierr = MatAssemblyEnd(*Prod,MAT_FINAL_ASSEMBLY); CHKERRQ(ierr);

   ierr = VecDestroy(col_vec); CHKERRQ(ierr);
   ierr = VecDestroy(mat_col_vec); CHKERRQ(ierr);
