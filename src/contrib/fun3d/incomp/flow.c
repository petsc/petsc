...
/*---------------------------------------------------------------------*/
{
  ...
  for (tsCtx->itstep = 0; (tsCtx->itstep < max_steps) &&
    ...
    PetscAssertFalse(nfailsCum >= 2,PETSC_COMM_SELF,1,"Unable to find a Newton Step");
    if (print_flag) {
...
/*---------------------------------------------------------------------*/
{
  ...
  if (rank == 0) {
    if (size == 1) {
      ...
      PetscAssertFalse(!fptr,PETSC_COMM_SELF,1,"Cannot open file %s",part_file);
 ...
 */
  {
    ...
    ierr = MPI_Allreduce(partLoc,partMax,7,MPI_INT,MPI_MAX,comm);CHKERRMPI(ierr);
    ierr = MPI_Allreduce(partLoc,partMin,7,MPI_INT,MPI_MIN,comm);CHKERRMPI(ierr);
    ierr = MPI_Allreduce(partLoc,partSum,7,MPI_INT,MPI_SUM,comm);CHKERRMPI(ierr);
...
static PetscErrorCode PetscFWrite_FUN3D(MPI_Comm comm,FILE *fp,void *data,PetscInt n,PetscDataType dtype,PetscBool base64)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(n < 0,comm,PETSC_ERR_ARG_OUTOFRANGE,"Trying to write a negative amount of data %" PetscInt_FMT,n);
...
#  define GetEdge(eptr,i,n1,n2) do { n1 = eptr[i+0*nedge]-1; n2 = eptr[i+1*nedge]-1; } while (0)
  ...
  *iconn  = NULL;
  ...
  for (i=0; i<nnodes; i++) {
    ...
    for (j=0; j<ntmp1; j++) {
      node1 = tmp1[j];
      PetscAssertFalse(node1 < 0 || nnodes <= node1,PETSC_COMM_SELF,1,"node index %" PetscInt_FMT " out of range [0,%" PetscInt_FMT ")",node1,nnodes);
      PetscAssertFalse(node1 <= node0,PETSC_COMM_SELF,1,"forward neighbor of %" PetscInt_FMT " is %" PetscInt_FMT ", should be larger",node0,node1);
      ...
      for (k=0; k<ntmp2; k++) {
        node2 = tmp2[k];
        PetscAssertFalse(node2 < 0 || nnodes <= node2,PETSC_COMM_SELF,1,"node index %" PetscInt_FMT " out of range [0,%" PetscInt_FMT ")",node2,nnodes);
        PetscAssertFalse(node2 <= node1,PETSC_COMM_SELF,1,"forward neighbor of %" PetscInt_FMT " is %" PetscInt_FMT ", should be larger",node1,node2);
        ...
        for (l=0; l<ntmp3; l++) {
          node3 = tmp3[l];
          PetscAssertFalse(node3 < 0 || nnodes <= node3,PETSC_COMM_SELF,1,"node index %" PetscInt_FMT " out of range [0,%" PetscInt_FMT ")",node3,nnodes);
          PetscAssertFalse(node3 <= node2,PETSC_COMM_SELF,1,"forward neighbor of %" PetscInt_FMT " is %" PetscInt_FMT ", should be larger",node2,node3);
          PetscAssertFalse(ncell > acell,PETSC_COMM_SELF,PETSC_ERR_SUP,"buffer exceeded");
          if (ntmp3 < 3) continue;
          conn[ncell][0] = node0;
          conn[ncell][1] = node1;
          conn[ncell][2] = node2;
          ...
          if (0) {
            ...
            PetscAssertFalse(ntmp3 > 2,PETSC_COMM_SELF,1,"More than two ways to complete a tetrahedron using a common triangle");
          }
          ncell++;
        }
      }
...
static PetscErrorCode WritePVTU(AppCtx *user,const char *fname,PetscBool base64)
{
  ...
  PetscAssertFalse(bs != 4,PETSC_COMM_WORLD,PETSC_ERR_ARG_INCOMP,"expected block size 4, got %" PetscInt_FMT,bs);
  ierr = VecGetSize(Xloc,&nloc);CHKERRQ(ierr);
  PetscAssertFalse(nloc/bs != nvertices,PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"expected nloc/bs=%" PetscInt_FMT " to match nvertices=%" PetscInt_FMT,nloc/bs,nvertices);
  PetscAssertFalse(nvertices != grid->nvertices,PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"expected nvertices=%" PetscInt_FMT " to match grid->nvertices=%" PetscInt_FMT,nvertices,grid->nvertices);
...
#else
  PetscAssertFalse(size > 1,PETSC_COMM_SELF,1,"Parallel case not supported in non-interlaced case");
...
/* call the output frame.out */
  ...
  PetscAssertFalse(!(output = fopen("frame.out","a")),PETSC_COMM_SELF,1,"can't open frame.out");
...
{
  PetscErrorCode ierr;
  PetscAssertFalse((*gen_start = start_counters(event0,event1)) < 0,PETSC_COMM_SELF,1,"Error in start_counters");
  ierr = PetscTime(&time_start_counters);CHKERRQ(ierr);
  return 0;
}
...
int EventCountersEnd(int gen_start,PetscScalar time_start_counters)
{
  ...
  PetscAssertFalse((gen_read = read_counters(event0,&_counter0,event1,&_counter1)) < 0,PETSC_COMM_SELF,1,"Error in read_counter");
  ierr = PetscTime(&&time_read_counters);CHKERRQ(ierr);
  PetscAssertFalse(gen_read != gen_start,PETSC_COMM_SELF,1,"Lost Counters!! Aborting ...");
  counter0      += _counter0;
  counter1      += _counter1;
