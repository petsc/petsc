...
while flag > .5
  ...
  flag = receive(p);	
  for i=1:N,	
    cellz1(:,i) = solution(cell_df( (9*i-8):2:9*i-1 ));
    cellz2(:,i) = solution(cell_df( (9*i-7):2:9*i -1 ));
    pval = solution(cell_df(9*i));
    cellz3(:,i) = [pval pval pval pval]';
  end
%
  figure(1)
...
%
  figure(3)
  fill3(cellx,celly,cellz3,cellz3)
end
