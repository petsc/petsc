...
int InputGrid(AGrid *agrid)
{
 ...
 vertices = agrid->vertices = (double *) PetscMalloc(agrid->nv*(agrid->xintervals+2)*(agrid->yintervals+2)*sizeof(double)); CHKPTRQ(vertices);
...
int GenerateNodes(AGrid *agrid)
{
 ...
 for(i=0;i<agrid->n_quads;i++){
   for(j=0;j<2*agrid->nv;j++){ cell_df[agrid->nt*i + j] = cell_vdf[2*agrid->nv*i + j];}
   for(j=0;j<agrid->np;j++){ cell_df[agrid->nt*i +2*agrid->nv+ j] = cell_pdf[agrid->np*i + j];}
 }
