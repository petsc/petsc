...
int AppCtxCreateVector(AppCtx *appctx)
{
  ...
  ierr = VecDuplicate(algebra->b, &algebra->u);CHKERRQ(ierr);
  ierr = VecDuplicate(algebra->b, &algebra->g);CHKERRQ(ierr);
...
int ierr; 
  ...
  ierr = MatDuplicate(algebra->A, MAT_DO_NOT_COPY_VALUES,&algebra->M);CHKERRQ(ierr);
  ierr = MatDuplicate(algebra->A,MAT_DO_NOT_COPY_VALUES, &algebra->C);CHKERRQ(ierr);
 ierr = MatDuplicate(algebra->A, MAT_DO_NOT_COPY_VALUES,&algebra->D);CHKERRQ(ierr);
