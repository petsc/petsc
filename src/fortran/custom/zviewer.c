...
#ifndef lint
static char vcid[] = "$Id: zviewer.c,v 1.1 1995/09/04 04:37:46 bsmith Exp bsmith $";
#endif
...
void viewermatlabopen_(char *name,int *port,Viewer *lab, int *__ierr,int len1 )
{
  Viewer vv;
  char   *c1;
  if (!name[len1] == 0) {
  ...
  *__ierr = ViewerMatlabOpen(c1,*port,&vv);
