...
PetscErrorCode setupStream(PetscInt device, PetscBool cpuTiming);
...
int main(int argc, char *argv[])
{
  ...
  ierr = PetscPrintf(PETSC_COMM_SELF, "[Single-Precision Device-Only STREAM Benchmark implementation in CUDA]\n");CHKERRQ(ierr);
  ...
  ierr = setupStream(device, cpuTiming);
  if (ierr >= 0) {
...
///////////////////////////////////////////////////////////////////////////////
PetscErrorCode setupStream(PetscInt deviceNum, PetscBool cpuTiming)
{
...
PetscErrorCode printResultsReadable(float times[][NTIMES]) {
  PetscErrorCode ierr;
  PetscInt       j, k;
  float	avgtime[5] = {0., 0., 0., 0., 0.};
  float	maxtime[5] = {0., 0., 0., 0., 0.};
  float	mintime[5] = {1e30,1e30,1e30,1e30,1e30};
