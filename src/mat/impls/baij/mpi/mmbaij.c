...
PetscErrorCode MatSetUpMultiply_MPIBAIJ(Mat mat)
{
  ...
  PetscCall(PetscTableCreate(B->mbs,baij->Nbs+1,&gid1_lid1));
  for (i=0; i<B->mbs; i++) {
...
PetscErrorCode MatDisAssemble_MPIBAIJ(Mat A)
{
  ...
  PetscCall(VecDestroy(&baij->lvec)); baij->lvec = NULL;
  PetscCall(VecScatterDestroy(&baij->Mvctx)); baij->Mvctx = NULL;
  if (baij->colmap) {
#if defined(PETSC_USE_CTABLE)
...
PetscErrorCode  MatDiagonalScaleLocal_MPIBAIJ(Mat A,Vec scale)
{
  ...
  PetscFunctionBegin;
  if (!uglyrmapd) {
    PetscCall(MatMPIBAIJDiagonalScaleLocalSetUp(A,scale));
  }
