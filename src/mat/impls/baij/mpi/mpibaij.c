...
PetscErrorCode MatCreateColmap_MPIBAIJ_Private(Mat mat)
{
  ...
  for (i=0; i<nbs; i++) {
    PetscCall(PetscTableAdd(baij->colmap,baij->garray[i]+1,i*bs+1,INSERT_VALUES));
  }
#else
...
PetscErrorCode MatAssemblyEnd_MPIBAIJ(Mat mat,MatAssemblyType mode)
{
  ...
  */
  ...
  if (!mat->was_assembled && mode == MAT_FINAL_ASSEMBLY) {
    PetscCall(MatSetUpMultiply_MPIBAIJ(mat));
  }
...
PetscErrorCode MatDestroy_MPIBAIJ(Mat mat)
{
  ...
  PetscCall(PetscObjectComposeFunction((PetscObject)mat,"MatConvert_mpibaij_mpibstrm_C",NULL));
#if defined(PETSC_HAVE_HYPRE)
...
PetscErrorCode MatPermute_MPIBAIJ(Mat A,IS rowp,IS colp,Mat *B)
{
  ...
  /* clean up */
  if (pcomm!=comm) {
    PetscCall(ISDestroy(&crowp));
  }
  if (size>1) {
    PetscCall(ISDestroy(&lcolp));
  }
  PetscFunctionReturn(0);
}
