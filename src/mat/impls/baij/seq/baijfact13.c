...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_3(Mat C,Mat A,const MatFactorInfo *info)
{
  ...
  for (i=0; i<n; i++) {
    ...
    ierr = Kernel_A_gets_inverse_A_3(w,shift);CHKERRQ(ierr);
  }
  ...
  for (i=0; i<n; i++){
    ...
    for(k = 0;k < nzL;k++){
      ...
      if (flg) {
        ...
        ierr = Kernel_A_gets_A_times_B_3(pc,pv,mwork);CHKERRQ(ierr);
        ...
        for (j=0; j<nz; j++) {
          ...
          v    = rtmp + bs2*pj[j];
          ierr = Kernel_A_gets_A_minus_B_times_C_3(v,pc,pv);CHKERRQ(ierr);
          pv  += bs2;          
        }
    ...
    ierr = Kernel_A_gets_inverse_A_3(pv,shift);CHKERRQ(ierr);
...
#undef __FUNCT__  
#define __FUNCT__ "MatLUFactorNumeric_SeqBAIJ_3_newdatastruct_v2"
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_3_newdatastruct_v2(Mat B,Mat A,const MatFactorInfo *info)
{
  Mat            C=B;
  Mat_SeqBAIJ    *a=(Mat_SeqBAIJ*)A->data,*b=(Mat_SeqBAIJ *)C->data;
  IS             isrow = b->row,isicol = b->icol;
...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_3_NaturalOrdering(Mat C,Mat A,const MatFactorInfo *info)
{
  Mat_SeqBAIJ    *a = (Mat_SeqBAIJ*)A->data,*b = (Mat_SeqBAIJ *)C->data;
  PetscErrorCode ierr;
  PetscInt       i,j,n = a->mbs,*bi = b->i,*bj = b->j;
...
#undef __FUNCT__  
#define __FUNCT__ "MatLUFactorNumeric_SeqBAIJ_3_NaturalOrdering_newdatastruct_v2"
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_3_NaturalOrdering_newdatastruct_v2(Mat B,Mat A,const MatFactorInfo *info)
{
  Mat            C=B;
  Mat_SeqBAIJ    *a=(Mat_SeqBAIJ*)A->data,*b=(Mat_SeqBAIJ *)C->data;
  PetscErrorCode ierr;
