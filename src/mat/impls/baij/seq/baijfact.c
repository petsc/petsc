...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_2(Mat B,Mat A,const MatFactorInfo *info)
{
  ...
  for (i=0; i<n; i++) {
    ...
    for (k=0; k < nzL; k++) {
      ...
      if (flg) {
        ...
        CHKERRQ(PetscKernel_A_gets_A_times_B_2(pc,pv,mwork));
        ...
        for (j=0; j<nz; j++) {
          ...
          v    = rtmp + 4*pj[j];
          CHKERRQ(PetscKernel_A_gets_A_minus_B_times_C_2(v,pc,pv));
          pv  += 4;
        }
        CHKERRQ(PetscLogFlops(16.0*nz+12)); /* flops = 2*bs^3*nz + 2*bs^3 - bs2) */
      }
    }
    ...
    CHKERRQ(PetscKernel_A_gets_inverse_A_2(pv,shift,allowzeropivot,&zeropivotdetected));
  ...
  CHKERRQ(PetscLogFlops(1.333333333333*2*2*2*n)); /* from inverting diagonal blocks */
...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_2_inplace(Mat B,Mat A,const MatFactorInfo *info)
{
  ...
  CHKERRQ(ISGetIndices(isrow,&r));
  CHKERRQ(ISGetIndices(isicol,&ic));
  CHKERRQ(PetscMalloc1(4*(n+1),&rtmp));
  ...
  for (i=0; i<n; i++) {
    ...
    while (row < i) {
      ...
      if (p1 != (PetscScalar)0.0 || p2 != (PetscScalar)0.0 || p3 != (PetscScalar)0.0 || p4 != (PetscScalar)0.0) {
        ...
        CHKERRQ(PetscLogFlops(16.0*nz+12.0));
      }
      row = *ajtmp++;
    }
    /* finished row so stick it into b->a */
    ...
    CHKERRQ(PetscKernel_A_gets_inverse_A_2(w,shift,allowzeropivot,&zeropivotdetected));
  ...
  CHKERRQ(PetscLogFlops(1.333333333333*8*b->mbs)); /* from inverting diagonal blocks */
  PetscFunctionReturn(0);
}
/*
    ...
    while (row < i) {
      ...
      if (*pc != 0.0) {
        ...
        *pc        = multiplier;
        nz         = bi[row+1] - diag_offset[row] - 1;
        for (j=0; j<nz; j++) rtmp[pj[j]] -= multiplier * pv[j];
        CHKERRQ(PetscLogFlops(1.0+2.0*nz));
      }
      row = *ajtmp++;
    }
    /* finished row so stick it into b->a */
...
PetscErrorCode MatLUFactor_SeqBAIJ(Mat A,IS row,IS col,const MatFactorInfo *info)
{
  ...
  CHKERRQ(PetscLogObjectParent((PetscObject)A,(PetscObject)((Mat_SeqBAIJ*)(A->data))->icol));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatCholeskyFactorNumeric_SeqBAIJ_N(Mat C,Mat A,const MatFactorInfo *info)
{
  ...
  if (bs > 1) { /* convert A to a SBAIJ matrix and apply Cholesky factorization from it */
    if (!a->sbaijMat) {
      CHKERRQ(MatConvert(A,MATSEQSBAIJ,MAT_INITIAL_MATRIX,&a->sbaijMat));
    }
    CHKERRQ((a->sbaijMat)->ops->choleskyfactornumeric(C,a->sbaijMat,info));
...
PetscErrorCode MatCholeskyFactorNumeric_SeqBAIJ_N_NaturalOrdering(Mat C,Mat A,const MatFactorInfo *info)
{
  ...
  CHKERRQ(PetscMalloc3(am,&rtmp,am,&il,am,&jl));
...
PetscErrorCode MatICCFactorSymbolic_SeqBAIJ(Mat fact,Mat A,IS perm,const MatFactorInfo *info)
{
  ...
  if (bs > 1) {
    ...
    CHKERRQ(MatICCFactorSymbolic(fact,a->sbaijMat,perm,info));
    PetscFunctionReturn(0);
  }
  ...
  if (!levels && perm_identity) {
    ...
    B    = fact;
    CHKERRQ(MatSeqSBAIJSetPreallocation(B,1,0,ui));
  ...
  CHKERRQ(PetscMalloc1(am+1,&ui));
  ui[0] = 0;
  CHKERRQ(PetscMalloc1(2*am+1,&cols_lvl));
  ...
  CHKERRQ(PetscFreeSpaceGet(PetscRealIntMultTruncate(fill,PetscIntSumTruncate(ai[am]/2,am/2)),&free_space));

  current_space = free_space;

  CHKERRQ(PetscFreeSpaceGet(PetscRealIntMultTruncate(fill,PetscIntSumTruncate(ai[am]/2,am/2)),&free_space_lvl));
  ...
  CHKERRQ(PetscLogObjectMemory((PetscObject)B,(ui[am]-am)*(sizeof(PetscInt)+sizeof(MatScalar))));
...
PetscErrorCode MatCholeskyFactorSymbolic_SeqBAIJ(Mat fact,Mat A,IS perm,const MatFactorInfo *info)
{
  ...
  if (bs > 1) { /* convert to seqsbaij */
    ...
    CHKERRQ(MatCholeskyFactorSymbolic(fact,a->sbaijMat,perm,info));
    PetscFunctionReturn(0);
  }
  ...
  CHKERRQ(PetscFreeSpaceGet(PetscRealIntMultTruncate(fill,PetscIntSumTruncate(ai[mbs]/2,mbs/2)),&free_space));
  ...
  /* put together the new matrix in MATSEQSBAIJ format */
  B    = fact;
  CHKERRQ(MatSeqSBAIJSetPreallocation(B,bs,MAT_SKIP_ALLOCATION,NULL));
  ...
  CHKERRQ(PetscLogObjectMemory((PetscObject)B,(ui[mbs]-mbs)*(sizeof(PetscInt)+sizeof(MatScalar))));
  b->maxnz = b->nz = ui[mbs];
...
PetscErrorCode MatSolve_SeqBAIJ_N_NaturalOrdering(Mat A,Vec bb,Vec xx)
{
  ...
  CHKERRQ(PetscArraycpy(t,b,bs)); /* copy 1st block of b to t */
  ...
  for (i=1; i<n; i++) {
    ...
    nz   = ai[i+1] - ai[i];
    s    = t + bs*i;
    CHKERRQ(PetscArraycpy(s,b+bs*i,bs)); /* copy i_th block of b to t */
    for (k=0;k<nz;k++) {
  ...
  for (i=n-1; i>=0; i--) {
    ...
    CHKERRQ(PetscArraycpy(x+i*bs,t+i*bs,bs));
  }
...
PetscErrorCode MatSolve_SeqBAIJ_N(Mat A,Vec bb,Vec xx)
{
  ...
  /* forward solve the lower triangular */
  CHKERRQ(PetscArraycpy(t,b+bs*r[0],bs));
  for (i=1; i<n; i++) {
    v    = aa + bs2*ai[i];
    vi   = aj + ai[i];
    nz   = ai[i+1] - ai[i];
    s    = t + bs*i;
    CHKERRQ(PetscArraycpy(s,b+bs*r[i],bs));
    for (m=0; m<nz; m++) {
  ...
  for (i=n-1; i>=0; i--) {
    ...
    CHKERRQ(PetscArraycpy(x + bs*c[i],t+i*bs,bs));
  }
...
static PetscErrorCode MatBlockAbs_private(PetscInt nbs,PetscInt bs2,PetscScalar *blockarray,PetscReal *absarray)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscArrayzero(absarray,nbs+1));
  for (i=0; i<nbs; i++) {
    for (j=0; j<bs2; j++) {
...
PetscErrorCode MatILUDTFactor_SeqBAIJ(Mat A,IS isrow,IS iscol,const MatFactorInfo *info,Mat *fact)
{
  ...
  CHKERRQ(PetscMalloc1(mbs+1,&bdiag));

  /* allocate row pointers bi */
  CHKERRQ(PetscMalloc1(2*mbs+2,&bi));
  ...
  CHKERRQ(PetscMalloc1(nnz_max,&bj));
  nnz_max = nnz_max*bs2;
  CHKERRQ(PetscMalloc1(nnz_max,&ba));

  /* put together the new matrix */
  CHKERRQ(MatSeqBAIJSetPreallocation(B,bs,MAT_SKIP_ALLOCATION,NULL));
  ...
  b->icol  = isicol;
  CHKERRQ(PetscMalloc1(bs*(mbs+1),&b->solve_work));
  ...
  CHKERRQ(PetscMalloc2(mbs,&im,mbs,&jtmp));
  /* rtmp, vtmp: working arrays for sparse and contiguous row entries of active row */
  CHKERRQ(PetscMalloc2(mbs*bs2,&rtmp,mbs*bs2,&vtmp));
  CHKERRQ(PetscMalloc1(mbs+1,&vtmp_abs));
  ...
  for (i=0; i<mbs; i++) {
    ...
    CHKERRQ(PetscLLClean(mbs,mbs,nzi,lnk,jtmp,lnkbt));
    ...
    while  (row < i) {
      ...
      CHKERRQ(MatBlockAbs_private(1,bs2,pc,vtmp_abs));
      if (vtmp_abs[0] > dt) { /* apply tolerance dropping rule */
        ...
        /* CHKERRQ(PetscLogFlops(bslog*(nz+1.0)-bs)); */
      }
      row = *bjtmp++;
    }
    ...
    while (jtmp[j] < i) { /* L-part. Note: jtmp is sorted */
      CHKERRQ(PetscArraycpy(vtmp+bs2*j,rtmp+bs2*jtmp[j],bs2));
      nzi_bl++; j++;
    }
    nzi_bu = nzi - nzi_bl -1;
    ...
    CHKERRQ(MatBlockAbs_private(nzi,bs2,vtmp,vtmp_abs));
    ...
    ncut = nzi_al + dtcount;
    if (ncut < nzi_bl) {
      CHKERRQ(PetscSortSplitReal(ncut,nzi_bl,vtmp_abs,jtmp));
    ...
    for (j=0; j<ncut; j++) {
      bjtmp[j] = jtmp[j];
      CHKERRQ(PetscArraycpy(batmp+bs2*j,rtmp+bs2*bjtmp[j],bs2));
    }
    bi[i+1] = bi[i] + ncut;
    nzi     = ncut + 1;
    ...
    ncut = nzi_au + dtcount;
    if (ncut < nzi_bu) {
      CHKERRQ(PetscSortSplitReal(ncut,nzi_bu,vtmp_abs+nzi_bl+1,jtmp+nzi_bl+1));
    ...
    CHKERRQ(PetscArraycpy(batmp,rtmp+bs2*i,bs2));
    *bjtmp = i;
    ...
    for (k=0; k<ncut; k++) {
      ...
      CHKERRQ(PetscArraycpy(batmp+bs2*k,rtmp+bs2*bjtmp[k],bs2));
    }
    ...
    CHKERRQ(PetscKernel_A_gets_inverse_A(bs,batmp,v_pivots,v_work,allowzeropivot,&zeropivotdetected));
    if (zeropivotdetected) B->factorerrortype = MAT_FACTOR_NUMERIC_ZEROPIVOT;
  } /* for (i=0; i<mbs; i++) */
  ...
  CHKERRQ(PetscLogFlops(bs2*B->cmap->n));
