...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_2(Mat B,Mat A,const MatFactorInfo *info)
{
  ...
  for (i=0; i<n; i++) {
    ...
    ierr = PetscKernel_A_gets_inverse_A_2(pv,shift,!A->erroriffailure,&zeropivotdetected);CHKERRQ(ierr);
...
PetscErrorCode MatLUFactorNumeric_SeqBAIJ_2_inplace(Mat B,Mat A,const MatFactorInfo *info)
{
  ...
  for (i=0; i<n; i++) {
    ...
    ierr = PetscKernel_A_gets_inverse_A_2(w,shift,!A->erroriffailure,&zeropivotdetected);CHKERRQ(ierr);
