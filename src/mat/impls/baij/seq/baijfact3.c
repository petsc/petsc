/*$Id: baijfact3.c,v 1.1 2001/01/06 15:35:15 bsmith Exp bsmith $*/
/*
...
ierr = PetscMalloc(jmax*sizeof(int),&(      ajtmp ));CHKERRQ(ierr);
  ...
  b->a          = (MatScalar*)PetscMalloc((ainew[n]+1)*sizeof(MatScalar)*bs2);CHKERRQ(ierr);
  b->j          = ajnew;
  b->i          = ainew;
  b->diag       = idnew;
  b->ilen       = 0;
  ...
  b->icol       = isicol;
ierr = PetscMalloc((bs*n+bs)*sizeof(Scalar),&  b->solve_work );CHKERRQ(ierr);
