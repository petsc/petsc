...
#  if defined(PETSC_HAVE_SSE)
          ...
          if (n==(unsigned short)n) {
            ...
            CHKERRQ(PetscInfo(inA,"Using special SSE, in-place natural ordering, ushort j index factor BS=4\n"));
          } else {
...
/*            } */
            ...
            CHKERRQ(PetscInfo(inA,"Using special SSE, in-place natural ordering, int j index factor BS=4\n"));
          }
#  else
...
PetscErrorCode MatLUFactorSymbolic_SeqBAIJ(Mat B,Mat A,IS isrow,IS iscol,const MatFactorInfo *info)
{
  ...
  CHKERRQ(PetscMalloc1((bdiag[0]+1)*bs2,&b->a));
  b->j             = bj;
  b->i             = bi;
  b->diag          = bdiag;
  ...
  b->icol = isicol;
  CHKERRQ(PetscMalloc1(bs*n+bs,&b->solve_work));
  CHKERRQ(PetscLogObjectMemory((PetscObject)B,(bdiag[0]+1)*(sizeof(PetscInt)+sizeof(PetscScalar)*bs2)));
  ...
  CHKERRQ(MatSeqBAIJSetNumericFactorization(B,both_identity));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatLUFactorSymbolic_SeqBAIJ_inplace(Mat B,Mat A,IS isrow,IS iscol,const MatFactorInfo *info)
{
  ...
  CHKERRQ(PetscMalloc1((bi[n]+1)*bs2,&b->a));
  b->j             = bj;
  b->i             = bi;
  b->diag          = bdiag;
  b->free_diag     = PETSC_TRUE;
  ...
  CHKERRQ(PetscMalloc1(bs*n+bs,&b->solve_work));
  CHKERRQ(PetscLogObjectMemory((PetscObject)B,(bi[n]-n)*(sizeof(PetscInt)+sizeof(PetscScalar)*bs2)));
  ...
  CHKERRQ(MatSeqBAIJSetNumericFactorization_inplace(B,both_identity));
  PetscFunctionReturn(0);
}
