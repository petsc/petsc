...
PetscErrorCode PETSCMAT_DLLEXPORT MatInvertBlockDiagonal_SeqBAIJ(Mat A)
{
  ...
  switch (bs){
	...
	mdiag += 36;
      ...
      break;
    default: 
      SETERRQ1(((PetscObject)A)->comm,PETSC_ERR_SUP,"not supported for block size %D",bs);
  }
...
#undef __FUNCT__  
#define __FUNCT__ "MatLoadnew_SeqBAIJ"
PetscErrorCode MatLoadnew_SeqBAIJ(PetscViewer viewer,Mat newmat)
{
  Mat_SeqBAIJ    *a;
  PetscErrorCode ierr;
  PetscInt       i,nz,header[4],*rowlengths=0,M,N,bs=1;
