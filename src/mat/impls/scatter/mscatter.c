...
PetscErrorCode  MatScatterSetVecScatter(Mat mat,VecScatter scatter)
{
  ...
  if (mat->rmap->n != scatter->vscat.to_n) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number of local rows in matrix %" PetscInt_FMT " not equal local scatter size %" PetscInt_FMT,mat->rmap->n,scatter->vscat.to_n);
  if (mat->cmap->n != scatter->vscat.from_n) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number of local columns in matrix %" PetscInt_FMT " not equal local scatter size %" PetscInt_FMT,mat->cmap->n,scatter->vscat.from_n);
