...
*/
#undef __FUNCT__  
#define __FUNCT__ "MatCholeskyFactorNumeric_SeqSBAIJ_5_NaturalOrdering"
PetscErrorCode MatCholeskyFactorNumeric_SeqSBAIJ_5_NaturalOrdering(Mat A,MatFactorInfo *info,Mat *B)
{
  Mat            C = *B;
  ...
  for (k = 0; k<mbs; k++){
    ...
    while (i < mbs){
      ...
      ierr = PetscLogFlops(125*4);CHKERRQ(ierr);
      ...
      if (jmin < jmax){
        ...
        ierr = PetscLogFlops(2*125*(jmax-jmin));CHKERRQ(ierr);
  ...
  C->ops->solve          = MatSolve_SeqSBAIJ_5_NaturalOrdering;
  C->ops->solvetranspose = MatSolve_SeqSBAIJ_5_NaturalOrdering;
  C->ops->forwardsolve   = MatForwardSolve_SeqSBAIJ_5_NaturalOrdering;
  C->ops->backwardsolve  = MatBackwardSolve_SeqSBAIJ_5_NaturalOrdering;
