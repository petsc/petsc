...
PetscErrorCode MatCholeskyFactorNumeric_SeqSBAIJ_4_NaturalOrdering(Mat A,MatFactorInfo *info,Mat *B)
{
  Mat            C = *B;
  ...
  ierr = PetscMalloc(32*sizeof(MatScalar),&dk);CHKERRQ(ierr);
  uik  = dk + 16;     
  ...
  for (k = 0; k<mbs; k++){
    ...
    while (i < mbs){
      ...
      ierr = PetscLogFlops(64*4);CHKERRQ(ierr);
      ...
      if (jmin < jmax){
        ...
        ierr = PetscLogFlops(2*64*(jmax-jmin));CHKERRQ(ierr);
  ...
  C->ops->solve          = MatSolve_SeqSBAIJ_4_NaturalOrdering;
  C->ops->solvetranspose = MatSolve_SeqSBAIJ_4_NaturalOrdering;
  C->ops->forwardsolve   = MatForwardSolve_SeqSBAIJ_4_NaturalOrdering;
  C->ops->backwardsolve  = MatBackwardSolve_SeqSBAIJ_4_NaturalOrdering;
