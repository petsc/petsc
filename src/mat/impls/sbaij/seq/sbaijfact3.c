...
PetscErrorCode MatCholeskyFactorNumeric_SeqSBAIJ_3(Mat C,Mat A,MatFactorInfo *info)
{
  ...
  ierr = PetscMalloc(18*sizeof(MatScalar),&dk);CHKERRQ(ierr);
  uik  = dk + 9;     
  ...
  if (!a->permute){
    ...
    ai   = a->inew; aj = a->jnew; 
    ierr = PetscMalloc(9*ai[mbs]*sizeof(MatScalar),&aa);CHKERRQ(ierr); 
  ...
  C->ops->solve          = MatSolve_SeqSBAIJ_3;
  C->ops->solvetranspose = MatSolve_SeqSBAIJ_3;
