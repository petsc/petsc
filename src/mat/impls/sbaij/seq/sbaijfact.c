...
/*$Id: sbaijfact.c,v 1.51 2000/11/28 17:29:37 bsmith Exp bsmith $*/
/*
...
ierr = PetscMalloc(umax*sizeof(int),&(      jutmp ));CHKERRQ(ierr);
  ...
  b->a          = (MatScalar*)PetscMalloc((iu[mbs]+1)*sizeof(MatScalar)*bs2);CHKERRQ(ierr);
  b->j          = ju;
  b->i          = iu;
  b->diag       = 0;
  b->ilen       = 0;
  ...
  b->icol       = perm;
  ierr          = PetscObjectReference((PetscObject)perm);CHKERRQ(ierr);
ierr = PetscMalloc((bs*mbs+bs)*sizeof(Scalar),&  b->solve_work );CHKERRQ(ierr);
  ...
  if (perm_identity){
    switch (bs) {
      case 1:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=1\n");
        break;
      case 2:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=2\n");
        break;
      case 3:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:sing special in-place natural ordering factor and solve BS=3\n");
        break; 
      case 4:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=4\n"); 
        break;
      case 5:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=5\n"); 
        break;
      case 6: 
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=6\n");
        break; 
      case 7:
        ...
        PetscLogInfo(A,"MatIncompleteCholeskyFactorSymbolic_SeqSBAIJ:Using special in-place natural ordering factor and solve BS=7\n");
      break; 
      default:
...
ierr = PetscMalloc(bs*sizeof(int),&(  pivots));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(bs2*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
    ierr = PetscMemcpy(aa,a->a,bs2*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr);
    a2anew  = (int*)PetscMalloc(ai[mbs]*sizeof(int));CHKERRQ(ierr); 
...
ierr = PetscMalloc(98*sizeof(MatScalar),&(  dk    ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(49*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
ierr = PetscMalloc(72*sizeof(MatScalar),&(  dk    ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(36*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
ierr = PetscMalloc(50*sizeof(MatScalar),&(  dk    ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(25*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
ierr = PetscMalloc(32*sizeof(MatScalar),&(  dk    ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(16*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
ierr = PetscMalloc(18*sizeof(MatScalar),&(  dk  ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(9*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
ierr = PetscMalloc(8*sizeof(MatScalar),&(  dk  ));CHKERRQ(ierr);
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(4*ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
...
int MatCholeskyFactorNumeric_SeqSBAIJ_1(Mat A,Mat *B)
{
  ...
  if (!a->permute){
    ...
    ai = a->inew; aj = a->jnew; 
    aa = (MatScalar*)PetscMalloc(ai[mbs]*sizeof(MatScalar));CHKERRQ(ierr); 
