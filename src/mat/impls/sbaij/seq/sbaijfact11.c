...
#undef __FUNCT__  
#define __FUNCT__ "MatCholeskyFactorNumeric_SeqSBAIJ_7"
PetscErrorCode MatCholeskyFactorNumeric_SeqSBAIJ_7(Mat C,Mat A,MatFactorInfo *info)
{
  ...
  for (k = 0; k<mbs; k++){
    ...
    while (i < mbs){
      ...
      ierr = PetscLogFlops(343*4);CHKERRQ(ierr);
	  ...
	  u43 = u[43]; u44 = u[44]; u45 = u[45]; u46 = u[46]; u47 = u[47]; u48 = u[48];
        ...
        ierr = PetscLogFlops(2*343*(jmax-jmin));CHKERRQ(ierr);
  ...
  C->ops->solve          = MatSolve_SeqSBAIJ_7;
  C->ops->solvetranspose = MatSolve_SeqSBAIJ_7;
