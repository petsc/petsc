/*$Id: sbaij.c,v 1.59 2001/06/22 00:20:57 buschelm Exp bsmith $*/
...
#undef __FUNCT__  
#define __FUNCT__ "MatGetOwnershipRange_SeqSBAIJ"
...
static int MatView_SeqSBAIJ_Binary(Mat A,PetscViewer viewer)
{
  ...
  ierr  = PetscMalloc((a->s_nz+1)*bs2*sizeof(int),&jj);CHKERRQ(ierr);
  count = 0;
  for (i=0; i<a->mbs; i++) {
    for (j=0; j<bs; j++) {
  ...
  ierr  = PetscMalloc((a->s_nz+1)*bs2*sizeof(Scalar),&aa);CHKERRQ(ierr);
  count = 0;
  for (i=0; i<a->mbs; i++) {
    for (j=0; j<bs; j++) {
      for (k=a->i[i]; k<a->i[i+1]; k++) {
...
int MatICCFactor_SeqSBAIJ(Mat inA,IS row,PetscReal fill,int level)
{
  ...
  */
  ...
  default:
    ...
    if (!a->solve_work) {
      ierr = PetscMalloc((A->m+a->bs)*sizeof(Scalar),&a->solve_work);CHKERRQ(ierr);
