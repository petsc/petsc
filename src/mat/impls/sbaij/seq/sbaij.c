...
PetscErrorCode PETSCMAT_DLLEXPORT MatSeqSBAIJSetColumnIndices(Mat mat,PetscInt *indices)
{
  ...
  if (f) {
    ...
    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"Wrong type of matrix to set column indices");
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatCopy_SeqSBAIJ(Mat A,Mat B,MatStructure str)
{
  ...
  if (str == SAME_NONZERO_PATTERN && (A->ops->copy == B->ops->copy)) {
    ...
    if (a->i[A->rmap->N] != b->i[B->rmap->N]) {
      SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Number of nonzeros in two matrices are different");
    }
...
#undef __FUNCT__  
#define __FUNCT__ "MatSetBlockSize_SeqSBAIJ"
PetscErrorCode MatSetBlockSize_SeqSBAIJ(Mat A,PetscInt bs)
{
  PetscInt rbs,cbs;
  PetscErrorCode ierr;
...
PetscErrorCode PETSCMAT_DLLEXPORT MatSeqSBAIJSetPreallocation_SeqSBAIJ(Mat B,PetscInt bs,PetscInt nz,PetscInt *nnz)
{
  ...
  if (nnz && newbs != bs) {
    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Cannot change blocksize from command line if setting nnz");
  }
  bs = newbs;
  ...
  if (mbs*bs != B->rmap->N) {
    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number rows, cols must be divisible by blocksize");
  }
...
PetscErrorCode MatLoad_SeqSBAIJ(PetscViewer viewer, const MatType type,Mat *A)
{
  ...
  if (header[3] < 0) {
    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_UNEXPECTED,"Matrix stored in special format, cannot load as SeqSBAIJ");
  }
