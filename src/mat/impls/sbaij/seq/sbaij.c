...
#undef __FUNCT__  
#define __FUNCT__ "MatRelax_SeqSBAIJ_1_ushort"
PetscErrorCode MatRelax_SeqSBAIJ_1_ushort(Mat A,Vec bb,PetscReal omega,MatSORType flag,PetscReal fshift,PetscInt its,PetscInt lits,Vec xx)
{
  Mat_SeqSBAIJ         *a = (Mat_SeqSBAIJ*)A->data;
...
PetscErrorCode MatAssemblyEnd_SeqSBAIJ(Mat A,MatAssemblyType mode)
{
  ...
  if (A->cmap->n < 65536 && A->cmap->bs == 1) {
    if (!a->jshort) {
      ...
      A->ops->relax = MatRelax_SeqSBAIJ_1_ushort;
    }
  }
  PetscFunctionReturn(0);
}
