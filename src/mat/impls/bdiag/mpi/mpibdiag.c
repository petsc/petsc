#ifndef lint
static char vcid[] = "$Id: mpibdiag.c,v 1.104 1996/12/18 00:06:54 balay Exp balay $";
#endif
/*
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatSetValues_MPIBDiag"
                            ...
                            int *idxn,Scalar *v,InsertMode addv)
{
  ...
  for ( i=0; i<m; i++ ) {
    ...
    if (idxm[i] >= mbd->M) SETERRQ(1,"Row too large");
    if (idxm[i] >= rstart && idxm[i] < rend) {
      row = idxm[i] - rstart;
      for ( j=0; j<n; j++ ) {
        if (idxn[j] < 0) SETERRQ(1,"Negative column");
        if (idxn[j] >= mbd->N) SETERRQ(1,"Column too large");
        if (roworiented) {
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetValues_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatAssemblyBegin_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatAssemblyEnd_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetBlockSize_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatZeroEntries_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatZeroRows_MPIBDiag"
static int MatZeroRows_MPIBDiag(Mat A,IS is,Scalar *diag)
{
  ...
  for ( i=0; i<N; i++ ) {
    ...
    if (!found) SETERRQ(1,"row out of range");
  }
  nsends = 0;  for ( i=0; i<size; i++ ) {nsends += procs[i];} 
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatMult_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatMultAdd_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatMultTrans_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatMultTransAdd_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetInfo_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetDiagonal_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatDestroy_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatView_MPIBDiag_Binary"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatView_MPIBDiag_ASCIIorDraw"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatView_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatSetOption_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetSize_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetLocalSize_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetOwnershipRange_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatGetRow_MPIBDiag"
static int MatGetRow_MPIBDiag(Mat matin,int row,int *nz,int **idx,Scalar **v)
{
  Mat_MPIBDiag *mat = (Mat_MPIBDiag *) matin->data;
  int          lrow;

  if (row < mat->rstart || row >= mat->rend)SETERRQ(1,"only for local rows")
  lrow = row - mat->rstart;
  return MatGetRow(mat->A,lrow,nz,idx,v);
}

#undef __FUNCTION__  
#define __FUNCTION__ "MatRestoreRow_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatNorm_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatPrintHelp_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatScale_MPIBDiag"
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatCreateMPIBDiag"
/*@C
                     ...
                     int *diag,Scalar **diagv,Mat *A)
{
  ...
  *A = 0;
  ...
  if (nd && diag == PETSC_NULL) {
    ...
    if (nd2 != nd)
      SETERRQ(1,"Incompatible number of diags and diagonal vals");
  } else if (flg2) {
    SETERRQ(1,"Must specify number of diagonals with -mat_bdiag_ndiag");
  }

  if (bs <= 0) SETERRQ(1,"Blocksize must be positive");
  if ((N%bs)) SETERRQ(1,"Invalid block size - bad column number");
  ...
  if (M == PETSC_DECIDE) {
    if ((m%bs)) SETERRQ(1,"Invalid block size - bad local row number");
    MPI_Allreduce(&m,&M,1,MPI_INT,MPI_SUM,comm);
  }
  if (m == PETSC_DECIDE) {
    if ((M%bs)) SETERRQ(1,"Invalid block size - bad global row number");
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatBDiagGetData"
/*@C
...
int MatBDiagGetData(Mat mat,int *nd,int *bs,int **diag,int **bdlen,Scalar ***diagv)
{
  ...
  } else SETERRQ(1,"Valid only for MATSEQBDIAG and MATMPIBDIAG formats");
  *nd    = dmat->nd;
  *bs    = dmat->bs;
  *diag  = dmat->diag;
  *bdlen = dmat->bdlen;
...
#undef __FUNCTION__  
#define __FUNCTION__ "MatLoad_MPIBDiag"
