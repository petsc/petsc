...
static int MatSetValues_SeqBDiag_1(Mat A,int m,int *im,int n,int *in,Scalar *v,InsertMode is)
{
  ...
  for ( kk=0; kk<m; kk++ ) { /* loop over added rows */
    ...
    for (j=0; j<n; j++) {
      if (in[j] < 0) SETERRQ(1,0,"Negative col.");
      if (in[j] >= a->n) SETERRQ(1,0,"Col. too large");
...
static int MatGetValues_SeqBDiag_1(Mat A,int m,int *im,int n,int *in,Scalar *v)
{
  ...
  for ( kk=0; kk<m; kk++ ) { /* loop over rows */
    ...
    if (row >= a->m) SETERRQ(1,0,"Row too large");
    for (j=0; j<n; j++) {
      if (in[j] < 0) SETERRQ(1,0,"Negative column");
      if (in[j] >= a->n) SETERRQ(1,0,"Column too large");
      ldiag = row - in[j]; /* diagonal number */
      dfound = 0;
      for (k=0; k<a->nd; k++) {
        if (a->diag[k] == ldiag) {
                             ...
                             double shift,int its,Vec xx)
{
  ...
  if (mainbd == -1) SETERRQ(1,0,"Main diagonal not set");
  dd = a->diagv[mainbd];
...
{ 
  ...
  for (d=0; d<nd; d++) {
    ...
    if (anew->bdlen[d] != a->bdlen[nd-d-1]) SETERRQ(1,0,"Incompatible diagonal lengths");
...
int MatView_SeqBDiag(PetscObject obj,Viewer viewer)
{
  Mat         A = (Mat) obj;
  ViewerType  vtype;
  int         ierr;
...
int MatDestroy_SeqBDiag(PetscObject obj)
{
  Mat          A = (Mat) obj;
  Mat_SeqBDiag *a = (Mat_SeqBDiag *) A->data;
  int          i, bs = a->bs;
...
static int MatGetDiagonal_SeqBDiag_N(Mat A,Vec v)
{
  ...
  if (a->mainbd == -1) SETERRQ(1,0,"Main diagonal not set");
...
int MatZeroRows_SeqBDiag(Mat A,IS is,Scalar *diag)
{
  ...
  if (diag) {
    if (a->mainbd == -1) SETERRQ(1,0,"Main diagonal does not exist");
    dd = a->diagv[a->mainbd];
...
int MatDiagonalScale_SeqBDiag(Mat A,Vec ll,Vec rr)
{
  ...
  if (ll) {
    ...
    if (bs == 1) {
      ...
      PLogFlops(a->nz);
    } else SETERRQ(1,0,"Not yet done for bs>1");
  }
  if (rr) {
