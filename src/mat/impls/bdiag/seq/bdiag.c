...
PetscErrorCode MatAssemblyEnd_SeqBDiag(Mat A,MatAssemblyType mode)
{
  ...
  ierr = PetscInfo((A,"MatAssemblyEnd_SeqBDiag:Number diagonals %D,memory used %D, block size %D\n",a->nd,a->maxnz,A->bs));CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatSetOption_SeqBDiag(Mat A,MatOption op)
{
  ...
  case MAT_USE_HASH_TABLE:
    ierr = PetscInfo((A,"MatSetOption_SeqBDiag:Option ignored\n"));CHKERRQ(ierr);
    break;
  case MAT_SYMMETRIC:
...
PetscErrorCode MatLoad_SeqBDiag(PetscViewer viewer, MatType type,Mat *A)
{
  ...
  */
  ...
  if (extra_rows) {
    ierr = PetscInfo((0,"MatLoad_SeqBDiag:Padding loaded matrix to match blocksize\n"));CHKERRQ(ierr);
  }
