  ...
  for ( kk=0; kk<m; kk++ ) { /* loop over added rows */
    ...
    for (j=0; j<n; j++) {
      ...
      for (k=0; k<a->nd; k++) {
        if (a->diag[k] == ldiag) {
          dfound = 1;
          if (is == ADD_VALUES) a->diagv[k][shift + (in[j]%nb)*nb] += value;
	  else                  a->diagv[k][shift + (in[j]%nb)*nb] = value;
          break;
        }
      }
      if (!dfound) {
        if (a->nonew || a->nonew_diag) {
          ...
          if (diag_new[a->nd] > 0) {
            diagv_new[a->nd] -= nb*nb*diag_new[a->nd];
          }
...
{
  Mat_SeqBDiag *a = (Mat_SeqBDiag *) A->data;
  int          kk, ldiag, shift, row, dfound, j, k, nb = a->nb;
  Scalar       zero = 0.0;
...
{ 
  Mat_SeqBDiag    *a = (Mat_SeqBDiag *) A->data;
  int             nd = a->nd, nb = a->nb, nb_diag,nb_nb = nb*nb;
  int             *a_diag = a->diag,*a_bdlen = a->bdlen;
  Scalar          *vin, *vout,**a_diagv = a->diagv;
  register Scalar *pvin, *pvout, *dv;
  register int    d,  k, len;
  ...
  for (d=0; d<nd; d++) {
    ...
    for (k=0; k<len; k++) {
      Kernel_v_gets_v_plus_A_times_w(nb,pvout,dv,pvin);
      pvout += nb; pvin += nb; dv += nb_nb; 
    }
    PLogFlops(2*nb_nb*len);
  }
  VecRestoreArray(xx,&vin); 
  VecRestoreArray(yy,&vout);
  return 0;
...
{ 
  Mat_SeqBDiag    *a = (Mat_SeqBDiag *) A->data;
  int             ierr, nd = a->nd, nb = a->nb, nb_diag,nb_nb = nb*nb;
  int             *a_diag = a->diag,*a_bdlen = a->bdlen;
  Scalar          *vin, *vout,**a_diagv = a->diagv;
  register Scalar *pvin, *pvout, *dv;
  register int    d,  k, len;
...
{
  Mat_SeqBDiag    *a = (Mat_SeqBDiag *) A->data;
  int             ierr, nd = a->nd, nb = a->nb, diag,kshift, kloc;
  register Scalar *pvin, *pvout, *dv;
  register int    d, i, j, k, len;
  Scalar          *vin, *vout;
                             ...
                             double shift,int its,Vec xx)
{
  ...
  int          i, j, k, d, kbase, nb = a->nb, kloc;
  ...
  if (flag == SOR_APPLY_UPPER) {
    ...
    for ( k=0; k<mblock; k++ ) {
      ...
      for (i=0; i<nb; i++) {
        ...
        for (j=i+1; j<nb; j++) sum += dd[kbase + j*nb + i] * b[kloc + j];
      ...
      for ( k=0; k<mblock; k++ ) {
        ...
        for (i=0; i<nb; i++) {
          ...
          for (d=0; d<mainbd; d++) {
            ...
            if (k >= diag) {
              for (j=0; j<nb; j++)
                sum -= dv[k*nb*nb + j*nb + i] * x[(k-diag)*nb + j];
            }
          }
          for (j=0; j<i; j++){
      ...
      for ( k=0; k<mblock; k++ ) {
        ...
        for (i=0; i<nb; i++) {
          ...
          for (d=0; d<mainbd; d++) {
            ...
            if (bloc >= 0) {
              for (j=0; j<nb; j++)
                sum -= dv[k*nb*nb + j*nb + i] * x[bloc*nb + j];
            }
          }
...
int MatNorm_SeqBDiag_Columns(Mat A,double *tmp,int n)
{
  ...
  if (nb == 1) {
    ...
    for (d=0; d<nd; d++) {
      ...
      if (diag > 0) {	/* lower triangle */
        for (k=0; k<len; k++) {
          kloc = k*nb; kshift = kloc*nb + diag*nb; 
    ...
    *norm = 0.0;
    if (nb == 1) {
      ...
      for (d=0; d<nd; d++) {
        ...
        if (diag > 0) {
          for (k=0; k<len; k++) {
            kloc = k*nb; kshift = kloc*nb + nb*diag; 
            for (i=0; i<nb; i++) {	/* i = local row */
              for (j=0; j<nb; j++) {	/* j = local column */
...
{ 
  ...
  int          nb = a->nb, kshift,shifto,shiftn;
  Scalar       *dwork, *dvnew;
  ...
  ierr = MatCreateSeqBDiag(A->comm,a->n,a->m,nd,nb,diagnew,0,&tmat);CHKERRQ(ierr);
  PetscFree(diagnew);
  ...
  for (d=0; d<nd; d++) {
    ...
    shiftn = anew->diag[d];
    if (shifto > 0)  shifto = nb*nb*shifto; else shifto = 0;
    if (shiftn > 0)  shiftn = nb*nb*shiftn; else shiftn = 0;
    if (nb == 1) {
      ...
      for (k=0; k<anew->bdlen[d]; k++) {
        ...
        for (i=0; i<nb; i++) {	/* i = local row */
          for (j=0; j<nb; j++) {	/* j = local column */
            dvnew[shiftn + kshift + j + i*nb] = dwork[shifto + kshift + j*nb + i];
          }
        }
      }
    }
  ...
  if (matout != PETSC_NULL) {
    *matout = tmat;
    ...
    if (!a->user_alloc) { /* Free the actual diagonals */
      for (i=0; i<a->nd; i++) {
        if (a->diag[i] > 0) {
          PetscFree( a->diagv[i] + nb*nb*a->diag[i]  );
        } else {
          PetscFree( a->diagv[i] );
        }
      }
...
{
  Mat_SeqBDiag *a = (Mat_SeqBDiag *) A->data;
  int          i, j, n, len, ibase, nb = a->nb, iloc;
...
int MatScale_SeqBDiag(Scalar *alpha,Mat inA)
{
  ...
  int          one = 1, i, len, nb = a->nb;

  for (i=0; i<a->nd; i++) {
    len = nb*nb*a->bdlen[i];
    if (a->diag[i] > 0) {
      BLscal_( &len, alpha, a->diagv[i] + nb*nb*a->diag[i], &one );
    } else {
...
static int MatDiagonalScale_SeqBDiag(Mat A,Vec ll,Vec rr)
{
  ...
  if (ll) {
    ...
    if (nb == 1) {
      ...
      PLogFlops(a->nz);
    } else SETERRQ(1,"MatDiagonalScale_SeqBDiag:Not yet done for nb>1");
  }
  if (rr) {
                      ...
                      Scalar **diagv,Mat *A)
{
  ...
  *A = 0;
  ...
  ierr = OptionsGetIntArray(PETSC_NULL,"-mat_bdiag_diags",idiag,&nd2,&flg1);
         CHKERRQ(ierr);
  if (flg1) {
    diag = idiag;
    nd   = nd2;
  ...
  PLogObjectMemory(B,(nda*(nb+2))*sizeof(int) + nb*nda*sizeof(Scalar)
                    + nda*sizeof(Scalar*) + sizeof(Mat_SeqBDiag)
                    + sizeof(struct _Mat) + sizetot*sizeof(Scalar));

  if (!b->user_alloc) {
    for (i=0; i<nd; i++) {
      b->diagv[i] = (Scalar*)PetscMalloc(nb*nb*b->bdlen[i]*sizeof(Scalar));
      CHKPTRQ(b->diagv[i]);
      PetscMemzero(b->diagv[i],nb*nb*b->bdlen[i]*sizeof(Scalar));
    }
...
{ 
  ...
  int          i, ierr, len,diag,nb = a->nb;
  Mat          mat;

  ierr = MatCreateSeqBDiag(A->comm,a->m,a->n,a->nd,nb,a->diag,PETSC_NULL,matout);
  CHKERRQ(ierr);
  ...
  if (cpvalues == COPY_VALUES) {
    for (i=0; i<a->nd; i++) {
      len = a->bdlen[i] * nb * nb * sizeof(Scalar);
      diag = a->diag[i];
      if (diag > 0) {
        PetscMemcpy(newmat->diagv[i]+nb*nb*diag,a->diagv[i]+nb*nb*diag,len);
      } else {
