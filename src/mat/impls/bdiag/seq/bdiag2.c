...
#endif
            ierr = PetscVerboseInfo((A,"MatSetValues_SeqBDiag:Nonzero in diagonal %D that user did not allocate\n",ldiag));CHKERRQ(ierr);
          }
        } else {
          ierr = PetscVerboseInfo((A,"MatSetValues_SeqBDiag: Allocating new diagonal: %D\n",ldiag));CHKERRQ(ierr);
          a->reallocs++;
