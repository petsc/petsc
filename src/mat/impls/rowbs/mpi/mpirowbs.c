...
static PetscErrorCode MatView_MPIRowbs_Binary(Mat mat,PetscViewer viewer)
{
  ...
  if (!rank) {
    ...
    for (i=1; i<size; i++) {
      ...
      ierr = MPI_Recv(cols,maxnz,MPIU_INT,i,((PetscObject)mat)->tag,comm,&status);CHKERRQ(ierr);
      ierr = MPI_Get_count(&status,MPIU_INT,&mnz);CHKERRQ(ierr);
      ierr = PetscBinaryWrite(fd,cols,nz,PETSC_INT,PETSC_FALSE);CHKERRQ(ierr);
    }
...
PetscErrorCode MatGetInfo_MPIRowbs(Mat A,MatInfoType flag,MatInfo *info)
{
  ...
  isend[3] = ((PetscObject)A)->mem;  isend[4] = info->mallocs;
...
/*24*/ MatZeroRows_MPIRowbs,
       0,
       0,
       0,
       0,
/*29*/ MatSetUpPreallocation_MPIRowbs,
       0,
       0,
       0,
       0,
...
/*54*/ 0,
       ...
       0,
       0,
/*59*/ MatGetSubMatrix_MPIRowbs,
       MatDestroy_MPIRowbs,
       MatView_MPIRowbs,
       0,
       MatUseScaledForm_MPIRowbs,
/*64*/ MatScaleSystem_MPIRowbs,
