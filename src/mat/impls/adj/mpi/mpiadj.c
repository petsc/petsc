...
static PetscErrorCode MatCreateSubMatrix_MPIAdj_data(Mat adj,IS irows, IS icols, PetscInt **sadj_xadj,PetscInt **sadj_adjncy,PetscInt **sadj_values)
{
  ...
  CHKERRQ(ISGetLocalSize(irows,&nlrows_is));
  CHKERRQ(ISGetIndices(irows,&irows_indices));
  CHKERRQ(PetscMalloc1(nlrows_is,&iremote));
  /* construct sf graph*/
  nleaves = nlrows_is;
  for (i=0; i<nlrows_is; i++) {
    owner = -1;
    rlocalindex = -1;
    CHKERRQ(PetscLayoutFindOwnerIndex(rmap,irows_indices[i],&owner,&rlocalindex));
    iremote[i].rank  = owner;
    iremote[i].index = rlocalindex;
  }
  CHKERRQ(MatGetRowIJ(adj,0,PETSC_FALSE,PETSC_FALSE,&nlrows_mat,&xadj,&adjncy,&done));
  CHKERRQ(PetscCalloc4(nlrows_mat,&ncols_send,nlrows_is,&xadj_recv,nlrows_is+1,&ncols_recv_offsets,nlrows_is,&ncols_recv));
  nroots = nlrows_mat;
  for (i=0; i<nlrows_mat; i++) {
    ncols_send[i] = xadj[i+1]-xadj[i];
  }
  CHKERRQ(PetscSFCreate(comm,&sf));
  CHKERRQ(PetscSFSetGraph(sf,nroots,nleaves,NULL,PETSC_OWN_POINTER,iremote,PETSC_OWN_POINTER));
  CHKERRQ(PetscSFSetType(sf,PETSCSFBASIC));
  CHKERRQ(PetscSFSetFromOptions(sf));
  CHKERRQ(PetscSFBcastBegin(sf,MPIU_INT,ncols_send,ncols_recv,MPI_REPLACE));
  CHKERRQ(PetscSFBcastEnd(sf,MPIU_INT,ncols_send,ncols_recv,MPI_REPLACE));
  CHKERRQ(PetscSFBcastBegin(sf,MPIU_INT,xadj,xadj_recv,MPI_REPLACE));
  CHKERRQ(PetscSFBcastEnd(sf,MPIU_INT,xadj,xadj_recv,MPI_REPLACE));
  ...
  CHKERRQ(PetscCalloc1(Ncols_recv,&iremote));
  CHKERRQ(PetscCalloc1(Ncols_recv,&adjncy_recv));
  nleaves = Ncols_recv;
  Ncols_recv = 0;
  for (i=0; i<nlrows_is; i++) {
    CHKERRQ(PetscLayoutFindOwner(rmap,irows_indices[i],&owner));
  ...
  if (a->useedgeweights) CHKERRQ(PetscCalloc1(Ncols_recv,&values_recv));
  nroots = Ncols_send;
  CHKERRQ(PetscSFCreate(comm,&sf));
  CHKERRQ(PetscSFSetGraph(sf,nroots,nleaves,NULL,PETSC_OWN_POINTER,iremote,PETSC_OWN_POINTER));
  CHKERRQ(PetscSFSetType(sf,PETSCSFBASIC));
  CHKERRQ(PetscSFSetFromOptions(sf));
  CHKERRQ(PetscSFBcastBegin(sf,MPIU_INT,adjncy,adjncy_recv,MPI_REPLACE));
  CHKERRQ(PetscSFBcastEnd(sf,MPIU_INT,adjncy,adjncy_recv,MPI_REPLACE));
  if (a->useedgeweights) {
    CHKERRQ(PetscSFBcastBegin(sf,MPIU_INT,a->values,values_recv,MPI_REPLACE));
    CHKERRQ(PetscSFBcastEnd(sf,MPIU_INT,a->values,values_recv,MPI_REPLACE));
  }
  CHKERRQ(PetscSFDestroy(&sf));
  CHKERRQ(MatRestoreRowIJ(adj,0,PETSC_FALSE,PETSC_FALSE,&nlrows_mat,&xadj,&adjncy,&done));
  CHKERRQ(ISGetLocalSize(icols,&icols_n));
  CHKERRQ(ISGetIndices(icols,&icols_indices));
  rnclos = 0;
  for (i=0; i<nlrows_is; i++) {
    for (j=ncols_recv_offsets[i]; j<ncols_recv_offsets[i+1]; j++) {
      CHKERRQ(PetscFindInt(adjncy_recv[j], icols_n, icols_indices, &loc));
      if (loc<0) {
        adjncy_recv[j] = -1;
  ...
  CHKERRQ(ISRestoreIndices(icols,&icols_indices));
  CHKERRQ(PetscCalloc1(rnclos,&sadjncy));
  if (a->useedgeweights) CHKERRQ(PetscCalloc1(rnclos,&svalues));
  CHKERRQ(PetscCalloc1(nlrows_is+1,&sxadj));
  rnclos = 0;
  for (i=0; i<nlrows_is; i++) {
    for (j=ncols_recv_offsets[i]; j<ncols_recv_offsets[i+1]; j++) {
  ...
  if (sadj_xadj)  { *sadj_xadj = sxadj;} else    CHKERRQ(PetscFree(sxadj));
  if (sadj_adjncy) { *sadj_adjncy = sadjncy;} else CHKERRQ(PetscFree(sadjncy));
  if (sadj_values) {
    if (a->useedgeweights) *sadj_values = svalues; else *sadj_values=NULL;
  } else {
    if (a->useedgeweights) CHKERRQ(PetscFree(svalues));
  }
  CHKERRQ(PetscFree4(ncols_send,xadj_recv,ncols_recv_offsets,ncols_recv));
  CHKERRQ(PetscFree(adjncy_recv));
  if (a->useedgeweights) CHKERRQ(PetscFree(values_recv));
...
static PetscErrorCode MatCreateSubMatrices_MPIAdj_Private(Mat mat,PetscInt n,const IS irow[],const IS icol[],PetscBool subcomm,MatReuse scall,Mat *submat[])
{
   ...
   */
  for (i=0; i<n; i++) {
    CHKERRQ(ISGetLocalSize(irow[i],&irow_n));
    CHKERRQ(ISGetLocalSize(icol[i],&icol_n));
    nindx = nindx>(irow_n+icol_n)? nindx:(irow_n+icol_n);
  }
  CHKERRQ(PetscMalloc1(nindx,&indices));
  /* construct a submat */
  for (i=0; i<n; i++) {
    if (subcomm) {
      CHKERRQ(PetscObjectGetComm((PetscObject)irow[i],&scomm_row));
      CHKERRQ(PetscObjectGetComm((PetscObject)icol[i],&scomm_col));
      CHKERRMPI(MPI_Comm_compare(scomm_row,scomm_col,&issame));
      ...
      CHKERRMPI(MPI_Comm_compare(scomm_row,PETSC_COMM_SELF,&issame));
    ...
    CHKERRQ(MatCreateSubMatrix_MPIAdj_data(mat,irow[i],icol[i],&sxadj,&sadjncy,&svalues));
    CHKERRQ(ISGetLocalSize(irow[i],&irow_n));
    CHKERRQ(ISGetLocalSize(icol[i],&icol_n));
    CHKERRQ(ISGetIndices(irow[i],&irow_indices));
    CHKERRQ(PetscArraycpy(indices,irow_indices,irow_n));
    CHKERRQ(ISRestoreIndices(irow[i],&irow_indices));
    CHKERRQ(ISGetIndices(icol[i],&icol_indices));
    CHKERRQ(PetscArraycpy(indices+irow_n,icol_indices,icol_n));
    CHKERRQ(ISRestoreIndices(icol[i],&icol_indices));
    nindx = irow_n+icol_n;
    CHKERRQ(PetscSortRemoveDupsInt(&nindx,indices));
    /* renumber columns */
    for (j=0; j<irow_n; j++) {
      for (k=sxadj[j]; k<sxadj[j+1]; k++) {
        CHKERRQ(PetscFindInt(sadjncy[k],nindx,indices,&loc));
        PetscCheckFalse(loc<0,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"can not find col %" PetscInt_FMT,sadjncy[k]);
        sadjncy[k] = loc;
      }
    }
    if (scall==MAT_INITIAL_MATRIX) {
      CHKERRQ(MatCreateMPIAdj(scomm_row,irow_n,icol_n,sxadj,sadjncy,svalues,submat[i]));
    } else {
       Mat                sadj = *(submat[i]);
       Mat_MPIAdj         *sa  = (Mat_MPIAdj*)((sadj)->data);
       CHKERRQ(PetscObjectGetComm((PetscObject)sadj,&scomm_mat));
       CHKERRMPI(MPI_Comm_compare(scomm_row,scomm_mat,&issame));
       ...
       CHKERRQ(PetscArraycpy(sa->i,sxadj,irow_n+1));
       CHKERRQ(PetscArraycpy(sa->j,sadjncy,sxadj[irow_n]));
       if (svalues) CHKERRQ(PetscArraycpy(sa->values,svalues,sxadj[irow_n]));
       CHKERRQ(PetscFree(sxadj));
       CHKERRQ(PetscFree(sadjncy));
       if (svalues) CHKERRQ(PetscFree(svalues));
    }
  }
...
static PetscErrorCode MatCreateSubMatrices_MPIAdj(Mat mat,PetscInt n,const IS irow[],const IS icol[],MatReuse scall,Mat *submat[])
{
  ...
  CHKERRQ(MatCreateSubMatrices_MPIAdj_Private(mat,n,irow,icol,PETSC_FALSE,scall,submat));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatView_MPIAdj_ASCII(Mat A,PetscViewer viewer)
{
  ...
  if (format == PETSC_VIEWER_ASCII_INFO) {
    ...
    for (i=0; i<m; i++) {
      CHKERRQ(PetscViewerASCIISynchronizedPrintf(viewer,"row %" PetscInt_FMT ":",i+A->rmap->rstart));
      for (j=a->i[i]; j<a->i[i+1]; j++) {
        if (a->values) {
          CHKERRQ(PetscViewerASCIISynchronizedPrintf(viewer," (%" PetscInt_FMT ", %" PetscInt_FMT ") ",a->j[j], a->values[j]));
        } else {
          CHKERRQ(PetscViewerASCIISynchronizedPrintf(viewer," %" PetscInt_FMT " ",a->j[j]));
        }
      }
...
static PetscErrorCode MatView_MPIAdj(Mat A,PetscViewer viewer)
{
  ...
  if (iascii) {
    CHKERRQ(MatView_MPIAdj_ASCII(A,viewer));
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatDestroy_MPIAdj(Mat mat)
{
  ...
  if (a->freeaij) {
    if (a->freeaijwithfree) {
      ...
      CHKERRQ(PetscFree(a->i));
      CHKERRQ(PetscFree(a->j));
      CHKERRQ(PetscFree(a->values));
    }
  }
  CHKERRQ(PetscFree(a->rowvalues));
  CHKERRQ(PetscFree(mat->data));
  CHKERRQ(PetscObjectChangeTypeName((PetscObject)mat,NULL));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)mat,"MatMPIAdjSetPreallocation_C",NULL));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)mat,"MatMPIAdjCreateNonemptySubcommMat_C",NULL));
  PetscFunctionReturn(0);
}
  ...
  *nz = a->i[row+1] - a->i[row];
  if (v) {
    ...
    if (a->rowvalues_alloc < *nz) {
      ...
      CHKERRQ(PetscMalloc1(a->rowvalues_alloc,&a->rowvalues));
    }
    for (j=0; j<*nz; j++) {
...
static PetscErrorCode MatEqual_MPIAdj(Mat A,Mat B,PetscBool * flg)
{
  ...
  CHKERRQ(PetscArraycmp(a->i,b->i,A->rmap->n+1,&flag));

  /* if a->j are the same */
  CHKERRQ(PetscMemcmp(a->j,b->j,(a->nz)*sizeof(PetscInt),&flag));

  CHKERRMPI(MPIU_Allreduce(&flag,flg,1,MPIU_BOOL,MPI_LAND,PetscObjectComm((PetscObject)A)));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatRestoreRowIJ_MPIAdj(Mat A,PetscInt oshift,PetscBool symmetric,PetscBool blockcompressed,PetscInt *m,const PetscInt *inia[],const PetscInt *inja[],PetscBool  *done)
{
  ...
  if (oshift) {
    PetscCheckFalse(!ia,PetscObjectComm((PetscObject)A),PETSC_ERR_ARG_WRONGSTATE,"If oshift then you must passed in inia[] argument");
    PetscCheckFalse(!ja,PetscObjectComm((PetscObject)A),PETSC_ERR_ARG_WRONGSTATE,"If oshift then you must passed in inja[] argument");
    for (i=0; i<=(*m); i++) (*ia)[i]--;
    for (i=0; i<(*ia)[*m]; i++) {
      (*ja)[i]--;
    }
...
PetscErrorCode  MatConvertFrom_MPIAdj(Mat A,MatType type,MatReuse reuse,Mat *newmat)
{
  ...
  for (i=0; i<m; i++) {
    CHKERRQ(MatGetRow(A,i+rstart,&len,&rj,NULL));
    for (j=0; j<len; j++) {
      if (rj[j] == i+rstart) {len--; break;}    /* don't count diagonal */
    }
    nzeros += len;
    CHKERRQ(MatRestoreRow(A,i+rstart,&len,&rj,NULL));
  }

  /* malloc space for nonzeros */
  CHKERRQ(PetscMalloc1(nzeros+1,&a));
  CHKERRQ(PetscMalloc1(N+1,&ia));
  CHKERRQ(PetscMalloc1(nzeros+1,&ja));

  nzeros = 0;
  ia[0]  = 0;
  for (i=0; i<m; i++) {
    CHKERRQ(MatGetRow(A,i+rstart,&len,&rj,&ra));
    cnt  = 0;
    for (j=0; j<len; j++) {
    ...
    CHKERRQ(MatRestoreRow(A,i+rstart,&len,&rj,&ra));
    nzeros += cnt;
    ia[i+1] = nzeros;
  }
  ...
  CHKERRQ(MatSetSizes(B,m,PETSC_DETERMINE,PETSC_DETERMINE,N));
  CHKERRQ(MatSetType(B,type));
  CHKERRQ(MatMPIAdjSetPreallocation(B,ia,ja,a));
...
static PetscErrorCode  MatMPIAdjSetPreallocation_MPIAdj(Mat B,PetscInt *i,PetscInt *j,PetscInt *values)
{
  ...
  CHKERRMPI(MPIU_Allreduce((int*)&useedgeweights,(int*)&b->useedgeweights,1,MPI_INT,MPI_MAX,PetscObjectComm((PetscObject)B)));
...
static PetscErrorCode MatMPIAdjCreateNonemptySubcommMat_MPIAdj(Mat A,Mat *B)
{
  ...
  PetscFunctionBegin;
  *B    = NULL;
  CHKERRQ(PetscObjectGetComm((PetscObject)A,&acomm));
  CHKERRMPI(MPI_Comm_size(acomm,&size));
  CHKERRMPI(MPI_Comm_size(acomm,&rank));
  ...
  CHKERRQ(PetscMalloc1(nranks,&ranks));
  for (i=0,nranks=0; i<size; i++) {
    if (ranges[i+1] - ranges[i] > 0) ranks[nranks++] = i;
  }
  CHKERRMPI(MPI_Comm_group(acomm,&agroup));
  CHKERRMPI(MPI_Group_incl(agroup,nranks,ranks,&bgroup));
  CHKERRQ(PetscFree(ranks));
  CHKERRMPI(MPI_Comm_create(acomm,bgroup,&bcomm));
  CHKERRMPI(MPI_Group_free(&agroup));
  CHKERRMPI(MPI_Group_free(&bgroup));
  if (bcomm != MPI_COMM_NULL) {
    PetscInt   m,N;
    Mat_MPIAdj *b;
    CHKERRQ(MatGetLocalSize(A,&m,NULL));
    CHKERRQ(MatGetSize(A,NULL,&N));
    CHKERRQ(MatCreateMPIAdj(bcomm,m,N,a->i,a->j,a->values,B));
    b          = (Mat_MPIAdj*)(*B)->data;
    b->freeaij = PETSC_FALSE;
    CHKERRMPI(MPI_Comm_free(&bcomm));
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode  MatMPIAdjToSeq_MPIAdj(Mat A,Mat *B)
{
  ...
  CHKERRMPI(MPI_Allreduce(&nz,&NZ,1,MPIU_INT,MPI_SUM,PetscObjectComm((PetscObject)A)));

  CHKERRQ(PetscMPIIntCast(nz,&mnz));
  CHKERRQ(PetscMalloc2(size,&allnz,size,&dispnz));
  CHKERRMPI(MPI_Allgather(&mnz,1,MPI_INT,allnz,1,MPI_INT,PetscObjectComm((PetscObject)A)));
  dispnz[0] = 0; for (i=1; i<size; i++) dispnz[i] = dispnz[i-1]+ allnz[i-1];
  if (adj->values) {
    CHKERRQ(PetscMalloc1(NZ,&Values));
    CHKERRMPI(MPI_Allgatherv(adj->values,mnz,MPIU_INT,Values,allnz,dispnz,MPIU_INT,PetscObjectComm((PetscObject)A)));
  }
  CHKERRQ(PetscMalloc1(NZ,&J));
  CHKERRMPI(MPI_Allgatherv(adj->j,mnz,MPIU_INT,J,allnz,dispnz,MPIU_INT,PetscObjectComm((PetscObject)A)));
  CHKERRQ(PetscFree2(allnz,dispnz));
  CHKERRMPI(MPI_Scan(&nz,&nzstart,1,MPIU_INT,MPI_SUM,PetscObjectComm((PetscObject)A)));
  nzstart -= nz;
  /* shift the i[] values so they will be correct after being received */
  for (i=0; i<m; i++) adj->i[i] += nzstart;
  CHKERRQ(PetscMalloc1(M+1,&II));
  CHKERRQ(PetscMPIIntCast(m,&mm));
  CHKERRQ(PetscMalloc2(size,&allm,size,&dispm));
  CHKERRMPI(MPI_Allgather(&mm,1,MPI_INT,allm,1,MPI_INT,PetscObjectComm((PetscObject)A)));
  dispm[0] = 0; for (i=1; i<size; i++) dispm[i] = dispm[i-1]+ allm[i-1];
  CHKERRMPI(MPI_Allgatherv(adj->i,mm,MPIU_INT,II,allm,dispm,MPIU_INT,PetscObjectComm((PetscObject)A)));
  CHKERRQ(PetscFree2(allm,dispm));
  II[M] = NZ;
  /* shift the i[] values back */
  for (i=0; i<m; i++) adj->i[i] -= nzstart;
  CHKERRQ(MatCreateMPIAdj(PETSC_COMM_SELF,M,N,II,J,Values,B));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMPIAdjCreateNonemptySubcommMat(Mat A,Mat *B)
{
  ...
  CHKERRQ(PetscUseMethod(A,"MatMPIAdjCreateNonemptySubcommMat_C",(Mat,Mat*),(A,B)));
  PetscFunctionReturn(0);
}
...
PETSC_EXTERN PetscErrorCode MatCreate_MPIAdj(Mat B)
{
  ...
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatMPIAdjSetPreallocation_C",MatMPIAdjSetPreallocation_MPIAdj));
  ...
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatMPIAdjToSeq_C",MatMPIAdjToSeq_MPIAdj));
  CHKERRQ(PetscObjectChangeTypeName((PetscObject)B,MATMPIADJ));
  PetscFunctionReturn(0);
}
...
PetscErrorCode  MatCreateMPIAdj(MPI_Comm comm,PetscInt m,PetscInt N,PetscInt *i,PetscInt *j,PetscInt *values,Mat *A)
{
  ...
  CHKERRQ(MatSetSizes(*A,m,PETSC_DETERMINE,PETSC_DETERMINE,N));
  CHKERRQ(MatSetType(*A,MATMPIADJ));
  CHKERRQ(MatMPIAdjSetPreallocation(*A,i,j,values));
  PetscFunctionReturn(0);
}
