...
static PetscErrorCode MatLoad_BlockMat(Mat newmat, PetscViewer viewer)
{
  ...
  for (i=0; i<mbs; i++) { /* loops for block rows */
    ...
    while (PETSC_TRUE) {  /* loops over blocks in block row */
      ...
      if (cnt >= amat->maxnz) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Number of blocks found greater than expected %" PetscInt_FMT,cnt);
      if (!flg || currentcol >= i) {
...
static PetscErrorCode MatAssemblyEnd_BlockMat(Mat A,MatAssemblyType mode)
{
  ...
  a->nz = ai[m];
  for (i=0; i<a->nz; i++) {
    if (PetscUnlikelyDebug(!aa[i])) SETERRQ3(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Null matrix at location %" PetscInt_FMT " column %" PetscInt_FMT " nz %" PetscInt_FMT,i,aj[i],a->nz);
