  ...
  if (size == 1) {
    ...
    if (type == NORM_FROBENIUS) {
      ...
      *nrm = 0.0;
      ...
      PetscCallMPI(MPIU_Allreduce(tmp,tmp2,A->cmap->N,MPIU_REAL,MPIU_SUM,PetscObjectComm((PetscObject)A)));
...
PetscErrorCode MatGetColumnReductions_MPIDense(Mat A,PetscInt type,PetscReal *reductions)
{
  ...
  if (type == NORM_INFINITY) {
    PetscCallMPI(MPIU_Allreduce(work,reductions,n,MPIU_REAL,MPIU_MAX,A->hdr.comm));
  } else {
    PetscCallMPI(MPIU_Allreduce(work,reductions,n,MPIU_REAL,MPIU_SUM,A->hdr.comm));
  }
...
PetscErrorCode MatDenseGetSubMatrix_MPIDense(Mat A,PetscInt cbegin,PetscInt cend,Mat *v)
{
  ...
  if (!a->cmat) {
    ...
    PetscCall(PetscLayoutReference(A->rmap,&a->cmat->rmap));
...
PetscErrorCode  MatCreateDense(MPI_Comm comm,PetscInt m,PetscInt n,PetscInt M,PetscInt N,PetscScalar *data,Mat *A)
{
  ...
  if (size > 1) {
    ...
    PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE,&havedata,1,MPIU_BOOL,MPI_LOR,comm));
