  ...
  if (size == 1) { /* sequential case */
    ...
    case 1:
      ...
      if (fin) {
        ...
        PetscCall(VecCreateMPIWithArray(comm, 1, n1, N1, (const PetscScalar *)data_fin, fin));
      ...
      if (bout) {
        ...
        PetscCall(PetscIntCast(local_n1, &n1));
        PetscCall(PetscIntCast(fft->N, &N1));
        PetscCall(VecCreateMPIWithArray(comm, 1, n1, N1, (const PetscScalar *)data_bout, bout));
...
static PetscErrorCode VecScatterPetscToFFTW_FFTW(Mat A, Vec x, Vec y)
{
  ...
  if (size == 1) {
    ...
    Mat_FFTW *fftw = (Mat_FFTW *)fft->data;
    PetscInt  ndim = fft->ndim, *dim = fft->dim, n1;
    ...
    case 1:
      ...
      PetscCall(ISCreateStride(comm, n1, nstart, 1, &list1));
      PetscCall(ISCreateStride(comm, n1, nlow, 1, &list2));
    ...
    case 2:
      ...
      PetscCall(PetscIntCast(local_0_start * dim[1], &nstart));
      PetscCall(PetscIntCast(low, &nlow));
      PetscCall(ISCreateStride(comm, n1, nstart, 1, &list1));
      PetscCall(ISCreateStride(comm, n1, nlow, 1, &list2));
      PetscCall(VecScatterCreate(x, list1, y, list2, &vecscat));
      ...
      for (i = 0; i < local_n0; i++) {
        for (j = 0; j < dim[1]; j++) {
          ...
          PetscCall(PetscIntCast(local_0_start * dim[1] + tempindx, &indx1[tempindx]));
    ...
    case 3:
      ...
      PetscCall(PetscIntCast(local_0_start * dim[1] * dim[2], &nstart));
      PetscCall(PetscIntCast(low, &nlow));
      PetscCall(ISCreateStride(comm, n1, nstart, 1, &list1));
      PetscCall(ISCreateStride(comm, n1, nlow, 1, &list2));
      PetscCall(VecScatterCreate(x, list1, y, list2, &vecscat));
      ...
      for (i = 0; i < local_n0; i++) {
        for (j = 0; j < dim[1]; j++) {
          for (k = 0; k < dim[2]; k++) {
            ...
            PetscCall(PetscIntCast(local_0_start * dim[1] * dim[2] + tempindx, &indx1[tempindx]));
            PetscCall(PetscIntCast(low + tempindx1, &indx2[tempindx]));
          }
        }
      }
    ...
    default:
      ...
      PetscCall(PetscIntCast(local_0_start * (fftw->partial_dim), &nstart));
      PetscCall(PetscIntCast(low, &nlow));
      PetscCall(ISCreateStride(comm, n1, nstart, 1, &list1));
      PetscCall(ISCreateStride(comm, n1, nlow, 1, &list2));
      PetscCall(VecScatterCreate(x, list1, y, list2, &vecscat));
...
static PetscErrorCode VecScatterFFTWToPetsc_FFTW(Mat A, Vec x, Vec y)
{
  ...
  if (size == 1) {
    ...
    case 1:
      ...
      PetscCall(PetscIntCast(local_n1, &n1));
      PetscCall(PetscIntCast(local_1_start, &nstart));
      PetscCall(PetscIntCast(low, &nlow));
      PetscCall(ISCreateStride(comm, n1, nstart, 1, &list1));
      PetscCall(ISCreateStride(comm, n1, nlow, 1, &list2));
      PetscCall(VecScatterCreate(x, list1, y, list2, &vecscat));
    ...
    default:
      ...
      j = low;
      for (i = 0, k = 1; i < (PetscInt)local_n0 * partial_dim; i++, k++) {
        PetscCall(PetscIntCast(local_0_start * partial_dim + i, &indx1[i]));
        indx2[i] = j;
        if (k % dim[ndim - 1] == 0) j += NM;
        j++;
      }
...
PETSC_EXTERN PetscErrorCode MatCreate_FFTW(Mat A)
{
  ...
  #else
      fftw_mpi_local_size_1d(dim[0], comm, FFTW_FORWARD, FFTW_ESTIMATE, &local_n0, &local_0_start, &local_n1, &local_1_start);
      PetscCall(PetscIntCast(local_n0, &fft->n));
      PetscCall(PetscIntCast(local_n1, &n1));
      PetscCall(MatSetSizes(A, n1, fft->n, fft->N, fft->N));
  #endif
      break;
    case 2:
  #if defined(PETSC_USE_COMPLEX)
