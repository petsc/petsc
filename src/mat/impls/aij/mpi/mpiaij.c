...
static char vcid[] = "$Id: mpiaij.c,v 1.222 1997/10/28 14:22:37 bsmith Exp bsmith $";
#endif
...
int MatGetDiagonal_MPIAIJ(Mat A,Vec v)
{
  ...
  PetscFunctionBegin;
  if (a->M != a->N) SETERRQ(1,0,"Supports only square matrix where A->A is diag block");
...
int MatLoad_MPIAIJ(Viewer viewer,MatType type,Mat *newmat)
{
  ...
  if (!rank) {
    ...
    if (header[3] < 0) {
      SETERRQ(1,1,"Matrix stored in special format on disk, cannot load as MPIAIJ");
    }
  }
