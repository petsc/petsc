...
PetscErrorCode MatSetValues_MPIAIJ(Mat mat, PetscInt m, const PetscInt im[], PetscInt n, const PetscInt in[], const PetscScalar v[], InsertMode addv)
{
  ...
  const PetscBool ignorezeroentries = (PetscBool)(!mat->structure_only && a->ignorezeroentries);
  Mat             B                 = aij->B;
...
PetscErrorCode MatAssemblyEnd_MPIAIJ(Mat mat, MatAssemblyType mode)
{
  ...
  */
  ...
  if ((!mat->was_assembled && mode == MAT_FINAL_ASSEMBLY) || !((Mat_SeqAIJ *)aij->A->data)->nonew) {
    mat->nonzerostate = aij->A->nonzerostate + aij->B->nonzerostate;
...
static PetscErrorCode MatZeroRows_MPIAIJ(Mat A, PetscInt N, const PetscInt rows[], PetscScalar diag, Vec x, Vec b)
{
  ...
  if (!((Mat_SeqAIJ *)mat->A->data)->keepnonzeropattern || !((Mat_SeqAIJ *)mat->A->data)->nonew) {
    A->nonzerostate = mat->A->nonzerostate + mat->B->nonzerostate;
...
static PetscErrorCode MatIsTranspose_MPIAIJ(Mat Amat, Mat Bmat, PetscReal tol, PetscBool *f)
{
  ...
  PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE, f, 1, MPI_C_BOOL, MPI_LAND, PetscObjectComm((PetscObject)Amat)));
  ...
  if (aij->size == 1) {
    ...
    if (type == NORM_FROBENIUS) {
      ...
      *norm = 0.0;
      ...
      PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE, norm, 1, MPIU_REAL, MPIU_MAX, PetscObjectComm((PetscObject)mat)));
...
PetscErrorCode MatCreateSubMatrix_MPIAIJ(Mat mat, IS isrow, IS iscol, MatReuse call, Mat *newmat)
{
  ...
  PetscBool sameRowDist = PETSC_FALSE, tsameDist[2];
  MPI_Comm  comm;
...
PetscErrorCode MatMPIAIJGetLocalMat_Private(Mat A, MatReuse scall, PetscBool structure_only, Mat *A_loc)
{
  ...
  if (scall == MAT_INITIAL_MATRIX) {
    ...
    if (!structure_only) PetscCall(PetscMalloc1(ci[am], &ca));
    k = 0;
    for (i = 0; i < am; i++) {
      ncols_o = bi[i + 1] - bi[i];
      ncols_d = ai[i + 1] - ai[i];
...
PetscErrorCode MatSetPreallocationCOO_MPIAIJ(Mat mat, PetscCount coo_n, PetscInt coo_i[], PetscInt coo_j[])
{
  ...
  */
  ...
  PetscCall(MatSetOption(mpiaij->A, MAT_STRUCTURE_ONLY, mat->structure_only));
  ...
  PetscCall(MatSetOption(mpiaij->B, MAT_STRUCTURE_ONLY, mat->structure_only));
  ...
  mat->nonzerostate  = mpiaij->A->nonzerostate + mpiaij->B->nonzerostate;
  PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE, &mat->nonzerostate, 1, MPIU_INT64, MPI_SUM, PetscObjectComm((PetscObject)mat)));
