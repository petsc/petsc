...
PetscErrorCode MatZeroRows_MPIAIJ(Mat A,IS is,const PetscScalar *diag)
{
  ...
  PetscInt       *lens,*lrows,*values,rstart=l->rstart;
...
PetscErrorCode MatView_MPIAIJ_Binary(Mat mat,PetscViewer viewer)
{
  ...
  PetscInt          nzmax,*column_indices,j,k,col,*garray = aij->garray,cnt,cstart = aij->cstart,rnz;
  ...
  if (!rank) {
    MPI_Status status;
    ierr = PetscBinaryWrite(fd,row_lengths,mat->m,PETSC_INT,PETSC_TRUE);CHKERRQ(ierr);
    for (i=1; i<size; i++) {
    ...
    ierr = MPI_Send(row_lengths,mat->m,MPIU_INT,0,tag,mat->comm);CHKERRQ(ierr);
  }
...
PetscErrorCode MatGetRow_MPIAIJ(Mat matin,PetscInt row,PetscInt *nz,PetscInt **idx,PetscScalar **v)
{
  ...
  PetscErrorCode ierr;
  PetscInt       i,*cworkA,*cworkB,**pcA,**pcB,cstart = mat->cstart;
  PetscInt       nztot,nzA,nzB,lrow,rstart = mat->rstart,rend = mat->rend;
  PetscInt       *cmap,*idx_p;
...
PetscErrorCode MatNorm_MPIAIJ(Mat mat,NormType type,PetscReal *norm)
{
  ...
  PetscErrorCode ierr;
  PetscInt       i,j,cstart = aij->cstart;
  PetscReal      sum = 0.0;
  PetscScalar    *v;
...
{ 
  ...
  PetscErrorCode ierr;
  PetscInt       M = A->M,N = A->N,m,*ai,*aj,row,*cols,i,*ct;
  Mat            B;
  PetscScalar    *array;
...
PetscErrorCode MatDuplicate_MPIAIJ(Mat matin,MatDuplicateOption cpvalues,Mat *newmat)
{
  ...
  *newmat       = 0;
  ...
  ierr       = PetscMemcpy(a->rowners,oldmat->rowners,2*(a->size+2)*sizeof(PetscInt));CHKERRQ(ierr);
...
PetscErrorCode MatSetColoring_MPIAIJ(Mat A,ISColoring coloring)
{
  ...
  if (coloring->ctype == IS_COLORING_LOCAL) {
    ...
    /* set coloring for diagonal portion */
    ierr = PetscMalloc((a->A->n+1)*sizeof(PetscInt),&larray);CHKERRQ(ierr);
    for (i=0; i<a->A->n; i++) {
      larray[i] = i + a->cstart;
    }
    ...
    /* set coloring for off-diagonal portion */
    ierr = PetscMalloc((a->B->n+1)*sizeof(PetscInt),&larray);CHKERRQ(ierr);
...
PetscErrorCode MatMerge(MPI_Comm comm,Mat inmat,PetscInt n,MatReuse scall,Mat *outmat)
{
  ...
  */
  if (scall == MAT_INITIAL_MATRIX){ 
    ...
    for (i=0;i<m;i++) {
      ierr = MatGetRow(inmat,i,&nnz,&indx,PETSC_NULL);CHKERRQ(ierr);
      ierr = MatPreallocateSet(i+rstart,nnz,indx,dnz,onz);CHKERRQ(ierr);
      ierr = MatRestoreRow(inmat,i,&nnz,&indx,PETSC_NULL);CHKERRQ(ierr);
    }
...
PetscErrorCode MatMerge_SeqsToMPINumeric(Mat seqmat,Mat mpimat) 
{
  ...
  PetscInt             N=mpimat->N,i,j,*owners,*ai=a->i,*aj=a->j; 
  PetscInt             proc,m;
...
PetscErrorCode MatMerge_SeqsToMPISymbolic(MPI_Comm comm,Mat seqmat,PetscInt m,PetscInt n,Mat *mpimat) 
{
  ...
  PetscInt             M=seqmat->m,N=seqmat->n,i,*owners,*ai=a->i,*aj=a->j;
...
PetscErrorCode MatGetLocalMat(Mat A,MatReuse scall,Mat *A_loc) 
{
  ...
  if (scall == MAT_INITIAL_MATRIX){
    ...
    nnz_max = 0;
    for (i=0; i<am; i++){
      nnz[i] = (ai[i+1] - ai[i]) + (bi[i+1] - bi[i]);
      if (nnz[i] > nnz_max) nnz_max = nnz[i];
    }
...
-    scall - either MAT_INITIAL_MATRIX or MAT_REUSE_MATRIX
   ...
   Output Parameter:
+    Bseq - the sequential matrix generated

    Level: developer

@*/
PetscErrorCode MatGetBrowsOfAoCols(Mat A,Mat B,MatReuse scall,PetscInt **startsj,PetscScalar **bufaa,Mat *Both) 
{
  ...
  PetscMPIInt       *rprocs,*sprocs,tagi=ctx->tag,tagj,taga,rank; 
  PetscInt          *rowlen,*bufj,*bufJ,ncols,aBn;
  PetscScalar       *rvalues,*svalues,*botha,*bufa,*bufA;
  PetscInt          i,j,k,nrecvs,nsends,count,*rrow,*srow,*rstarts,*rstartsj,*sstarts,*sstartsj,len;
  MPI_Request       *rwaits,*swaits;
  MPI_Status        *rstatus,*sstatus;
  PetscInt          row,*bothi,*bothj;
  const PetscInt    *cols;
  ...
  if (scall == MAT_INITIAL_MATRIX){
    /* printf("[%d] create Both \n",rank); */
  } else if (scall == MAT_REUSE_MATRIX){
    /* printf("[%d] Both exit \n",rank); */
    ierr = MatDestroy(*Both);
  } else {
  ...
  for (i=0; i<nrecvs; i++){
    ...
    count = rstarts[i+1]-rstarts[i];
    ierr = MPI_Irecv(rowlen,count,MPIU_INT,rprocs[i],tagi,comm,rwaits+i);CHKERRQ(ierr);
  }

  /* pack the outgoing message */
  ierr = PetscMalloc((nsends+nrecvs+3)*sizeof(PetscInt),startsj);CHKERRQ(ierr); 
  sstartsj = *startsj;
  ...
  for (i=0; i<nsends; i++){
    ...
    count = sstarts[i+1]-sstarts[i]; /* num of rows */
    for (j=0; j<count; j++) {
      row = srow[k] + b->rowners[rank]; /* global row idx */
      ierr = MatGetRow(B,row,&rowlen[j],PETSC_NULL,PETSC_NULL);CHKERRQ(ierr); /* rowlength */
      len += rowlen[j];  
      ierr = MatRestoreRow(B,row,&ncols,PETSC_NULL,PETSC_NULL);CHKERRQ(ierr);
      k++;
    } 
    ierr = MPI_Isend(rowlen,count,MPIU_INT,sprocs[i],tagi,comm,swaits+i);CHKERRQ(ierr);
    sstartsj[i+1] = len;  /* starting point of i-th outgoing msg in bufj and bufa */
  }
  /* sends and recvs of i-array are completed */
  if (nsends) {
  ...
  /* create i-array of Both */
  aBn = a->B->n;
  if (rstarts[nrecvs] != aBn) SETERRQ2(1,"rstarts[nrecvs]=%d != aBn: %d",rstarts[nrecvs],aBn);
  ierr = PetscMalloc((aBn+2)*sizeof(PetscInt),&bothi);CHKERRQ(ierr);
  bothi[0] = 0;
  ...
  for (i=0; i<nrecvs; i++){
    ...
    count = rstarts[i+1]-rstarts[i];
    for (j=0; j<count; j++) {
      bothi[k+1] = bothi[k] + rowlen[j]; 
      len += rowlen[j]; k++;
    }
    rstartsj[i+1] = len;
  }

  /* create j and a arrays of Both */
  ierr = PetscMalloc((bothi[aBn]+1)*sizeof(PetscInt),&bothj);CHKERRQ(ierr);
  ierr = PetscMalloc((bothi[aBn]+1)*sizeof(PetscScalar),&botha);CHKERRQ(ierr);

  /* j and a array */
  /*---------------*/
  ierr = PetscObjectGetNewTag((PetscObject)A,&tagj);CHKERRQ(ierr);
  ierr = PetscObjectGetNewTag((PetscObject)A,&taga);CHKERRQ(ierr);

  /*  post receives of j-array */
  for (i=0; i<nrecvs; i++){
    count = rstartsj[i+1]-rstartsj[i]; /* length of the msg received */
    ierr = MPI_Irecv(bothj+rstartsj[i],count,MPIU_INT,rprocs[i],tagj,comm,rwaits+i);CHKERRQ(ierr);
  }
  ...
  for (i=0; i<nsends; i++){
    ...
    for (j=0; j<count; j++) {
      row  = srow[k] + b->rowners[rank]; /* global row idx */
      ierr = MatGetRow(B,row,&ncols,&cols,&vals);CHKERRQ(ierr);
      for (l=0; l<ncols; l++){
        *bufJ++ = cols[l];
        *bufA++ = vals[l]; 
      }
      ierr = MatRestoreRow(B,row,&ncols,&cols,&vals);CHKERRQ(ierr);  
      k++;
    }
    ierr = MPI_Isend(bufj+sstartsj[i],sstartsj[i+1]-sstartsj[i],MPIU_INT,sprocs[i],tagj,comm,swaits+i);CHKERRQ(ierr); 
  }
  /* sends and recvs of j-array are completed */
  if (nsends) {
    ierr = MPI_Waitall(nsends,swaits,sstatus);CHKERRQ(ierr);
  }  
  if (nrecvs){
  ...
  for (i=0; i<nrecvs; i++){
    count = rstartsj[i+1]-rstartsj[i]; /* length of the msg received */
    ierr = MPI_Irecv(botha+rstartsj[i],count,MPIU_SCALAR,rprocs[i],taga,comm,rwaits+i);CHKERRQ(ierr);
  }
  k = 0; 
  for (i=0; i<nsends; i++){
    count = sstartsj[i+1]-sstartsj[i];
    ierr = MPI_Isend(bufa+sstartsj[i],count,MPIU_SCALAR,sprocs[i],taga,comm,swaits+i);CHKERRQ(ierr); 
  }
  /* sends and recvs of a-array are completed */
  if (nsends) {
    ierr = MPI_Waitall(nsends,swaits,sstatus);CHKERRQ(ierr);
  }  
  if (nrecvs){
  ...
  ierr = MatCreateSeqAIJWithArrays(PETSC_COMM_SELF,aBn,B->N,bothi,bothj,botha,Both);CHKERRQ(ierr);

  /* MatCreateSeqAIJWithArrays flags matrix so PETSc doesn't free the user's arrays. */
  /* Since these are PETSc arrays, change flags to free them as necessary. */
  both = (Mat_SeqAIJ *)(*Both)->data;
  both->freedata = PETSC_TRUE;
  both->nonew    = 0;
