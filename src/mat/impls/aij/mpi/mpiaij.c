...
{ 
  ...
  ierr = PetscLogInfo((aij->A,"MatAssemblyBegin_MPIAIJ:Stash has %D entries, uses %D mallocs.\n",nstash,reallocs));CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatSetOption_MPIAIJ(Mat A,MatOption op)
{
  ...
  case MAT_YES_NEW_DIAGONALS:
    ierr = PetscLogInfo((A,"MatSetOption_MPIAIJ:Option ignored\n"));CHKERRQ(ierr);
    break;
...
PetscErrorCode MatFileSplit(Mat A,char *outfile)
{
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_SELF,name,FILE_MODE_WRITE,&out);CHKERRQ(ierr);
  ierr = PetscFree(name);
