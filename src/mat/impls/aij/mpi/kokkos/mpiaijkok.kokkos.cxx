...
PetscErrorCode MatMPIAIJSetPreallocation_MPIAIJKokkos(Mat mat,PetscInt d_nz,const PetscInt d_nnz[],PetscInt o_nz,const PetscInt o_nnz[])
{
  ...
  if (!mpiaij->A) {
    CHKERRQ(MatCreate(PETSC_COMM_SELF,&mpiaij->A));
    CHKERRQ(MatSetSizes(mpiaij->A,mat->rmap->n,mat->cmap->n,mat->rmap->n,mat->cmap->n));
    CHKERRQ(PetscLogObjectParent((PetscObject)mat,(PetscObject)mpiaij->A));
  }
  if (!mpiaij->B) {
    PetscMPIInt size;
    CHKERRMPI(MPI_Comm_size(PetscObjectComm((PetscObject)mat),&size));
    CHKERRQ(MatCreate(PETSC_COMM_SELF,&mpiaij->B));
    CHKERRQ(MatSetSizes(mpiaij->B,mat->rmap->n,size > 1 ? mat->cmap->N : 0,mat->rmap->n,size > 1 ? mat->cmap->N : 0));
    CHKERRQ(PetscLogObjectParent((PetscObject)mat,(PetscObject)mpiaij->B));
  }
  CHKERRQ(MatSetType(mpiaij->A,MATSEQAIJKOKKOS));
  CHKERRQ(MatSetType(mpiaij->B,MATSEQAIJKOKKOS));
  CHKERRQ(MatSeqAIJSetPreallocation(mpiaij->A,d_nz,d_nnz));
  CHKERRQ(MatSeqAIJSetPreallocation(mpiaij->B,o_nz,o_nnz));
...
PetscErrorCode MatMult_MPIAIJKokkos(Mat mat,Vec xx,Vec yy)
{
  ...
  CHKERRQ(VecScatterBegin(mpiaij->Mvctx,xx,mpiaij->lvec,INSERT_VALUES,SCATTER_FORWARD));
  CHKERRQ((*mpiaij->A->ops->mult)(mpiaij->A,xx,yy));
  CHKERRQ(VecScatterEnd(mpiaij->Mvctx,xx,mpiaij->lvec,INSERT_VALUES,SCATTER_FORWARD));
  CHKERRQ((*mpiaij->B->ops->multadd)(mpiaij->B,mpiaij->lvec,yy,yy));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMultAdd_MPIAIJKokkos(Mat mat,Vec xx,Vec yy,Vec zz)
{
  ...
  PetscCheckFalse(nt != mat->cmap->n,PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Incompatible partition of mat (%" PetscInt_FMT ") and xx (%" PetscInt_FMT ")",mat->cmap->n,nt);
  CHKERRQ(VecScatterBegin(mpiaij->Mvctx,xx,mpiaij->lvec,INSERT_VALUES,SCATTER_FORWARD));
  CHKERRQ((*mpiaij->A->ops->multadd)(mpiaij->A,xx,yy,zz));
  CHKERRQ(VecScatterEnd(mpiaij->Mvctx,xx,mpiaij->lvec,INSERT_VALUES,SCATTER_FORWARD));
  CHKERRQ((*mpiaij->B->ops->multadd)(mpiaij->B,mpiaij->lvec,zz,zz));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMultTranspose_MPIAIJKokkos(Mat mat,Vec xx,Vec yy)
{
  ...
  CHKERRQ((*mpiaij->B->ops->multtranspose)(mpiaij->B,xx,mpiaij->lvec));
  CHKERRQ((*mpiaij->A->ops->multtranspose)(mpiaij->A,xx,yy));
  CHKERRQ(VecScatterBegin(mpiaij->Mvctx,mpiaij->lvec,yy,ADD_VALUES,SCATTER_REVERSE));
  CHKERRQ(VecScatterEnd(mpiaij->Mvctx,mpiaij->lvec,yy,ADD_VALUES,SCATTER_REVERSE));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMPIAIJGetLocalMatMerge_MPIAIJKokkos(Mat mat,MatReuse reuse,IS *glob,Mat *C)
{
  ...
  CHKERRQ(MatSeqAIJKokkosMergeMats(Ad,Ao,reuse,C));
  if (glob) {
                                           ...
                                           MatRowMapKokkosView& rowoffset,Mat& C)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(MatSeqAIJKokkosSyncDevice(B)); /* Make sure B->spptr is accessible */
  ...
  if (reuse == MAT_REUSE_MATRIX) {
    ...
    CHKERRQ(PetscSFBcastBegin(bcastSF,MPIU_SCALAR,abuf.data(),Ca.data(),MPI_REPLACE)); /* TODO: get memtype for abuf */
    CHKERRQ(PetscSFBcastEnd  (bcastSF,MPIU_SCALAR,abuf.data(),Ca.data(),MPI_REPLACE));
    ...
    CHKERRMPI(PetscObjectGetComm((PetscObject)ownerSF,&comm));
    CHKERRQ(PetscSFGetGraph(ownerSF,&nroots,&nleaves,NULL,NULL));
    ...
    CHKERRQ(PetscMalloc1(nroots,&Browlens));
    ...
    Ci_h[0] = 0;
    CHKERRQ(PetscSFBcastWithMemTypeBegin(ownerSF,MPIU_INT,PETSC_MEMTYPE_HOST,Browlens,PETSC_MEMTYPE_HOST,&Ci_h[1],MPI_REPLACE));
    CHKERRQ(PetscSFBcastEnd(ownerSF,MPIU_INT,Browlens,&Ci_h[1],MPI_REPLACE));
    ...
    CHKERRQ(PetscSFGetLeafRanks(ownerSF,&niranks,&iranks,&ioffset,&irootloc)); /* irootloc[] contains indices of rows I need to send to each receiver */
    CHKERRQ(PetscSFGetRootRanks(ownerSF,&nranks,&ranks,&roffset,NULL/*rmine*/,NULL/*rremote*/)); /* recv info */
    ...
    */
    CHKERRQ(PetscMalloc3(niranks+1,&sdisp,nranks,&rdisp,niranks+nranks,&reqs));
    ...
    CHKERRMPI(PetscCommGetNewTag(comm,&tag));
    for (i=0; i<nranks; i++)  CHKERRMPI(MPI_Irecv(&rdisp[i],1,MPIU_INT,ranks[i],tag,comm,&reqs[i]));
    for (i=0; i<niranks; i++) CHKERRMPI(MPI_Isend(&sdisp[i],1,MPIU_INT,iranks[i],tag,comm,&reqs[nranks+i]));
    CHKERRMPI(MPI_Waitall(niranks+nranks,reqs,MPI_STATUSES_IGNORE));
    ...
    PetscSFNode *iremote;
    CHKERRQ(PetscMalloc1(nleaves2,&iremote));
    ...
    CHKERRQ(PetscSFCreate(comm,&bcastSF));
    CHKERRQ(PetscSFSetGraph(bcastSF,nroots2,nleaves2,NULL/*ilocal*/,PETSC_OWN_POINTER,iremote,PETSC_OWN_POINTER));
    ...
    */
    ...
    CHKERRQ(PetscSFBcastBegin(bcastSF,MPIU_INT,   jbuf.data(),Cj.view_device().data(),MPI_REPLACE));
    CHKERRQ(PetscSFBcastBegin(bcastSF,MPIU_SCALAR,abuf.data(),Ca.view_device().data(),MPI_REPLACE));
    CHKERRQ(PetscSFBcastEnd  (bcastSF,MPIU_INT,   jbuf.data(),Cj.view_device().data(),MPI_REPLACE));
    CHKERRQ(PetscSFBcastEnd  (bcastSF,MPIU_SCALAR,abuf.data(),Ca.view_device().data(),MPI_REPLACE));
    ...
    CHKERRQ(PetscFree3(sdisp,rdisp,reqs));
    CHKERRQ(PetscFree(Browlens));
                                            ...
                                            KokkosCsrMatrix& C)
{
  ...
  if (reuse == MAT_REUSE_MATRIX) {
    /* Send Aa to abuf */
    CHKERRMPI(PetscSFReduceBegin(reduceSF,MPIU_SCALAR,akok->a_device_data(),abuf.data(),MPI_REPLACE));
    CHKERRMPI(PetscSFReduceEnd  (reduceSF,MPIU_SCALAR,akok->a_device_data(),abuf.data(),MPI_REPLACE));
    ...
    CHKERRQ(PetscObjectGetComm((PetscObject)ownerSF,&comm));
    ...
    CHKERRQ(PetscSFSetUp(ownerSF));
    CHKERRQ(PetscSFGetLeafRanks(ownerSF,&niranks,&iranks,&ioffset,&rows)); /* recv info: iranks[] will send rows to me */
    CHKERRQ(PetscSFGetRootRanks(ownerSF,&nranks,&ranks,&roffset,NULL/*rmine*/,NULL/*rremote*/)); /* send info */
    CHKERRQ(PetscSFGetGraph(ownerSF,&nroots,&nleaves,NULL,NULL));
    ...
    CHKERRQ(PetscMalloc2(Am,&srowlens,niranks+nranks,&reqs));
    for (i=0; i<Am; i++) srowlens[i] = Ai[i+1] - Ai[i];
    rrowlens[0] = 0;
    rrowlens++; /* shift the pointer to make the following expression more readable */
    for (i=0; i<niranks; i++)CHKERRMPI(MPI_Irecv(&rrowlens[ioffset[i]],ioffset[i+1]-ioffset[i],MPIU_INT,iranks[i],tag,comm,&reqs[i]));
    for (i=0; i<nranks; i++) CHKERRMPI(MPI_Isend(&srowlens[roffset[i]],roffset[i+1]-roffset[i],MPIU_INT,ranks[i],tag,comm,&reqs[niranks+i]));
    CHKERRMPI(MPI_Waitall(niranks+nranks,reqs,MPI_STATUSES_IGNORE));
    ...
    CHKERRQ(PetscCalloc1(Cm,&currowlens)); /* Init with zero, to be added to */
    ...
    CHKERRQ(PetscMalloc2(niranks,&sdisp,nranks,&rdisp));
    ...
    PetscSFNode *iremote;
    CHKERRQ(PetscMalloc1(nleaves2,&iremote)); /* no free, since memory will be given to reduceSF */
    for (i=0; i<nranks; i++) {
    ...
    CHKERRQ(PetscSFCreate(comm,&reduceSF));
    CHKERRQ(PetscSFSetGraph(reduceSF,nroots2,nleaves2,NULL,PETSC_OWN_POINTER,iremote,PETSC_OWN_POINTER));
    CHKERRQ(PetscFree2(sdisp,rdisp));
    ...
    CHKERRMPI(PetscSFReduceBegin(reduceSF,MPIU_INT,   Ajg.data(),           jbuf.data(),MPI_REPLACE));
    CHKERRMPI(PetscSFReduceEnd  (reduceSF,MPIU_INT,   Ajg.data(),           jbuf.data(),MPI_REPLACE));
    CHKERRMPI(PetscSFReduceBegin(reduceSF,MPIU_SCALAR,akok->a_device_data(),abuf.data(),MPI_REPLACE));
    CHKERRMPI(PetscSFReduceEnd  (reduceSF,MPIU_SCALAR,akok->a_device_data(),abuf.data(),MPI_REPLACE));
    ...
    CHKERRQ(PetscFree2(srowlens,reqs));
  } else SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Unsupported MatReuse enum %d",reuse);
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatSetMPIAIJKokkosWithGlobalCSRMatrix(Mat C,MatReuse reuse,const KokkosCsrMatrix& csrmat,MatRowMapKokkosView& Cdstart)
{
  ...
  if (reuse == MAT_REUSE_MATRIX) {
    ...
    CHKERRQ(PetscLayoutGetRange(C->cmap,&cstart,&cend)); /* Not MatGetOwnershipRangeColumn() since C has not been preallocated yet */
    ...
    */
    ...
    CHKERRQ(MatCreateSeqAIJKokkosWithCSRMatrix(PETSC_COMM_SELF,cdkok,&Cd));
    CHKERRQ(MatCreateSeqAIJKokkosWithCSRMatrix(PETSC_COMM_SELF,cokok,&Co));
    CHKERRQ(MatSetMPIAIJKokkosWithSplitSeqAIJKokkosMatrices(C,Cd,Co)); /* Coj will be converted to local ids within */
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatSeqAIJCompactOutExtraColumns_SeqAIJKokkos(Mat C,MatColIdxKokkosView& l2g)
{
  ...
  CHKERRQ(MatSeqAIJCompactOutExtraColumns_SeqAIJ(C,&l2gmap));
  ckok = static_cast<Mat_SeqAIJKokkos*>(C->spptr);
  ...
  CHKERRQ(ISLocalToGlobalMappingGetIndices(l2gmap,&garray));
  CHKERRQ(ISLocalToGlobalMappingGetSize(l2gmap,&sz));
  ...
  CHKERRQ(ISLocalToGlobalMappingRestoreIndices(l2gmap,&garray));
...
static PetscErrorCode MatProductSymbolic_MPIAIJKokkos_AB(Mat_Product *product,Mat A,Mat B,MatMatStruct_AB *mm)
{
  ...
  CHKERRQ(MatMPIAIJGetLocalMatMerge(B,MAT_INITIAL_MATRIX,&glob,&mm->B_local));
  CHKERRQ(MatProductCreate(Ad,mm->B_local,NULL,&C1));
  CHKERRQ(MatProductSetType(C1,MATPRODUCT_AB));
  ...
  CHKERRQ(MatProductCreate(Ao,mm->B_other,NULL,&C2));
  CHKERRQ(MatProductSetType(C2,MATPRODUCT_AB));
  ...
  mm->C1 = C1;
  mm->C2 = C2;
  CHKERRQ(ISRestoreIndices(glob,&garray));
...
static PetscErrorCode MatProductSymbolic_MPIAIJKokkos_AtB(Mat_Product *product,Mat A,Mat B,PetscBool localB,PetscInt N,const ConstMatColIdxKokkosView& l2g,MatMatStruct_AtB *mm)
{
  ...
  PetscFunctionBegin;
  /* C1 = Ad^t * B */
  CHKERRQ(MatProductCreate(Ad,B,NULL,&C1));
  CHKERRQ(MatProductSetType(C1,MATPRODUCT_AtB));
  CHKERRQ(MatProductSetFill(C1,product->fill));
  C1->product->api_user = product->api_user;
  CHKERRQ(MatProductSetFromOptions(C1));
  PetscCheck(C1->ops->productsymbolic,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Missing symbolic op for %s",MatProductTypes[C1->product->type]);
  ...
  /* C2 = Ao^t * B */
  CHKERRQ(MatProductCreate(Ao,B,NULL,&C2));
  CHKERRQ(MatProductSetType(C2,MATPRODUCT_AtB));
  CHKERRQ(MatProductSetFill(C2,product->fill));
  C2->product->api_user = product->api_user;
  CHKERRQ(MatProductSetFromOptions(C2));
  PetscCheck(C2->ops->productsymbolic,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Missing symbolic op for %s",MatProductTypes[C2->product->type]);
...
PetscErrorCode MatProductNumeric_MPIAIJKokkos(Mat C)
{
  ...
  if (ptype == MATPRODUCT_AB) {
    ...
    CHKERRQ((*ab->C2->ops->productnumeric)(ab->C2));
    ...
    /* C1 = Ad^t * B_local */
    CHKERRQ(MatMPIAIJGetLocalMatMerge(B,MAT_REUSE_MATRIX,NULL/*glob*/,&atb->B_local));
    ...
    if (atb->C1->product->A != Ad) CHKERRQ(MatProductReplaceMats(Ad,NULL,NULL,atb->C1));
    CHKERRQ((*atb->C1->ops->productnumeric)(atb->C1));
    ...
    if (atb->C1->product->A != Bd) CHKERRQ(MatProductReplaceMats(Bd,NULL,NULL,atb->C1));
    CHKERRQ((*atb->C1->ops->productnumeric)(atb->C1));
  ...
  CHKERRQ(MatSetMPIAIJKokkosWithGlobalCSRMatrix(C,MAT_REUSE_MATRIX,mm->C_global,mm->Cdstart));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatProductSymbolic_MPIAIJKokkos(Mat C)
{
  ...
  if (ptype == MATPRODUCT_AB) {
    ...
    CHKERRQ(MatProductSymbolic_MPIAIJKokkos_AB(product,A,B,mmdata->mmAB));
    mm   = static_cast<MatMatStruct*>(mmdata->mmAB);
  } else if (ptype == MATPRODUCT_AtB) {
    mmdata->mmAtB = new MatMatStruct_AtB();
    auto atb      = mmdata->mmAtB;
    CHKERRQ(MatMPIAIJGetLocalMatMerge(B,MAT_INITIAL_MATRIX,&glob,&atb->B_local));
    CHKERRQ(ISGetIndices(glob,&garray));
    CHKERRQ(ISGetSize(glob,&sz));
    l2g  = Kokkos::create_mirror_view_and_copy(DefaultMemorySpace(),ConstMatColIdxKokkosViewHost(garray,sz));
    CHKERRQ(MatProductSymbolic_MPIAIJKokkos_AtB(product,A,atb->B_local,PETSC_TRUE,N,l2g,atb));
    CHKERRQ(ISRestoreIndices(glob,&garray));
    CHKERRQ(ISDestroy(&glob));
    mm   = static_cast<MatMatStruct*>(atb);
  } else if (ptype == MATPRODUCT_PtAP) { /* BtAB */
    ...
    auto ab       = mmdata->mmAB;
    auto atb      = mmdata->mmAtB;
    CHKERRQ(MatProductSymbolic_MPIAIJKokkos_AB(product,A,B,ab));
    auto tmp = new Mat_SeqAIJKokkos(ab->C_global); /* Memory will be owned by ab->C_petsc */
    CHKERRQ(MatCreateSeqAIJKokkosWithCSRMatrix(PETSC_COMM_SELF,tmp,&ab->C_petsc));
    CHKERRQ(MatProductSymbolic_MPIAIJKokkos_AtB(product,B,ab->C_petsc,PETSC_FALSE,N,l2g/*not used*/,atb));
    mm   = static_cast<MatMatStruct*>(atb);
  }
  /* Split the C_global into petsc A, B format */
  CHKERRQ(MatSetMPIAIJKokkosWithGlobalCSRMatrix(C,MAT_INITIAL_MATRIX,mm->C_global,mm->Cdstart));
...
static PetscErrorCode MatSetValuesCOO_MPIAIJKokkos(Mat mat,const PetscScalar v[],InsertMode imode)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscGetMemType(v,&memtype)); /* Return PETSC_MEMTYPE_HOST when v is NULL */
  ...
  if (imode == INSERT_VALUES) {
    ...
    CHKERRQ(MatSeqAIJGetKokkosViewWrite(B,&Ba));
    ...
    CHKERRQ(MatSeqAIJGetKokkosView(A,&Aa)); /* read & write matrix values */
    CHKERRQ(MatSeqAIJGetKokkosView(B,&Ba));
  }
  ...
  CHKERRQ(PetscSFReduceWithMemTypeBegin(mpiaij->coo_sf,MPIU_SCALAR,PETSC_MEMTYPE_KOKKOS,vsend.data(),PETSC_MEMTYPE_KOKKOS,v2.data(),MPI_REPLACE));
  ...
  CHKERRQ(PetscSFReduceEnd(mpiaij->coo_sf,MPIU_SCALAR,vsend.data(),v2.data(),MPI_REPLACE));
  ...
  if (imode == INSERT_VALUES) {
    CHKERRQ(MatSeqAIJRestoreKokkosViewWrite(A,&Aa)); /* Increase A & B's state etc. */
    CHKERRQ(MatSeqAIJRestoreKokkosViewWrite(B,&Ba));
  } else {
    CHKERRQ(MatSeqAIJRestoreKokkosView(A,&Aa));
    CHKERRQ(MatSeqAIJRestoreKokkosView(B,&Ba));
  }
  PetscFunctionReturn(0);
}
...
PETSC_INTERN PetscErrorCode MatConvert_MPIAIJ_MPIAIJKokkos(Mat A, MatType mtype, MatReuse reuse, Mat* newmat)
{
  ...
  CHKERRQ(PetscStrallocpy(VECKOKKOS,&B->defaultvectype));
  CHKERRQ(PetscObjectChangeTypeName((PetscObject)B,MATMPIAIJKOKKOS));

  a = static_cast<Mat_MPIAIJ*>(A->data);
  if (a->A) CHKERRQ(MatSetType(a->A,MATSEQAIJKOKKOS));
  if (a->B) CHKERRQ(MatSetType(a->B,MATSEQAIJKOKKOS));
  if (a->lvec) CHKERRQ(VecSetType(a->lvec,VECSEQKOKKOS));
  ...
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatMPIAIJSetPreallocation_C",MatMPIAIJSetPreallocation_MPIAIJKokkos));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatMPIAIJGetLocalMatMerge_C",MatMPIAIJGetLocalMatMerge_MPIAIJKokkos));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatSetPreallocationCOO_C",   MatSetPreallocationCOO_MPIAIJKokkos));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatSetValuesCOO_C",          MatSetValuesCOO_MPIAIJKokkos));
  PetscFunctionReturn(0);
}
/*MC
...
PETSC_EXTERN PetscErrorCode MatCreate_MPIAIJKokkos(Mat A)
{
  ...
  CHKERRQ(MatConvert_MPIAIJ_MPIAIJKokkos(A,MATMPIAIJKOKKOS,MAT_INPLACE_MATRIX,&A));
  PetscFunctionReturn(0);
}
...
PetscErrorCode  MatCreateAIJKokkos(MPI_Comm comm,PetscInt m,PetscInt n,PetscInt M,PetscInt N,PetscInt d_nz,const PetscInt d_nnz[],PetscInt o_nz,const PetscInt o_nnz[],Mat *A)
{
  ...
  if (size > 1) {
    CHKERRQ(MatSetType(*A,MATMPIAIJKOKKOS));
...
PetscErrorCode MatKokkosGetDeviceMatWrite(Mat A, PetscSplitCSRDataStructure *B)
{
  ...
  if (size == 1) {
    CHKERRQ(MatSeqAIJKokkosGetDeviceMat(A,&d_mat));
    CHKERRQ(MatSeqAIJKokkosModifyDevice(A)); /* Since we are going to modify matrix values on device */
  } else {
    Mat_MPIAIJ  *aij = (Mat_MPIAIJ*)A->data;
    CHKERRQ(MatSeqAIJKokkosGetDeviceMat(aij->A,&d_mat));
    CHKERRQ(MatSeqAIJKokkosModifyDevice(aij->A));
    CHKERRQ(MatSeqAIJKokkosModifyDevice(aij->B));
  ...
  if (A->assembled) {
    if (A->was_assembled) {
      CHKERRQ(PetscInfo(A,"Assemble more than once already\n"));
    }
    ...
    CHKERRQ(PetscInfo(A,"Warning !assemble ??? assembled=%" PetscInt_FMT "\n",A->assembled));
  }
  if (!d_mat) {
    ...
    CHKERRQ(PetscInfo(A,"Create device matrix in Kokkos\n"));
    if (size == 1) {
      Amat = A;
      ...
      CHKERRQ(PetscCalloc1(A->cmap->N,&colmap));
      CHKERRQ(PetscLogObjectMemory((PetscObject)A,(A->cmap->N)*sizeof(PetscInt)));
    ...
    CHKERRMPI(MPI_Comm_rank(comm,&h_mat.rank));
    ...
    CHKERRQ(MatSeqAIJKokkosSetDeviceMat(Amat,&h_mat));
    CHKERRQ(MatSeqAIJKokkosGetDeviceMat(Amat,&d_mat));
    CHKERRQ(PetscInfo(A,"Create device Mat n=%" PetscInt_FMT " nnz=%" PetscInt_FMT "\n",h_mat.diag.n, nnz));
  }
...
PETSC_INTERN PetscErrorCode MatAIJKokkosPrintOffloadMask(Mat A)
{
  ...
  if (size == 1) {
    CHKERRQ(MatSeqAIJKokkosGetOffloadMask(A,&amask));
    CHKERRQ(PetscPrintf(PETSC_COMM_SELF,"%s\n",amask));
  } else {
    Ad  = ((Mat_MPIAIJ*)A->data)->A;
    Ao  = ((Mat_MPIAIJ*)A->data)->B;
    CHKERRQ(MatSeqAIJKokkosGetOffloadMask(Ad,&amask));
    CHKERRQ(MatSeqAIJKokkosGetOffloadMask(Ao,&bmask));
    CHKERRQ(PetscPrintf(PETSC_COMM_SELF,"Diag : Off-diag = %s : %s\n",amask,bmask));
  }
  PetscFunctionReturn(0);
}
