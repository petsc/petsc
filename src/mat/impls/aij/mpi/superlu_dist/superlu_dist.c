...
PetscErrorCode MatSuperluDistGetDiagU_SuperLU_DIST(Mat F,PetscScalar *diagU)
{
  ...
  PetscFunctionBegin;
  PetscCallExternalNoErrorCode("SuperLU_DIST:pGetDiagU",pGetDiagU(F->rmap->N,&lu->LUstruct,&lu->grid,CASTDOUBLECOMPLEX diagU));
...
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0)
  if (context->use3d) {
    PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridexit3d",superlu_gridexit3d(&context->grid3d));
  } else
#endif
    PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridexit",superlu_gridexit(&context->grid));
...
static PetscErrorCode MatDestroy_SuperLU_DIST(Mat A)
{
  ...
  if (lu->CleanUpSuperLU_Dist) {
    ...
    PetscCallExternalNoErrorCode("SuperLU_DIST:Destroy_CompRowLoc_Matrix_dist",Destroy_CompRowLoc_Matrix_dist(&lu->A_sup));
    if (lu->options.SolveInitialized) {
      PetscCallExternalNoErrorCode("SuperLU_DIST:SolveFinalize",SolveFinalize(&lu->options, &lu->SOLVEstruct));
    }
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0)
    if (lu->use3d) {
      if (lu->grid3d.zscp.Iam == 0) {
        PetscCallExternalNoErrorCode("SuperLU_DIST:Destroy_LU",Destroy_LU(A->cmap->N, &lu->grid3d.grid2d, &lu->LUstruct));
      } else {
        PetscCallExternalNoErrorCode("SuperLU_DIST:DeAllocLlu_3d",DeAllocLlu_3d(lu->A_sup.ncol, &lu->LUstruct, &lu->grid3d));
        PetscCallExternalNoErrorCode("SuperLU_DIST:DeAllocGlu_3d",DeAllocGlu_3d(&lu->LUstruct));
      }
      PetscCallExternalNoErrorCode("SuperLU_DIST:Destroy_A3d_gathered_on_2d",Destroy_A3d_gathered_on_2d(&lu->SOLVEstruct, &lu->grid3d));
    } else
#endif
      PetscCallExternalNoErrorCode("SuperLU_DIST:Destroy_LU",Destroy_LU(A->cmap->N, &lu->grid, &lu->LUstruct));
    PetscCallExternalNoErrorCode("SuperLU_DIST:ScalePermstructFree",ScalePermstructFree(&lu->ScalePermstruct));
    PetscCallExternalNoErrorCode("SuperLU_DIST:LUstructFree",LUstructFree(&lu->LUstruct));

    /* Release the SuperLU_DIST process grid only if the matrix has its own copy, that is it is not in the communicator context */
    if (lu->comm_superlu) {
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0)
      if (lu->use3d) {
        PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridexit3d",superlu_gridexit3d(&lu->grid3d));
      } else
#endif
        PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridexit",superlu_gridexit(&lu->grid));
    }
  }
  /*
...
static PetscErrorCode MatSolve_SuperLU_DIST(Mat A,Vec b_mpi,Vec x)
{
  ...
  PetscCallExternalNoErrorCode("SuperLU_DIST:PStatInit",PStatInit(&stat));        /* Initialize the statistics variables. */
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0) && !PetscDefined(MISSING_GETLINE)
  if (lu->use3d)
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx3d",pgssvx3d(&lu->options,&lu->A_sup,&lu->ScalePermstruct,CASTDOUBLECOMPLEX bptr,m,1,&lu->grid3d,&lu->LUstruct,&lu->SOLVEstruct,berr,&stat,&info));
  else
#endif
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx",pgssvx(&lu->options,&lu->A_sup,&lu->ScalePermstruct,CASTDOUBLECOMPLEX bptr,m,1,&lu->grid,&lu->LUstruct,&lu->SOLVEstruct,berr,&stat,&info));
  PetscCheck(!info,PETSC_COMM_SELF,PETSC_ERR_LIB,"pdgssvx fails, info: %d",info);

  if (lu->options.PrintStat) PetscCallExternalNoErrorCode("SuperLU_DIST:PStatPrint",PStatPrint(&lu->options, &stat, &lu->grid));  /* Print the statistics. */
  PetscCallExternalNoErrorCode("SuperLU_DIST:PStatFree",PStatFree(&stat));
...
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0) && !PetscDefined(MISSING_GETLINE)
  if (lu->use3d)
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx3d",pgssvx3d(&lu->options,&lu->A_sup,&lu->ScalePermstruct,CASTDOUBLECOMPLEX bptr,m,nrhs,&lu->grid3d,&lu->LUstruct,&lu->SOLVEstruct,berr,&stat,&info));
  else
#endif
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx",pgssvx(&lu->options,&lu->A_sup,&lu->ScalePermstruct,CASTDOUBLECOMPLEX bptr,m,nrhs,&lu->grid,&lu->LUstruct,&lu->SOLVEstruct,berr,&stat,&info));
...
static PetscErrorCode MatLUFactorNumeric_SuperLU_DIST(Mat F,Mat A,const MatFactorInfo *info)
{
  ...
  if (lu->options.Fact == DOFACT) { /* first numeric factorization */
    PetscCallExternalNoErrorCode("SuperLU_DIST:allocateA_dist",allocateA_dist(Aloc->rmap->n, nz, CASTDOUBLECOMPLEXSTAR &lu->val, &lu->col, &lu->row));
...
#endif
        PetscCallExternalNoErrorCode("SuperLU_DIST:Destroy_LU",Destroy_LU(A->rmap->N, &lu->grid, &lu->LUstruct));
  ...
  if (lu->options.Fact == DOFACT) {
    PetscCallExternalNoErrorCode("SuperLU_DIST:Create_CompRowLoc_Matrix_dist",Create_CompRowLoc_Matrix_dist(&lu->A_sup, A->rmap->N, A->cmap->N, nz, A->rmap->n, A->rmap->rstart, CASTDOUBLECOMPLEX lu->val, lu->col, lu->row, SLU_NR_loc, SLU, SLU_GE));
  }
...
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0) && !PetscDefined(MISSING_GETLINE)
  if (lu->use3d) {
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx3d",pgssvx3d(&lu->options, &lu->A_sup, &lu->ScalePermstruct, 0, A->rmap->n, 0, &lu->grid3d, &lu->LUstruct, &lu->SOLVEstruct, berr, &stat, &sinfo));
  } else
#endif
    PetscCallExternalNoErrorCode("SuperLU_DIST:pgssvx",pgssvx(&lu->options, &lu->A_sup, &lu->ScalePermstruct, 0, A->rmap->n, 0, &lu->grid, &lu->LUstruct, &lu->SOLVEstruct, berr, &stat, &sinfo));
  if (sinfo > 0) {
  ...
  if (lu->options.PrintStat) {
    PetscCallExternalNoErrorCode("SuperLU_DIST:PStatPrint",PStatPrint(&lu->options, &stat, &lu->grid));  /* Print the statistics. */
  }
  PetscCallExternalNoErrorCode("SuperLU_DIST:PStatFree",PStatFree(&stat));
...
static PetscErrorCode MatLUFactorSymbolic_SuperLU_DIST(Mat F,Mat A,IS r,IS c,const MatFactorInfo *info)
{
...
#if PETSC_PKG_SUPERLU_DIST_VERSION_GE(7,2,0)
    if (lu->use3d) {
      PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridinit3d",superlu_gridinit3d(context ? context->comm : lu->comm_superlu, lu->nprow, lu->npcol,lu->npdep, &lu->grid3d));
      if (context) {context->grid3d = lu->grid3d; context->use3d = lu->use3d;}
    } else {
#endif
      PetscCallExternalNoErrorCode("SuperLU_DIST:superlu_gridinit",superlu_gridinit(context ? context->comm : lu->comm_superlu, lu->nprow, lu->npcol, &lu->grid));
  ...
  PetscCallExternalNoErrorCode("SuperLU_DIST:ScalePermstructInit",ScalePermstructInit(M, N, &lu->ScalePermstruct));
  PetscCallExternalNoErrorCode("SuperLU_DIST:LUstructInit",LUstructInit(N, &lu->LUstruct));
