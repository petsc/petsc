...
PetscErrorCode  MatFDColoringApply_BAIJ(Mat J,MatFDColoring coloring,Vec x1,MatStructure *flag,void *sctx)
{
  ...
  if (!vscale) {
    if (ctype == IS_COLORING_GLOBAL && coloring->htype[0] == 'd') {
      /* contain the "diagonal" on processor scalings followed by the off processor* - garray must be non-bloced! */
      Mat_MPIBAIJ    *baij=(Mat_MPIBAIJ*)J->data;
      PetscInt       *garray;
      ...
      ierr = VecCreateGhost(PetscObjectComm((PetscObject)J),J->cmap->n,PETSC_DETERMINE,baij->B->cmap->n,garray,&vscale);CHKERRQ(ierr);
...
PetscErrorCode  MatFDColoringApply_AIJ(Mat J,MatFDColoring coloring,Vec x1,MatStructure *flag,void *sctx)
{
  ...
  if (bcols > 1 && ctype == IS_COLORING_GHOSTED) { /* only supported for seqaij matrix */
...
PetscErrorCode MatFDColoringSetUp_MPIXAIJ(Mat mat,ISColoring iscoloring,MatFDColoring c)
{
  ...
  PetscInt               ctype=c->ctype,*spidxA,*spidxB,nz,bs,bs2,spidx,*color_start;
  Mat                    A,B;
  ...
  ierr = PetscMalloc3(m+1,PetscInt,&rowhit,m+1,PetscScalar*,&valaddrhit,nis+1,PetscInt,&color_start);CHKERRQ(ierr);
  nz = 0;
  ...
  if (!isBAIJ && bcols > 1) {
    PetscInt nbcols=0,brows=c->brows,mbs=c->m,*row_start,*nrows_new,nz_new,row_end;
    MatEntry *Jentry_new;
