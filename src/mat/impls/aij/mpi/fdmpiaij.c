...
PetscErrorCode  MatFDColoringApply_MPIAIJ(Mat J,MatFDColoring coloring,Vec x1,MatStructure *flag,void *sctx)
{
  ...
  PetscFunctionBegin;
  printf("MatFDColoringApply_MPIAIJ ...\n");
...
PetscErrorCode MatFDColoringCreate_MPIAIJ_new(Mat mat,ISColoring iscoloring,MatFDColoring c)
{
  ...
  //printf("MatFDColoringCreate_MPIAIJ_new...\n");
  ...
  for (i=0; i<nis; i++) {
    ...
    if (ctype == IS_COLORING_GLOBAL) {
      ...
      if (i == 0){
        printf("[%d] color %d ncolumn %d nctot %d\n",rank,i,c->ncolumns[i],nctot);
      }
    ...
    for (j=0; j<nctot; j++) {
      ...
      if (col >= cstart && col < cend) { /* column is in diagonal block of matrix A */
        ...
        for (k=0; k<m; k++) {
          ...
          //if (rank==0) printf("[%d] (%d %d B[%d]=%g)\n",rank,*rows,col,spidx,B_val[spidx]);
          //if (rank==0) printf("[%d] (%d %d B), colb %d\n",rank,*rows,col,colb);
          rowhit[*rows++] = col + 1;
        }
      }
    }
