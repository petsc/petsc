...
static int MatIncreaseOverlap_MPIAIJ_Once(Mat C,int imax,IS *is)
{
  ...
  { 
    ...
    len   = (imax)*(sizeof(PetscBT) + sizeof(int*)+ sizeof(int)) + 
      (m)*imax*sizeof(int)  + (m/BITS_PER_BYTE+1)*imax*sizeof(char) + 1;
    ...
    t_p   = (char *)(d_p   + m*imax);
    for (i=0; i<imax; i++) {
      table[i] = t_p + (m/BITS_PER_BYTE+1)*i;
      data[i]  = d_p + (m)*i;
    }
  }
...
int MatGetSubMatrix_MPIAIJ_All(Mat A,MatReuse scall,Mat **Bin)
{
  ...
  if (scall == MAT_INITIAL_MATRIX) {
    ...
    */
    ...
    /* ---------------------------------------------------------------
         Create the sequential matrix
    */
    ierr  = MatCreateSeqAIJ(PETSC_COMM_SELF,A->M,A->N,0,lens,&B);CHKERRQ(ierr);
...
{ 
  ...
  for (i=0; i<nrqs; ++i) {
    ...
    ierr = PetscMalloc((rbuf2[index][0]+1)*sizeof(PetscScalar),&rbuf4[index]);CHKERRQ(ierr);
    ierr = MPI_Irecv(rbuf3[index],rbuf2[index][0],MPI_INT,r_status2[i].MPI_SOURCE,tag2,comm,r_waits3+index);CHKERRQ(ierr);
    ierr = MPI_Irecv(rbuf4[index],rbuf2[index][0],MPIU_SCALAR,r_status2[i].MPI_SOURCE,tag3,comm,r_waits4+index);CHKERRQ(ierr);
  } 
      ...
      for (j=1,max1=rbuf1_i[0]; j<=max1; j++) { 
        ...
        for (k=0; k<kmax; k++,ct1++) {
          ...
          if (!shift) {
            ...
            ierr = PetscMemcpy(cols,t_cols,ncols*sizeof(int));CHKERRQ(ierr);
  ...
  ierr = PetscMalloc((nrqr+1)*sizeof(MPI_Request),&s_waits4);CHKERRQ(ierr);
  {
    ...
    int    *b_j = b->j,shift = a->indexshift;
      ...
      for (j=1,max1=rbuf1_i[0]; j<=max1; j++) {
        ...
        for (k=0; k<kmax; k++,ct1++) {
          ...
          if (!shift) {
            ...
            ierr = PetscMemcpy(vals,t_vals,ncols*sizeof(PetscScalar));CHKERRQ(ierr);
