...
PetscErrorCode MatDestroy_MPIAIJ_MatGetSubmatrices(Mat C)
{
  ...
  PetscFunctionBegin;
  if (submatj->id) PetscFunctionReturn(0);
...
PetscErrorCode MatGetSubMatrices_MPIAIJ_SingleIS(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,Mat *submat[])
{
  ...
  ierr = MatGetSubMatrices_MPIAIJ_SingleIS_Local(C,ismax,isrow,iscol,scall,allcolumns,*submat);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatGetSubMatrices_MPIAIJ(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,Mat *submat[])
{
  ...
  if (C->submat_singleis) { /* flag is set in PCSetUp_ASM() to skip several MPIU_Allreduce() */
...
PetscErrorCode MatGetSubMatrices_MPIAIJ_Local(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,PetscBool *allcolumns,Mat *submats)
{
  ...
  PetscInt       **rbuf3,*req_source1,*req_source2,**sbuf_aj,**rbuf2,max1,max2;
  ...
  size = c->size;
  rank = c->rank;
  if (ismax == 0) printf("[%d] MatGetSubMatrices_MPIAIJ_Local, scall %d, ismax = 0!!! \n",rank,scall);
  //printf("[%d] MatGetSubMatrices_MPIAIJ_Local, scall %d\n",rank,scall);
  ...
  ierr = PetscMalloc1(ismax,&issorted);CHKERRQ(ierr);
  ...
  ierr = PetscMalloc1(ismax,&smats);CHKERRQ(ierr);
  ierr = PetscMalloc1(ismax,&row2proc);CHKERRQ(ierr);
  ...
  if (scall == MAT_REUSE_MATRIX) {
    ...
    for (i=0; i<ismax; i++) {
      ...
      /* Initial matrix as if empty -- rm later!!! */
      ...
      cmap[i]       = smat_i->cmap;
#if defined(PETSC_USE_CTABLE)
      //rmap_loc   = smat_i->rmap_loc;
      //cmap_loc   = smat_i->cmap_loc;
#endif
    }
  }
  ...
  ierr = PetscMalloc3(nrqr,&sbuf2,nrqr,&req_size,nrqr,&req_source1);CHKERRQ(ierr);
  {
    ...
    for (i=0; i<nrqr; ++i) {
      ...
      req_source1[idex] = r_status1[i].MPI_SOURCE;
      /* form the header */
      sbuf2_i[0] = req_size[idex];
      for (j=1; j<start; j++) sbuf2_i[j] = rbuf1_i[j];

      ierr = MPI_Isend(sbuf2_i,end,MPIU_INT,req_source1[idex],tag1,comm,s_waits2+i);CHKERRQ(ierr);
    }
  }
  ierr = PetscFree(r_status1);CHKERRQ(ierr);
  ierr = PetscFree(r_waits1);CHKERRQ(ierr);
  ...
  for (i=0; i<nrqs; ++i) {
    ...
    req_source2[i] = r_status2[i].MPI_SOURCE;
    ierr = MPI_Irecv(rbuf3[idex],rbuf2[idex][0],MPIU_INT,req_source2[i],tag2,comm,r_waits3+idex);CHKERRQ(ierr);
  }
  ...
  ierr = PetscMalloc1(nrqr+1,&s_waits3);CHKERRQ(ierr);
  {
    ...
    for (i=0; i<nrqr; i++) {
      ...
      ierr = MPI_Isend(sbuf_aj_i,req_size[i],MPIU_INT,req_source1[i],tag2,comm,s_waits3+i);CHKERRQ(ierr);
    }
  }
  ...
  } // endof scall == MAT_INITIAL_MATRIX ||!ismax
  ...
  if (scall == MAT_INITIAL_MATRIX) {
    for (i=0; i<ismax; i++) {
      ...
      smat_i->cmap        = cmap[i];
#if defined(PETSC_USE_CTABLE)
      smat_i->rmap_loc    = NULL;//rmap_loc;
      smat_i->cmap_loc    = NULL;//cmap_loc;
#endif
  ...
  for (i=0; i<nrqs; ++i) {
    ...
    ierr = MPI_Irecv(rbuf4[idex],rbuf2[idex][0],MPIU_SCALAR,req_source2[i],tag3,comm,r_waits4+idex);CHKERRQ(ierr);
  }
  ...
  ierr = PetscMalloc1(nrqr+1,&s_waits4);CHKERRQ(ierr);
  {
    ...
    for (i=0; i<nrqr; i++) {
      ...
      ierr = MPI_Isend(sbuf_aa_i,req_size[i],MPIU_SCALAR,req_source1[i],tag3,comm,s_waits4+i);CHKERRQ(ierr);
    }
  }
  ...
  ierr = PetscFree(issorted);CHKERRQ(ierr);
