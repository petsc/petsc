...
PetscErrorCode MatGetSubMatrices_MPIAIJ(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,Mat *submat[])
{
  ...
  if (scall == MAT_REUSE_MATRIX) {
    if (!ismax) {
      ...
      Mat_SeqAIJ *subc = (Mat_SeqAIJ*)((*submat)[0]->data);
      Mat_SubMat *smat_i   = subc->submatis1;
      if (!smat_i) printf("smat_i = NULL! wantallmatrix %d\n",wantallmatrix);
    }
  }
...
PetscErrorCode MatGetSubMatrices_MPIAIJ_Local(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,Mat *submats)
{
  ...
  PetscBool      *issorted,*allcolumns,colflag;
  ...
  for (i=0; i<ismax; i++) {
    ...
    ierr = ISGetLocalSize(iscol[i],&j);CHKERRQ(ierr);
    if (colflag && j == C->cmap->N) {
      ...
      for (i=0; i<nrqr; i++) {
        ...
        for (j=1,max1=rbuf1_i[0]; j<=max1; j++) {
          ...
          for (k=0; k<kmax; k++,ct1++) {
            ...
            /* load the column indices for this row into sorted cols */
            cols = sbuf_aj_i + ct2;
