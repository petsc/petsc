...
#undef __FUNCT__
#define __FUNCT__ "SeqAIJCRL_create_aijcrl"
PetscErrorCode SeqAIJCRL_create_aijcrl(Mat A)
{
...
PetscErrorCode MatAssemblyEnd_SeqAIJCRL(Mat A, MatAssemblyType mode)
{
  ...
  ierr = SeqAIJCRL_create_aijcrl(A);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode PETSCMAT_DLLEXPORT MatConvert_SeqAIJ_SeqAIJCRL(Mat A,const MatType type,MatReuse reuse,Mat *newmat)
{
  PetscErrorCode ierr;
  Mat            B = *newmat;
  Mat_AIJCRL     *aijcrl;
  ...
  if (A->assembled) {
    ierr = SeqAIJCRL_create_aijcrl(B);CHKERRQ(ierr);
  }
...
@*/
PetscErrorCode PETSCMAT_DLLEXPORT MatCreateSeqAIJCRL(MPI_Comm comm,PetscInt m,PetscInt n,PetscInt nz,const PetscInt nnz[],Mat *A)
{
  PetscErrorCode ierr;
...
PetscErrorCode PETSCMAT_DLLEXPORT MatCreate_SeqAIJCRL(Mat A)
{
  PetscErrorCode ierr;
