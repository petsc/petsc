...
/*  printf("i %d row %d nz %d idx %d fm %d\n",i,row,nz,idx,fm);  */
  ...
  aijnew->maxnz = aijnew->nz = ainew[n] + shift;
...
int MatLUFactorNumeric_SeqAIJ(Mat mat,Mat *infact)
{
  ...
  for ( i=0; i<n; i++ ) {
    ...
    pv = aijnew->a + ai[i] + shift;
    pj = aijnew->j + ai[i] + shift;
    nz = ai[i+1] - ai[i];
    if (rtmp[i] == 0.0) {SETERRQ(1,"MatLUFactorNumeric_SeqAIJ:Zero pivot");}
    rtmp[i] = 1.0/rtmp[i];
    for ( j=0; j<nz; j++ ) {pv[j] = rtmp[pj[j]+shift];}
  } 
  PETSCFREE(rtmp);
  ierr = ISRestoreIndices(isicol,&ic); CHKERRQ(ierr);
  ierr = ISRestoreIndices(isrow,&r); CHKERRQ(ierr);
...
int MatSolveTrans_SeqAIJ(Mat mat,Vec bb, Vec xx)
{
  ...
  for ( i=n-1; i>=0; i-- ){
    v   = aa + aij->diag[i] - 1 + shift;
    vi  = aj + aij->diag[i] - 1 + shift;
    nz  = aij->diag[i] - ai[i];
    while (nz--) {
      tmp[*vi-- + shift] -= (*v--)*tmp[i];
    }
                             ...
                             int levels,Mat *fact)
{
  ...
  for ( prow=0; prow<n; prow++ ) {
    ...
    while ( row < prow ) {
      ...
      nnz     = ainew[row+1] - ainew[row] - nz + shift;
      if (*xi++ + shift != row) {
