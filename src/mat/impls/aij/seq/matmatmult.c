...
PetscErrorCode MatMatMultNumeric_SeqAIJ_SeqDense(Mat A,Mat B,Mat C)
{
  ...
  PetscFunctionBegin;
  if (PetscUnlikely(B->rmap->n != A->cmap->n)) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number columns in A %" PetscInt_FMT " not equal rows in B %" PetscInt_FMT "\n",A->cmap->n,B->rmap->n);
  if (PetscUnlikely(A->rmap->n != C->rmap->n)) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number rows in C %" PetscInt_FMT " not equal rows in A %" PetscInt_FMT "\n",C->rmap->n,A->rmap->n);
  if (PetscUnlikely(B->cmap->n != C->cmap->n)) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Number columns in B %" PetscInt_FMT " not equal columns in C %" PetscInt_FMT "\n",B->cmap->n,C->cmap->n);
