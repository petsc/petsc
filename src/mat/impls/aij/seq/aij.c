...
PetscErrorCode MatView_SeqAIJ(Mat A,PetscViewer viewer)
{
  ...
  if (iascii) {
    ...
    Mat_SeqAIJ     *a = (Mat_SeqAIJ*)A->data;
    ierr = PetscViewerSocketPutSparse_Private(viewer,A->rmap.n,A->cmap.n,a->nz,a->a,a->i,a->j);CHKERRQ(ierr);
#endif
  } else if (isbinary) {
...
PetscErrorCode MatDestroy_SeqAIJ(Mat A)
{
  ...
  ierr = PetscObjectComposeFunctionDynamic((PetscObject)A,"MatMatMult_seqaij_seqdense_C","",PETSC_NULL);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
void PETSCMAT_DLLEXPORT matsetvaluesseqaij_(Mat *AA,PetscInt *mm,const PetscInt im[],PetscInt *nn,const PetscInt in[],const PetscScalar v[],InsertMode *isis)
{
  ...
  for (k=0; k<m; k++) { /* loop over added rows */
    ...
    if (row < 0) continue;
#if defined(PETSC_USE_DEBUG)  
    if (row >= A->rmap.n) SETERRABORT(A->comm,PETSC_ERR_ARG_OUTOFRANGE,"Row too large");
#endif
    rp   = aj + ai[row]; ap = aa + ai[row];
    rmax = imax[row]; nrow = ailen[row]; 
    low  = 0;
    ...
    for (l=0; l<n; l++) { /* loop over added columns */
      if (in[l] < 0) continue;
#if defined(PETSC_USE_DEBUG)  
      if (in[l] >= A->cmap.n) SETERRABORT(A->comm,PETSC_ERR_ARG_OUTOFRANGE,"Column too large");
#endif
      col = in[l];
      if (roworiented) {
        value = v[l + k*n]; 
