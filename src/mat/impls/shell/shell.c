...
static PetscErrorCode MatShellPreZeroRight(Mat A,Vec x,Vec *xx)
{
  ...
  *xx = x;
  ...
  if (shell->zcols) {
    if (!shell->right_work) {
      PetscCall(MatCreateVecs(A,&shell->right_work,NULL));
    }
  ...
  *xx = NULL;
  if (!shell->zrows) {
    *xx = x;
    ...
    if (!shell->left_work) {
      PetscCall(MatCreateVecs(A,NULL,&shell->left_work));
    }
...
static PetscErrorCode MatZeroRowsColumns_Local_Shell(Mat mat,PetscInt nr,PetscInt rows[],PetscInt nc,PetscInt cols[],PetscScalar diag,PetscBool rc)
{
  ...
  PetscFunctionBegin;
  if (!shell->zvals) {
    PetscCall(MatCreateVecs(mat,NULL,&shell->zvals));
  }
  if (!shell->zvals_w) {
    PetscCall(VecDuplicate(shell->zvals,&shell->zvals_w));
  }
...
static PetscErrorCode MatProductSymbolic_Shell_X(Mat D)
{
  ...
  if (matmat->resultname) {
    ...
    if (!flg) {
      PetscCall(MatSetType(D,matmat->resultname));
    }
  }
...
PetscErrorCode MatCopy_Shell(Mat A,Mat B,MatStructure str)
{
  ...
  if (shellA->dshift) {
    if (!shellB->dshift) {
      PetscCall(VecDuplicate(shellA->dshift,&shellB->dshift));
    }
  ...
  if (shellA->left) {
    if (!shellB->left) {
      PetscCall(VecDuplicate(shellA->left,&shellB->left));
    }
  ...
  if (shellA->right) {
    if (!shellB->right) {
      PetscCall(VecDuplicate(shellA->right,&shellB->right));
    }
  ...
  if (shellA->zrows) {
    ...
    if (shellA->zcols) {
      PetscCall(ISDuplicate(shellA->zcols,&shellB->zcols));
    }
...
static PetscErrorCode MatDiagonalScale_Shell(Mat Y,Vec left,Vec right)
{
  ...
  if (right) {
    ...
    if (shell->zrows) {
      if (!shell->left_work) {
        PetscCall(MatCreateVecs(Y,NULL,&shell->left_work));
      }
