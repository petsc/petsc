...
PetscErrorCode MatGetValues_SeqSELL(Mat A,PetscInt m,const PetscInt im[],PetscInt n,const PetscInt in[],PetscScalar v[])
{
  ...
  for (k=0; k<m; k++) { /* loop over requested rows */
    ...
    for (l=0; l<n; l++) { /* loop over requested columns */
      col = in[l];
      if (col<0) continue;
      if (PetscDefined(USE_DEBUG) && col >= A->cmap->n) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Column too large: row %D max %D",col,A->cmap->n-1);
...
PetscErrorCode MatSetValues_SeqSELL(Mat A,PetscInt m,const PetscInt im[],PetscInt n,const PetscInt in[],const PetscScalar v[],InsertMode is)
{
  ...
  for (k=0; k<m; k++) { /* loop over added rows */
    ...
    for (l=0; l<n; l++) { /* loop over added columns */
      col = in[l];
      if (col<0) continue;
      if (PetscDefined(USE_DEBUG) && col >= A->cmap->n) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Col too large: row %D max %D",col,A->cmap->n-1);
      if (a->roworiented) {
        value = v[l+k*n];
      } else {
        value = v[k+l*m];
