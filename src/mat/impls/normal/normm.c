...
PetscErrorCode MatIncreaseOverlap_Normal(Mat A,PetscInt is_max,IS is[],PetscInt ov)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(ov < 0,PetscObjectComm((PetscObject)A),PETSC_ERR_ARG_OUTOFRANGE,"Negative overlap specified");
...
PetscErrorCode MatCreateSubMatrices_Normal(Mat mat,PetscInt n,const IS irow[],const IS icol[],MatReuse scall,Mat *submat[])
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(a->left || a->right || irow != icol,PetscObjectComm((PetscObject)mat),PETSC_ERR_SUP,"Not implemented");
...
PetscErrorCode MatPermute_Normal(Mat A,IS rowp,IS colp,Mat *B)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(rowp != colp,PetscObjectComm((PetscObject)A),PETSC_ERR_ARG_INCOMP,"Row permutation and column permutation must be the same");
...
PetscErrorCode MatDuplicate_Normal(Mat A, MatDuplicateOption op, Mat *B)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(a->left || a->right,PetscObjectComm((PetscObject)A),PETSC_ERR_SUP,"Not implemented");
...
PetscErrorCode MatProductSymbolic_Normal_Dense(Mat C)
{
  ...
  A = C->product->A;
  a = (Mat_Normal*)A->data;
  PetscAssertFalse(a->left,PetscObjectComm((PetscObject)C),PETSC_ERR_SUP,"Not implemented");
  B = C->product->B;
