...
int main(int argc, char **argv)
{
  ...
  PetscCall(VecCreateSeq(PETSC_COMM_SELF, n, &a));
  ...
  PetscCheck(vec_norm == mat_norm, PETSC_COMM_SELF, PETSC_ERR_PLIB, "Norms don't match");
  ...
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatMult is not like VecPointwiseMultiply");
  ...
  PetscCall(VecAXPY(c2, -1.0, c));
  PetscCall(VecNorm(c2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatMultAdd gave unexpected value");
  ...
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatDuplicate with MAT_COPY_VALUES did not make a duplicate vector");
  PetscCall(MatDestroy(&A_dup));
  ...
  PetscCall(VecAXPY(a2, -1.0, a));
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatShift gave different result from VecShift");
  ...
  PetscCall(VecAXPY(a2, -1.0, a));
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatScale gave different result from VecScale");
  ...
  PetscCall(VecAXPY(a2, -1.0, a));
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatDiagonalScale gave unexpected result");
  ...
  PetscCall(VecAXPY(a2, -1.0, a));
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatDiagonalSet gave unexpected result");
  ...
  PetscCheck(state_pre == state_post, PETSC_COMM_SELF, PETSC_ERR_PLIB, "State changed on noop");
  ...
  PetscCall(MatDiagonalRestoreInverseDiagonal(A, &A_inv_diag));
  PetscCall(PetscObjectStateGet((PetscObject)A, &state_post));
  PetscCheck(state_pre != state_post, PETSC_COMM_SELF, PETSC_ERR_PLIB, "State not changed on mutation");
  ...
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatDiagonalGetInverse gave unexpected result");

  PetscCall(MatZeroEntries(A));
  PetscCall(MatNorm(A, NORM_INFINITY, &mat_norm));
  PetscCheck(mat_norm == 0.0, PETSC_COMM_SELF, PETSC_ERR_PLIB, "MatZeroEntries gave unexpected result");
