...
{
  PetscFunctionBegin;
  if (a_idx>=ail->size) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"a_idx >= ail->size: a_idx=%" PetscInt_FMT ".",a_idx);
  *pos = ail->array[a_idx];
  PetscFunctionReturn(0);
}
...
PetscErrorCode PetscCDAppendID(PetscCoarsenData *ail, PetscInt a_idx, PetscInt a_id)
{
  ...
  if (a_idx>=ail->size) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Index %" PetscInt_FMT " out of range.",a_idx);
...
PetscErrorCode PetscCDAppendRemove(PetscCoarsenData *ail, PetscInt a_destidx, PetscInt a_srcidx)
{
  ...
  PetscFunctionBegin;
  if (a_srcidx>=ail->size) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Index %" PetscInt_FMT " out of range.",a_srcidx);
  if (a_destidx>=ail->size) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Index %" PetscInt_FMT " out of range.",a_destidx);
  if (a_destidx==a_srcidx) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"a_destidx==a_srcidx %" PetscInt_FMT ".",a_destidx);
...
PetscErrorCode PetscCDGetASMBlocks(const PetscCoarsenData *ail, const PetscInt a_bs, Mat mat, PetscInt *a_sz, IS **a_local_is)
{
  ...
  if (*a_sz != kk) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"*a_sz %" PetscInt_FMT " != kk %" PetscInt_FMT,*a_sz,kk);
...
#define BF_SZ 10000
          ...
          if (count > BF_SZ) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"buffer too small for receive: %d",count);
          ...
          while (n--) {
            ...
            if (lid_matched[lid1]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Recieved deleted gid %" PetscInt_FMT ", deleted by (lid) %" PetscInt_FMT " from proc %" PetscInt_FMT,sub_it,gid1,kk);
            lid_matched[lid1] = PETSC_TRUE; /* keep track of what we've done this round */
            ierr              = PetscCDSizeAt(agg_llists, lid1, &kk);CHKERRQ(ierr);
            count2           += kk + 2;
            count3++; /* number of verts requested (n) */
          }
          if (count2 > count3*CHUNCK_SIZE) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"Irecv will be too small: %" PetscInt_FMT,count2);
          ...
          if (nSend2==REQ_BF_SIZE) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"buffer too small for requests: %" PetscInt_FMT,nSend2);
