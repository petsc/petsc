/*$Id: sp1wd.c,v 1.39 2001/03/23 23:22:51 balay Exp $*/
...
int MatOrdering_1WD(Mat mat,const MatOrderingType type,IS *row,IS *col)
{
  ...
  ierr = PetscMalloc((5*nrow+1) * sizeof(int),&mask);CHKERRQ(ierr);
  xls  = mask + nrow;
  ls   = xls + nrow + 1;
  xblk = ls + nrow;
  perm = xblk + nrow;
