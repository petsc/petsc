...
PetscErrorCode  MatGetOrdering(Mat mat,MatOrderingType type,IS *rperm,IS *cperm)
{
  ...
  PetscAssertFalse(!type,PETSC_COMM_SELF,PETSC_ERR_ARG_NULL,"Ordering type cannot be null");
  ...
  PetscAssertFalse(mmat != nmat,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Must be square matrix, rows %" PetscInt_FMT " columns %" PetscInt_FMT,mmat,nmat);

  ierr = MatOrderingRegisterAll();CHKERRQ(ierr);
  ierr = PetscFunctionListFind(MatOrderingList,type,&r);CHKERRQ(ierr);
  PetscAssertFalse(!r,PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Unknown or unregistered type: %s",type);
