...
PETSC_INTERN PetscErrorCode MatGetOrdering_WBM(Mat mat, MatOrderingType type, IS *row, IS *col)
{
  ...
  if (ierr) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"HSL mc64id_dist returned %d",ierr);
  ...
  if (ierr) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_LIB,"HSL mc64ad_dist returned %d",ierr);
