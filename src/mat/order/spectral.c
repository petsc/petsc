...
PETSC_INTERN PetscErrorCode MatGetOrdering_Spectral(Mat A, MatOrderingType type, IS *row, IS *col)
{
  ...
  ierr = MatCreateLaplacian(A, eps, PETSC_FALSE, &L);CHKERRQ(ierr);
  {
    ...
    PetscAssertFalse(norm > 1.0e-10,PetscObjectComm((PetscObject) y), PETSC_ERR_PLIB, "Invalid graph Laplacian");
...
#else
  {
    ...
    evInd = perm[0];
    PetscAssertFalse(PetscUnlikely((realpart[evInd] > 1.0e-12) || (imagpart[evInd] > 1.0e-12)),PetscObjectComm((PetscObject) L), PETSC_ERR_PLIB, "Graph Laplacian must have lowest eigenvalue 0");
    evInd = perm[1];
    PetscAssertFalse(PetscUnlikely((realpart[evInd] < 1.0e-12) && (imagpart[evInd] < 1.0e-12)),PetscObjectComm((PetscObject) L), PETSC_ERR_PLIB, "Graph Laplacian must have only one zero eigenvalue");
    evInd = perm[0];
    for (i = 0; i < n; ++i) {
      PetscAssertFalse(PetscUnlikely(PetscAbsReal(eigvec[evInd*n+i] - eigvec[evInd*n+0]) > 1.0e-10),PetscObjectComm((PetscObject) L), PETSC_ERR_PLIB, "Graph Laplacian must have constant lowest eigenvector ev_%" PetscInt_FMT " %g != ev_0 %g", i, (double)(eigvec[evInd*n+i]), (double)(eigvec[evInd*n+0]));
    }
