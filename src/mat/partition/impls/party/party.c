...
static PetscErrorCode MatPartitioningApply_Party(MatPartitioning part,IS *partitioning)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(part->use_edge_weights,PetscObjectComm((PetscObject)part),PETSC_ERR_SUP,"Party does not support edge weights");
...
PetscErrorCode MatPartitioningPartySetCoarseLevel_Party(MatPartitioning part,PetscReal level)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(level<0.0 || level>1.0,PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Party: level of coarsening out of range [0.0-1.0]");
