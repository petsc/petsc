...
static PetscErrorCode MatPartitioningApply_Chaco(MatPartitioning part,IS *partitioning)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(part->use_edge_weights,PetscObjectComm((PetscObject)part),PETSC_ERR_SUP,"Chaco does not support edge weights");
...
#if defined(PETSC_HAVE_UNISTD_H)
  fd_stdout = dup(1);
  PetscAssertFalse(pipe(fd_pipe),PETSC_COMM_SELF,PETSC_ERR_SYS,"Could not open pipe");
  close(1);
  dup2(fd_pipe[1],1);
...
#endif
  PetscAssertFalse(ierr,PETSC_COMM_SELF,PETSC_ERR_LIB,"Chaco failed");
...
PetscErrorCode MatPartitioningChacoSetCoarseLevel_Chaco(MatPartitioning part,PetscReal level)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(level<0.0 || level>1.0,PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Chaco: level of coarsening out of range [0.0-1.0]");
...
PetscErrorCode MatPartitioningChacoSetEigenTol_Chaco(MatPartitioning part,PetscReal tol)
{
  ...
  if (tol==PETSC_DEFAULT) chaco->eigtol = 0.001;
    ...
    PetscAssertFalse(tol<=0.0,PetscObjectComm((PetscObject)part),PETSC_ERR_ARG_OUTOFRANGE,"Tolerance must be positive");
    chaco->eigtol = tol;
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatPartitioningChacoSetEigenNumber_Chaco(MatPartitioning part,PetscInt num)
{
  ...
  if (num==PETSC_DEFAULT) chaco->eignum = 1;
    ...
    PetscAssertFalse(num<1 || num>3,PetscObjectComm((PetscObject)part),PETSC_ERR_ARG_OUTOFRANGE,"Can only specify 1, 2 or 3 eigenvectors");
    chaco->eignum = num;
  }
  PetscFunctionReturn(0);
}
