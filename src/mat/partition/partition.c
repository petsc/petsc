...
static PetscErrorCode MatPartitioningApply_Square(MatPartitioning part,IS *partitioning)
{
  ...
  PetscFunctionBegin;
  ierr = MPI_Comm_size(PetscObjectComm((PetscObject)part),&size);CHKERRMPI(ierr);
  PetscAssertFalse(part->n != size,PetscObjectComm((PetscObject)part),PETSC_ERR_SUP,"Currently only supports one domain per processor");
  p = (PetscInt)PetscSqrtReal((PetscReal)part->n);
  PetscAssertFalse(p*p != part->n,PetscObjectComm((PetscObject)part),PETSC_ERR_SUP,"Square partitioning requires \"perfect square\" number of domains");

  ierr = MatGetSize(part->adj,&N,NULL);CHKERRQ(ierr);
  n    = (PetscInt)PetscSqrtReal((PetscReal)N);
  PetscAssertFalse(n*n != N,PetscObjectComm((PetscObject)part),PETSC_ERR_SUP,"Square partitioning requires square domain");
  PetscAssertFalse(n%p != 0,PETSC_COMM_SELF,PETSC_ERR_SUP,"Square partitioning requires p to divide n");
...
PETSC_INTERN PetscErrorCode MatPartitioningSizesToSep_Private(PetscInt p, PetscInt sizes[], PetscInt seps[], PetscInt level[])
{
  ...
  PetscFunctionBegin;
  l2p = PetscLog2Real(p);
  PetscAssertFalse(l2p - (PetscInt)PetscLog2Real(p),PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"%" PetscInt_FMT " is not a power of 2",p);
...
PetscErrorCode  MatPartitioningApplyND(MatPartitioning matp,IS *partitioning)
{
  ...
  PetscAssertFalse(!matp->adj->assembled,PetscObjectComm((PetscObject)matp),PETSC_ERR_ARG_WRONGSTATE,"Not for unassembled matrix");
  PetscAssertFalse(matp->adj->factortype,PetscObjectComm((PetscObject)matp),PETSC_ERR_ARG_WRONGSTATE,"Not for factored matrix");
  PetscAssertFalse(!matp->ops->applynd,PetscObjectComm((PetscObject)matp),PETSC_ERR_SUP,"Nested dissection not provided by MatPartitioningType %s",((PetscObject)matp)->type_name);
...
PetscErrorCode  MatPartitioningApply(MatPartitioning matp,IS *partitioning)
{
  ...
  PetscAssertFalse(!matp->adj->assembled,PetscObjectComm((PetscObject)matp),PETSC_ERR_ARG_WRONGSTATE,"Not for unassembled matrix");
  PetscAssertFalse(matp->adj->factortype,PetscObjectComm((PetscObject)matp),PETSC_ERR_ARG_WRONGSTATE,"Not for factored matrix");
  PetscAssertFalse(!matp->ops->apply,PetscObjectComm((PetscObject)matp),PETSC_ERR_ARG_WRONGSTATE,"Must set type with MatPartitioningSetFromOptions() or MatPartitioningSetType()");
...
PetscErrorCode  MatPartitioningSetType(MatPartitioning part,MatPartitioningType type)
{
  ...
  PetscAssertFalse(!r,PetscObjectComm((PetscObject)part),PETSC_ERR_ARG_UNKNOWN_TYPE,"Unknown partitioning type %s",type);
