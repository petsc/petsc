...
PetscErrorCode MatSetFromOptions(Mat B)
{
  ...
  flg = PETSC_FALSE;
  PetscCall(PetscOptionsBool("-mat_ignore_zero_entries", "For AIJ/SELL/IS matrices this will stop zero values from creating a zero location in the matrix", "MatSetOption", flg, &flg, &set));
...
- flg - bind to the CPU if value of `PETSC_TRUE`
  ...
  backend, `MATAIJKOKKOS` always use its own operation implementations (in contrast to CPU-bound
...
PetscErrorCode MatSetValuesCOO(Mat A, const PetscScalar coo_v[], InsertMode imode)
{
  ...
  if (f) {
    if (!A->structure_only) PetscCall((*f)(A, coo_v, imode)); // all known COO implementations do not use MatStash. They do their own off-proc communication
  ...
  } else if (!A->structure_only) PetscCall(MatSetValuesCOO_Basic(A, coo_v, imode)); // fall back to MatSetValues, which might use MatStash
