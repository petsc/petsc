...
int StashBuild_Private(Stash *stash)
{
  ...
  stash->n     = 0;
  stash->array = (Scalar *) MALLOC( stash->nmax*(2*sizeof(int) +
                            sizeof(Scalar))); CHKPTR(stash->array);
  stash->idx   = (int *) (stash->array + stash->nmax); CHKPTR(stash->idx);
  stash->idy   = (int *) (stash->idx + stash->nmax); CHKPTR(stash->idy);
  return 0;
}
...
int StashDestroy_Private(Stash *stash)
{
  ...
  if (stash->array) {FREE(stash->array); stash->array = 0;}
  return 0;
}
