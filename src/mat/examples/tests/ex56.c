#ifndef lint
static char vcid[] = "$Id: ex56.c,v 1.2 1997/02/06 23:26:31 balay Exp balay $";
#endif
...
{
  Mat         A;
  int         bs=3,m=4,i,j,val = 10,ierr,flg;
  Scalar      x[6][9];
  int         row[2],col[3];
  ...
  if (flg) { ierr = MatSetOption(A,MAT_COLUMN_ORIENTED); CHKERRA(ierr); }
