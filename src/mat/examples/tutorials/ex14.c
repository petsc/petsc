...
#undef __FUNCT__
#define __FUNCT__ "TestMatFwkGlobal"
PetscErrorCode TestMatFwkGlobal() {
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD, "Testing global MatFwk\n"); CHKERRQ(ierr);
  ...
  */
  ...
  /* We only need to construct a single block and then reuse it */
  B = PETSC_NULL;
  for(i = 0; i < 4; ++i) {
    values[i] *= h;
  }
  for(i = 0; i < e; ++i) {
    if(B == PETSC_NULL) {
      ...
      ierr = MatFwkSetBlock(M, i, i, B); CHKERRQ(ierr);
    }
  }
...
#undef __FUNCT__
#define __FUNCT__ "TestMatFwkLocal"
PetscErrorCode TestMatFwkLocal() {
  Mat M; /* Matrix framekwork */
  Mat B; /* Framework block */
  PetscInt e,n;
  PetscReal h;
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD, "Testing local MatFwk\n"); CHKERRQ(ierr);
  ...
  /* Construct vectors with local elementwise layouts of degrees of freedom */
  /* Local number of elementwise degrees of freedom is twice the local number of elements e*/
  ierr = VecCreateMPI(PETSC_COMM_WORLD, e*2, PETSC_DECIDE, &V); CHKERRQ(ierr);

  /* Now create the matrix framework with no scatter or gather */
  ierr = MatCreate(PETSC_COMM_WORLD, &M); CHKERRQ(ierr);
  n = 2*e;
  ierr = MatSetSizes(M, n, n, PETSC_DECIDE, PETSC_DECIDE); CHKERRQ(ierr);
  ierr = MatSetType(M, MATFWK); CHKERRQ(ierr);
  ...
  ierr = MatFwkSetScatter(M, PETSC_NULL, e, blocks); CHKERRQ(ierr);
  ierr = MatFwkSetGather(M, PETSC_NULL, e, blocks); CHKERRQ(ierr);
...
int main(int argc,char **args) {
  ...
  ierr = TestMatFwkGlobal();
  ierr = TestMatFwkLocal();
