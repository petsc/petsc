...
int main(int argc,char **args)
{
  ...
  if (i < 0 || j < 0 || k1 < 0 || k2 < 0) {
    PetscCall(PetscPrintf(PETSC_COMM_SELF,"Error (compare A and sA): MatGetInfo()\n"));
  }
  ...
  if (rnorm > tol) {
    PetscCall(PetscPrintf(PETSC_COMM_SELF,"Error: MatNorm_FROBENIUS, NormA=%16.14e NormsB=%16.14e\n",(double)norm1,(double)norm2));
  }
  PetscCall(MatNorm(A,NORM_INFINITY,&norm1));
  PetscCall(MatNorm(sB,NORM_INFINITY,&norm2));
  rnorm = PetscAbsReal(norm1-norm2)/norm2;
  if (rnorm > tol) {
    PetscCall(PetscPrintf(PETSC_COMM_SELF,"Error: MatNorm_INFINITY(), NormA=%16.14e NormsB=%16.14e\n",(double)norm1,(double)norm2));
  }
  ...
  for (i=0; i<40; i++) {
    ...
    norm1 -= norm2;
    if (norm1<-tol || norm1>tol) {
      PetscCall(PetscPrintf(PETSC_COMM_SELF,"Error: MatMult(), norm1-norm2: %g\n",(double)norm1));
    }
  }
  ...
  for (lf=-1; lf<10; lf += inc) {
    ...
    */
    ...
    if (lf == -1) {
      ...
      if (10*norm1 < norm2) {
        PetscCall(PetscPrintf(PETSC_COMM_SELF,"MatForwardSolve and BackwardSolve: Norm of error=%g, bs=%" PetscInt_FMT "\n",(double)norm2,bs));
      }
    }
    ...
    if (10*norm1 < norm2 && lf-inc != -1) {
      PetscCall(PetscPrintf(PETSC_COMM_SELF,"lf=%" PetscInt_FMT ", %" PetscInt_FMT ", Norm of error=%g, %g\n",lf-inc,lf,(double)norm1,(double)norm2));
    }
    norm1 = norm2;
  ...
  for (i=0; i<10; i++) {
    ...
    if (norm2>tol) {
      PetscCall(PetscPrintf(PETSC_COMM_SELF,"Error:MatSolve(), norm2: %g\n",(double)norm2));
    }
  }
  PetscCall(MatDestroy(&sFactor));
#endif
