...
#define CHKERRQPARMETIS(n) \
  PetscAssertFalse(n == METIS_ERROR_INPUT,PETSC_COMM_SELF,PETSC_ERR_LIB,"ParMETIS error due to wrong inputs and/or options"); \
  else PetscAssertFalse(n == METIS_ERROR_MEMORY,PETSC_COMM_SELF,PETSC_ERR_LIB,"ParMETIS error due to insufficient memory"); \
  else PetscAssertFalse(n == METIS_ERROR,PETSC_COMM_SELF,PETSC_ERR_LIB,"ParMETIS general error"); \
...
int main(int argc, char *argv[])
{
  ...
  PetscAssertFalse(!flg,PETSC_COMM_WORLD,PETSC_ERR_USER,"Must specify -prefix");
  ...
  red = fread(vtxdist, sizeof(idx_t), size+1, fp);PetscAssertFalse(red != (size_t) (size+1),PETSC_COMM_SELF,PETSC_ERR_SYS,"Unable to read from data file");

  ni = vtxdist[rank+1]-vtxdist[rank];

  ierr = PetscMalloc1(ni+1,&xadj);CHKERRQ(ierr);

  red = fread(xadj, sizeof(idx_t), ni+1, fp);PetscAssertFalse(red != (size_t) (ni+1),PETSC_COMM_SELF,PETSC_ERR_SYS,"Unable to read from data file");

  ierr = PetscMalloc1(xadj[ni],&adjncy);CHKERRQ(ierr);

  for (i=0; i<ni; i++) {
    red = fread(&adjncy[xadj[i]], sizeof(idx_t), xadj[i+1]-xadj[i], fp);PetscAssertFalse(red != (size_t) (xadj[i+1]-xadj[i]),PETSC_COMM_SELF,PETSC_ERR_SYS,"Unable to read from data file");
  }
  ...
  red = fread(xyz, sizeof(PetscReal), ndims*ni, fp);PetscAssertFalse(red != (size_t) (ndims*ni),PETSC_COMM_SELF,PETSC_ERR_SYS,"Unable to read from data file");
