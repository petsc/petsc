...
int main(int argc,char **argv)
{
  ...
  if (r != 0.0) SETERRQ(PetscObjectComm((PetscObject)B),PETSC_ERR_PLIB,"Error MatDuplicate + MatCopy + MatAXPY %g",(double)r);
  ...
  ierr = MatNorm(B,NORM_INFINITY,&r);CHKERRQ(ierr);
  if (r != 0.0) SETERRQ(PetscObjectComm((PetscObject)B),PETSC_ERR_PLIB,"Error MatDuplicate + MatCopy + MatAXPY_Basic %g",(double)r);
  ...
  if (m == n) {
    ...
    if (r > tol) SETERRQ(PetscObjectComm((PetscObject)B),PETSC_ERR_PLIB,"Error MatPtAP %g",(double)r);
