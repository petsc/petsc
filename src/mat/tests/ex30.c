...
int main(int argc,char **args)
{
  ...
  if (!LU && lf==0) {
    ...
    */
    ...
    if (PetscAbs(norm2 - norm2_inplace) > tol) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"ILU(0) %g and in-place ILU(0) %g give different residuals",(double)norm2,(double)norm2_inplace);
    ierr = MatDestroy(&A);CHKERRQ(ierr);
  }
  ...
  if (!CHOLESKY && lf==0 && !matordering) {
    ...
    */
    ...
    ierr = VecAXPY(y,-1.0,x);CHKERRQ(ierr);
    ierr = VecNorm(y,NORM_2,&norm2_inplace);CHKERRQ(ierr);
    if (PetscAbs(norm2 - norm2_inplace) > tol) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"ICC(0) %g and in-place ICC(0) %g give different residuals",(double)norm2,(double)norm2_inplace);
    ierr = MatDestroy(&A);CHKERRQ(ierr);
  }
