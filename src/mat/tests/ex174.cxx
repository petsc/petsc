...
int main(int argc, char **args)
{
  ...
  /* Load PETSc matrices */
  PetscCall(PetscOptionsGetString(NULL, NULL, "-fA", file[0], sizeof(file[0]), NULL));
  ...
  PetscCall(PetscOptionsGetString(NULL, NULL, "-fB", file[1], sizeof(file[1]), &flgB));
  if (flgB) {
  ...
  PetscCall(PetscObjectTypeCompare((PetscObject)A, MATELEMENTAL, &isElemental));
  if (isElemental) {
    Ae      = A;
    Be      = B;
    ...
    PetscCall(MatConvert(A, MATELEMENTAL, MAT_INITIAL_MATRIX, &Ae));
