...
int main(int argc,char **args)
{
  ...
  for (test = 0; test < ntest; test++) {
    ...
    if (!flg) {
      PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error mult add\n",test));
    }
    PetscCall(MatMultTransposeAddEqual(A,S,10,&flg));
    if (!flg) {
      PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error mult add (T)\n",test));
    }
    if (testzerorows) {
      Mat       ST,B,C,BT,BTT;
      IS        zr;
      ...
      if (b1) {
        ...
        if (err >= tol) {
          PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error b %g\n",test,(double)err));
        }
      ...
      PetscCall(MatNorm(C,NORM_FROBENIUS,&err));
      if (err >= tol) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error mat mult transpose after %s %g\n",test,ronl ? "MatZeroRows" : "MatZeroRowsColumns",(double)err));
      }
    ...
    if (testgetdiag && cong) { /* MatGetDiagonal() */
      ...
      if (err >= tol) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error diag %g\n",test,(double)err));
      }
    ...
    if (testsubmat) {
      ...
      if (!flg) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error submatrix mult add\n",test));
      }
      PetscCall(MatMultTransposeAddEqual(sA,sS,10,&flg));
      if (!flg) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error submatrix mult add (T)\n",test));
      }
      ...
      if (err >= tol) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error mat submatrix %g\n",test,(double)err));
      }
      ...
      PetscCall(MatMultTransposeAddEqual(sA,sS,10,&flg));
      if (!flg) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error submatrix (T) mult add (T)\n",test));
      }
      PetscCall(MatConvert(sA,MATDENSE,MAT_INITIAL_MATRIX,&dA));
      PetscCall(MatConvert(sS,MATDENSE,MAT_INITIAL_MATRIX,&dS));
      PetscCall(MatAXPY(dA,-1.0,dS,SAME_NONZERO_PATTERN));
      PetscCall(MatNorm(dA,NORM_FROBENIUS,&err));
      if (err >= tol) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error mat submatrix (T) %g\n",test,(double)err));
      }
      PetscCall(MatDestroy(&sA));
      PetscCall(MatDestroy(&sS));
      PetscCall(MatDestroy(&dA));
    ...
    if (testaxpy) {
      ...
      PetscCall(MatMultAddEqual(A,S,10,&flg));
      if (!flg) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error axpy mult add\n",test));
      }
      PetscCall(MatMultTransposeAddEqual(A,S,10,&flg));
      if (!flg) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,"[test %" PetscInt_FMT "] Error axpy mult add (T)\n",test));
      }
