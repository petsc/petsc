...
int main(int argc,char **args)
{
  ...
  PetscAssertFalse(!flg,PETSC_COMM_WORLD,PETSC_ERR_USER_INPUT,"Input file not specified");
  ...
  for (ct=0; ct<100; ct++) {
    ...
    for (i=0,j=0; i<ncols1 && j<ncols2; j++) {
      ...
      PetscAssertFalse(v1[i] != v2[j],PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatGetRow() failed - vals incorrect.");
    }
    PetscAssertFalse(j<ncols2,PETSC_COMM_SELF,PETSC_ERR_PLIB,"MatGetRow() failed - cols incorrect");
