...
int main(int argc,char **args)
{
  ...
  PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Error in MatConvert(): A != sA");
  ...
  if (TestSubMat) {
    ...
    ierr = MatMultEqual(A,sA,10,&flg);CHKERRQ(ierr);
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"A != sA");
    ...
    ierr = MatMultEqual(A,sA,10,&flg);CHKERRQ(ierr);
    PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"MatCreateSubmatrices(): A != sA");
