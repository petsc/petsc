...
static PetscErrorCode AssembleMatrix(MPI_Comm comm,Mat *A)
{
  ...
  CHKERRQ(MatSetSizes(B,5,6,PETSC_DETERMINE,PETSC_DETERMINE));
  ...
  CHKERRQ(MatSetValue(B,me-1,me,me*me,INSERT_VALUES));
...
int main(int argc, char *argv[])
{
  ...
  idxcol[2] = ms+4;
  idxcol[3] = ms+5;
  CHKERRQ(ISCreateGeneral(PETSC_COMM_WORLD,4,idxcol,PETSC_USE_POINTER,&iscol));
