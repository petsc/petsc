#ifndef lint
static char vcid[] = "$Id: qcg.c,v 1.12 1995/11/07 13:36:35 bsmith Exp curfman $";
#endif
/*
...
static int KSPSolve_GenQCG(KSP itP,int *its);
static int KSPSolve_ICQCG(KSP itP,int *its);
...
$  3 if convergence is reached along a truncated step.
  ...
  Notes:
  KSPSolve_QCG() is coded for use with general preconditioners, using just calls to
  PCApply().  In contract, KSPSolve_QICCCG() is coded explicitly with forward/backward
  solves for incomplete Cholesky preconditioning, since this version requires fewer
  triangular solves than the general case.
 */
int KSPSolve_QCG(KSP itP,int *its)
{
  ...
  if (pctype == PCICC && (!OptionsHasName(PETSC_NULL,"-ksp_general_qcg")))
    return KSPSolve_ICQCG(itP,its);
  else 
    return KSPSolve_GenQCG(itP,its);
}
 ...
 */
  ...
  *its = 0;
  ...
  /* ierr = MatForwardSolve(Pmat,B,BS); CHKERRQ(ierr); */
  ...
  /* ierr = MatBackwardSolve(Pmat,X,X); CHKERRQ(ierr); */  /* Unscale x */
  ierr = VecCopy(X,WA2); CHKERRQ(ierr);
  ierr = PCApply(pc,WA2,X); CHKERRQ(ierr);
