...
static int PCDestroy_SPAI(PetscObject obj)
{
  PC       pc = (PC) obj;
  PC_SPAI *ispai = (PC_SPAI *) pc->data;
...
static int PCView_SPAI(PetscObject obj,Viewer viewer)
{
  PC         pc = (PC)obj;
  PC_SPAI    *ispai = (PC_SPAI *) pc->data;
  FILE       *fd;
  ...
  if (vtype  == ASCII_FILE_VIEWER || vtype == ASCII_FILES_VIEWER) {  
    ...
    PetscFPrintf(pc->comm,fd,"    maxapi %d\n",ispai->maxapi);
...
int MatConvertToSPAI(Mat A,matrix **B)
{
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Allgather(&mnl, 1, MPI_INT,M->mnls, 1, MPI_INT,MPI_COMM_WORLD);
  M->start_indices[0] = 0;
  for (i=1; i<size; i++) {
  ...
  MPI_Allreduce(&len,&M->maxnz,1,MPI_INT,MPI_MAX,comm);

  for ( i=rstart; i<rend; i++ ) {
    row_indx             = i-rstart;
    len                  = num_row_ptr[row_indx];
    rows->ptrs[row_indx] = (dist_ptr *) PetscMalloc(len*sizeof(dist_ptr));
    CHKPTRQ(rows->ptrs[row_indx]);
    rows->A[row_indx]    = (double *) PetscMalloc(len*sizeof(double));
    CHKPTRQ(rows->A[row_indx]);
    rows->adrs[row_indx] = (double **) PetscMalloc(len*sizeof(double *));
    CHKPTRQ(rows->adrs[row_indx]);
  }
