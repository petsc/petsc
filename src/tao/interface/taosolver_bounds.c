...
PetscErrorCode TaoComputeVariableBounds(Tao tao)
{
  ...
  PetscValidHeaderSpecific(tao,TAO_CLASSID,1);
  if (!tao->XL || !tao->XU) {
    PetscAssertFalse(!tao->solution,PetscObjectComm((PetscObject)tao),PETSC_ERR_ORDER,"TaoSetInitialVector must be called before TaoComputeVariableBounds");
...
PetscErrorCode TaoComputeConstraints(Tao tao, Vec X, Vec C)
{
  ...
  PetscAssertFalse(!tao->ops->computeconstraints,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"TaoSetConstraintsRoutine() has not been called");
...
PetscErrorCode TaoComputeEqualityConstraints(Tao tao, Vec X, Vec CE)
{
  ...
  PetscAssertFalse(!tao->ops->computeequalityconstraints,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"TaoSetEqualityConstraintsRoutine() has not been called");
...
PetscErrorCode TaoComputeInequalityConstraints(Tao tao, Vec X, Vec CI)
{
  ...
  PetscAssertFalse(!tao->ops->computeinequalityconstraints,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"TaoSetInequalityConstraintsRoutine() has not been called");
