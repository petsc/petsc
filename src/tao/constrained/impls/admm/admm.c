...
static PetscErrorCode TaoSolve_ADMM(Tao tao)
{
  ...
  if (am->regswitch != TAO_ADMM_REGULARIZER_SOFT_THRESH) {
    PetscAssertFalse(!am->subsolverX->ops->computejacobianequality,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"Must call TaoADMMSetMisfitConstraintJacobian() first");
    PetscAssertFalse(!am->subsolverZ->ops->computejacobianequality,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"Must call TaoADMMSetRegularizerConstraintJacobian() first");
    if (am->constraint != NULL) {
  ...
  if (!is_reg_shell) {
    ...
    case (TAO_ADMM_REGULARIZER_USER):
      PetscAssertFalse(!am->ops->regobjgrad,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"Must call TaoADMMSetRegularizerObjectiveAndGradientRoutine() first if one wishes to use TAO_ADMM_REGULARIZER_USER with non-TAOSHELL type");
      break;
...
static PetscErrorCode TaoSetUp_ADMM(Tao tao)
{
  ...
  if (!am->constraint) {
    ...
    PetscAssertFalse(M != N,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_WRONGSTATE,"Solution vector and constraint vector must be of same size!");
  }
