...
PetscErrorCode TaoLineSearchView(TaoLineSearch ls, PetscViewer viewer)
{
  ...
  if (!viewer) {
    PetscCall(PetscViewerASCIIGetStdout(((PetscObject)ls)->comm, &viewer));
  }
...
PetscErrorCode TaoLineSearchDestroy(TaoLineSearch *ls)
{
  ...
  if ((*ls)->usemonitor) {
    PetscCall(PetscViewerDestroy(&(*ls)->viewer));
  }
...
PetscErrorCode TaoLineSearchComputeObjective(TaoLineSearch ls, Vec x, PetscReal *f)
{
  ...
  if (ls->usetaoroutines) {
    ...
    if (ls->ops->computeobjective) PetscCallUser("TaoLineSearch callback objective",(*ls->ops->computeobjective)(ls,x,f,ls->userctx_func));
    else if (ls->ops->computeobjectiveandgradient) {
      PetscCall(VecDuplicate(x,&gdummy));
      PetscCallUser("TaoLineSearch callback objective",(*ls->ops->computeobjectiveandgradient)(ls,x,f,gdummy,ls->userctx_funcgrad));
      PetscCall(VecDestroy(&gdummy));
    } else PetscCallUser("TaoLineSearch callback objective",(*ls->ops->computeobjectiveandgts)(ls,x,ls->stepdirection,f,&gts,ls->userctx_funcgts));
...
PetscErrorCode TaoLineSearchComputeObjectiveAndGradient(TaoLineSearch ls, Vec x, PetscReal *f, Vec g)
{
  ...
  if (ls->usetaoroutines) {
    ...
    PetscCall(PetscLogEventBegin(TAOLINESEARCH_Eval,ls,0,0,0));
    if (ls->ops->computeobjectiveandgradient) PetscCallUser("TaoLineSearch callback objective/gradient",(*ls->ops->computeobjectiveandgradient)(ls,x,f,g,ls->userctx_funcgrad));
    else {
      ...
      PetscCallUser("TaoLineSearch callback objective",(*ls->ops->computeobjective)(ls,x,f,ls->userctx_func));
      ...
      PetscCallUser("TaoLineSearch callback gradient",(*ls->ops->computegradient)(ls,x,g,ls->userctx_grad));
    }
    PetscCall(PetscLogEventEnd(TAOLINESEARCH_Eval,ls,0,0,0));
...
PetscErrorCode TaoLineSearchComputeGradient(TaoLineSearch ls, Vec x, Vec g)
{
  ...
  if (ls->usetaoroutines) {
    ...
    PetscCall(PetscLogEventBegin(TAOLINESEARCH_Eval,ls,0,0,0));
    if (ls->ops->computegradient) PetscCallUser("TaoLineSearch callback gradient",(*ls->ops->computegradient)(ls,x,g,ls->userctx_grad));
    else {
      ...
      PetscCallUser("TaoLineSearch callback gradient",(*ls->ops->computeobjectiveandgradient)(ls,x,&fdummy,g,ls->userctx_funcgrad));
    }
    PetscCall(PetscLogEventEnd(TAOLINESEARCH_Eval,ls,0,0,0));
  }
  ls->ngeval++;
...
PetscErrorCode TaoLineSearchComputeObjectiveAndGTS(TaoLineSearch ls, Vec x, PetscReal *f, PetscReal *gts)
{
  ...
  PetscCall(PetscLogEventBegin(TAOLINESEARCH_Eval,ls,0,0,0));
  PetscCallUser("TaoLineSearch callback objective/gts",(*ls->ops->computeobjectiveandgts)(ls,x,ls->stepdirection,f,gts,ls->userctx_funcgts));
  PetscCall(PetscLogEventEnd(TAOLINESEARCH_Eval,ls,0,0,0));
  PetscCall(PetscInfo(ls,"TaoLineSearch Function evaluation: %14.12e\n",(double)(*f)));
  ls->nfeval++;
  PetscFunctionReturn(0);
