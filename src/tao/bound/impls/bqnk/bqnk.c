...
PetscErrorCode TaoSetUp_BQNK(Tao tao)
{
  ...
  PetscAssertFalse(!is_lmvm,PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "Matrix must be an LMVM-type");
  ierr = MatGetOption(bqnk->B, MAT_SYMMETRIC, &is_sym);CHKERRQ(ierr);
  PetscAssertFalse(!is_sym,PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "LMVM matrix must be symmetric");
...
PetscErrorCode TaoGetLMVMMatrix(Tao tao, Mat *B)
{
  ...
  PetscAssertFalse(!flg,PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "LMVM Matrix only exists for quasi-Newton algorithms");
  *B = bqnk->B;
  PetscFunctionReturn(0);
}
...
PetscErrorCode TaoSetLMVMMatrix(Tao tao, Mat B)
{
  ...
  PetscAssertFalse(!flg,PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "Given matrix is not an LMVM matrix");
  if (bqnk->B) {
    ierr = MatDestroy(&bqnk->B);CHKERRQ(ierr);
  }
