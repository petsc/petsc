  ...
  if (bnk->is_nash || bnk->is_stcg || bnk->is_gltr) {
    ...
    case BNK_INIT_INTERPOLATION:
      ...
      for (j = 0; j < j_max; ++j) {
        ...
        if (f_min < bnk->f) {
          /* We found a solution better than the initial, so let's test and accept it */
          bnk->f = f_min;
...
PetscErrorCode TaoSetUp_BNK(Tao tao)
{
  ...
  if (bnk->max_cg_its > 0) {
    if (!bnk->bncg_sol) {ierr = VecDuplicate(tao->solution,&bnk->bncg_sol);CHKERRQ(ierr);}
