...
PetscErrorCode TaoBNKInitialize(Tao tao, PetscInt initType, PetscBool *needH)
{
  ...
  PetscAssertFalse(PetscIsInfOrNanReal(bnk->f) || PetscIsInfOrNanReal(resnorm),PetscObjectComm((PetscObject)tao),PETSC_ERR_USER, "User provided compute function generated Inf or NaN");
  ...
  if (is_bfgs) {
    ...
    PetscAssertFalse(!sym_set || !is_symmetric,PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "LMVM matrix in the LMVM preconditioner must be symmetric.");
  } else if (is_jacobi) {
    ...
    case BNK_INIT_INTERPOLATION:
      ...
      for (j = 0; j < j_max; ++j) {
        ...
        for (i = 0; i < i_max; ++i) {
          ...
          PetscAssertFalse(PetscIsInfOrNanReal(bnk->f),PetscObjectComm((PetscObject)tao),PETSC_ERR_USER, "User provided compute function generated Inf or NaN");
  ...
  if (kspTR) {
    ...
    if (0.0 == tao->trust) {
      ...
      if (bnk->dnorm > 0.0) {
        ...
        PetscAssertFalse(bnk->dnorm == 0.0,PetscObjectComm((PetscObject)tao),PETSC_ERR_PLIB, "Initial direction zero");
      }
    }
  } else {
    ierr = KSPSolve(tao->ksp, bnk->G_inactive, bnk->X_inactive);CHKERRQ(ierr);
