...
static const char *CG_Table[64] = {"gd", "hs", "fr", "pr", "prp", "dy", "hz", "dk", "kd", "ssml_bfgs", "ssml_dfp", "ssml_brdn"};
...
static PetscErrorCode TaoSolve_BNCG(Tao tao)
{
  ...
  while(1) {
    ...
    /* Steffenson is currently an experimental (broken) feature. Do not use. */
    if (cg->use_steffenson) {
      ierr = TaoBNCGSteffensonAcceleration(tao);CHKERRQ(ierr);
    }
    if (tao->reason != TAO_CONTINUE_ITERATING) PetscFunctionReturn(0);
  }
  PetscFunctionReturn(0);
...
static PetscErrorCode TaoSetUp_BNCG(Tao tao)
{
  ...
  if (cg->use_steffenson){
    if (!cg->steffnm1) {
      ierr = VecDuplicate(tao->solution, &cg->steffnm1);CHKERRQ(ierr);
    }
    if (!cg->steffn) {
      ierr = VecDuplicate(tao->solution, &cg->steffn);CHKERRQ(ierr);
    }
    if (!cg->steffnp1) {
      ierr = VecDuplicate(tao->solution, &cg->steffnp1);CHKERRQ(ierr);
    }
    if (!cg->steffva) {
      ierr = VecDuplicate(tao->solution, &cg->steffva);CHKERRQ(ierr);
    }
    if (!cg->steffvatmp) {
      ierr = VecDuplicate(tao->solution, &cg->steffvatmp);CHKERRQ(ierr);
    }
  }
  if (cg->diag_scaling){
...
static PetscErrorCode TaoSetFromOptions_BNCG(PetscOptionItems *PetscOptionsObject,Tao tao)
{
    ...
    ierr = PetscOptionsBool("-tao_bncg_use_steffenson","(developer - incomplete, do not use) use vector-Steffenson acceleration","",cg->use_steffenson,&cg->use_steffenson,NULL);CHKERRQ(ierr);
    ...
    ierr = PetscOptionsBool("-tao_bncg_recycle","(developer) enable recycling the existing solution and gradient at the start of a new solve","",cg->recycle,&cg->recycle,NULL);CHKERRQ(ierr);
...
.      -tao_bncg_use_steffenson <b> (not implemented) - use Steffenson acceleration
...
PETSC_INTERN PetscErrorCode TaoBNCGStepDirectionUpdate(Tao tao, PetscReal gnorm2, PetscReal step, PetscReal fold, PetscReal gnorm2_old, PetscBool cg_restart, PetscReal dnorm, PetscReal ginner)
{
  ...
  PetscReal         tmp = 1.0, ynorm, ynorm2, snorm = 1.0, dk_yk=1.0, gd;
  ...
  if (tao->niter >= 1){
    ...
    if (step*dnorm < PETSC_MACHINE_EPSILON || step*dk_yk < PETSC_MACHINE_EPSILON) cg_restart = 1;
  ...
  /* A note on diagonal scaling (to be added to paper): For the FR, PR, PRP, and DY methods, the diagonally scaled versions must be derived as a preconditioned CG method rather than as a Hessian initialization like in the Broyden methods. */

  /* In that case, one writes the objective function as f(x) \equiv f(Ay). Gradient evaluations yield g(x_k) = A g(Ay_k) = A g(x_k). Furthermore, the direction d_k \equiv (x_k - x_{k-1})/step according to HZ (2006) becomes A^{-1} d_k, such that d_k^T g_k remains the same under preconditioning. Note that A is diagonal, such that A^T = A. */

  /* This yields questions like what the dot product d_k^T y_k should look like. HZ mistakenly treats that as the same under preconditioning, but that is not necessarily true. */

  /* Observe y_k \equiv g_k - g_{k-1}, and under the P.C. transformation, we get d_k^T y_k = (d_k^T A_k^{-T} A_k g_k - d_k^T A_k^{-T} A_{k-1} g_{k-1}), yielding d_k^T y_k = d_k^T g_k - d_k^T (A_k^{-T} A_{k-1} g_{k-1}), which is NOT the same if our preconditioning matrix is updated between iterations. This same issue is found when considering dot products of the form g_{k+1}^T y_k. */

  /* As of now, for PR, PRP, and DY, the above considerations have not been fully taken into account, explaining their poorer performance. FR is implemented correctly, and yields some marginal improvement on performance. Working on the rest now. */
  ...
  if (cg_restart) {
    ...
    case CG_PolakRibierePlus:
      ...
      break;
    case CG_DaiYuan: /* TODO: suspect this is broken in the diag part - test4 */
      if (!cg->diag_scaling){
        ...
        ierr = MatMult(cg->B, tao->gradient, cg->g_work);CHKERRQ(ierr);
    ...
    case CG_SSML_BROYDEN:
      ...
      if (!cg->diag_scaling){
        /* Instead of a regular convex combination, we will solve a quadratic formula. */
        ierr = TaoBNCGComputeScalarScaling(ynorm2, step*dk_yk, snorm*snorm, &tau_bfgs, cg->bfgs_scale);CHKERRQ(ierr);
        ierr = TaoBNCGComputeScalarScaling(ynorm2, step*dk_yk, snorm*snorm, &tau_dfp, cg->dfp_scale);CHKERRQ(ierr);
        ierr = VecDot(cg->yk, tao->gradient, &gkp1_yk);CHKERRQ(ierr);
      ...
      break;
    case CG_HagerZhang:
      /* Their 2006 paper. Comes from deleting the y_k term from SSML_BFGS, introducing a theta parameter, and using a cutoff for beta. See DK 2013 pg. 315 for a review of CG_DESCENT 5.3. One can use the dynamic restart strategy they implement, but it doesn't work well for us. */
      ierr = VecDot(tao->gradient, tao->stepdirection, &gd);CHKERRQ(ierr);
      ierr = VecDot(cg->G_old, tao->stepdirection, &gd_old);CHKERRQ(ierr);
      ierr = VecWAXPY(cg->sk, -1.0, cg->X_old, tao->solution);CHKERRQ(ierr);
      snorm = dnorm*step;
  ...
  cg_restart = 0; /* Will check in next iteration */
  PetscFunctionReturn(0);
}
...
PETSC_INTERN PetscErrorCode TaoBNCGSteffensonAcceleration(Tao tao)
{
  ...
  if (tao->niter > 2 && tao->niter % 2 == 0){
    ...
    ierr = VecDot(cg->W, cg->W, &mag1);CHKERRQ(ierr);
    ierr = VecAXPBYPCZ(cg->W, -1.0, 1.0, -1.0, cg->steffn, cg->steffnp1);CHKERRQ(ierr);
    ierr = VecDot(cg->W, cg->W, &mag2);CHKERRQ(ierr);
    ...
    ierr = VecAXPBYPCZ(cg->steffva, 1.0, -1.0, 0.0, cg->steffnp1, cg->steffn);CHKERRQ(ierr);
    ierr = VecAXPBYPCZ(cg->steffva, 1.0, -1.0, 1.0, cg->steffnm1, cg->steffn);CHKERRQ(ierr);
...
PETSC_INTERN PetscErrorCode TaoBNCGConductIteration(Tao tao, PetscReal gnorm)
{
  ...
  if (cg->cg_type != CG_GradientDescent) {
    ...
    if (PetscIsInfOrNanReal(gd) || (gd/(dnorm*dnorm) <= -1e10 || gd/(dnorm*dnorm) >= -1e-10)) {
      ...
      PetscPrintf(PETSC_COMM_SELF, "gtd/(dtd) is small or positive: %20.19e\n", gd/(dnorm*dnorm));
      ierr = TaoBNCGResetUpdate(tao, gnorm2);CHKERRQ(ierr);
      ierr = TaoBNCGBoundStep(tao, cg->as_type, tao->stepdirection);CHKERRQ(ierr);
      ++cg->descent_error;
