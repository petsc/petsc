...
int main(int argc, char **argv)
{
  ...
  ierr = TaoSetJacobianStateRoutine(tao, user.Js, PETSC_NULL, user.JsInv, FormJacobianState, (void *)&user); CHKERRQ(ierr); 
...
PetscErrorCode EllipticInitialize(AppCtx *user)
{
  ...
  */
  ...
  for (i=0; i<user->ns; i++){
    ...
    ierr = VecScatterCreate(user->y,is_from_y,user->suby,PETSC_NULL,&user->yi_scatter[i]); CHKERRQ(ierr);
    istart = istart + hi-lo;
  ...
  */
  ...
  for (i=0; i<user->ns; i++){
    ...
    ierr = VecScatterCreate(user->d,is_from_d,user->subd,PETSC_NULL,&user->di_scatter[i]); CHKERRQ(ierr);
    istart = istart + hi-lo;
  ...
  if (!user->use_ptap) {
    ...
    ierr = MatMPIAIJSetPreallocation(user->Diag,1,PETSC_NULL,0,PETSC_NULL); CHKERRQ(ierr);
    ierr = MatSeqAIJSetPreallocation(user->Diag,1,PETSC_NULL); CHKERRQ(ierr);
