...
static PetscErrorCode TaoSetup_SQPCON(TaoSolver tao)
{
  ...
  PetscFunctionBegin;
  if (!tao->state_is) SETERRQ(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONGSTATE,"SQPCON Solver requires an initial state index set -- use TaoSetStateIS()");
...
PetscErrorCode TaoCreate_SQPCON(TaoSolver tao)
{
  ...
  ierr = PetscNewLog(tao,&sqpconP);CHKERRQ(ierr);
