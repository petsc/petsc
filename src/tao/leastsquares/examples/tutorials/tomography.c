...
/* static char help[] = "Reads PETSc matrix and vector from files. Verify their norms\n\n"; */
#include <petscmat.h>
int mainDummy(int argc,char **argv)
{
  ...
  ierr = VecCreate(PETSC_COMM_WORLD,&xGT);CHKERRQ(ierr);
  ierr = VecLoad(xGT,fd);CHKERRQ(ierr);  
  ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);
  ...
  ierr = VecDestroy(&xGT);CHKERRQ(ierr);
  ierr = PetscFinalize();
  return ierr;
}
