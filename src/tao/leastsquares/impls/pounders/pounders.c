...
static PetscErrorCode pounders_feval(Tao tao, Vec x, Vec F, PetscReal *fsum)
{
  ...
  PetscAssertFalse(PetscIsInfOrNanReal(*fsum),PETSC_COMM_SELF,PETSC_ERR_USER, "User provided compute function generated Inf or NaN");
  PetscFunctionReturn(0);
}
...
static PetscErrorCode gqtwrap(Tao tao,PetscReal *gnorm, PetscReal *qmin)
{
  ...
  if (!mfqP->usegqt) {
    ...
    PetscAssertFalse(maxval > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"upper bound < lower bound in subproblem");
    /* Make sure xu > tao->solution > xl */
    ierr = VecCopy(mfqP->subxl,mfqP->subpdel);CHKERRQ(ierr);
    ierr = VecAXPY(mfqP->subpdel,-1.0,mfqP->subx);CHKERRQ(ierr);
    ierr = VecMax(mfqP->subpdel,NULL,&maxval);CHKERRQ(ierr);
    PetscAssertFalse(maxval > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"initial guess < lower bound in subproblem");

    ierr = VecCopy(mfqP->subx,mfqP->subpdel);CHKERRQ(ierr);
    ierr = VecAXPY(mfqP->subpdel,-1.0,mfqP->subxu);CHKERRQ(ierr);
    ierr = VecMax(mfqP->subpdel,NULL,&maxval);CHKERRQ(ierr);
    PetscAssertFalse(maxval > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"initial guess > upper bound in subproblem");
...
/* Computes the parameters of the quadratic Q(x) = c + g'*x + 0.5*x*G*x'
  ...
  PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine getrf returned with value %d",info);
  ...
  if (np == mfqP->n+1) {
    ...
    PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine potrf returned with value %d",info);
  }
  ...
  for (k=0;k<mfqP->m;k++) {
    if (np != mfqP->n+1) {
      ...
      PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine potrs returned with value %d",info);
    ...
    PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine getrs returned with value %d",info);
...
static PetscErrorCode morepoints(TAO_POUNDERS *mfqP)
{
  ...
  while (mfqP->nmodelpoints < mfqP->npmax && point>=0) {
    ...
    PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine geqrf returned with value %d",info);
    ...
    PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine ormqr returned with value %d",info);
    ...
    PetscAssertFalse(info != 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine gesvd returned with value %d",info);
  ...
  for (i=mfqP->nHist-1;i>=0;i--) {
    ...
    if (normd <= c) {
      ...
      if (!mfqP->q_is_I) {
        ...
        PetscAssertFalse(info < 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"ormqr returned value %d",info);
      }
      ...
      if (proj >= mfqP->theta1) { /* add this index to model */
        ...
        PetscAssertFalse(info < 0,PETSC_COMM_SELF,PETSC_ERR_LIB,"geqrf returned value %d",info);
        mfqP->q_is_I = 0;
      }
...
static PetscErrorCode TaoSolve_POUNDERS(Tao tao)
{
  ...
  if (tao->XL && tao->XU) {
    ...
    PetscAssertFalse(val > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"X0 > upper bound");

    /* Check x0 >= xl */
    ierr = VecCopy(tao->XL,mfqP->Xhist[0]);CHKERRQ(ierr);
    ierr = VecAXPY(mfqP->Xhist[0],-1.0,tao->solution);CHKERRQ(ierr);
    ierr = VecMax(mfqP->Xhist[0],NULL,&val);CHKERRQ(ierr);
    PetscAssertFalse(val > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"X0 < lower bound");
    ...
    ierr = VecAXPY(mfqP->Xhist[0],-1.0,tao->XU);CHKERRQ(ierr);
    ierr = VecMax(mfqP->Xhist[0],NULL,&val);CHKERRQ(ierr);
    PetscAssertFalse(val > 1e-10,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"X0 + delta > upper bound");
  }
