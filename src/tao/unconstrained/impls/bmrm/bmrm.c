  ...
  while (tao->reason == TAO_CONTINUE_ITERATING) {
    ...
    if (rank == 0) {
      ...
      for (i=0; i<=tao->niter; i++) {
        PetscAssertFalse(!pgrad,PETSC_COMM_SELF,PETSC_ERR_PLIB,"Assert that there are at least tao->niter+1 pgrad available");
...
PetscInt project(PetscInt n,PetscReal *a,PetscReal b,PetscReal *c,PetscReal *l,PetscReal *u,PetscReal *x,PetscReal *lam_ext,TAO_DF *df)
{
  ...
  *lam_ext = 0;
  ...
  PetscAssertFalse(PetscAbsReal(dlambda) > BMRM_INFTY,PETSC_COMM_SELF,PETSC_ERR_PLIB,"L2N2_DaiFletcherPGM detected Infeasible QP problem!");
