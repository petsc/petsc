...
static PetscErrorCode TaoSolve_NLS(Tao tao)
{
  ...
  */
  ...
  if (is_nash || is_stcg || is_gltr) {
    PetscAssertFalse(tao->trust0 < 0.0,PetscObjectComm((PetscObject)tao),PETSC_ERR_ARG_OUTOFRANGE,"Initial radius negative");
    ...
    case NLS_INIT_INTERPOLATION:
      ...
      for (j = 0; j < j_max; ++j) {
        ...
        if (fmin < f) {
          ...
          ierr = TaoGradientNorm(tao, tao->gradient,NORM_2,&gnorm);CHKERRQ(ierr);
          PetscAssertFalse(PetscIsInfOrNanReal(gnorm),PetscObjectComm((PetscObject)tao),PETSC_ERR_USER, "User provided compute gradient generated Inf or NaN");
          needH = 1;
    ...
    if (is_nash || is_stcg || is_gltr) {
      ...
      if (0.0 == tao->trust) {
        ...
        if (norm_d > 0.0) {
          ...
          PetscAssertFalse(norm_d == 0.0,PETSC_COMM_SELF,PETSC_ERR_PLIB, "Initial direction zero");
        }
      }
    } else {
      ierr = KSPSolve(tao->ksp, tao->gradient, nlsP->D);CHKERRQ(ierr);
