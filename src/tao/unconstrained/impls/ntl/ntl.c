...
static PetscErrorCode TaoSolve_NTL(Tao tao)
{
  ...
  PetscAssertFalse(!is_nash && !is_stcg && !is_gltr,PetscObjectComm((PetscObject)tao),PETSC_ERR_SUP,"TAO_NTR requires nash, stcg, or gltr for the KSP");
  ...
  while (tao->reason == TAO_CONTINUE_ITERATING) {
    ...
    if (0.0 == tao->trust) {
      ...
      if (norm_d > 0.0) {
        ...
        PetscAssertFalse(norm_d == 0.0,PetscObjectComm((PetscObject)tao),PETSC_ERR_PLIB, "Initial direction zero");
      }
    }
    ...
    /* Check for converged */
    ierr = VecNorm(tao->gradient, NORM_2, &gnorm);CHKERRQ(ierr);
    PetscAssertFalse(PetscIsInfOrNanReal(f) || PetscIsInfOrNanReal(gnorm),PetscObjectComm((PetscObject)tao),PETSC_ERR_USER,"User provided compute function generated Not-a-Number");
    needH = 1;
