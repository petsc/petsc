...
static PetscErrorCode MatDiagonalSet_ADA(Mat M,Vec D, InsertMode mode)
{
  ...
  PetscFunctionBegin;
  PetscAssertFalse(mode == INSERT_VALUES,PetscObjectComm((PetscObject)M),PETSC_ERR_SUP,"Cannot insert diagonal entries of this matrix type, can only add");
...
static PetscErrorCode MatCreateSubMatrix_ADA(Mat mat,IS isrow,IS iscol,MatReuse cll, Mat *newmat)
{
  ...
  PetscAssertFalse(!isequal,PETSC_COMM_SELF,PETSC_ERR_SUP,"Only for identical column and row indices");
  ierr = MatShellGetContext(mat,&ctx);CHKERRQ(ierr);
  ...
  ierr = MatShellGetContext(*newmat,&ctx);CHKERRQ(ierr);
