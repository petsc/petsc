...
PetscErrorCode VecBlockSetSubVec( Vec V, const PetscInt idxm,const Vec vec );
...
PetscErrorCode VecSetUp_Block( Vec V )
{
	...
	/* Create space */
	ierr = PetscMalloc( sizeof(Vec) * ctx->nb, &ctx->v );CHKERRQ(ierr);
	for( i=0; i<ctx->nb; i++ ) {
		ctx->v[i] = PETSC_NULL;
...
PetscErrorCode VecDuplicate_Block( Vec x, Vec *y )
{
	...
	for( i=0; i<bx->nb; i++ ) {
		...
		/* Only required to VecDestroy() IF block allocates memory, should use the BlockSetValue to be safe */
		//	VecDestroy( by->v[i] );
		//	by->v[i] = Y;
...
PetscErrorCode VecView_Block( Vec x, PetscViewer viewer )
{
	...
	PetscTypeCompare((PetscObject)viewer,PETSCVIEWERASCII,&isascii);
	if (isascii) {
		...
		for( i=0; i<bx->nb; i++ ) {
			...
			PetscViewerASCIIPrintf( viewer, "(%D) : name=\"%s\", type=%s, rows=%D \n",i,name,type,NR);
