...
PetscErrorCode VecAYPX_SeqCUDA(Vec yin,PetscScalar alpha,Vec xin)
{
  ...
  ierr = PetscLogGpuTimeBegin();CHKERRQ(ierr); // hannah: added here
  ...
  ierr = PetscLogGpuTimeEnd();CHKERRQ(ierr); // hannah: added here
  ...
  while (current_y_index < nv)
  {
    switch (nv - current_y_index) {
      ...
      case 4:
        ...
        ierr = PetscLogGpuTimeEnd();CHKERRQ(ierr); // hannah: not include cudaMemcpy?
...
PetscErrorCode VecSet_SeqCUDA(Vec xin,PetscScalar alpha)
{
  ...
  ierr = PetscLogGpuTimeBegin();CHKERRQ(ierr); // hannah was here
  if (alpha == (PetscScalar)0.0) {
  ...
  ierr = PetscLogGpuTimeEnd();CHKERRQ(ierr); // hannah was here
  ierr = WaitForGPU();CHKERRCUDA(ierr);
