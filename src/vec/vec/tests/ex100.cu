...
int main(int argc,char **args)
{
  ...
  CHKERRQ(VecSetType(u, VECCUDA));
  CHKERRQ(VecGetType(u, &vectype));
  CHKERRQ(PetscPrintf(PETSC_COMM_WORLD, "Before load, vectype is : %s\n", (char*)vectype));
  CHKERRQ(VecLoad(u,viewer));
  CHKERRQ(VecGetType(u, &vectype));
  CHKERRQ(PetscPrintf(PETSC_COMM_WORLD, "After load, vectype is : %s\n", (char*)vectype));
  CHKERRQ(PetscViewerDestroy(&viewer));
  CHKERRQ(VecView(u,PETSC_VIEWER_STDOUT_WORLD));
