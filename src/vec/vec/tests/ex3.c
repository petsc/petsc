...
int main(int argc, char **argv)
{
  ...
  PetscCheck(size >= 2, PETSC_COMM_WORLD, PETSC_ERR_WRONG_MPI_SIZE, "Must be run with at least two processors");
  ...
  if (rank == 1) {
    idx = 2;
    PetscCall(VecSetValues(y, 1, &idx, &three, INSERT_VALUES));
    idx = 0;
    PetscCall(VecSetValues(y, 1, &idx, &two, INSERT_VALUES));
    idx = 0;
    PetscCall(VecSetValues(y, 1, &idx, &one, INSERT_VALUES));
  } else {
    idx = 7;
    PetscCall(VecSetValues(y, 1, &idx, &three, INSERT_VALUES));
  }
