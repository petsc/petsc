...
!  duplicating individual ones.
      ...
      call VecDuplicateVecsF90(x,ithree,z,ierr);CHKERRQ(ierr)

!  Set the vectors to entries to a constant value.

      call VecSet(x,one,ierr);CHKERRQ(ierr)
      call VecSet(y,two,ierr);CHKERRQ(ierr)
      call VecSet(z(1),one,ierr);CHKERRQ(ierr)
      call VecSet(z(2),two,ierr);CHKERRQ(ierr)
      call VecSet(z(3),three,ierr);CHKERRQ(ierr)

!  Demonstrate various basic vector routines.

      call VecDot(x,x,dot,ierr);CHKERRQ(ierr)
      call VecMDot(x,ithree,z,dots,ierr);CHKERRQ(ierr)
...
!  (when using real numbers) it is undefined.
 ...
 120  format ('All other values should be near zero')
      ...
      call VecScale(x,two,ierr);CHKERRQ(ierr)
 ...
 130  format ('VecScale ',1pe8.2)
      ...
      call VecNorm(w,NORM_2,norm,ierr);CHKERRQ(ierr)
      v = abs(norm-2.0*sqrt(nfloat))
      if (v .gt. -1.d-10 .and. v .lt. 1.d-10) v = 0.0
      if (rank .eq. 0) write(6,140) v
 140  format ('VecCopy ',1pe8.2)

      call VecAXPY(y,three,x,ierr);CHKERRQ(ierr)
      call VecNorm(y,NORM_2,norm,ierr);CHKERRQ(ierr)
      v = abs(norm-8.0*sqrt(nfloat))
      if (v .gt. -1.d-10 .and. v .lt. 1.d-10) v = 0.0
      if (rank .eq. 0) write(6,150) v
 150  format ('VecAXPY ',1pe8.2)

      call VecAYPX(y,two,x,ierr);CHKERRQ(ierr)
      call VecNorm(y,NORM_2,norm,ierr);CHKERRQ(ierr)
 ...
 160  format ('VecAYXP ',1pe8.2)
      ...
      call VecSwap(x,y,ierr);CHKERRQ(ierr)
      call VecNorm(y,NORM_2,norm,ierr);CHKERRQ(ierr)
      v = abs(norm-2.0*sqrt(nfloat))
      if (v .gt. -1.d-10 .and. v .lt. 1.d-10) v = 0.0
 ...
 180  format ('VecSwap ',1pe8.2)
      ...
      call VecWAXPY(w,two,x,y,ierr);CHKERRQ(ierr)
      call VecNorm(w,NORM_2,norm,ierr);CHKERRQ(ierr)
 ...
 190  format ('VecWAXPY ',1pe8.2)
      ...
      call VecPointwiseMult(w,y,x,ierr);CHKERRQ(ierr)
      call VecNorm(w,NORM_2,norm,ierr);CHKERRQ(ierr)
 ...
 200  format ('VecPointwiseMult ',1pe8.2)
      ...
      call VecPointwiseDivide(w,x,y,ierr);CHKERRQ(ierr)
      call VecNorm(w,NORM_2,norm,ierr);CHKERRQ(ierr)
 ...
 210  format ('VecPointwiseDivide ',1pe8.2)
      ...
      dots(3) = two
      call VecSet(x,one,ierr);CHKERRQ(ierr)
      call VecMAXPY(x,ithree,dots,z,ierr);CHKERRQ(ierr)
      call VecNorm(z(1),NORM_2,norm,ierr);CHKERRQ(ierr)
      v = abs(norm-sqrt(nfloat))
      if (v .gt. -1.d-10 .and. v .lt. 1.d-10) v = 0.0
      call VecNorm(z(2),NORM_2,norm,ierr);CHKERRQ(ierr)
      v1 = abs(norm-2.0*sqrt(nfloat))
      if (v1 .gt. -1.d-10 .and. v1 .lt. 1.d-10) v1 = 0.0
      call VecNorm(z(3),NORM_2,norm,ierr);CHKERRQ(ierr)
...
!  are no longer needed.
      ...
      call VecDestroyVecsF90(ithree,z,ierr);CHKERRQ(ierr)
