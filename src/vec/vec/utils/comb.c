...
int PETSCVEC_DLLEXPORT Petsc_DelReduction(MPI_Comm comm,int keyval,void* attr_val,void* extra_state)
{
  ...
  PetscFunctionBegin;
  ierr = PetscInfo((0,"Petsc_DelReduction:Deleting reduction data in an MPI_Comm %ld\n",(long)comm));CHKERRQ(ierr);
...
PetscErrorCode PETSCVEC_DLLEXPORT PetscSplitReductionGet(MPI_Comm comm,PetscSplitReduction **sr)
{
  ...
  if (!flag) {  /* doesn't exist yet so create it and put it in */
    ...
    ierr = PetscInfo((0,"PetscSplitReductionGet:Putting reduction data in an MPI_Comm %ld\n",(long)comm));CHKERRQ(ierr);
  }
