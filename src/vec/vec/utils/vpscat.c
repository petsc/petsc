...
PetscErrorCode VecScatterLocalOptimize_Private(VecScatter_Seq_General *gen_to,VecScatter_Seq_General *gen_from)
{
  ...
  if (!n_nonmatching) {
    ...
    ierr = PetscInfo((0,"VecScatterLocalOptimize_Private:Reduced %D to 0\n", n));CHKERRQ(ierr);
  } else if (n_nonmatching == n) {
    gen_to->nonmatching_computed = PETSC_FALSE;
    ierr = PetscInfo((0,"VecScatterLocalOptimize_Private:All values non-matching\n"));CHKERRQ(ierr);
  } else {
    gen_to->nonmatching_computed= PETSC_TRUE;
    ...
    ierr = PetscInfo((0,"VecScatterLocalOptimize_Private:Reduced %D to %D\n",n,n_nonmatching));CHKERRQ(ierr);
  } 
  PetscFunctionReturn(0);
}
...
PetscErrorCode VecScatterLocalOptimizeCopy_Private(VecScatter_Seq_General *gen_to,VecScatter_Seq_General *gen_from,PetscInt bs)
{
  ...
  ierr = PetscInfo((0,"VecScatterLocalOptimizeCopy_Private:Local scatter is a copy, optimizing for it\n"));CHKERRQ(ierr);
  ...
  */
  {
    ...
    if (flg) {
      ...
      ierr = PetscInfo((0,"VecScatterCopy_PtoP_X:Using VecScatter ready receiver mode\n"));CHKERRQ(ierr);
    } else {
      out->postrecvs               = 0;
      ...
      if (flg) {
        ierr = PetscInfo((0,"VecScatterCopy_PtoP_X:Using VecScatter Ssend mode\n"));CHKERRQ(ierr);
      }
      for (i=0; i<out_to->n; i++) {
        if (!flg) {
...
PetscErrorCode VecScatterCreate_PtoS(PetscInt nx,PetscInt *inidx,PetscInt ny,PetscInt *inidy,Vec xin,Vec yin,PetscInt bs,VecScatter ctx)
{
  ...
  */
  ...
  if (bs > 1) {
    ...
    if (flgs) {
      ierr = PetscInfo((0,"VecScatterCreate_PtoS:Using VecScatter Ssend mode\n"));CHKERRQ(ierr);
    }
    for (i=0; i<from->n; i++) {
      ierr = MPI_Recv_init(Srvalues+bs*rstarts[i],bs*rstarts[i+1]-bs*rstarts[i],MPIU_SCALAR,rprocs[i],tag,comm,rwaits+i);CHKERRQ(ierr);
      if (!flgs) {
    ...
    if (flg) {
      ...
      ierr = PetscInfo((0,"VecScatterCreate_PtoS:Using VecScatter ready receiver mode\n"));CHKERRQ(ierr);
    } else {
      ctx->postrecvs           = 0;
    ...
    ierr = PetscInfo((0,"VecScatterCreate_PtoS:Using blocksize %D scatter\n",bs));CHKERRQ(ierr);
    ...
    ierr = PetscInfo((0,"VecScatterCreate_PtoS:Using nonblocked scatter\n"));CHKERRQ(ierr);
    ctx->postrecvs = 0;
...
PetscErrorCode VecScatterCreate_StoP(PetscInt nx,PetscInt *inidx,PetscInt ny,PetscInt *inidy,Vec yin,PetscInt bs,VecScatter ctx)
{
  ...
  */
  ...
  if (bs > 1) {
    ierr = PetscInfo((0,"VecScatterCreate_StoP:Using blocksize %D scatter\n",bs));CHKERRQ(ierr);
    ctx->copy        = VecScatterCopy_PtoP_X;
    switch (bs) {
    case 12: 
      ctx->begin     = VecScatterBegin_PtoP_12;
    ...
    ierr = PetscInfo((0,"VecScatterCreate_StoP:Using nonblocked scatter\n"));CHKERRQ(ierr);
    ctx->begin     = VecScatterBegin_PtoP;
    ctx->end       = VecScatterEnd_PtoP; 
    ctx->copy      = VecScatterCopy_PtoP;
  }
...
#endif
  if (duplicate) {
    ierr = PetscInfo((0,"VecScatterCreate_PtoP:Duplicate to from indices passed in VecScatterCreate(), they are ignored\n"));CHKERRQ(ierr);
  }
