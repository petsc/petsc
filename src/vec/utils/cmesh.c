#ifndef lint
static char vcid[] = "$Id: cmesh.c,v 1.22 1996/02/26 18:02:57 curfman Exp curfman $";
#endif
...
.   m,n - the local number of mesh points in the x and y directions
.   x,y - the locations of the local mesh points (optional, use PETSC_NULL
...
{
  int           xin = 1, yin = 1, c1, c2, c3, c4, i, N, rank, ierr, flag, Nloc;
  int           isend[2], irecv[2], mglob, nglob;
  ...
  if (!flag) {
    ...
    ierr = VecMax(V,0,&max); CHKERRQ(ierr); ierr = VecMin(V,0,&min); CHKERRQ(ierr);
    scale = (200.0 - 32.0)/(max - min);
    ierr = VecScale(&scale,V); CHKERRQ(ierr);

    isend[0] = m; isend[1] = n;
    MPI_Allreduce(isend,irecv,2,MPI_INT,MPI_SUM,win->comm);
    mglob = irecv[0]; nglob = irecv[1];
    if (!x) {
      /* must determine local starting point */
      xin = 0; 
    ...
    for ( i=0; i<Nloc; i++ ) {
      if (!((i+1) % mglob) ) continue;  /* last column on right is skipped */
      if (i+mglob+1 >= N) continue;
