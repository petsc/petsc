...
PetscErrorCode MPIPetsc_Type_unwrap(MPI_Datatype a,MPI_Datatype *atype,PetscBool *flg)
{
  ...
  *atype = a;
  ...
  if (combiner == MPI_COMBINER_DUP) {
    PetscAssertFalse(nints != 0 || naddrs != 0 || ntypes != 1,PETSC_COMM_SELF,PETSC_ERR_LIB,"Unexpected returns from MPI_Type_get_envelope()");
    ...
    *flg = PETSC_TRUE;
    ...
    PetscAssertFalse(nints != 1 || naddrs != 0 || ntypes != 1,PETSC_COMM_SELF,PETSC_ERR_LIB,"Unexpected returns from MPI_Type_get_envelope()");
