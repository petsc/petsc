...
PetscErrorCode PetscSFLinkNvshmemCheck(PetscSF sf, PetscMemType rootmtype, const void *rootdata, PetscMemType leafmtype, const void *leafdata, PetscBool *use_nvshmem)
{
  ...
  */
  ...
  if (sf->use_nvshmem && !sf->checked_nvshmem_eligibility) {
    ...
    */
    if (sf->use_nvshmem) {
      ...
      PetscCallMPI(MPIU_Allreduce(MPI_IN_PLACE, &hasNullRank, 1, MPIU_INT, MPI_LOR, comm));
  ...
  if (sf->use_nvshmem) {
    ...
    if (PetscDefined(USE_DEBUG)) {                                                                                       /* Check in debug mode. Note MPI_Allreduce is expensive, so only in debug mode */
      PetscCallMPI(MPIU_Allreduce(&oneCuda, &allCuda, 1, MPIU_INT, MPI_LAND, comm));
