...
PetscErrorCode PetscLayoutCreateFromRanges(MPI_Comm comm,const PetscInt range[],PetscCopyMode mode,PetscInt bs,PetscLayout *newmap)
{
  ...
  if (PetscDefined(USE_DEBUG)) {  /* just check that n, N and bs are consistent */
    ...
    if (tmp != map->N) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Sum of local lengths %" PetscInt_FMT " does not equal global length %" PetscInt_FMT ", my local length %" PetscInt_FMT ".\nThe provided PetscLayout is wrong.",tmp,map->N,map->n);
    if (map->bs > 1) {
      if (map->n % map->bs) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Local size %" PetscInt_FMT " must be divisible by blocksize %" PetscInt_FMT,map->n,map->bs);
    }
    if (map->bs > 1) {
      if (map->N % map->bs) SETERRQ(map->comm,PETSC_ERR_PLIB,"Global size %" PetscInt_FMT " must be divisible by blocksize %" PetscInt_FMT,map->N,map->bs);
    }
  }
  /* lock the layout */
...
PetscErrorCode PetscLayoutSetUp(PetscLayout map)
{
  ...
  PetscFunctionBegin;
  if (map->setupcalled && (map->n != map->oldn || map->N != map->oldN)) SETERRQ(map->comm,PETSC_ERR_ARG_WRONGSTATE,"Layout is already setup with (local=%" PetscInt_FMT ",global=%" PetscInt_FMT "), cannot call setup again with (local=%" PetscInt_FMT ",global=%" PetscInt_FMT ")", map->oldn, map->oldN, map->n, map->N);
...
PetscErrorCode PetscLayoutSetISLocalToGlobalMapping(PetscLayout in,ISLocalToGlobalMapping ltog)
{
  ...
  if (ltog) {
    ...
    if (in->bs > 0 && (bs != 1) && in->bs != bs) SETERRQ(in->comm,PETSC_ERR_PLIB,"Blocksize of layout %" PetscInt_FMT " must match that of mapping %" PetscInt_FMT " (or the latter must be 1)",in->bs,bs);
...
{
  PetscFunctionBegin;
  if (map->bs > 1 && n % map->bs) SETERRQ(map->comm,PETSC_ERR_ARG_INCOMP,"Local size %" PetscInt_FMT " not compatible with block size %" PetscInt_FMT,n,map->bs);
  map->n = n;
  PetscFunctionReturn(0);
}
...
PetscErrorCode PetscLayoutSetBlockSize(PetscLayout map,PetscInt bs)
{
  ...
  if (map->n > 0 && map->n % bs) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Local size %" PetscInt_FMT " not compatible with block size %" PetscInt_FMT,map->n,bs);
  if (map->mapping) {
    PetscInt       obs;
    PetscErrorCode ierr;
