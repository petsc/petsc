...
PetscErrorCode  ISCreate(MPI_Comm comm,IS *is)
{
  ...
  ierr = PetscHeaderCreate(*is,_p_IS,struct _ISOps,IS_CLASSID,-1,"IS",comm,ISDestroy_,ISView);CHKERRQ(ierr);
...
PetscErrorCode  ISSetType(IS is, const ISType method)
{
  ...
  ierr = PetscFListFind(ISList, ((PetscObject)is)->comm, method,(void (**)(void)) &r);CHKERRQ(ierr);
