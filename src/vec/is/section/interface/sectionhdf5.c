...
static PetscErrorCode PetscSectionLoad_HDF5_SingleField(PetscSection s, PetscViewer viewer)
{
...
#if defined(PETSC_USE_DEBUG)
  if (N1 != N) SETERRQ(comm, PETSC_ERR_ARG_SIZ, "Unable to load s->atlasDof: sum of local sizes (%" PetscInt_FMT ") != global size (%" PetscInt_FMT "): local size on this process is %" PetscInt_FMT, N1, N, n);
#endif
...
#if defined(PETSC_USE_DEBUG)
  if (N1 != N) SETERRQ(comm, PETSC_ERR_ARG_SIZ, "Unable to load s->atlasOff: sum of local sizes (%" PetscInt_FMT ") != global size (%" PetscInt_FMT "): local size on this process is %" PetscInt_FMT, N1, N, n);
#endif
  ...
  if (hasConstraints) {
    ...
    ierr = PetscViewerHDF5ReadSizes(viewer, "atlasDof", NULL, &N);CHKERRQ(ierr);
#if defined(PETSC_USE_DEBUG)
    if (N1 != N) SETERRQ(comm, PETSC_ERR_ARG_SIZ, "Unable to load s->bc->atlasDof: sum of local sizes (%" PetscInt_FMT ") != global size (%" PetscInt_FMT "): local size on this process is %" PetscInt_FMT, N1, N, n);
#endif
    ...
    ierr = PetscViewerHDF5ReadSizes(viewer, "atlasOff", NULL, &N);CHKERRQ(ierr);
#if defined(PETSC_USE_DEBUG)
    if (N1 != N) SETERRQ(comm, PETSC_ERR_ARG_SIZ, "Unable to load s->bc->atlasOff: sum of local sizes (%" PetscInt_FMT ") != global size (%" PetscInt_FMT "): local size on this process is %" PetscInt_FMT, N1, N, n);
#endif
    ...
    if (M1 != M) SETERRQ(comm, PETSC_ERR_ARG_SIZ, "Unable to load s->bcIndices: sum of local sizes (%" PetscInt_FMT ") != global size (%" PetscInt_FMT "): local size on this process is %" PetscInt_FMT, M1, M, m);
#endif
...
PetscErrorCode PetscSectionLoad_HDF5_Internal(PetscSection s, PetscViewer viewer)
{
  ...
  if (s->pStart < 0 && s->pEnd < 0) n = PETSC_DECIDE;
    ...
    if (s->pStart != 0) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "s->pStart must be 0 (got %" PetscInt_FMT ")", s->pStart);
    if (s->pEnd < 0) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "s->pEnd must be >= 0, (got %" PetscInt_FMT ")", s->pEnd);
    n = s->pEnd;
  }
