...
!     Test IS of size 0
      ...
      if (n .ne. 0) then
        SETERRQ(PETSC_COMM_SELF,1,'Wrong result from ISCreateStride ')
      endif
      call ISStrideGetInfo(is,start,stride,ierr)
      if (start .ne. 0) then
         SETERRQ(PETSC_COMM_SELF,1,'Wrong result from ISStrideGetInfo ')
      endif
      if (stride .ne. 2) then
        SETERRQ(PETSC_COMM_SELF,1,'Wrong result from ISStrideGetInfo ')
      endif
      ...
      if (.not. flag) then
        SETERRQ(PETSC_COMM_SELF,1,'Wrong result from PetscObjectTypeCompare')
      endif
...
!     Test ISGetIndices()
      ...
      do 10, i=1,10000
        if (ii(i+iis) .ne. -11 + 3*i) then
          SETERRQ(PETSC_COMM_SELF,1,'Wrong result from ISGetIndices')
        endif
 10   continue
      call ISRestoreIndices(is,ii,iis,ierr)
      call ISDestroy(is,ierr)
