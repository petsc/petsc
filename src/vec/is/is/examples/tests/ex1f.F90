...
!     Test IS of size 0
       ...
       if (n .ne. 0) then SETERRA(PETSC_COMM_SELF,1,'Error getting size of zero IS')
...
!     fortran indices start from 1 - but IS indices start from 0
 ...
 10   continue
      ...
      do 20, i=1,n
        if (ii(i+iis) .ne. indices(i)) then SETERRA(PETSC_COMM_SELF,1,'Error getting indices')
 20   continue
...
!     Check identity and permutation
      ...
      if (flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking permutation')
      call ISIdentity(is,flag,ierr);CHKERRA(ierr)
      if (.not. flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking identity')
      call ISSetPermutation(is,ierr);CHKERRA(ierr)
      call ISSetIdentity(is,ierr);CHKERRA(ierr)
      call ISPermutation(is,flag,ierr);CHKERRA(ierr)
      if (.not. flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking permutation second time')
      call ISIdentity(is,flag,ierr);CHKERRA(ierr)
      if (.not. flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking identity second time')

!     Check equality of index sets

      call ISEqual(is,is,flag,ierr);CHKERRA(ierr)
      if (.not. flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking equal')

!     Sorting

      call ISSort(is,ierr);CHKERRA(ierr)
      call ISSorted(is,flag,ierr);CHKERRA(ierr)
      if (.not. flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking sorted')

!     Thinks it is a different type?

      call PetscObjectTypeCompare(is,ISSTRIDE,flag,ierr);CHKERRA(ierr)
      if (flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking stride')
      call PetscObjectTypeCompare(is,ISBLOCK,flag,ierr);CHKERRA(ierr)
      if (flag) then SETERRA(PETSC_COMM_SELF,1,'Error checking block')
...
!     Inverting permutation
 ...
 30   continue
      ...
      do 40, i=1,n
        if (ii(iis+i) .ne. n - i) then SETERRA(PETSC_COMM_SELF,1,'Error getting permutation indices')
 40   continue
