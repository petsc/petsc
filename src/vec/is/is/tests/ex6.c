...
int main(int argc,char **argv)
{
  ...
  }
  {
    ...
    PetscAssertFalse(cnt != n,PETSC_COMM_SELF,PETSC_ERR_PLIB,"inconsistent count");
    ierr = ISGeneralSetIndices(is[1],n,indices,PETSC_COPY_VALUES);CHKERRQ(ierr);
    ierr = PetscFree(indices);CHKERRQ(ierr);
  }

  ierr = ISEqual(is[0],is[1],&flg);CHKERRQ(ierr);
  PetscAssertFalse(!flg,PETSC_COMM_SELF,PETSC_ERR_PLIB,"is[0] should be equal to is[1]");
