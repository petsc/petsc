...
static PetscErrorCode TestGeneral(void)
{
  ...
  PetscAssertFalse(location != keylocation,comm,PETSC_ERR_PLIB,"Key %" PetscInt_FMT " not at %" PetscInt_FMT ": %" PetscInt_FMT,key,keylocation,location);
  ierr = ISLocate(is,nonkey,&location);CHKERRQ(ierr);
  PetscAssertFalse(location >= 0,comm,PETSC_ERR_PLIB,"Nonkey %" PetscInt_FMT " found at %" PetscInt_FMT,nonkey,location);
  ierr = ISSort(is);CHKERRQ(ierr);
  ierr = ISLocate(is,key,&location);CHKERRQ(ierr);
  PetscAssertFalse(location != sortedlocation,comm,PETSC_ERR_PLIB,"Key %" PetscInt_FMT " not at %" PetscInt_FMT ": %" PetscInt_FMT,key,sortedlocation,location);
  ierr = ISLocate(is,nonkey,&location);CHKERRQ(ierr);
  PetscAssertFalse(location >= 0,comm,PETSC_ERR_PLIB,"Nonkey %" PetscInt_FMT " found at %" PetscInt_FMT,nonkey,location);
...
static PetscErrorCode TestStride(void)
{
  ...
  for (i = 0; i < 2; i++) {
    ...
    PetscAssertFalse(location >= 0,comm,PETSC_ERR_PLIB,"Nonkey %" PetscInt_FMT " found at %" PetscInt_FMT,nonkey[i],location);
  }
  ierr = ISDestroy(&is);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
