...
PetscErrorCode ISRenumber(IS subset, IS subset_mult, PetscInt *N, IS *subset_n)
{
  ...
  if (N) { /* compute total size of new subset if requested */
    *N   = start + nlocals;
    ierr = MPI_Comm_size(PetscObjectComm((PetscObject)subset),&commsize);CHKERRMPI(ierr);
    ierr = MPI_Bcast(N,1,MPIU_INT,commsize-1,PetscObjectComm((PetscObject)subset));CHKERRMPI(ierr);
  }
...
static PetscErrorCode ISSetInfo_Internal(IS is, ISInfo info, ISInfoType type, ISInfoBool ipermanent, PetscBool flg)
{
  ...
  case IS_IDENTITY:
    ...
    break;
  default:
    PetscAssertFalse(type == IS_LOCAL,PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Unknown IS property");
...
PetscErrorCode ISSetInfo(IS is, ISInfo info, ISInfoType type, PetscBool permanent, PetscBool flg)
{
  ...
  PetscAssertFalse(((int) info) <= IS_INFO_MIN || ((int) info) >= IS_INFO_MAX,errcomm,PETSC_ERR_ARG_OUTOFRANGE,"Options %d is out of range",(int)info);
...
static PetscErrorCode ISGetInfo_Sorted(IS is, ISInfoType type, PetscBool *flg)
{
  ...
  comm = PetscObjectComm((PetscObject)is);
  ierr = MPI_Comm_size(comm, &size);CHKERRMPI(ierr);
  ierr = MPI_Comm_size(comm, &rank);CHKERRMPI(ierr);
  if (type == IS_GLOBAL && is->ops->sortedglobal) {
    ...
    if (type == IS_LOCAL || size == 1) {
      *flg = sortedLocal;
      ...
      ierr = MPI_Allreduce(&sortedLocal, flg, 1, MPIU_BOOL, MPI_LAND, comm);CHKERRMPI(ierr);
      if (*flg) {
        ...
        ierr = MPI_Exscan(&max, &maxprev, 1, MPIU_INT, MPI_MAX, comm);CHKERRMPI(ierr);
  ...
  if (type == IS_GLOBAL && is->ops->uniqueglobal) {
    ...
    if (type == IS_LOCAL || size == 1) {
      *flg = uniqueLocal;
      ...
      ierr = MPI_Allreduce(&uniqueLocal, flg, 1, MPIU_BOOL, MPI_LAND, comm);CHKERRMPI(ierr);
      if (*flg) {
  ...
  if (type == IS_GLOBAL && is->ops->permglobal) {
    ...
    if (type == IS_LOCAL || size == 1) {
      *flg = permLocal;
      ...
      ierr = MPI_Allreduce(&permLocal, flg, 1, MPIU_BOOL, MPI_LAND, comm);CHKERRMPI(ierr);
    }
    ierr = PetscFree(idx);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
  ...
  if (type == IS_GLOBAL && is->ops->intervalglobal) {
    ...
    if (type == IS_LOCAL || size == 1) {
      *flg = intervalLocal;
      ...
      ierr = MPI_Allreduce(&intervalLocal, flg, 1, MPIU_BOOL, MPI_LAND, comm);CHKERRMPI(ierr);
      if (*flg) {
        PetscInt  n, min = PETSC_MAX_INT, max = PETSC_MIN_INT;
        PetscInt  maxprev;
  ...
  if (type == IS_GLOBAL && is->ops->intervalglobal) {
    ...
    *flg = PETSC_FALSE;
    if (isinterval) {
      ...
      ierr = MPI_Bcast(&min, 1, MPIU_INT, 0, comm);CHKERRMPI(ierr);
    ...
    *flg = PETSC_FALSE;
    ...
    if (type == IS_LOCAL || size == 1) {
      *flg = identLocal;
      ...
      ierr = MPI_Allreduce(&identLocal, flg, 1, MPIU_BOOL, MPI_LAND, comm);CHKERRMPI(ierr);
    }
    ierr = ISRestoreIndices(is, &idx);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
...
PetscErrorCode  ISSetPermutation(IS is)
{
  ...
  if (PetscDefined(USE_DEBUG)) {
    ...
    if (size == 1) {
      ...
      ierr = PetscIntSortSemiOrdered(n,idx);CHKERRQ(ierr);
      for (i=0; i<n; i++) {
        PetscAssertFalse(idx[i] != i,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Index set is not a permutation");
      }
      ierr = PetscFree(idx);CHKERRQ(ierr);
...
PetscErrorCode  ISDestroy(IS *is)
{
  ...
  if ((*is)->complement) {
    ...
    PetscAssertFalse(refcnt > 1,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONGSTATE, "Nonlocal IS has not been restored");
...
PetscErrorCode  ISInvertPermutation(IS is,PetscInt nlocal,IS *isout)
{
  ...
  PetscAssertFalse(!isperm,PetscObjectComm((PetscObject)is),PETSC_ERR_ARG_WRONG,"Not a permutation");
  ...
  if (isidentity) {
    ...
    ierr = MPI_Allreduce(&issame, &isallsame, 1, MPIU_BOOL, MPI_LAND, PetscObjectComm((PetscObject)is));CHKERRMPI(ierr);
    issame = isallsame;
  }
  if (issame) {
...
static PetscErrorCode ISGatherTotal_Private(IS is)
{
  ...
  ierr = MPI_Allgatherv((void*)lindices,nn,MPIU_INT,is->total,sizes,offsets,MPIU_INT,comm);CHKERRMPI(ierr);
...
PetscErrorCode  ISRestoreTotalIndices(IS is, const PetscInt *indices[])
{
  ...
  if (size == 1) {
    ...
    PetscAssertFalse(is->total != *indices,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Index array pointer being restored does not point to the array obtained from the IS.");
  }
  PetscFunctionReturn(0);
}
/*@C
...
PetscErrorCode  ISRestoreNonlocalIndices(IS is, const PetscInt *indices[])
{
  ...
  PetscValidHeaderSpecific(is,IS_CLASSID,1);
  PetscValidPointer(indices,2);
  PetscAssertFalse(is->nonlocal != *indices,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Index array pointer being restored does not point to the array obtained from the IS.");
  PetscFunctionReturn(0);
}
...
PetscErrorCode  ISRestoreNonlocalIS(IS is, IS *complement)
{
  ...
  PetscValidHeaderSpecific(is,IS_CLASSID,1);
  PetscValidPointer(complement,2);
  PetscAssertFalse(*complement != is->complement,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Complement IS being restored was not obtained with ISGetNonlocalIS()");
  ierr = PetscObjectGetReference((PetscObject)(is->complement), &refcnt);CHKERRQ(ierr);
  PetscAssertFalse(refcnt <= 1,PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Duplicate call to ISRestoreNonlocalIS() detected");
...
PetscErrorCode ISLoad(IS is, PetscViewer viewer)
{
  ...
  PetscAssertFalse(!isbinary && !ishdf5,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Invalid viewer; open viewer with PetscViewerBinaryOpen()");
...
PetscErrorCode  ISSetBlockSize(IS is,PetscInt bs)
{
  ...
  PetscAssertFalse(bs < 1,PetscObjectComm((PetscObject)is),PETSC_ERR_ARG_OUTOFRANGE,"Block size %" PetscInt_FMT ", must be positive",bs);
  if (PetscDefined(USE_DEBUG)) {
    ...
    if (indices) {
      ...
      PetscAssertFalse(length%bs != 0,PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Local size %D not compatible with block size %D",length,bs);
      for (i=0;i<length/bs;i+=bs) {
        for (j=0;j<bs-1;j++) {
          PetscAssertFalse(indices[i*bs+j] != indices[i*bs+j+1]-1,PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"Block size %" PetscInt_FMT " is incompatible with the indices: non consecutive indices %" PetscInt_FMT " %" PetscInt_FMT,bs,indices[i*bs+j],indices[i*bs+j+1]);
        }
      }
    }
