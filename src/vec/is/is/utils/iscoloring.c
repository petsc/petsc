...
/*@
    ISColoringCreate - Generates an `ISColoring` context from lists (provided by each MPI rank) of colors for each node.
...
/*@
    ISPartitioningToNumbering - Takes an `IS' that represents a partitioning (the MPI rank that each local entry belongs to) and on each MPI rank
