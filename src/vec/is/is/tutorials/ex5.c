...
int main(int argc,char **argv)
{
  ...
  /*
     ...
     creates a mapping
  */
  CHKERRQ(PetscIntView(n,indices,PETSC_VIEWER_STDOUT_WORLD));
  CHKERRQ(ISLocalToGlobalMappingCreate(PETSC_COMM_WORLD,2,n,indices,PETSC_COPY_VALUES,&mapping));

  /*
     Map a set of local indices to their global values
  */
  CHKERRQ(PetscIntView(m,input,PETSC_VIEWER_STDOUT_WORLD));
  ...
  */
  ...
  CHKERRQ(PetscIntView(13,inglobals,PETSC_VIEWER_STDOUT_WORLD));
  ...
  */
  CHKERRQ(PetscIntView(13,inglobals,PETSC_VIEWER_STDOUT_WORLD));
  CHKERRQ(ISGlobalToLocalMappingApplyBlock(mapping,IS_GTOLM_DROP,13,inglobals,&m,outlocals));
