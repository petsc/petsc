...
PETSC_EXTERN PetscErrorCode PFCreate_Identity(PF pf,void *value)
{
  ...
  PetscFunctionBegin;
  if (pf->dimout != pf->dimin) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Input dimension must match output dimension for Identity function, dimin = %" PetscInt_FMT " dimout = %" PetscInt_FMT,pf->dimin,pf->dimout);
