...
%
function [X,F,flag]=MFQnls(func,X0,n,npmax,nfmax,gradtol,delta,m,mtype)
% --DEPENDS ON-------------------------------------------------------------
...
while nf<nfmax
    ...
    if (rho >= eta1)  || ((rho>eta0) && (valid))
        ...
        Displace = (X(nf,:)-X(xkin,:))/delta; %% LATER NEED TO MAKE SURE DELTA NOT MESSING UP THINGS HERE!!!!
        
        for j=1:m
            C(j) = C(j)+Displace*G(:,j)+.5*Displace*H(:,:,j)*Displace'; % Need this for mfq
            G(:,j) = G(:,j) + H(:,:,j)*Displace'; % Need this for mfq
        end
...
%            F(nf,:) = func(X(nf,:))';
    ...
    RES=zeros(np,m);
    for i=1:np
        D(i,:) = (X(ModelIn(i),:)-X(xkin,:))/deltaold;
        for j=1:m
            RES(i,j) = -C(j)-D(i,:)*(G(:,j)+.5*H(:,:,j)*D(i,:)')+F(ModelIn(i),j);
        end
    end

    
    % 6b. Update the quadratic model
    %[dum1,Gres,Hres] = getQuadnls(L,Z,M,N,F(ModelIn,:),Fres(ModelIn),F(xkin,:),mtype,m);
    [Cdel,Gdel,Hdel] = getQuadnlsmfq(L,Z,M,N,RES,Fres(ModelIn),F(xkin,:),mtype,m);
