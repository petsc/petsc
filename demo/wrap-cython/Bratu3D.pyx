...
def formInitGuess(Vec x, DMDA da, double lambda_):
    cdef int ierr
    cdef Params p = {"lambda_" : lambda_}
    ierr = FormInitGuess(da.dm, x.vec, &p)
    if ierr != 0: raise Error(ierr)

def formFunction(SNES snes, Vec x, Vec f, DMDA da, double lambda_):
    cdef int ierr
    cdef Params p = {"lambda_" : lambda_}
    ierr = FormFunction(da.dm, x.vec, f.vec, &p)
    if ierr != 0: raise Error(ierr)

def formJacobian(SNES snes, Vec x, Mat J, Mat P, DMDA da, double lambda_):
    cdef int ierr
    cdef Params p = {"lambda_" : lambda_}
