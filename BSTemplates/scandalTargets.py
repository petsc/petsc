...
class Defaults:
  ...
  def __init__(self, project, sources = None, bootstrapPackages = []):
    ...
    self.addLanguage('C')
    # Get runtime AST
    self.usingSIDL.repositoryDirs.append(os.path.join(self.usingSIDL.getRootDir(), 'sidl', 'sidl.sidl'))
    return
  ...
  def getRepositoryTargets(self):
    action = scandal.CompileSIDLRepository(compilerFlags = self.usingSIDL.getCompilerFlags())
    action.outputDir = self.usingSIDL.repositoryDir
    action.repositoryDirs.extend(self.usingSIDL.repositoryDirs)
    return [target.Target(None, [scandal.TagSIDL(), action])]

  def getSIDLServerCompiler(self, lang, rootDir, generatedRoots):
    action = scandal.CompileSIDLServer(fileset.ExtensionFileSet(generatedRoots, self.compileExt), compilerFlags = self.usingSIDL.getCompilerFlags())
  ...
  def getSIDLServerTargets(self):
    ...
    for lang in self.usingSIDL.serverLanguages:
      ...
      targets.append(target.Target(None, [scandal.TagSIDL(), target.If(self.isNewSidl, genActions, defActions)]))
    return targets

  def getSIDLClientCompiler(self, lang, rootDir):
    compiler           = scandal.CompileSIDLClient(fileset.ExtensionFileSet(rootDir, self.compileExt), compilerFlags = self.usingSIDL.getCompilerFlags())
  ...
  def getSIDLClientTargets(self):
    ...
    for lang in self.usingSIDL.clientLanguages:
      targets.append(target.Target(None, [scandal.TagSIDL(), self.getSIDLClientCompiler(lang, self.usingSIDL.getClientRootDir(lang))]))
    ...
    for package in self.getPackages():
      for lang in self.usingSIDL.internalClientLanguages[package]:
        targets.append(target.Target(None, [scandal.TagSIDL(), self.getSIDLClientCompiler(lang, self.usingSIDL.getServerRootDir(lang, package))]))
    return targets
